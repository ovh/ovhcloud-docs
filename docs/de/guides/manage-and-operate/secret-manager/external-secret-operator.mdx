...
Erstellen Sie eine `secret.yaml`-Datei:
...
Das Feld `stringData` akzeptiert den Token im Klartext — Kubernetes kodiert ihn beim Speichern des Secrets in Base64. Wenn Sie stattdessen das Feld `data` verwenden möchten, müssen Sie den Token vorher selbst in Base64 kodieren (`echo -n "<token>" | base64`).

Wenden Sie sie mit dem Befehl `kubectl apply -f secret.yaml` an.

Alternativ, falls der `user_pat` in der Umgebungsvariable `PAT_TOKEN` gespeichert ist, erstellen Sie das Secret direkt mit kubectl, das die Kodierung ebenfalls übernimmt:
...
Wenn Sie das Secret mit `kubectl get secret ovhcloud-vault-token -n external-secrets -o yaml` prüfen, erscheint der Wert Base64-kodiert unter `data`.
