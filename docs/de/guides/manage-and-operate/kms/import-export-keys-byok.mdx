...
- Sie kennen Ihre `okmsId` und Ihren regionalen Endpoint. Beide werden von <ApiLink section="/okms" version="v2" method="GET" route={"/okms/resource"}>GET /okms/resource</ApiLink> zurückgegeben und im Tab **Allgemeine Informationen** im <ManagerLink to="/#/okms/key-management-service">Dashboard Ihrer OKMS-Domain</ManagerLink> angezeigt.
...
Erstellen Sie einen RSA-Schlüssel, der ausschließlich der Umschließung dient. Setzen Sie stets sowohl `wrapKey` als auch `unwrapKey`. `wrapKey` und `unwrapKey` schließen sich mit `sign` und `verify` gegenseitig aus.
