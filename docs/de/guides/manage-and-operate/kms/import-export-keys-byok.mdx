...
Nach dem Umschließen mit dem öffentlichen Zielschlüssel senden Sie den entstandenen JWE-Compact-Serialization-`ciphertext` in der Importanfrage. Ein Beispiel in Go finden Sie unter [Vollständiges Beispiel in Go](#vollständiges-beispiel-in-go).
...
### Vollständiges Beispiel in Go

Die folgenden Beispiele führen beide Zeremonien mit einem RSA-Schlüssel durch. Sie verwenden das [Go SDK](https://pkg.go.dev/github.com/ovh/okms-sdk-go) für die Aufrufe an OVHcloud KMS und [go-jose](https://pkg.go.dev/github.com/go-jose/go-jose/v4) für die JWE-Operationen auf der anderen Seite. Beide Beispiele kodieren das Schlüsselmaterial in PKCS#8 (DER), was dem `keyFormatType` `PKCS8` entspricht.

#### Einen lokal erzeugten Schlüssel importieren

Die Go-Standardbibliothek erzeugt den Schlüssel, den das Programm lokal mit dem öffentlichen Transportschlüssel umschließt und anschließend importiert. Sein Klartext erreicht OVHcloud KMS nie.

```go
// 1. Erzeugen Sie den Transportschlüssel auf OVHcloud KMS und rufen Sie seinen öffentlichen Teil ab.
...
// 2. Erzeugen Sie den zu importierenden Schlüssel und kodieren Sie ihn in PKCS#8 (DER).
...
// 3. Umschließen Sie das Schlüsselmaterial als JWE: Schlüsselumschließung mit RSA-OAEP-256, Inhaltsverschlüsselung mit A256GCM.
...
// 4. Importieren Sie den umschlossenen Schlüssel. Das KMS hebt die Umschließung auf und leitet Typ und Größe aus dem Material ab.
...
Die JWE-Inhaltsverschlüsselung (`enc`) muss `A256GCM` sein.

#### Einen Schlüssel exportieren und das JWE entschlüsseln

Das Ziel (hier das lokale Programm) erzeugt das Transportschlüsselpaar und importiert nur dessen öffentlichen Teil in OVHcloud KMS. Anschließend entschlüsselt das Ziel das exportierte JWE mit dem privaten Teil.

```go
// 1. Erzeugen Sie das Transportschlüsselpaar am Ziel.
destKey, err := rsa.GenerateKey(rand.Reader, 4096)
if err != nil {
  return err
}

// 2. Importieren Sie nur seinen öffentlichen Teil in OVHcloud KMS, mit der Verwendung wrapKey.
...
// 3. Machen Sie den Schlüssel extrahierbar und exportieren Sie ihn anschließend umschlossen durch den öffentlichen Zielschlüssel.
extractable := true
...
// 4. Entschlüsseln Sie das JWE mit dem privaten Zielschlüssel und parsen Sie das PKCS#8-Material.
