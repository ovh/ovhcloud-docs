...
4. Importieren Sie auf dem Ziel den umschlossenen Schlüssel mit `POST /v1/servicekey` und einer `wrappedKeys`-Nutzlast. Das KMS hebt die Umschließung des Ciphertexts mit dem privaten Transportschlüssel auf und speichert den resultierenden Serviceschlüssel.
...
Erstellen Sie einen RSA-Schlüssel, der ausschließlich der Umschließung dient. Setzen Sie stets sowohl `wrapKey` als auch `unwrapKey`. `wrapKey` und `unwrapKey` sowie `sign` und `verify` schließen sich gegenseitig aus.
...
Rufen Sie `GET /v1/servicekey/{keyId}` mit den unten stehenden Abfrageparametern für die Umschließung auf. Sind diese Parameter vorhanden, liefert die API `wrappedKeys` statt Klartext-`keys`.
