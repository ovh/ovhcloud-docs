...
description: Erfahren Sie, wie Sie Personal Access Tokens, Servicekonten oder Zugriffszertifikate konfigurieren, um sich bei der regionalen REST-API Ihrer OKMS-Domain zu authentifizieren
...
- Sie verfügen über ein [OVHcloud Kundenkonto](/guides/account-and-service-management/account-information/ovhcloud-account-creation).
...
| **Servicekonto** | Machine-to-Machine-Integrationen | Unterstützt | Nicht unterstützt |
...
:::tip
Für den Zugriff auf die REST-API sollten Sie einen **Personal Access Token (PAT)** oder ein **Servicekonto** bevorzugen. Reservieren Sie Zugriffszertifikate für **KMIP** oder mTLS-basierte Clients.
:::
...
<Tabs>
  ...
  <Tab label="Servicekonto">
    #### Schritt 1 — Ein Servicekonto erstellen

    Falls Sie noch keines haben, erstellen Sie ein [OVHcloud Servicekonto](/guides/manage-and-operate/api/manage-service-account).

    #### Schritt 2 — Eine IAM-Richtlinie erstellen

    Erstellen Sie eine [IAM-Richtlinie](/guides/account-and-service-management/account-information/iam-policy-ui), die dem Servicekonto die erforderlichen OKMS-Aktionen auf Ihrer Domain gewährt. Weitere Informationen finden Sie weiter unten unter [IAM-Rechte für OKMS](#iam-rights-for-okms).

    #### Das Servicekonto auf der Datenebene verwenden

    Rufen Sie über den Client-Credentials-Flow ein OAuth2-Zugriffstoken ab, wie unter [So verwenden Sie Servicekonten für die Verbindung mit den OVHcloud APIs](/guides/account-and-service-management/account-information/authenticate-api-with-service-account) beschrieben, und übergeben Sie es dann als Bearer-Token:
  ...
  <Tab label="Zugriffszertifikat">
    ...
    Falls Sie noch keine haben, erstellen Sie eine OVHcloud Identität (lokaler Benutzer, Benutzergruppe oder Servicekonto). Weitere Informationen finden Sie unter [Vorstellung der Identitäten](/guides/manage-and-operate/iam/identities-management).
...
Sie können sie auch über den Swagger-Link auf dem <ManagerLink to="/#/dedicated/security/kms">Dashboard Ihrer OKMS-Domain</ManagerLink> öffnen.
...
2. Geben Sie unter **oAuth2ClientCredentials** die `client_id` und das `client_secret` Ihres Servicekontos ein. Weitere Informationen zum Abrufen dieser Zugangsdaten finden Sie unter [So verwenden Sie Servicekonten für die Verbindung mit den OVHcloud APIs](/guides/account-and-service-management/account-information/authenticate-api-with-service-account).
