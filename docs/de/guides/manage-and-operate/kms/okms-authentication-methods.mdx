...
description: Erfahren Sie, wie Sie Personal Access Tokens, Dienstkonten oder Zugriffszertifikate konfigurieren, um sich bei der regionalen REST-API Ihrer OKMS-Domain zu authentifizieren
...
| **Dienstkonto** | Machine-to-Machine-Integrationen | Unterstützt | Nicht unterstützt |
...
:::tip
Für den Zugriff auf die REST-API sollten Sie einen **Personal Access Token (PAT)** oder ein **Dienstkonto** bevorzugen. Reservieren Sie Zugriffszertifikate für **KMIP** oder mTLS-basierte Clients.
:::
...
<Tabs>
  ...
  <Tab label="Dienstkonto">
    #### Schritt 1 — Ein Dienstkonto erstellen

    Falls Sie noch keines haben, erstellen Sie ein [OVHcloud Dienstkonto](/guides/manage-and-operate/api/manage-service-account).

    #### Schritt 2 — Eine IAM-Richtlinie erstellen

    Erstellen Sie eine [IAM-Richtlinie](/guides/account-and-service-management/account-information/iam-policy-ui), die dem Dienstkonto die erforderlichen OKMS-Aktionen auf Ihrer Domain gewährt. Weitere Informationen finden Sie weiter unten unter [IAM-Rechte für OKMS](#iam-rights-for-okms).

    #### Das Dienstkonto auf der Datenebene verwenden

    Rufen Sie über den Client-Credentials-Flow ein OAuth2-Zugriffstoken ab, wie unter [So verwenden Sie Dienstkonten für die Verbindung mit den OVHcloud APIs](/guides/account-and-service-management/account-information/authenticate-api-with-service-account) beschrieben, und übergeben Sie es dann als Bearer-Token:
  ...
  <Tab label="Zugriffszertifikat">
    ...
    Falls Sie noch keine haben, erstellen Sie eine OVHcloud Identität (lokaler Benutzer, Benutzergruppe oder Dienstkonto). Weitere Informationen finden Sie unter [Vorstellung der Identitäten](/guides/manage-and-operate/iam/identities-management).
...
2. Geben Sie unter **oAuth2ClientCredentials** die `client_id` und das `client_secret` Ihres Dienstkontos ein. Weitere Informationen zum Abrufen dieser Zugangsdaten finden Sie unter [So verwenden Sie Dienstkonten für die Verbindung mit den OVHcloud APIs](/guides/account-and-service-management/account-information/authenticate-api-with-service-account).
