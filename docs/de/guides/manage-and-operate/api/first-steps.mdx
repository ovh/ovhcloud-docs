...
Die im <ApiLink>OVHcloud API-Portal</ApiLink> verfügbare API erlaubt es Ihnen, OVHcloud Produkte zu bestellen, zu verwalten, zu aktualisieren und zu konfigurieren, ohne ein grafisches Interface wie das Kundencenter zu verwenden.
...
- Sie sind auf der Webseite der <ApiLink>OVHcloud API</ApiLink>.
...
Klicken Sie auf der <ApiLink>OVHcloud API Seite</ApiLink> auf <code className="action">Explore the OVH API</code>, um die Liste der API-Funktionen anzuzeigen.
...
Klicken Sie hierzu auf folgenden Link: <CreateToken>OVHcloud Token-Erstellungsseite</CreateToken>.
...
Der **CK**-Token kann für die Übertragung von Rechten verwendet werden. Weitere Informationen finden Sie in dieser Anleitung: [How to manage a customer’s account via OVHcloud API](/guides/manage-and-operate/api/api-right-delegation) (EN).
...
Um Schlüssel aufzulisten und zu widerrufen, können Sie die <ApiLink>API-Konsole</ApiLink> oder das <ManagerLink to="/">OVHcloud Kundencenter</ManagerLink> verwenden:

<Tabs>
  <Tab label="API-Konsole">
    - Auflisten der Schlüssel-IDs mit dem folgenden Aufruf:
    
    <Api version="v1" section="/me" method="GET" route={"/me/api/application"} />
    
    - Mit dem folgenden Aufruf erhalten Sie wichtige Informationen:
    
    <Api version="v1" section="/me" method="GET" route={"/me/api/application/\\{applicationId\\}"} />
...
[OVHcloud Kunden-Account über die API verwalten](/guides/manage-and-operate/api/api-right-delegation) (EN)
