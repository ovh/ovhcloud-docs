...
<Tabs>
  ...
  <Tab label="Schritt 2">
    **Am Projekt anmelden und Migration erstellen**
    ...
    - **Quell-Account**:
        - **Account-Typ**: Wählen Sie `OVHcloud` und dann `MX Plan` oder `Automatische Erkennung`. Klicken Sie auf <code className="action">Anmelden</code>, um sich mit Ihrem OVHcloud Account zu identifizieren und den Dienst sowie die zu migrierende Adresse automatisch auszuwählen (Beispiel: `contact@mydomain.ovh`). Geben Sie dann das Passwort für diesen E-Mail-Account ein.
    - **Ziel-Account**:
        - **Account-Typ**: Wählen Sie `OVHcloud` und dann `Zimbra`. Klicken Sie auf <code className="action">Anmelden</code>, um sich mit Ihrem OVHcloud Account zu identifizieren und den Zimbra-Dienst sowie die Zieladresse auszuwählen (Beispiel: `contact2@mydomain.ovh`). Geben Sie dann das Passwort für diesen E-Mail-Account ein.
