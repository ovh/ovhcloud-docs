...
<Tabs>
  <Tab label="Schritt 1">
    Klicken Sie im Bereich `Web Cloud` auf <ManagerLink to="/#/web-domains/domain">Domainnamen</ManagerLink>, wählen Sie den betreffenden Domainnamen aus, den Sie für die Erstellung der Hosts in [Schritt 3](#step3) verwendet haben, und öffnen Sie dann den Tab **DNS-Zone**.
  ...
  <Tab label="Schritt 3">
    Wählen Sie im sich öffnenden Formular den DNS-Eintragstyp *A* oder *AAAA* je nach dem Typ der zugehörigen IP, die Sie hinzufügen möchten.
  </Tab>
  <Tab label="Schritt 4">
    Geben Sie die *Subdomain* und die *IPv4*-Adresse (A) oder *IPv6*-Adresse (AAAA) ein und klicken Sie dann auf <code className="action">Hinzufügen</code>. Falls erforderlich, folgen Sie den Anweisungen in unserer Anleitung "[OVHcloud DNS-Zone bearbeiten](/guides/web-cloud/domains/dns-zone-edit)".
  </Tab>
</Tabs>
...
<Tabs>
  <Tab label="Schritt 1">
    Klicken Sie im Bereich `Web Cloud` auf <ManagerLink to="/#/web-domains/domain">Domainnamen</ManagerLink>, wählen Sie den betreffenden Domainnamen aus, für den Sie die DNS-Server individualisiert haben, und öffnen Sie dann den Tab **DNS-Zone**.
  </Tab>
  <Tab label="Schritt 2">
    ...
    Die DNS-Zone erscheint im *Text*-Modus:
  ...
  <Tab label="Schritt 3">
    Ersetzen Sie in diesem Fenster **nur in den Einträgen vom Typ *NS*** die DNS-Servernamen durch Ihre eigenen benutzerdefinierten DNS-Servernamen, und **erhöhen Sie dabei** den ersten numerischen Wert der *SOA*-Zeile um "1". Wenn Sie Ihre Änderungen vorgenommen haben, folgen Sie den auf dem Bildschirm angezeigten Schritten, um zu bestätigen.
