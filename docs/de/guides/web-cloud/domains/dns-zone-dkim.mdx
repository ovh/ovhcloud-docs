...
Wenn Sie eine E-Mail über **contact@mydomain.ovh** versenden, wird dem Header der E-Mail eine mit privatem Schlüssel verschlüsselte Signatur hinzugefügt.
...
<ZoneTabs>
  ...
  <Tab label="Zimbra" availableIn={['eu']}>
    ...
    **Tipp zur Erstellung eines CNAME-Eintrags**
    ...
    Öffnen Sie den Tab <code className="action">DNS-Zone</code> und klicken Sie dann oberhalb der Tabelle auf <code className="action">Eintrag hinzufügen</code>. Wählen Sie im Formular, das sich öffnet, den Typ `CNAME` aus und füllen Sie die Felder entsprechend den von Ihnen notierten Werten aus.
    :::
  </Tab>
</ZoneTabs>
...
Um DKIM zu konfigurieren, besuchen Sie die <ApiLink>OVHcloud API-Seite</ApiLink> und melden Sie sich an:
...
<Tabs>
  ...
  <Tab label="4. DNS-Eintrag konfigurieren">
    ...
    Gehen Sie auf den Tab <code className="action">DNS-Zone</code> und klicken Sie dann oberhalb der Tabelle auf <code className="action">Eintrag hinzufügen</code>. Wählen Sie im Formular, das sich öffnet, den Typ `CNAME` aus und füllen Sie die Felder entsprechend den von Ihnen ermittelten Werten aus.
    ...
    <img className="thumbnail" alt="CNAME-Eintrag über die DNS-Zone hinzufügen" src="/images/assets/screens/control-panel/product-selection/web-cloud/domain-dns/dns-zone/nx-cname-form.png" loading="lazy" />

    Nachdem Sie die Werte eingegeben haben, klicken Sie auf <code className="action">Hinzufügen</code>.
...
<Tabs>
  ...
  <Tab label="5. DKIM aktivieren">
    ...
    *Beispielausgabe:*
    ...
    :::tip
    Sie haben damit alle notwendigen Schritte unternommen, um DKIM zu aktivieren. Um sicherzustellen, dass DKIM aktiviert ist, prüfen Sie mithilfe des Abschnitts [API - Die verschiedenen Zustände von DKIM](#dkim-status) dieser Anleitung, ob `status:` den Wert `inProduction` hat. Ist das der Fall, ist Ihr DKIM aktiv.<br/><br/> **Wenn Sie zwei Selektoren erstellt haben**, sollte der zweite Selektor `status:` den Wert `ready` haben.
    :::
  </Tab>
</Tabs>
...
<ZoneTabs>
  <Tab label="MX Plan und Zimbra" availableIn={['eu']}>
    ...
    - `error`: Fehler beim Installationsvorgang. Erstellen Sie ein [Ticket für den Support](/links/support-contact) und geben Sie die betreffende Domain an.<br/>
...
Öffnen Sie den Tab <code className="action">DNS-Zone</code> und klicken Sie dann oberhalb der Tabelle auf <code className="action">Eintrag hinzufügen</code>. Es gibt 3 Möglichkeiten, einen Eintrag hinzuzufügen, um die DKIM-Konfiguration in Ihrer DNS-Zone durchzuführen:
...
<img className="thumbnail" alt="DKIM-Eintrag über die DNS-Zone hinzufügen" src="/images/assets/screens/control-panel/product-selection/web-cloud/domain-dns/dns-zone/nx-dkim-form.png" loading="lazy" />

- **Selektor** (Subdomain): Geben Sie den Namen des DKIM-Selektors ein, den Ihr E-Mail-Dienst bereitstellt. Das Suffix `._domainkey` wird automatisch hinzugefügt und Ihr Domainname wird am Ende angehängt.
...
- **Öffentlicher Schlüssel (base64)**: Fügen Sie den von Ihrem E-Mail-Dienst bereitgestellten öffentlichen Schlüssel ein. Dies ist der Schlüssel, mit dem Empfänger Ihre DKIM-Signatur überprüfen.
- **Schlüsseltyp**: Der Signaturalgorithmus des Schlüssels — `RSA` (Standard) oder `ed25519`.
- **Testmodus**: Belassen Sie ihn im Produktivbetrieb auf `Deaktiviert`. Wenn er aktiviert ist, behandeln Empfänger mit DKIM signierte Testnachrichten wie unsignierte, sodass eine fehlgeschlagene Prüfung die Zustellung nicht beeinträchtigt, während Sie Ihre Konfiguration validieren.
...
<Tabs>
  ...
  <Tab label="502">
    ...
    Bei der Konfiguration des DKIM ist ein Fehler aufgetreten. Wenn sich Ihre Konfiguration nach 24 Stunden noch in diesem Status befindet, können Sie ein [Support-Ticket eröffnen](/links/support-contact).
  </Tab>
  <Tab label="503">
...
<ZoneTabs>
  <Tab label="Exchange und E-Mail Pro" availableIn={['eu']}>
    ...
    - `deleting`: DKIM wird gelöscht. Nach dem Löschen folgen Sie dem Abschnitt "[API - Vollständige DKIM-Konfiguration](#firststep)".
    ...
    - `todo`: Der Task wurde initialisiert und muss gestartet werden. Wenn er sich nach 24 Stunden noch in diesem Status befindet, können Sie ein [Support-Ticket](/links/support-contact) unter Angabe der Selektor-ID erstellen.
  </Tab>
  <Tab label="E-Mails und Zimbra" availableIn={['eu']}>
    ...
    - `error`: Fehler beim Installationsvorgang. Erstellen Sie ein [Ticket für den Support](/links/support-contact) unter Angabe der betreffenden Domain.
