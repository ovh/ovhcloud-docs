...
- Bessere Kontrolle über die Verwendung des Domainnamens und Erkennung von Spoofing-Versuchen mithilfe von Berichten, die bei fehlerhafter E-Mail-Authentifizierung versendet werden. Außerdem wird die Sicherheit erhöht, indem die SPF-Protokolle mit DKIM verbunden werden.
...
Sie müssen DMARC zunächst auf dem E-Mail-Dienst aktivieren, der mit Ihrem Domainnamen verbunden ist.

Es gibt zwei Möglichkeiten, DMARC in Ihrer OVHcloud DNS-Zone zu konfigurieren:

- Über einen [DMARC-Eintrag](#dmarc-record). Dieser erlaubt eine vereinfachte Konfiguration von DMARC. Füllen Sie einfach die Felder mit den DMARC-Einstellungen aus, die Sie für Ihre Konfiguration benötigen. Dieser Eintrag wird von DNS-Servern als TXT-Eintrag gelesen.
- Über einen [TXT-Eintrag](#txt-record). Dieser Standardeintrag kann im OVHcloud Kundencenter für die DMARC-Konfiguration verwendet werden. Damit können Sie alle DMARC-Einstellungen integrieren, einschließlich derer, die über den OVHcloud DMARC-Eintrag fehlen. Er muss jedoch den Syntaxregeln des DMARC-Protokolls entsprechen.
...
Klicken Sie oberhalb der Tabelle auf <code className="action">Eintrag hinzufügen</code> und wählen Sie dann den Typ **DMARC** aus. Im Formular können Sie jeden Teil des Eintrags festlegen:
...
- **Version**: Obligatorisches Feld, das die Version des DMARC-Protokolls bestimmt (`DMARC1`).

- **Regel für die Domain (p=)**: Die vom Empfänger auf Anfrage des Inhabers der Sender-Domain einzunehmende Regelung. Die Richtlinie gilt für die abgefragte Domain und die Subdomains, außer der Subdomain-Tag **sp=** enthält andere Anweisungen. Mögliche Werte sind:
...
- **Prozentsatz der gefilterten Nachrichten (pct=)** (zwischen 0 und 100, Standardwert 100): Prozentsatz des Nachrichtenflusses, auf den die DMARC-Richtlinie angewendet werden soll. PCT soll Domaininhabern ermöglichen, den DMARC-Mechanismus schrittweise zu implementieren.

- **URI für globale Berichtserstellung (rua=)**: Adressen, an die Berichte gesendet werden sollen (durch Kommas getrennte Liste im Text-Format). Es kann ein beliebiger gültiger URI angegeben werden. Vor dem E-Mail-Empfänger muss "mailto:" stehen (Beispiel: `mailto:address@example.com`).
...
- **Ausrichtungsmodus für SPF (aspf=)** (Standardwert: `r`): Gibt den SPF-Alignment-Modus an. Die Werte sind:
    - `r` (Relaxed) für den flexiblen Modus: E-Mails können zum Beispiel von einer Subdomain der angegebenen Domain gesendet werden. Man spricht hier von einem partiellen Alignment.
    - `s` (Strict) für den strikten Modus: E-Mails müssen von der angegebenen Domain und ausschließlich von dieser gesendet werden. Das Ergebnis ist somit "aligned".
...
<img className="thumbnail" alt="DMARC-Eintrag über die DNS-Zone hinzufügen" src="/images/assets/screens/control-panel/product-selection/web-cloud/domain-dns/dns-zone/nx-dmarc-form.png" loading="lazy" />
...
Klicken Sie oberhalb der Tabelle auf <code className="action">Eintrag hinzufügen</code> und wählen Sie dann den Typ **TXT** aus.
...
Zur Veranschaulichung dieses ersten Beispiels wurde der [DMARC-Eintrag](#dmarc-record) in der DNS-Zone verwendet und die folgenden Einstellungen angewendet. Das Ergebnis lautet:
...
- **pct=100**: Die DMARC-Richtlinie gilt für 100% der Nachrichten aller E-Mails des Domaininhabers.
