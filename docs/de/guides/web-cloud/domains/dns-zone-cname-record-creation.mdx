...
Klicken Sie oberhalb der Tabelle auf <code className="action">Eintrag hinzufügen</code> und wählen Sie den Typ **CNAME** aus. Geben Sie im Feld **Subdomain** die betreffende Subdomain ein (zum Beispiel `www` für `www.domain.tld`) und im Feld **Ziel** den Domainnamen oder die Subdomain, auf die Sie verweisen möchten (zum Beispiel `domain.tld`). Klicken Sie anschließend auf <code className="action">Hinzufügen</code>.
