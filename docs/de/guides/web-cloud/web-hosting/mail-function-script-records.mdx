...
Wenn Sie die E-Mail-Accounts Ihres MX Plans oder Ihres [OVHcloud Webhostings](/links/web/hosting) verwalten möchten, lesen Sie unsere Dokumentation zu [Hosted E-Mail - MX Plan](/guides/web-cloud/email-and-collaborative-solutions/mx-plan/landing-page-mx-plan).
:::

:::tip
Diese Anleitung betrifft den Versand über die "mail()"-Funktion von PHP, der sich über Ihr OVHcloud Kundencenter überwachen lässt. Sie können E-Mails von Ihrem Shared Hosting aus auch über eine [authentifizierte SMTP-Verbindung](#SMTP) mit einem E-Mail-Account versenden, der mit Ihrer Domain verknüpft ist. Um zwischen den beiden Methoden zu wählen und den SMTP-Versand zu konfigurieren, lesen Sie die Anleitung [Webhosting - Best Practices für den E-Mail-Versand](/guides/web-cloud/web-hosting/email-sending-best-practices).
:::
...
<Tabs>
  <Tab label="Schritt 1">
    Rufen Sie die Seite <ManagerLink to="/#/web/hosting">Hosting-Pakete</ManagerLink> auf und wählen Sie Ihr Webhosting aus.
  ...
  <Tab label="Schritt 2">
    Öffnen Sie den Tab <code className="action">Mehr</code> und klicken Sie dann auf <code className="action">E-Mail-Skripte</code>.
  ...
  <Tab label="Schritt 3">
    ...
    Diese Seite zeigt den von Ihren Skripten generierten automatischen E-Mail-Versand:
    ...
    - **Fehlerbericht an**: Erhalten Sie täglich einen Statusbericht an die E-Mail-Adresse Ihrer Wahl. Aktivieren Sie ihn, indem Sie auf <code className="action">Empfänger ändern</code> klicken. Dieser Bericht enthält versendete E-Mails, die bei OVHcloud eine Fehlermeldung zurückliefern. Mit dem Button <code className="action">E-Mails im Fehlerstatus</code> rechts auf der Seite <code className="action">E-Mail-Skripte</code> können Sie ebenfalls Fehlerberichte einsehen.
    ...
    - **Heute versandte E-Mails**: Heute gesendete automatische E-Mails.
    ...
    - **History der versandten E-Mails**: Grafik der E-Mails, die in den letzten Tagen von Ihrem Webhosting aus gesendet wurden.
    ...
    - **E-Mails bereinigen**: Löscht die in der Warteschleife befindlichen E-Mails und entsperrt den Versand von E-Mails. Aus Gründen der Vertraulichkeit kann OVHcloud nicht auf die E-Mails in der Warteschlange zugreifen. Sie können diese E-Mails nur anzeigen, wenn sie vor dem Versand in der Datenbank Ihrer Website gespeichert wurden.
    - **E-Mails im Fehlerstatus**: Öffnet die Logs der letzten Sendefehler. Dort finden Sie die betroffenen E-Mail-Adressen mit dem zugehörigen Fehler. Achtung, dieser Verlauf wird nicht zurückgesetzt, auch wenn Sie <code className="action">E-Mails bereinigen</code> oder <code className="action">Versand entsperren</code> nutzen.
    ...
    Um die gewünschte Aktion durchzuführen, klicken Sie auf den Button und dann auf <code className="action">Bestätigen</code>. In einigen Fällen kann es mehrere Minuten dauern, bis die Änderung wirksam wird.
  </Tab>
</Tabs>
...
Wenn Sie feststellen, dass die von Ihren Skripten generierten E-Mails nicht mehr versendet werden, obwohl der Status des Dienstes weiterhin den Versand erlaubt (*"Aktiv"* oder *"Force"*), überprüfen Sie Folgendes:
...
- **Stellen Sie sicher, dass Ihr Skript tatsächlich die "mail()"-Funktion verwendet**: Die Prüfungen und Statistiken auf dieser Seite betreffen ausschließlich E-Mails, die über "mail()" versendet werden. Wenn Ihr Skript oder die E-Mail-Einstellungen Ihrer Website einen SMTP-Server angeben, laufen die E-Mails über diesen Server und erscheinen hier nicht. Lesen Sie dazu den Abschnitt [Versand von E-Mails mit einem "SMTP" Skript](#SMTP).
...
Dieser Zustand tritt ein, wenn zu viele Ihrer automatisch versandten E-Mails mit einem Fehler zurückkommen.
...
Um diesen Zustand zu beheben, sichern Sie jedes Skript, das über Ihr Hosting E-Mails senden kann, mit einem "Captcha" System ab.
...
Um diesen Zustand zu beheben, führen Sie mindestens folgende Aktionen aus:
...
- Die E-Mail enthält ausführbare Dateien (EXE, BAT, PIF, XML, XLSX oder Dokumente mit Makros); das gilt auch für Dateien in komprimierten Archiven (.zip, .rar usw.).
...
Ein Shared Hosting erlaubt es Ihnen auch, E-Mails anstelle der "mail()"-Funktion über ein Skript zu versenden, das eine authentifizierte SMTP-Verbindung mit einem E-Mail-Account nutzt, der mit Ihrer Domain verknüpft ist. Das Versandkontingent hängt dann vom E-Mail-Account (etwa 200 E-Mails pro Stunde für einen MX Plan Account) und von der Quell-IP-Adresse ab (300 E-Mails pro Stunde und pro IP-Adresse, die mit den anderen Websites Ihres Hosting-Clusters geteilt wird), und die E-Mails sind im Bereich <code className="action">E-Mail-Skripte</code> Ihres OVHcloud Kundencenters nicht sichtbar.

Um die beiden Methoden zu vergleichen und den SMTP-Versand zu konfigurieren (Server, Port, PHPMailer-Beispiel, CMS-Plugins), lesen Sie die Anleitung [Webhosting - Best Practices für den E-Mail-Versand](/guides/web-cloud/web-hosting/email-sending-best-practices).

## Weiterführende Informationen <a name="go-further"></a>

[Webhosting - Best Practices für den E-Mail-Versand](/guides/web-cloud/web-hosting/email-sending-best-practices)
