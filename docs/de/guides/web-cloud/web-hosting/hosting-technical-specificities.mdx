...
Für in Webhostings enthalten Datenbanken oder Datenbankserver (Web Cloud Databases) in Gravelines (GRA), Frankreich und Beauharnois (BHS), Kanada wird täglich ein Backup der Datenbanken durchgeführt. Diese Backups sind verfügbar (über das <ManagerLink to="/">OVHcloud Kundencenter</ManagerLink> oder über die [OVHcloud API](/links/api)). Die Backups werden zusätzlich auf einer anderen Infrastruktur gespeichert. Diese Daten werden in einem Rechenzentrum in Straßburg (SBG) repliziert. Die Aufbewahrungsfrist für Backups beträgt 30 Tage.
