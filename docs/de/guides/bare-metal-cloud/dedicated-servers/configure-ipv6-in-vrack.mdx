...
Geben Sie die ID Ihres vRack (z. B. `pn-9000000`) als *path parameter* `serviceName` ein und setzen Sie den IPv6-Block im Feld *request body*:
...
Verwenden Sie den folgenden Aufruf, um zu überprüfen, ob IPv6 zugewiesen wurde:
...
Überprüfen, welches Subnetz gebrückt ist:
...
Aktivieren Sie SLAAC im Feld *request body*:
...
Beachten Sie, dass sich ein gegebenes Subnetz nicht mit einem anderen definierten Subnetz überschneiden darf und die Next-Hop-Adresse zum ersten Teil (gebrücktes /64-Subnetz) Ihres Additional-IPv6-Präfixes gehören muss.

Verwenden Sie den folgenden API-Aufruf, um ein solches Subnetz zu definieren:
...
Beachten Sie, dass dies zusätzliche Risiken mit sich bringt (wie den Verlust des Zugangs nicht nur zum Host, sondern auch zu allen VMs) und nicht empfohlen wird.
...
Beachten Sie, dass dies sehr spezifisch für den installierten Hypervisor ist (es kann sich um vSwitch- oder veth-Interfaces handeln). Lesen Sie die spezifische Hypervisor-Netzwerkdokumentation für diese Einrichtung.
