...
Öffnen Sie die <ApiLink>OVHcloud API-Konsole</ApiLink> und verwenden Sie den folgenden Aufruf:
...
Der Backup Storage ist nur mit NFSv3 kompatibel. Vergewissern Sie sich zunächst, dass Ihre IP-Blöcke auf den Speicher zugreifen und das NFS-Protokoll verwenden können. Je nach dem von Ihnen verwendeten Linux-Betriebssystem kann es sein, dass der NFS-Client installiert und die Dienste NFS und `portmap` gestartet werden müssen.
...
Stellen Sie bei Verbindungsproblemen außerdem sicher, dass die für NFSv3 erforderlichen UDP- und TCP-Ports in der Firewall sowohl für eingehenden als auch für ausgehenden Datenverkehr geöffnet sind.
Öffnen Sie Port 111 (UDP/TCP), Port 2049 (TCP, NFS) und den dynamischen Portbereich 32768–60999 (UDP/TCP) in beide Richtungen. Diese Ports werden von RPC für die Programme `mountd`, `lockd` und `statd` verwendet.

Erlauben Sie abschließend alle ausgehenden Verbindungen zur IP-Adresse Ihres Backup Storages.
