---
pageType: overview
title: Dedicated Server
description: "Konfiguration, Sicherung und Betrieb Ihrer OVHcloud Dedicated Server - Installation, Netzwerk, RAID, Backups und erweiterte Serveradministration."
text: "Entdecken Sie die Dedicated Server-Anleitungen"
essentialsTitle: "Die wichtigsten Anleitungen für den Einstieg"
essentials:
  - title: Erste Schritte mit einem Dedicated Server
    link: /guides/bare-metal-cloud/dedicated-servers/getting-started-with-dedicated-server
  - title: "Erste Schritte mit einem Kimsufi, So You Start oder Rise Dedicated Server"
    link: /guides/bare-metal-cloud/dedicated-servers/getting-started-with-dedicated-server-eco
  - title: SSH-Verbindungen verwenden
    link: /guides/bare-metal-cloud/dedicated-servers/ssh-introduction
  - title: "Authentifizierungsschlüssel für SSH-Verbindungen zu OVHcloud Servern erstellen und verwenden"
    link: /guides/bare-metal-cloud/dedicated-servers/creating-ssh-keys
  - title: "Öffentliche Authentifizierungsschlüssel im OVHcloud Kundencenter speichern"
    link: /guides/bare-metal-cloud/dedicated-servers/import-keys-control-panel
  - title: "Verwendung der IPMI-Konsole für Dedicated Server"
    link: /guides/bare-metal-cloud/dedicated-servers/ipmi
tutorials:
  - title: "PuTTY für SSH-Verbindungen und Authentifizierung verwenden"
    link: /guides/web-cloud/web-hosting/ssh-using-putty-on-windows
  - title: Tutorial - Configuring pfSense network bridge (EN)
    link: /guides/bare-metal-cloud/dedicated-servers/pfsense-bridging
  - title: "Webserver (LAMP) auf Debian oder Ubuntu installieren"
    link: /guides/bare-metal-cloud/dedicated-servers/installing-lamp-debian9-ubuntu18
goFurther:
  title: "Weiterführende Informationen"
  items:
    - title: OVHcloud Community besuchen
      link: https://community.ovhcloud.com/
    - title: "Roadmap & Changelog einsehen"
      link: https://www.ovhcloud.com/de/roadmap-changelog/
    - title: Discord beitreten
      link: https://discord.gg/ovhcloud
cta:
  title: "Fragen? OVHcloud Support kontaktieren"
  description: "Finden Sie die Antworten auf Ihre Fragen im Hilfe-Center."
  actions:
    - text: Zum Support
      link: https://help.ovhcloud.com/
      theme: brand
---
