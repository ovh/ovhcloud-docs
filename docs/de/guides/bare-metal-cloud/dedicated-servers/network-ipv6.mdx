...
Da ifcfg nun veraltet ist, erstellt NetworkManager die neuen Profile in diesem Format nicht mehr standardmäßig. Die Konfigurationsdatei befindet sich nun unter `/etc/NetworkManager/system-connections/`.
  ...
  <Tab label="Windows Server 2019 und höhere Versionen">
    ...
    **Schritt 2: Netzwerkkonfiguration Ihres Servers öffnen**
    ...
    Öffnen Sie das Windows-Startmenü, geben Sie `ncpa.cpl` ein und klicken Sie dann auf das vorgeschlagene Suchergebnis.

    <img className="thumbnail" alt="Fenster Netzwerkverbindungen in der Windows-Systemsteuerung" src="/images/bare-metal-cloud/dedicated-servers/network-ipv6/ipv6_network_connections.png" loading="lazy" />
...
Zögern Sie in jedem Fall nicht, sich an [unser Support-Team](/links/support-contact) zu wenden, um Ihre Konfigurationen zu überprüfen. Es müssen folgende Angaben gemacht werden:
