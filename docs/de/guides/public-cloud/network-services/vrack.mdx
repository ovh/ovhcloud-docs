...
Sie werden auf eine andere Seite weitergeleitet, um die Bestellung zu bestätigen. Es wird einige Minuten dauern, bis das vRack in Ihrem Account eingerichtet ist.
    ...
    Um die Konfiguration Ihres vRacks über das OVHcloud Kundencenter fortzusetzen, gehen Sie zu [Schritt 2: Ein privates Netzwerk im vRack erstellen](#create-pn-in-vrack), im Tab **Über das OVHcloud Kundencenter**.
  </Tab>
...
Wählen Sie im erscheinenden Pop-up-Fenster die privaten Netzwerke aus, die Sie an Ihre Instanz anhängen möchten, und klicken Sie auf <code className="action">Bestätigen</code>.
  ...
  <Tab label="Über die OVHcloud APIv6">
    Sie können eine vorhandene Instanz nicht über das OVHcloud Kundencenter in den vRack integrieren. Sie müssen Horizon, die OpenStack-API oder die OVHcloud APIv6 verwenden.
...
Wählen Sie Ihr Interface aus und bestätigen Sie.
  ...
  <Tab label="Über die OpenStack CLI">
    ...
    **Ein privates Netzwerkinterface hinzufügen**
    ...
    Um ein neues Interface anzuhängen, führen Sie aus:
...
Um ein Interface zu trennen, identifizieren Sie zuerst den Neutron-Port.  
...
Sobald Sie den Port identifiziert haben, führen Sie aus:
