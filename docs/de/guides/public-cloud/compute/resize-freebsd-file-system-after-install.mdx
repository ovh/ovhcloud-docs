---
title: Dateisystem auf FreeBSD anpassen
description: Erfahren Sie hier, wie Sie das Dateisystem einer Public Cloud Instanz oder eines VPS unter FreeBSD anpassen
...
:::info
FreeBSD installiert `sudo` nicht standardmäßig. Wenn `sudo` nicht verfügbar ist, wechseln Sie mit `su` zum Root-Benutzer und führen Sie den Befehl ohne `sudo` aus.
:::
...
Passen Sie die Partition `freebsd-zfs` an:
...
Sie werden feststellen, dass in diesem Beispiel `zroot` nun 50 GB groß ist. Der ZFS-Pool wurde nun erweitert.
