...
Klicken Sie auf der Seite <code className="action">Schnellzugriff</code> auf <code className="action">Instanz erstellen</code>. Wählen Sie anschließend ein kompatibles GPU-Instanzmodell aus, das den **Cloud GPU**-Instanzen entspricht, um von Ressourcen zu profitieren, die für grafische oder rechenintensive Workloads geeignet sind.

Folgen Sie anschließend den verbleibenden Schritten, wie in [dieser Anleitung](/guides/public-cloud/compute/getting-started#step-4-create-the-instance) beschrieben. Dieser Vorgang kann einige Minuten dauern.

<Tabs>
  <Tab label="Unter Linux">
    Alle von uns angebotenen Images können auf einer GPU-Instanz verwendet werden.

    Öffnen Sie beim Schritt zur Image-Auswahl den Tab <code className="action">Unix-Distributionen</code> und wählen Sie ein UNIX-Image, das Ihren Anforderungen entspricht.
  ...
  <Tab label="Unter Windows">
    ...
    Öffnen Sie beim Schritt zur Image-Auswahl den Tab <code className="action">Windows-Distributionen</code> und wählen Sie ein Windows-Image, das mit dem gewählten Instanzmodell kompatibel ist.
    ...
    **Verbindung zu einer Windows-Instanz**
    ...
    Nachdem die Instanz fertig erstellt ist, muss die Windows-Installation abgeschlossen werden (_sysprep_). Klicken Sie hierzu auf <code className="action">...</code> und dann auf <code className="action">Details der Instanz</code>. Wechseln Sie zum Tab <code className="action">VNC-Konsole</code>. Die Konsole sollte bereits das Post-Installationsfenster anzeigen.
