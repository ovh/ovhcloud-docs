...
- Disporre di un'istanza [Public Cloud](/guides/public-cloud/compute/getting-started) nel proprio account OVHcloud
...
:::info
Ogni tipo di volume è disponibile anche in versione cifrata (**LUKS**). Questi volumi garantiscono la confidenzialità dei dati. Sono disponibili nello Spazio Cliente OVHcloud e con gli strumenti indicati nella sezione seguente, specificando il tipo `<volume_type>-luks`.

:::

:::warning
**Impatto della cifratura (LUKS) sulle performance.** I volumi cifrati hanno un impatto sulle performance di circa il 10% rispetto a volumi equivalenti non cifrati.
