...
Una volta registrato il metodo di pagamento, verrà utilizzato per addebitare l'importo calcolato per la tua [fatturazione alla fine del mese](/guides/public-cloud/cross-functional/analyze-billing). Questo calcolo è effettuato in base al tempo di utilizzo di ogni risorsa, in funzione del prezzo delle risorse. Per le istanze, le tariffe sono generalmente indicate su base oraria per una maggiore leggibilità, ma la fatturazione è calcolata al secondo, con una durata minima di fatturazione di 1 minuto.
