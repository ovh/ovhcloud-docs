...
**Aggiunta di un'interfaccia di rete privata** <a id="adding-a-private-interface"></a>
...
### Ordine delle interfacce dopo un collegamento/scollegamento a caldo <a id="interface-order-after-reboot"></a>

:::warning
Collegare o scollegare interfacce di rete (porte) mentre l'istanza è in esecuzione (a caldo) può modificare l'ordine in cui il sistema operativo guest le rileva al riavvio successivo. L'interfaccia identificata come `eth0` (o qualsiasi altra interfaccia prevista, inclusa quella pubblica) può quindi cambiare. Verifica la nuova configurazione per evitare problemi di rete imprevisti.

Se devi riorganizzare più interfacce su un'istanza in esecuzione:

1. Scollega tutte le interfacce che desideri modificare. Scollegare l'interfaccia "Ext-Net" rilascia definitivamente il suo indirizzo IP pubblico, come indicato nella sezione [Rimozione di un'interfaccia di rete](#removing-a-private-interface).
2. Ricollegale una a una, nell'ordine in cui desideri che compaiano nel sistema operativo guest.
3. Effettua un *hard reboot* dell'istanza per verificare che l'ordine delle interfacce corrisponda a quello previsto prima di farci affidamento in produzione.

:::

### Rimozione di un'interfaccia di rete <a id="removing-a-private-interface"></a>
