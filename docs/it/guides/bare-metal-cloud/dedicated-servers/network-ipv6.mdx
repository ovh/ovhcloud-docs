...
A causa della riduzione di valore del ifcfg, NetworkManager non crea più i nuovi profili in questo formato per impostazione predefinita. Il file di configurazione è ora disponibile in `/etc/NetworkManager/system-connections/`.
  ...
  <Tab label="Windows Server 2019 e versioni successive">
    ...
    **Step 2: Apri la configurazione di rete del tuo server**
    ...
    Apri il menu Start di Windows, digita `ncpa.cpl`, quindi clicca sul risultato di ricerca suggerito.

    <img className="thumbnail" alt="Finestra Connessioni di rete nel pannello di controllo di Windows" src="/images/bare-metal-cloud/dedicated-servers/network-ipv6/ipv6_network_connections.png" loading="lazy" />
...
In ogni caso, non esitare a [contattare il nostro team di supporto](/links/support-contact) per richiedere una revisione delle configurazioni. Sarà necessario fornire:
