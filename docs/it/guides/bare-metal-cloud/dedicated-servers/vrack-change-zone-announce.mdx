...
Accedi alla pagina delle <ApiLink>API OVHcloud</ApiLink> e utilizza il comando qui sotto per modificare l’annuncio del blocco IP.

<Api version="v1" section="/vrack" method="GET" route={"/vrack"} />

Questa chiamata API consente di recuperare l'elenco di tutte le vRack create. Se i riferimenti mostrati non ti permettono di identificare la vRack, recupera l’informazione dallo <ManagerLink to="/">Spazio Cliente OVHcloud</ManagerLink>.
