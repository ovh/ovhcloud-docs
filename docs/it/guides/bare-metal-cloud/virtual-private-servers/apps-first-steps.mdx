...
:::info
Se il link è già scaduto, connettiti al tuo VPS via SSH con il nome utente ed esegui il comando `sudo whmlogin` per generarne uno nuovo.
:::
...
WHM utilizza di default l'utente root per l'autenticazione. Il link monouso presente nell'e-mail consente di accedere alla configurazione iniziale e di impostare la password di root. Agli accessi successivi a WHM, inserisci `root` come nome utente e la password che hai definito.
