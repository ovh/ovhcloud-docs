...
Click on the <code className="action">DNS zone</code> tab, then, in the table that appears, identify the row with the value `A` as its type. Open the actions menu <code className="action">⋮</code> on that row and click <code className="action">Edit the entry</code>.
