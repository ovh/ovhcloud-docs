---
pageType: overview
title: Server Privati Virtuali
description: "Distribuisci, configura e gestisci il tuo VPS OVHcloud - installazione, networking, backup e amministrazione quotidiana dei tuoi server privati virtuali."
text: "Scopri le guide Server Privati Virtuali"
essentialsTitle: "Le guide essenziali per iniziare"
essentials:
  - title: Prime attivazione di un VPS
    link: /guides/bare-metal-cloud/virtual-private-servers/starting-with-a-vps
  - title: "VPS - Gestione tramite lo Spazio Cliente OVHcloud"
    link: /guides/bare-metal-cloud/virtual-private-servers/understand-vps-control-panel
  - title: FAQ su VPS OVHcloud
    link: /guides/bare-metal-cloud/virtual-private-servers/vps-faq
  - title: "Configurazione degli account utente e dell'accesso root su un server"
    link: /guides/bare-metal-cloud/dedicated-servers/changing-root-password-linux-ds
  - title: Come eseguire le prime operazioni sulle connessioni SSH
    link: /guides/bare-metal-cloud/dedicated-servers/ssh-introduction
  - title: "Come creare e utilizzare chiavi di autenticazione per le connessioni SSH ai server OVHcloud"
    link: /guides/bare-metal-cloud/dedicated-servers/creating-ssh-keys
tutorials:
  - title: "Tutorial - Come utilizzare PuTTY per le connessioni SSH e l'autenticazione"
    link: /guides/web-cloud/web-hosting/ssh-using-putty-on-windows
  - title: Come utilizzare SFTP per trasferire i file
    link: /guides/bare-metal-cloud/dedicated-servers/transfer-data-via-sftp
  - title: "Recupero dei database in modalità Rescue"
    link: /guides/bare-metal-cloud/dedicated-servers/restore-bdd-rescue
goFurther:
  title: Per approfondire
  items:
    - title: Visita la community OVHcloud
      link: https://community.ovh.com/
    - title: "Consulta la roadmap e il changelog"
      link: https://www.ovhcloud.com/it/roadmap-changelog/
    - title: Unisciti a Discord
      link: https://discord.gg/ovhcloud
cta:
  title: "Hai domande? Accedi al supporto OVHcloud"
  description: "Trova la risposta alle tue domande nel nostro centro assistenza."
  actions:
    - text: Accedi al supporto
      link: https://help.ovhcloud.com/
      theme: brand
---
