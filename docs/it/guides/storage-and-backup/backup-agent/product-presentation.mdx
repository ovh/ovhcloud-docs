...
Il prodotto Backup Agent si basa su due prodotti dell'editore di software Veeam:
...
Una volta che l'agente ha effettuato dei backup, potrete ripristinare i vostri dati in due modi: ripristino dei file (FLR) o ripristino del server.

## Punti chiave

Numerosi punti di forza sono presenti in questa offerta:

- Prima politica di backup automatico con 30 giorni di conservazione.
- Possibilità di estendere la conservazione fino a 365 giorni.
...
Vi invitiamo a leggere le nostre altre guide per scoprire il prodotto.
