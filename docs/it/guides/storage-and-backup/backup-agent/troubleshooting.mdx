...
Puoi trovare i log del tuo Backup Agent nella cartella: `C:\ProgramData\Veeam\` o `/var/log`.
...
Devi scaricare l'agente dal tuo <ManagerLink to="/#/bmc-backup-agent-baremetal/service">Backup Agent</ManagerLink> e installarlo sul tuo nuovo sistema operativo.
...
<Tabs>
  <Tab label="Windows">
    ...
    **Esportare i log**
    ...
    2. Vai al menu <code className="action">Help</code> > <code className="action">Export Logs</code>.
    3. Seleziona la directory di destinazione per l'archivio dei log.
    4. Clicca su <code className="action">Export</code> per generare l'archivio.
