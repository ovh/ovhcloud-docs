...
<Tabs>
  <Tab label="Tramite l'API">
    ...
    :::info
    Nel campo `resources`, sostituisci il valore `urn` con l'URN del tuo servizio Enterprise File Storage (EFS).

    Utilizza la seguente chiamata API per recuperare l'URN del tuo servizio: <Api version="v2" section="/iam" method="GET" route={"/iam/resource"} />

    Utilizza il parametro di query `resourceName={serviceName}` con l'identificativo del tuo servizio Enterprise File Storage.

    Nella risposta dell'API, copia il valore `urn`. Ad esempio: `urn:v1:eu:resource:storageNetApp:xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx`.
    :::
  ...
  <Tab label="Tramite la CLI">
    ...
    Esegui il seguente comando per recuperare l'URN del tuo servizio:

    ```bash
    ovhcloud iam resource list --filter 'name=="{serviceName}"' -o json
    ```

    Sostituisci `{serviceName}` con l'identificativo del tuo servizio Enterprise File Storage.

    Nell'output del comando, copia il valore `urn`. Ad esempio: `urn:v1:eu:resource:storageNetApp:xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx`.
    :::
...
[Gestione degli account di servizio OVHcloud (EN)](/guides/manage-and-operate/api/manage-service-account)
