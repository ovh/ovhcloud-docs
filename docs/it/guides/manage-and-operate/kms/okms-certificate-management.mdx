...
Per comunicare con il dominio OKMS, è necessario un certificato di accesso. È uno dei tre [metodi di autenticazione OKMS](/guides/manage-and-operate/kms/okms-authentification-method); i certificati sono **obbligatori per KMIP** e consigliati quando è richiesta l'autenticazione client mTLS. Per l'utilizzo dell'API REST, un Personal Access Token (PAT) o un service account è generalmente più semplice da configurare.
