...
Dopo l'incapsulamento con la chiave pubblica di destinazione, invia nella richiesta di importazione il `ciphertext` JWE Compact Serialization ottenuto. Per un esempio in Go, consulta [Esempio completo in Go](#esempio-completo-in-go).
...
### Esempio completo in Go

Gli esempi seguenti eseguono entrambe le cerimonie con una chiave RSA, utilizzando l'[SDK Go](https://pkg.go.dev/github.com/ovh/okms-sdk-go) per le chiamate a OVHcloud KMS e [go-jose](https://pkg.go.dev/github.com/go-jose/go-jose/v4) per le operazioni JWE dall'altro lato. Entrambi gli esempi codificano il materiale di chiave in PKCS#8 (DER), che corrisponde al `keyFormatType` `PKCS8`.

#### Importare una chiave generata localmente

La libreria standard di Go genera la chiave, che il programma incapsula localmente con la chiave di trasporto pubblica e poi importa. Il suo testo in chiaro non raggiunge mai OVHcloud KMS.

```go
// 1. Crea la chiave di trasporto su OVHcloud KMS e recupera la sua parte pubblica.
...
// 2. Genera la chiave da importare e codificala in PKCS#8 (DER).
...
// 3. Incapsula il materiale di chiave come JWE: incapsulamento della chiave RSA-OAEP-256, cifratura del contenuto A256GCM.
...
// 4. Importa la chiave incapsulata. Il KMS la disincapsula e deduce il tipo e la dimensione dal materiale.
...
La cifratura del contenuto JWE (`enc`) deve essere `A256GCM`.

#### Esportare una chiave e decifrare il JWE

La destinazione (qui, il programma locale) genera la coppia di chiavi di trasporto e importa solo la sua parte pubblica in OVHcloud KMS. La destinazione decifra poi il JWE esportato con la parte privata.

```go
// 1. Genera la coppia di chiavi di trasporto sulla destinazione.
destKey, err := rsa.GenerateKey(rand.Reader, 4096)
if err != nil {
  return err
}

// 2. Importa solo la sua parte pubblica in OVHcloud KMS, con l'utilizzo wrapKey.
...
// 3. Rendi la chiave estraibile, poi esportala incapsulata dalla chiave pubblica di destinazione.
extractable := true
...
// 4. Decifra il JWE con la chiave privata di destinazione e analizza il materiale PKCS#8.
