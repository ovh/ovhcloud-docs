...
Crea una chiave RSA dedicata all'incapsulamento. Imposta sempre sia `wrapKey` sia `unwrapKey`. `wrapKey` e `unwrapKey` sono mutuamente esclusive con `sign` e `verify`.
...
Chiama `GET /v1/servicekey/{keyId}` con i parametri di query di incapsulamento seguenti. Quando questi parametri sono presenti, l'API restituisce `wrappedKeys` invece di `keys` in chiaro.
