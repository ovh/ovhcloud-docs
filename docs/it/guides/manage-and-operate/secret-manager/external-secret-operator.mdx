...
Il campo `stringData` accetta il token non codificato — Kubernetes lo codifica in base64 al momento della memorizzazione del secret. Se preferisci utilizzare il campo `data`, devi prima codificare tu stesso il token in base64 (`echo -n "<token>" | base64`).

Applicalo con il comando `kubectl apply -f secret.yaml`.

Oppure, se il `user_pat` è memorizzato nella variabile d'ambiente `PAT_TOKEN`, crea il secret direttamente con kubectl, che si occupa anche della codifica:
...
Quando esamini il secret con `kubectl get secret ovhcloud-vault-token -n external-secrets -o yaml`, il valore compare sotto `data`, codificato in base64.
