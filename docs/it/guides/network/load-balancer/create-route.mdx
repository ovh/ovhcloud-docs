...
Route management is only available through the <ApiLink>OVHcloud API</ApiLink>. It is only valid for the **http** and **tcp** protocols, and the path `/ipLoadbalancing/{serviceName}/{protocol}/route/` exposes the dedicated route API.
