...
| Limit | Value |
| :--- | :--- |
| Region | The OWAF is available only in the **GRA** (Gravelines) region |
...
4. Optionally, adjust the **Anomaly threshold**. This is the score at which a request is blocked. Each matching rule adds its score to the request's total (5 by default); if the total reaches or exceeds the threshold, the request is blocked. A lower value is stricter; a higher value is more permissive.
