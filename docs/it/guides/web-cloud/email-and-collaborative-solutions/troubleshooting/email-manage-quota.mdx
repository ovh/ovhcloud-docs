...
<Tabs>
  ...
  <Tab label="Roundcube: MX Plan" availableIn={['eu']}>
    Quando sei connesso alla Webmail Roundcube, la quota è visibile nella parte inferiore sinistra, materializzata da un diagramma circolare e la percentuale consumata.<br/><br/>
...
Nella scheda <code className="action">Account email</code>, clicca sul pulsante <code className="action">...</code>a destra dell'account da modificare e poi clicca su <code className="action">Modifica</code>.
  ...
  <Tab label="Email Pro" availableIn={['eu']}>
    L'offerta Email Pro dispone di una capacità unica di 15 GB. Se hai bisogno di maggiore spazio di storage, è necessario passare a un'offerta con maggiore spazio. Per farlo, leggi la sezione [cambiare offerta per aumentare la capacità](#switchingoffer) di questa guida.<br/>
  </Tab>
  <Tab label="Exchange" availableIn={['eu', 'ca']}>
