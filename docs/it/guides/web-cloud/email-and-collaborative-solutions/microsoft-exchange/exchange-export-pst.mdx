...
description: Esporta una casella email Exchange in formato PST dallo Spazio Cliente o dall'API OVHcloud, poi verifica e apri l'archivio
...
Questa funzionalità è disponibile per <Region zones={['eu']}>Hosted Exchange, Private Exchange e Trusted Exchange</Region><Region excludeZones={['eu']}>Hosted Exchange</Region>. Si applica alle singole caselle email: i gruppi, gli account condivisi e gli account risorsa non possono essere esportati.
...
- Disporre di un servizio <Region zones={['eu']}>[Hosted Exchange](/links/web/emails-hosted-exchange), [Private Exchange](/links/web/emails-private-exchange) o [Trusted Exchange](/links/web/emails-trusted-exchange)</Region><Region excludeZones={['eu']}>[Hosted Exchange](/links/web/emails-hosted-exchange)</Region>.
...
- Per il metodo API: disporre di credenziali API create nella <CreateToken rights="GET=/email/exchange/*&POST=/email/exchange/*&DELETE=/email/exchange/*">pagina di generazione dei token OVHcloud</CreateToken>, con almeno i diritti `GET`, `POST` e `DELETE` su `/email/exchange/*`.
...
Il processo si basa su 2 oggetti: `export` genera l'archivio e ne riporta l'avanzamento, mentre `exportURL` fornisce un URL di download temporaneo. Tutte le chiamate seguenti appartengono alla <ApiLink section="/email/exchange">sezione `/email/exchange`</ApiLink> dell'API OVHcloud.
...
Per aprire i file PST in Outlook classico per Windows, vai su <code className="action">File</code> > <code className="action">Apri ed esporta</code> > <code className="action">Apri file di dati di Outlook</code>, poi seleziona ogni file `.pst` estratto. Ad oggi, il nuovo Outlook per Windows non apre i file `.pst`.
