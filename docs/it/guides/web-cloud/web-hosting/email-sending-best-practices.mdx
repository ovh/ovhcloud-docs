...
Entrambi i metodi inviano email dal tuo sito. Si differenziano per il server da cui parte l'email, per il modo in cui l'invio è limitato e per il modo in cui puoi monitorarlo.
...
| **Identità del mittente** | L'indirizzo `From:` è dichiarato dallo script. Verifica che il tuo record SPF autorizzi i server OVHcloud. | L'indirizzo `From:` deve corrispondere all'account autenticato. L'email è firmata DKIM se DKIM è attivato per il tuo dominio. |
