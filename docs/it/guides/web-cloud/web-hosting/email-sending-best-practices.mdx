...
| **Quota di invio** | Per hosting e per ora, in base alla tua offerta. Consulta le [Specifiche tecniche degli hosting web](/guides/web-cloud/web-hosting/hosting-technical-specificities). | Per account email e per ora (circa 200 email per un account MX Plan), più **300 email all'ora per indirizzo IP di origine** sul server SMTP OVHcloud. Su un hosting condiviso, l'indirizzo IP in uscita è condiviso con gli altri siti dello stesso cluster. |
...
In tutti i casi, i parametri SMTP sono identici: server `smtp.mail.ovh.ca`, porta `465` (SSL/TLS) o `587` (STARTTLS), con autenticazione tramite l'indirizzo email completo e la relativa password.
