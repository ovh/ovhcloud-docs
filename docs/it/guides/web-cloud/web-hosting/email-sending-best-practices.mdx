...
| **Identità del mittente** | L'indirizzo `From:` è dichiarato dallo script. Verifica che il tuo record SPF autorizzi i server OVHcloud. | L'indirizzo `From:` deve corrispondere all'account autenticato. L'email è firmata <Tooltip term="dkim">DKIM</Tooltip> se DKIM è attivato per il tuo dominio. |
