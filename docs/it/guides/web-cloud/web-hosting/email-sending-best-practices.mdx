...
description: Scegli tra la funzione PHP mail() e l'invio SMTP autenticato per inviare email dal tuo hosting web e configura l'SMTP con il tuo account email OVHcloud
...
Il tuo hosting web offre due modi per inviare email dal tuo sito: la funzione PHP `mail()` nativa, che utilizza i server di invio del tuo hosting, e l'invio **SMTP autenticato** tramite un account email associato al tuo dominio. Entrambi sono legittimi. `mail()` è il metodo predefinito: non richiede alcuna configurazione, è soggetto a una quota per hosting e può essere monitorato dal tuo Spazio Cliente OVHcloud. L'SMTP autenticato è l'alternativa quando devi controllare l'identità del mittente: l'indirizzo `From:` è legato a un account autenticato e l'email porta la firma del tuo dominio. Le sue quote si applicano per account email e per indirizzo IP di origine, che su un hosting condiviso è comune a più siti.
...
- i metodi di monitoraggio delle email.

| | PHP `mail()` | SMTP autenticato |
|---|---|---|
| **Funzionamento** | Lo script affida l'email ai server di invio del tuo hosting. Non è necessaria alcuna configurazione. | Lo script si connette al server di posta OVHcloud con il nome utente e la password di un account email associato al tuo dominio. |
| **Quota di invio** | Per hosting e per ora, in base alla tua offerta. Consulta le [Specifiche tecniche degli hosting web](/guides/web-cloud/web-hosting/hosting-technical-specificities). | Per account email e per ora (circa 200 email per un account MX Plan), più **300 email all'ora per indirizzo IP di origine** sul server SMTP OVHcloud. Su un hosting condiviso, l'indirizzo IP in uscita è condiviso con gli altri siti del cluster. |
| **Monitoraggio** | Dalla scheda <code className="action">Script email</code> del tuo hosting nello Spazio Cliente OVHcloud: statistiche, report degli errori, blocco ed eliminazione. Consulta la guida [Gestire l'invio delle email automatiche](/guides/web-cloud/web-hosting/mail-function-script-records). | Non visibile nello Spazio Cliente OVHcloud. Monitora gli invii dai log della tua applicazione. |
| **Identità del mittente** | L'indirizzo `From:` è dichiarato dallo script. Il suo dominio deve essere associato al tuo hosting (multisito), altrimenti l'email non può essere autenticata. | L'indirizzo `From:` deve corrispondere all'account autenticato. L'email è firmata <Tooltip term="dkim">DKIM</Tooltip> se DKIM è attivato per il tuo dominio. |
| **Uso tipico** | Moduli di contatto, notifiche, qualsiasi script il cui volume rientra nella quota dell'hosting. | CMS con più plugin che inviano email, applicazioni che richiedono un mittente fisso e firmato, o indirizzo `From:` condiviso con la tua casella email. |

Qualunque sia il metodo scelto:

- I server destinatari (Gmail, Outlook, ecc.) verificano che il dominio del `From:` autorizzi il server di invio. Mantieni corretti i tuoi record [SPF](/guides/web-cloud/domains/dns-zone-spf), [DKIM](/guides/web-cloud/domains/dns-zone-dkim) e [DMARC](/guides/web-cloud/domains/dns-zone-dmarc) nella tua zona DNS.
- Un invio abusivo o di spam dal tuo hosting o dal tuo account email comporta un blocco automatico. Se le tue email non partono più, consulta la guida [Gestire l'invio delle email automatiche](/guides/web-cloud/web-hosting/mail-function-script-records).
...
- **SPF e DKIM**: l'email viene inviata da un server autorizzato per il tuo dominio e firmata DKIM se [DKIM è attivato](/guides/web-cloud/domains/dns-zone-dkim) per il tuo dominio.
...
Questo dominio non deve necessariamente essere gestito nel tuo account OVHcloud: puoi utilizzare un dominio registrato presso un altro registrar, purché sia associato al tuo hosting web. È questa associazione che attesta che sei autorizzato a inviare email a nome di quel dominio.

:::tip
Se il tuo dominio non è ancora associato al tuo hosting, aggiungilo come multisito dal tuo Spazio Cliente OVHcloud, poi verifica la configurazione DNS associata. Consulta la guida [Ospitare più siti su uno stesso hosting](/guides/web-cloud/web-hosting/multisites-configure-multisite).
:::

:::warning
Un invio effettuato per un dominio non associato al tuo hosting non può essere autenticato: presenta le caratteristiche di un invio non autenticato e sarà quindi rifiutato e registrato come errore.
:::
...
La funzione `wp_mail()` di WordPress utilizza in background la funzione PHP `mail()`, con un indirizzo `From:` definito da WordPress o da ciascun plugin. Gli invii rientrano quindi nella quota del tuo hosting e possono essere monitorati dallo Spazio Cliente OVHcloud.

Se desideri un mittente fisso e autenticato per tutte le email del tuo sito, installa un **plugin SMTP** che sostituisce `wp_mail()` con una connessione SMTP autenticata verso il tuo server di posta OVHcloud.
...
[FluentSMTP](https://it.wordpress.org/plugins/fluent-smtp/) è un'alternativa leggera e gratuita. La quota di invio del tuo account email si applica allo stesso modo.
