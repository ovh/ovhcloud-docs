...
- **Assicurati che il tuo script utilizzi effettivamente la funzione "mail()"**: le verifiche e le statistiche di questa pagina riguardano solo le email inviate tramite "mail()". Se il tuo script, o i parametri di invio del tuo sito web, specificano un server SMTP, le email passano da quel server e non compaiono qui. Consulta la [sezione SMTP](#SMTP) di questa guida.
