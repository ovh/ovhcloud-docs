...
<Tabs>
  ...
  <Tab label="NS">
    **N**ame **S**erver
    ...
    Definisci i server DNS associati alla tua zona DNS. Ad esempio, se i record NS della tua zona DNS visualizzano i server *dnsXX.ovh.ca* e *nsXX.ovh.ca*, sarà necessario utilizzarli nella scheda <code className="action">Server DNS</code> del tuo Spazio Cliente OVHcloud. Per maggiori informazioni, consulta la nostra guida [Modificare i server DNS di un nome di dominio OVHcloud](/guides/web-cloud/domains/dns-server-edit).
    </Region>
...
<Tabs>
  ...
  <Tab label="TLSA">
    **T**ransport **L**ayer **S**ecurity **A**uthentication
    ...
    Utilizzato per inserire l'impronta di un certificato SSL/TLS. Che permette di conservare l'*hash* di un certificato direttamente nella zona DNS del nome di dominio tramite un record DNS.
