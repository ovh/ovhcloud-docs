...
Quando configurate il vostro nome a dominio su una soluzione email OVHcloud, la configurazione automatica del DKIM è proposta e realizzata per default se non la disattivate.

Se il DKIM non è stato attivato quando avete aggiunto un nome a dominio alla vostra piattaforma email, dovrete avviare il processo di configurazione automatica tramite lo spazio client.
...
<ZoneTabs>
  <Tab label="Email (MX Plan)" availableIn={['eu']}>
    ...
    Nel caso in cui il vostro nome a dominio non sia gestito nello stesso spazio client OVHcloud della vostra piattaforma email o registrato fuori da OVHcloud, otterrete la finestra qui sotto:
    ...
    Questa vi invita a inserire due valori CNAME nella zona DNS del nome a dominio, il che permette di collegare questo nome a dominio ai selettori DKIM del vostro servizio email. È necessario inserire questi valori e assicurarvi che siano propagati prima di cliccare su <code className="action">Attivare</code>.
  </Tab>
  ...
  <Tab label="Zimbra" availableIn={['eu']}>
    ...
    **Suggerimento per creare un record CNAME**
    ...
    Dallo <ManagerLink to="/">Spazio Cliente OVHcloud</ManagerLink> in cui è ospitato il nome di dominio del tuo servizio email, nella sezione <code className="action">Web Cloud</code>, clicca su <code className="action">Domini</code> nella colonna di sinistra e seleziona il nome di dominio interessato.

    Apri la scheda <code className="action">Zona DNS</code>, poi clicca su <code className="action">Aggiungi una voce</code> sopra la tabella. Nel modulo che si apre, seleziona il tipo `CNAME` e completa i campi in base ai valori annotati.
    :::
  </Tab>
</ZoneTabs>
...
Per configurare il DKIM, recatevi sulla <ApiLink>pagina delle API OVHcloud</ApiLink> e accedete:
...
<Region zones={['eu']}>
Recatevi nella sezione `/email/domain/` (offerte MX Plan e Zimbra), `/email/exchange` (offerta Exchange) o `/email/pro` (offerta Email Pro) delle API e digitate "dkim" nel campo `Filter` per far apparire solo le funzioni API relative al DKIM.
</Region>
...
<Tabs>
  ...
  <Tab label="4. Configura record DNS">
    ...
    <img className="thumbnail" alt="Aggiunta di un record CNAME dalla zona DNS" src="/images/assets/screens/control-panel/product-selection/web-cloud/domain-dns/dns-zone/nx-cname-form.png" loading="lazy" />

    Una volta inseriti i valori, clicca su <code className="action">Aggiungi</code>.
...
<ZoneTabs>
  <Tab label="MX Plan e Zimbra" availableIn={['eu']}>
    ...
    - `error`: il processo di installazione ha riscontrato un errore. Ti invitiamo ad aprire un [ticket al supporto](/links/support-contact) e a specificare il dominio in questione.<br/>
...
Dallo <ManagerLink to="/">Spazio Cliente OVHcloud</ManagerLink>, apri la sezione <code className="action">Web Cloud</code>, clicca su <code className="action">Domini</code>, poi seleziona il nome di dominio interessato.

Apri la scheda <code className="action">Zona DNS</code>, poi clicca su <code className="action">Aggiungi una voce</code> sopra la tabella. Per configurare il DKIM nella tua zona DNS, è possibile aggiungere un record in 3 modi:
...
Questo record è chiamato DKIM sull'interfaccia ma in realtà è un record TXT nella zona. Il record DKIM ha lo scopo di facilitare la lettura dei diversi parametri del DKIM presentandoli sotto forma di campi indipendenti.

<img className="thumbnail" alt="Aggiunta di un record DKIM dalla zona DNS" src="/images/assets/screens/control-panel/product-selection/web-cloud/domain-dns/dns-zone/nx-dkim-form.png" loading="lazy" />

- **Selettore** (Sottodominio): inserisci il nome del selettore DKIM fornito dal tuo servizio di posta. Il suffisso `._domainkey` viene aggiunto automaticamente e il tuo nome di dominio viene aggiunto alla fine.
...
- **Chiave pubblica (base64)**: incolla la chiave pubblica fornita dal tuo servizio di posta. È la chiave che i destinatari utilizzano per verificare la tua firma DKIM.
- **Tipo di chiave**: l'algoritmo di firma della chiave, `RSA` (predefinito) o `ed25519`.
- **Modalità di test**: lasciala su `Disattivato` in produzione. Quando è attivata, i destinatari trattano i messaggi di test firmati con DKIM come messaggi non firmati, così una verifica non superata non incide sulla consegna mentre convalidi la tua configurazione.
...
<Tabs>
  ...
  <Tab label="502">
    ...
    Si è verificato un errore durante la configurazione del DKIM. Oltre le 24 ore, se la configurazione è ancora in questo stato, ti invitiamo ad aprire un [ticket al supporto](/links/support-contact).
  </Tab>
  <Tab label="503">
...
<ZoneTabs>
  ...
  <Tab label="MX Plan e Zimbra" availableIn={['eu']}>
    ...
    - `error`: il processo di installazione ha riscontrato un errore. Ti invitiamo ad aprire un [ticket presso il supporto](/links/support-contact) precisando il dominio in questione.
