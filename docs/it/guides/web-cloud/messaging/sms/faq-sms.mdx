...
1. **Crea le chiavi API** dalla <CreateToken>pagina di creazione dei token API di OVHcloud</CreateToken> autorizzando gli endpoint `/sms/*`.
...
L'API restituisce un identificativo di job per monitorare lo stato dell'invio. Sono disponibili anche librerie in Python, Node.js, Java e C#. La documentazione completa dell'API SMS è disponibile sulla <ApiLink section="/sms">console API OVHcloud</ApiLink>.
...
La documentazione interattiva completa è disponibile sulla <ApiLink section="/sms">console API OVHcloud</ApiLink>. L'autenticazione avviene tramite chiavi API (Application Key, Application Secret, Consumer Key).
