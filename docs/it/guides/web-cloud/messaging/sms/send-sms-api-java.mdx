...
Crea i tuoi identificativi di Script (tutte le chiavi in una sola volta) su questa pagina: <CreateToken rights="GET=/sms/&GET=/sms/*/jobs&POST=/sms/*/jobs">pagina di creazione dei token OVHcloud</CreateToken> (questo URL permette di avere automaticamente le autorizzazioni per i passaggi descritti in questa guida).
