...
<CreateToken rights="GET=/sms&GET=/sms/*&GET=/sms/*/jobs&POST=/sms/*/jobs">pagina di creazione dei token OVHcloud</CreateToken> (questo URL permette di avere automaticamente le autorizzazioni per i passaggi descritti in questa guida).
...
La <ApiLink section="/sms" regions={["eu"]}>console di API</ApiLink> ti permetterà di scoprire altri metodi per facilitare l’integrazione di servizi SMS quali: SMS che permettono la risposta (unicamente per gli account OVHcloud in Francia), invio massivo con file CSV, mailing o con conferma di ricezione...
