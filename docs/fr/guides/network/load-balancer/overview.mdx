---
pageType: overview
title: Load Balancer
description: "Répartissez le trafic sur votre infrastructure OVHcloud avec Load Balancer - provisionnement, certificats, listeners, fermes et règles de routage."
text: "Explorez les guides Load Balancer"
essentialsTitle: "Les guides indispensables pour bien débuter"
essentials:
  - title: "Gestion du service OVHcloud Load Balancer via l'espace client"
    link: /guides/network/load-balancer/use-lb
  - title: "Commande d'un certificat SSL gratuit"
    link: /guides/network/load-balancer/order-freecertificate
  - title: "Fonctionnalités principales de l'API Load Balancer"
    link: /guides/network/load-balancer/use-api-reference
  - title: "Transfert des logs (Log Forwarding) TCP / HTTP / HTTPS du Load Balancer OVHcloud"
    link: /guides/network/load-balancer/use-api-logs-customers
  - title: "Modes de répartition"
    link: /guides/network/load-balancer/create-balancing
goFurther:
  title: Aller plus loin
  items:
    - title: Visitez la communauté OVHcloud
      link: https://community.ovhcloud.com/
    - title: Consultez la roadmap & changelog
      link: https://www.ovhcloud.com/fr/roadmap-changelog/
    - title: Rejoignez Discord
      link: https://discord.gg/ovhcloud
cta:
  title: "Des questions ? Accédez au support OVHcloud"
  description: "Trouvez la réponse à vos questions dans notre centre d'aide."
  actions:
    - text: Accéder au support
      link: https://help.ovhcloud.com/csm/fr-home
      theme: brand
---
