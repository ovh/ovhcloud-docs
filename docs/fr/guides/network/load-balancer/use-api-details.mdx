...
Toutes les fonctions d'API qui sont utilisées dans la section */ipLoadbalancing* sont disponibles sur <ApiLink>cette page</ApiLink>.
...
#### Lister les zones disponibles à la commande pour le Load Balancer OVHcloud

<Api version="v1" section="/order" method="GET" route={"/order/catalog/public/ipLoadbalancing"} />


| Parameter | Required | Description |
|-----------|----------|-------------|
| `ovhSubsidiary` | Yes | Le code de la filiale OVHcloud (pays ou région) dont vous souhaitez consulter le catalogue, par exemple : `FR` (chaîne de caractères) |

Les valeurs acceptées, ainsi que la devise des tarifs renvoyés pour chacune, dépendent de votre zone commerciale :

<Tabs noSync>
  <Tab label="Europe / Afrique">

| Valeur | Pays ou région | Devise |
|--------|----------------|--------|
| `CZ` | République tchèque | EUR |
| `DE` | Allemagne | EUR |
| `ES` | Espagne | EUR |
| `FI` | Finlande | EUR |
| `FR` | France | EUR |
...
:::info
La valeur `EU` figure dans l'énumération de ce paramètre du schéma de l'API, mais elle n'est pas acceptée par cet appel.
:::

:::info
Les valeurs `CZ`, `FI` et `LT` retournent le catalogue de la filiale irlandaise (`IE`), qui propose moins de zones.
:::

  </Tab>
  <Tab label="Canada / Asie-Pacifique">
...
| `QC` | Québec | CAD |
| `SG` | Singapour | SGD |
| `WE` | Reste du monde (anglais) | USD |
| `WS` | Reste du monde (espagnol) | USD |

  </Tab>
</Tabs>

Les zones disponibles à la commande figurent dans la famille d'options `zone` de chaque offre (`plans[].addonFamilies`), par exemple `iplb-zone-lb1-gra`.
...
#### Lister les zones du service Load Balancer OVHcloud
...
#### Obtenir le statut global d'un service Load Balancer OVHcloud
