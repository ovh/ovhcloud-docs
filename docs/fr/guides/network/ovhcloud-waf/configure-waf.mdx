...
| Limite | Valeur |
| :--- | :--- |
| Région | L'OWAF est disponible uniquement dans la région **GRA** (Gravelines) |
...
4. Ajustez éventuellement le champ **Anomaly threshold**. Il s'agit du score à partir duquel une requête est bloquée. Chaque règle correspondante ajoute son score au total de la requête (5 par défaut) ; si le total atteint ou dépasse le seuil, la requête est bloquée. Une valeur plus basse est plus stricte ; une valeur plus élevée est plus permissive.
