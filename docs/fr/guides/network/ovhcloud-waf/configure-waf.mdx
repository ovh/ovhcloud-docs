...
| **Répondre aux obligations de conformité** | Démontrez le déploiement d'un WAF à vos auditeurs PCI-DSS, ISO 27001, NIS2 ou DORA grâce à un journal d'audit complet et exportable. |
...
- **Application immédiate** — les modifications de configuration sont appliquées immédiatement, sans redémarrage.
...
- **General Information** — voyez où se situe le WAF et où il transmet le trafic, pour y faire pointer votre domaine.
...
La colonne **Action** indique ce que fait le WAF lorsqu'une règle correspond, en tenant compte du mode d'application actuel :
...
Pendant l'alpha, l'action d'une règle est fixée par le jeu de règles géré et ne peut pas être modifiée individuellement depuis la console. Pour chaque règle, vous pouvez uniquement l'activer ou la désactiver (voir ci-dessous) ; le comportement d'application global (Detection, Blocking ou Disabled) se définit globalement dans l'onglet **Protection**.
