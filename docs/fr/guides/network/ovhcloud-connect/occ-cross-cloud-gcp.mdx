...
- Si vous transmettez la clé d'appairage via l'API : un accès aux <ApiLink>API OVHcloud</ApiLink>, avec des identifiants créés en suivant le guide « [Premiers pas avec les API OVHcloud](/guides/manage-and-operate/api/first-steps) »
...
Connectez-vous à votre <ManagerLink to="/">espace client OVHcloud</ManagerLink>. Dans la section <code className="action">Bare Metal Cloud</code>, ouvrez <code className="action">OVHcloud Connect</code> dans la rubrique <code className="action">Network</code> et sélectionnez votre service.
...
L'appel retourne une tâche.
  ...
  <Tab label="console >BETA_">

La clé d'appairage se saisit uniquement dans la console >BETA_. Ouvrez la <ManagerLink to="/beta/#/network/cloud-connect">console >BETA_</ManagerLink> et sélectionnez votre service.
...
| Suppression de la configuration PoP | La configuration PoP est supprimée lorsque vous dissociez la clé d'appairage (<code className="action">Dissocier la clé</code> dans la console >BETA_) ou lorsque vous résiliez la connexion. La dissociation du vRack retire également la clé. Le service n'est pas résilié et peut être associé de nouveau à Google. |
...
| Certaines routes ne sont pas apprises | Votre réseau annonce plus de `100` préfixes par session BGP. | Réduisez le nombre de préfixes annoncés. Si votre architecture en exige davantage, contactez notre [équipe du support](/links/support-contact). |
