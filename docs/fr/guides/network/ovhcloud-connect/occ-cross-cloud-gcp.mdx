...
| ASN OVHcloud | `16276` | ASN présenté par OVHcloud sur le rattachement Google. |
...
3. La configuration PoP de niveau 3 est appliquée automatiquement, avec l'ASN `16550` côté Google et l'ASN `16276` côté OVHcloud.
...
| ASN fixes | Le côté Google utilise l'ASN `16550` et le côté OVHcloud l'ASN `16276`. Ces valeurs sont définies par le processus de provisionnement et ne sont pas personnalisables. |
...
| Session BGP qui ne s'établit pas | Le rattachement est provisionné mais le routage n'a pas encore convergé, ou les paramètres du Cloud Router sont incomplets. | Vérifiez le statut du rattachement de VLAN et du Cloud Router dans Google Cloud. La session utilise l'ASN `16550` (Google) et `16276` (OVHcloud). |
