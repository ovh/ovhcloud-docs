...
Ce guide explique comment activer la transmission des logs de votre OVHcloud Connect vers Logs Data Platform (LDP), une plateforme qui vous permet de stocker, archiver, interroger et visualiser vos logs.
...
Maintenant que vos logs sont ingérés et stockés dans le data stream de votre Logs Data Platform, vous pouvez interroger vos logs et créer des tableaux de bord pour obtenir une représentation graphique de vos logs grâce à l’interface web de Graylog.
