...
2. **Configurez BGP sur les deux liens** avec les priorités de routes appropriées (en utilisant la Local Preference et l’AS-path prepending sur vos propres équipements) afin que le trafic privilégie un chemin tout en pouvant basculer sur l’autre.
...
- **MED (Multi-Exit Discriminator)** — **Non pris en charge** sur OVHcloud Connect. La sélection de chemin actif/passif repose sur l’AS-path prepending, qui s’applique uniquement côté client.
