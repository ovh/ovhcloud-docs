...
<summary>Puis-je porter un numéro spécial (SVA) vers OVHcloud ?</summary>

Oui. Un numéro spécial de Services à Valeur Ajoutée (SVA) peut être porté en numéro individuel comme en plage de numéros (SDA). Pour un numéro spécial français porté en numéro individuel, le RIO est obligatoire ; obtenez-le en appelant le 3179 depuis la ligne concernée ou en contactant votre opérateur actuel. Pour une plage de numéros, c'est un mandat de portabilité qui est requis : celui-ci vous est transmis par OVHcloud durant le processus. Dans tous les cas, la portabilité d'un numéro SVA suppose que votre identité ait été validée au préalable : tant que ce n'est pas le cas, l'espace client vous renvoie vers le formulaire de validation d'identité avant de pouvoir poursuivre la demande.

Pour plus de détails, consultez les guides « [Demander et suivre une portabilité de numéro](/guides/web-cloud/phone-and-fax/voip/demander-la-portabilite-de-mon-numero) » et « [Valider votre identité pour l'exploitation d'un numéro spécial SVA](/guides/web-cloud/phone-and-fax/voip/verification-identite-numeros-sva) ».
