...
Visual Studio Code vous demande de sélectionner le dossier que vous souhaitez télécharger sur votre hébergement web. Saisissez la valeur `name` précédemment définie dans le fichier « sftp.json ».
...
[Utiliser l’accès SSH de son hébergement web](/guides/web-cloud/web-hosting/ssh-on-webhosting). N’oubliez pas que pour utiliser le SSH, vous devez disposer d’une offre d’[hébergement web OVHcloud](/links/web/hosting) **Pro**, **Performance**, **Agency**, **Agency Plus** ou **Agency Max**.
