...
Les adresses IPv4 étant désormais moins disponibles, il est plus difficile d’ajouter de nouvelles machines sur le réseau Internet avec la norme IPv4. Toutefois, les connexions avec une adresse IPv6 sont utiles uniquement si, par exemple, votre site web est aussi disponible avec ce même protocole. Ainsi, plus il y aura de sites web accessibles en IPv6, plus les différents acteurs présents sur le réseau Internet basculeront leurs appareils/machines sur ce nouveau protocole.
...
Si la zone DNS active de votre nom de domaine est présente chez OVHcloud, utilisez nos guides « [Éditer une zone DNS OVHcloud](/guides/web-cloud/domains/dns-zone-edit) », puis « [Tout savoir sur les enregistrements DNS](/guides/web-cloud/domains/dns-zone-records) » pour créer une entrée DNS de type **AAAA**.
