...
Les deux méthodes envoient des e-mails depuis votre site web. Elles diffèrent par :
...
Ce domaine n’a pas besoin d’être géré au sein de votre compte OVHcloud : vous pouvez utiliser un domaine enregistré chez un autre bureau d’enregistrement, du moment qu’il est attaché à votre hébergement web. C’est cet attachement qui atteste que vous êtes autorisé à envoyer des e-mails au nom de ce domaine.
...
:::warning
Un envoi émis pour un domaine qui n’est pas attaché à votre hébergement ne peut pas être authentifié : il présente les caractéristiques d’un envoi non authentifié et sera donc rejeté et classé en erreur.
:::
...
Si vous souhaitez un expéditeur fixe et authentifié pour tous les e-mails de votre site, installez un **plugin SMTP** qui remplace `wp_mail()` par une connexion SMTP authentifiée vers votre serveur de messagerie OVHcloud.
