...
Votre hébergement web offre deux façons d’envoyer des e-mails depuis votre site : la fonction PHP `mail()` native, qui utilise les serveurs d’envoi de votre hébergement, et l’envoi **SMTP authentifié** via une adresse e-mail attachée à votre domaine. Les deux sont légitimes. `mail()` est la méthode par défaut : elle ne demande aucune configuration, elle est soumise à un quota par hébergement et elle se suit depuis votre espace client OVHcloud. Le SMTP authentifié est l’alternative lorsque vous devez maîtriser l’identité de l’expéditeur : l’adresse `From:` est liée à un compte authentifié et l’e-mail porte la signature de votre domaine. Ses quotas s’appliquent par adresse e-mail et par adresse IP source, partagée sur un hébergement mutualisé.

**Ce guide vous aide à choisir entre les deux méthodes et vous explique comment configurer un envoi SMTP authentifié depuis un hébergement mutualisé, que vous gériez un site WordPress, un autre CMS ou une application PHP.**
...
Les deux méthodes envoient des e-mails depuis votre site. Elles diffèrent par le serveur de départ de l’e-mail, par la façon dont l’envoi est limité et par la façon dont vous pouvez le suivre.
...
| **Suivi** | Depuis l’onglet <code className="action">Scripts e-mail</code> de votre hébergement dans l’espace client OVHcloud : statistiques, rapports d’erreurs, blocage et purge. Consultez le guide « [Suivre et gérer les e-mails automatisés de son hébergement web](/guides/web-cloud/web-hosting/mail-function-script-records) ». | Invisible dans l’espace client OVHcloud. Suivez les envois depuis les journaux de votre application. |
| **Identité de l’expéditeur** | L’adresse `From:` est déclarée par le script. Vérifiez que votre enregistrement SPF autorise les serveurs OVHcloud. | L’adresse `From:` doit correspondre au compte authentifié. L’e-mail est signé DKIM si DKIM est activé pour votre domaine. |
...
- Les serveurs destinataires (Gmail, Outlook, etc.) vérifient que le domaine du `From:` autorise le serveur d’envoi. Maintenez vos enregistrements [SPF](/guides/web-cloud/domains/dns-zone-spf), [DKIM](/guides/web-cloud/domains/dns-zone-dkim) et [DMARC](/guides/web-cloud/domains/dns-zone-dmarc) corrects dans votre zone DNS.
