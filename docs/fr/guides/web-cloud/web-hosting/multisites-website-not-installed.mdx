...
<Tabs>
  ...
  <Tab label="Étape 2">
    Cliquez sur le menu <code className="action">Noms de domaine</code>, choisissez le nom de domaine concerné, puis ouvrez l'onglet **Zone DNS**.
...
Cela signifie que votre nom de domaine n'est pas géré depuis votre espace client OVHcloud.<br/> Déterminez son « bureau d'enregistrement » ainsi que les serveurs DNS auxquels il est associé via notre outil [WHOIS](/links/web/domains-whois).<br/> Retrouvez et modifiez la zone DNS concernée en conséquence en suivant la rubrique dédiée du guide « [Partager son hébergement entre plusieurs sites - ajouter un nom de domaine externe](/guides/web-cloud/web-hosting/multisites-configure-multisite) ».
