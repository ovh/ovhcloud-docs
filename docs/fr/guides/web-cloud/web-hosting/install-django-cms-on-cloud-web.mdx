...
Lors de la configuration du moteur d’exécution Python (voir l’étape 1), nous avons défini le script de lancement de notre application comme étant « server.py ». Or Django CMS utilise un fichier nommé « manage.py ». Pour faire le lien entre Django CMS et votre moteur d’exécution, créez donc un lien symbolique entre « server.py » et « manage.py » :
