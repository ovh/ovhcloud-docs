...
description: "Gérer l'ensemble de vos sites WordPress depuis MainWP, inclus dans le Managed Hosting — mises à jour, actions groupées et surveillance centralisée."
...
<Tabs>
  <Tab label="Étape 1">
    Accédez à la page <ManagerLink to="/beta/#/web-cloud/wordpress">Managed hosting for WordPress</ManagerLink> de votre espace client OVHcloud et identifiez la ressource à gérer dans la liste affichée.
...
=======
    <img className="thumbnail" alt="Vue de la ressource Managed Hosting for WordPress dans la liste des hébergements WordPress" src="/images/guides/web-cloud/web-hosting/mainwp-site-management/managed-hosting-for-wordpress.png" loading="lazy" />
>>>>>>> develop
  </Tab>
  <Tab label="Étape 2">
    Cliquez sur <code className="action">Gérer</code> sous la ressource concernée. La page **Informations générales** s'affiche avec les détails de votre offre (abonnement, quotas, contacts).

    <img className="thumbnail" alt="Page Informations générales de la ressource avec le bouton Gérer mes sites en haut à droite" src="/images/guides/web-cloud/web-hosting/mainwp-site-management/mainwp-access.png" loading="lazy" />
  </Tab>
  <Tab label="Étape 3">
    Cliquez sur <code className="action">Gérer mes sites</code> en haut à droite. L'interface MainWP s'ouvre dans un nouvel onglet.
...
- **Ouvrir l'administration** : cliquez sur <code className="action">WP Admin</code> en face du site WordPress pour accéder directement à son tableau de bord WordPress.
...
- **Mettre à jour** : cliquez sur <code className="action">Mettre à jour</code> en face de chaque élément, ou <code className="action">Tout mettre à jour</code> pour appliquer toutes les mises à jour en une fois.

:::warning
Effectuez un backup ou un snapshot de votre site WordPress avant toute mise à jour majeure. Certaines mises à jour d'extensions peuvent provoquer des incompatibilités. Consultez le guide « [Sauvegarder ses sites web WordPress avec MainWP](/guides/web-cloud/web-hosting/mainwp-backup) » pour créer un point de restauration.
:::
...
- **Afficher la liste des sites WordPress** : depuis le tableau de bord MainWP, accédez à la section <code className="action">Sites</code>.
- **Sélectionner les sites WordPress cibles** : cochez les cases en face de chaque site WordPress à inclure dans l'action groupée. Utilisez <code className="action">Tout sélectionner</code> pour inclure l'ensemble de vos sites WordPress.
...
- **Lancer la mise à jour groupée** : cliquez sur <code className="action">Mises à jour</code> dans le menu d'actions groupées.
- **Choisir les composants à mettre à jour** : sélectionnez les éléments concernés (WordPress core, extensions, thèmes).
- **Appliquer** : cliquez sur <code className="action">Mettre à jour</code>. MainWP applique les mises à jour séquentiellement sur chaque site WordPress sélectionné.
...
- **Accéder à la gestion des extensions** : dans le menu MainWP, cliquez sur <code className="action">Extensions</code> (ou **Plugins**).
...
- **Appliquer** : cliquez sur <code className="action">Installer</code> ou <code className="action">Activer</code> selon le cas.
...
[Sauvegarder ses sites web WordPress avec MainWP](/guides/web-cloud/web-hosting/mainwp-backup)
