...
OVHcloud propose plusieurs types de certificats SSL sur les offres d’[hébergement mutualisé OVHcloud](/links/web/hosting). Ils sont présentés dans notre guide « [Hébergement web - Gérer un certificat SSL](/guides/web-cloud/web-hosting/ssl-on-webhosting) ». Les certificats SSL sont incontournables pour la sécurité de votre site web.
...
- `State or Province Name (full name) [Some-State]` : saisissez en majuscule le nom de votre région (ou de votre État si vous êtes, par exemple, aux USA).
...
Enregistrez ce fichier et conservez-le précieusement pour la suite de ce guide si votre fournisseur SSL vous le demande lors de votre future commande.
...
Si vous démarrez directement la lecture de ce guide à cette étape car vous disposez déjà d’un certificat SSL externe commandé auprès d’un fournisseur SSL, vérifiez que vous disposez bien uniquement des 3 fichiers suivants : `certificate.crt`, `private.key` et `ca_bundle.crt`. Si ce n’est pas le cas, consultez les informations [ci-dessus](#3files).
