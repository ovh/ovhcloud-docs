...
Repérez les enregistrements « CNAME » existants pour le sous-domaine « www » (exemple : `www.mydomain.ovh.`).
    ...
    **3 - Modification :** si un enregistrement « CNAME » existe pour le sous-domaine « www », ouvrez le menu d’actions <code className="action">⋮</code> sur la ligne concernée et cliquez sur <code className="action">Modifier l'entrée</code>. La ligne se déplie en un formulaire d’édition. Remplacez uniquement la **Cible** par `ext-cust.squarespace.com.`. Confirmez vos modifications.

    Si aucun enregistrement « CNAME » n’existe pour le sous-domaine « www », cliquez sur <code className="action">Ajouter une entrée</code> au-dessus du tableau, puis sélectionnez le type d’enregistrement DNS <code className="action">CNAME</code>, saisissez `www` dans le champ **Sous-domaine** et `ext-cust.squarespace.com.` dans le champ **Cible**. Cliquez sur <code className="action">Ajouter</code>.

    **4 - Ajout :** créez un enregistrement CNAME de vérification : au-dessus du tableau, cliquez sur <code className="action">Ajouter une entrée</code>, puis sélectionnez le type d’enregistrement DNS <code className="action">CNAME</code>. Saisissez votre `code unique récupéré chez SquareSpace` dans le champ **Sous-domaine**, puis `verify.squarespace.com.` dans le champ **Cible**. Cliquez sur <code className="action">Ajouter</code>.
  </Tab>
</Tabs>
