...
<Tabs>
  <Tab label="Étape 1">
    Dans la section `Web Cloud`, cliquez sur <ManagerLink to="/#/web-domains/domain">Noms de domaine</ManagerLink>, sélectionnez le nom de domaine que vous avez utilisé pour créer les hôtes lors de la [Partie 3](#step3), puis ouvrez l’onglet **Zone DNS**.
  ...
  <Tab label="Étape 3">
    Dans le formulaire qui s’ouvre, sélectionnez le type d’enregistrement DNS *A* ou *AAAA* en fonction du type d’IP associée que vous souhaitez ajouter.
  </Tab>
  <Tab label="Étape 4">
    Renseignez le *sous-domaine* et l’adresse *IPv4* (A) ou *IPv6* (AAAA), puis cliquez sur <code className="action">Ajouter</code>. Si nécessaire, reportez-vous aux instructions décrites dans notre documentation « [Éditer une zone DNS OVHcloud](/guides/web-cloud/domains/dns-zone-edit) ».
  </Tab>
</Tabs>
...
<Tabs>
  <Tab label="Étape 1">
    Dans la section `Web Cloud`, cliquez sur <ManagerLink to="/#/web-domains/domain">Noms de domaine</ManagerLink>, sélectionnez le nom de domaine pour lequel vous avez personnalisé les serveurs DNS, puis ouvrez l’onglet **Zone DNS**.
  </Tab>
  <Tab label="Étape 2">
    ...
    La zone DNS apparaît en mode *texte* :
  ...
  <Tab label="Étape 3">
    Dans cette fenêtre, remplacez **uniquement dans les enregistrements de type *NS*** les noms des serveurs DNS par vos propres noms de serveurs DNS personnalisés **sans oublier** d’incrémenter de « 1 » la première valeur numérique de la ligne *SOA*. Une fois vos modifications faites, suivez les étapes affichées à l’écran pour confirmer.
