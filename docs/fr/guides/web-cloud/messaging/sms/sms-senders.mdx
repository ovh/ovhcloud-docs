...
Certain(s) pays imposent en plus une déclaration réglementaire de l'expéditeur alphanumérique (Sender ID) auprès de leur régulateur avant de pouvoir l'utiliser :

| Pays | Déclaration requise | Canal |
|------|----------------------|-------|
| Espagne | Oui | [Déclarer un Sender ID pour l'Espagne](/guides/web-cloud/messaging/sms/declaration-sender-id-espagne) |
...
OVHcloud vous permet de choisir un type d’expéditeur pour envoyer vos SMS.
...
Il permet de recevoir une réponse via <code className="action">Message et campagne</code> > <code className="action">Gestion des SMS</code> > <code className="action">SMS reçus</code>.
...
:::info
Pour certains pays, une déclaration réglementaire supplémentaire est requise avant de pouvoir utiliser un expéditeur alphanumérique — voir le tableau de statut par pays en début de guide.
:::
