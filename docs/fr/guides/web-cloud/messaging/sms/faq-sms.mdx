...
1. **Créez des clés API** depuis la <CreateToken>page de création de tokens de l'API OVHcloud</CreateToken> en autorisant les endpoints `/sms/*`.
...
L'API retourne un identifiant de job permettant de suivre le statut d'envoi. Des librairies sont également disponibles en Python, Node.js, Java et C#. La documentation complète de l'API SMS est disponible sur la <ApiLink section="/sms">console API OVHcloud</ApiLink>.
...
La documentation interactive complète est disponible sur la <ApiLink section="/sms">console API OVHcloud</ApiLink>. L'authentification se fait via des clés API (Application Key, Application Secret, Consumer Key).
