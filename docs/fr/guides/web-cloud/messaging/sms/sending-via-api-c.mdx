...
description: "Découvrez comment envoyer des SMS avec l’API RESTful OVHcloud en C# : créez vos identifiants API, connectez-vous et envoyez votre premier SMS"
...
Créez vos identifiants de Script (toutes les clés en une fois) sur cette page: <CreateToken>page de création de token OVHcloud</CreateToken>.

:::info
L'URL suivante vous permet d'obtenir automatiquement les bons droits pour ce guide : <CreateToken rights="GET=/sms/&GET=/sms/*/jobs&POST=/sms/*/jobs">page de création de token OVHcloud</CreateToken>.
:::
...
L'environnement et les identifiants sont prêts : vous pouvez coder votre premier appel à l'API.
...
Vous pouvez désactiver le STOP via le paramètre `noStopClause`. À noter qu'avec le STOP vous ne pouvez envoyer de SMS de 20h à 8h du matin.

## Aller plus loin

La [console d'API](https://api.ovh.com/console/?section=%2Fsms&branch=v1) vous permettra de découvrir d'autres méthodes pour faciliter l'intégration de services tels que : SMS réponses, envoi en masse avec fichier CSV, publipostage, suivi des accusés de réception...
