...
Si votre offre actuelle n'inclut pas l'IPv6, il est possible de changer d'offre en suivant notre guide : [Comment changer l'offre de mon service OverTheBox](/guides/web-cloud/internet/overthebox/offer-migration).
...
En mode WireGuard, le service est par ailleurs compatible avec n'importe quel matériel prenant en charge WireGuard, sans installation de l'image OverTheBox. Pour en savoir plus, consultez le guide « [Comment configurer WireGuard sur OverTheBox](/guides/web-cloud/internet/overthebox/config-wireguard) ».

</details>

<details>
<summary>Quelle différence entre le mode de fonctionnement OverTheBox et le mode WireGuard ?</summary>

Un service OverTheBox peut fonctionner selon deux modes exclusifs (un seul à la fois) :

- **Mode WireGuard** : il établit une connexion point à point avec n'importe quel client compatible WireGuard (ordinateur portable, smartphone, ou un autre routeur prenant en charge WireGuard), sans installation de l'image OverTheBox. En revanche, ce mode ne prend pas en charge l'agrégation ni le basculement (failover) de connexions, et l'IPv6 n'est pas encore disponible.
- **Mode OverTheBox** : il est spécifique à l'image système et au matériel OverTheBox. Il offre l'agrégation et le basculement (failover) de plusieurs connexions Internet, ainsi que des services complémentaires comme l'accès à distance.

Pour basculer votre service en mode WireGuard, consultez le guide « [Comment configurer WireGuard sur OverTheBox](/guides/web-cloud/internet/overthebox/config-wireguard) ».

</details>

<details>
<summary>Ai-je besoin d'un équipement OverTheBox pour le mode de fonctionnement WireGuard ?</summary>

Non, l'équipement OverTheBox n'est pas nécessaire en mode WireGuard. N'importe quel client prenant en charge WireGuard peut utiliser un service OverTheBox en mode WireGuard. Pour en savoir plus, consultez le guide « [Comment configurer WireGuard sur OverTheBox](/guides/web-cloud/internet/overthebox/config-wireguard) ».
