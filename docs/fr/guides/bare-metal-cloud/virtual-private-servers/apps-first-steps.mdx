...
:::info
Si le lien a déjà expiré, connectez-vous en SSH au serveur à l'aide du nom d'utilisateur et exécutez la commande `sudo whmlogin` pour en générer un nouveau.
:::
...
WHM utilise par défaut l'utilisateur root pour l'authentification. Le lien à usage unique contenu dans l'e-mail permet d'accéder à la configuration initiale et de définir votre mot de passe root. Lors des connexions suivantes à WHM, saisissez `root` comme nom d'utilisateur et le mot de passe que vous avez défini.
