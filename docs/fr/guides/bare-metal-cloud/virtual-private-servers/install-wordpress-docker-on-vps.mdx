...
Cliquez sur l'onglet <code className="action">Zone DNS</code>. Dans le tableau qui s'affiche, identifiez la ligne ayant pour type la valeur `A`, ouvrez le menu d'actions <code className="action">⋮</code> sur cette ligne et cliquez sur <code className="action">Modifier l'entrée</code>.

Dans le formulaire d'édition, entrez l'adresse IP de votre VPS ou du serveur dédié dans le champ <code className="action">Cible</code>, puis confirmez vos modifications.
