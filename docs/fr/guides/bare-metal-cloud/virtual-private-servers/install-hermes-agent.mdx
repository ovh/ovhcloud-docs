...
description: "Découvrez comment déployer Hermes Agent en production sur un VPS, connecté à OVHcloud AI Endpoints, avec passerelle Telegram et tâches planifiées"
...
- Disposer d'un [VPS OVHcloud](/links/bare-metal/vps) (Debian 12 ou Ubuntu 22.04+). Consultez notre guide « [Premiers pas avec un VPS](/guides/bare-metal-cloud/virtual-private-servers/starting-with-a-vps) ».
...
- Disposer d'une clé API OVHcloud AI Endpoints. Consultez notre guide « [AI Endpoints - Premiers pas](/guides/public-cloud/ai-machine-learning/ai-endpoints-getting-started) » pour en générer une.
- (Facultatif, nécessaire à partir de l'étape 5) Obtenir un token de bot Telegram auprès de [@BotFather](https://t.me/BotFather), si vous souhaitez dialoguer avec l'agent depuis Telegram.
...
:::info
Le modèle `gpt-oss-120b` supporte l'appel d'outils (function calling), indispensable au fonctionnement d'un agent. Vous pouvez consulter le [catalogue AI Endpoints](/links/public-cloud/ai-endpoints-catalog) pour choisir un autre modèle compatible ; vérifiez alors sa fenêtre de contexte (64k minimum) et son support du function calling.
:::
...
- **HTTP 401/403** : la clé est invalide côté OVHcloud — régénérez-la depuis la section <code className="action">AI Endpoints</code> de votre <ManagerLink to="/#/public-cloud/pci/projects">projet Public Cloud</ManagerLink>.
...
:::info
Chaque exécution consomme des tokens AI Endpoints (facturation à l'usage). Une tâche hebdomadaire de synthèse représente typiquement quelques centaines de milliers de tokens par mois, soit quelques centimes avec `gpt-oss-120b`. Surveillez votre consommation depuis la section <code className="action">AI Endpoints</code> de votre <ManagerLink to="/#/public-cloud/pci/projects">projet Public Cloud</ManagerLink>. Pour le détail des tarifs et le cycle de vie des modèles, consultez notre guide « [AI Endpoints - Facturation et cycle de vie](/guides/public-cloud/ai-machine-learning/ai-endpoints-billing) ».
:::
