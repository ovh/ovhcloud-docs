...
Pour accéder à votre site web depuis un navigateur, liez d'abord le nom de domaine de votre site web WordPress à l'adresse IP de votre VPS ou de votre serveur dédié. Pour cela, modifiez l'enregistrement `A` de la zone DNS active afin qu'il pointe vers cette adresse IP, comme indiqué dans notre guide « [Éditer une zone DNS OVHcloud](/guides/web-cloud/domains/dns-zone-edit) ».
