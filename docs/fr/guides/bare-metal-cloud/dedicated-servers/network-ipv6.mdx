...
Le format ifcfg étant déprécié, NetworkManager ne crée plus par défaut les nouveaux profils dans ce format. Le fichier de configuration se trouve dans `/etc/NetworkManager/system-connections/`.
  ...
  <Tab label="Windows Server 2019 et versions ultérieures">
    ...
    **Étape 2 : Ouvrir la configuration réseau de votre serveur**
    ...
    Ouvrez le menu Démarrer de Windows, saisissez `ncpa.cpl`, puis cliquez sur le résultat de recherche proposé.

    <img className="thumbnail" alt="Fenêtre Connexions réseau dans le panneau de configuration Windows" src="/images/bare-metal-cloud/dedicated-servers/network-ipv6/ipv6_network_connections.png" loading="lazy" />
    ...
    **Étape 4 : Désactiver les identifiants d'interface IPv6 aléatoires**
    ...
    Ouvrez le menu Démarrer, recherchez <code className="action">Windows PowerShell</code>, faites un clic droit dessus et sélectionnez <code className="action">Exécuter en tant qu'administrateur</code>.

    <img className="thumbnail" alt="Exécuter PowerShell en tant qu'administrateur" src="/images/bare-metal-cloud/dedicated-servers/network-ipv6/ipv6_powershell_admin.png" loading="lazy" />
    ...
    <img className="thumbnail" alt="Désactiver les identifiants aléatoires" src="/images/bare-metal-cloud/dedicated-servers/network-ipv6/ipv6_powershell_randomize_identifiers.png" loading="lazy" />
...
Dans tous les cas, n'hésitez pas à [contacter notre équipe de support](/links/support-contact) pour demander à examiner vos configurations. Vous devrez fournir :
