...
Les [serveurs dédiés](/links/bare-metal/bare-metal) OVHcloud vous permettent de configurer les disques, le [RAID matériel](/guides/bare-metal-cloud/dedicated-servers/raid-hard), le [RAID logiciel](/guides/bare-metal-cloud/dedicated-servers/raid-soft), LVM, ZFS, etc. pendant [la réinstallation](/guides/bare-metal-cloud/dedicated-servers/getting-started-with-dedicated-server) de votre système d'exploitation depuis l’<ApiLink>API OVHcloud</ApiLink> ou depuis votre <ManagerLink to="/">espace client OVHcloud</ManagerLink>. Dans cet article, nous allons nous concentrer sur l'<ApiLink>API OVHcloud</ApiLink>.<br/>
...
- la première (diskGroupId=1) contient 2 disques de 480 Go chacun. Il s'agit du diskGroupId par défaut.
- la seconde (diskGroupId=2) contient 2 disques de 1,9 To chacun.

Exemple d'installation de Debian 13 (Trixie) sur le diskGroupId 2 :
...
:::info
Par défaut, si aucun diskGroupId n’est spécifié ou s’il est défini sur 0, le système d’exploitation est installé sur le diskGroupId 1 par défaut.
...
:::warning
Pour l'instant, l'API ne supporte que l'installation OS et la personnalisation du stockage sur 1 seule grappe de disques. Vous pouvez impliquer de 1 à tous les disques de la grappe choisie dans la personnalisation du stockage. Cependant, les disques restants de cette grappe seront effacés. Les disques appartenant aux autres grappes sont eux aussi effacés par défaut, mais leurs données peuvent être conservées : consultez la section [Effacement des données](#data-erasure). Dans tous les cas, ces disques resteront visibles par l'OS installé, et peuvent être utilisés/configurés ultérieurement pour stocker des données.
...
⁴ L'installateur ESXi ne prend pas en charge les schémas de partitionnement personnalisés. Le partitionnement est défini par l'éditeur du logiciel. Néanmoins, l’<ApiLink>API OVHcloud</ApiLink> peut vous donner une idée de ce à quoi ressemble le partitionnement : pour plus d'informations, consultez [OS & Compatibilité du partitionnement](#os-partitioning-compatibility).<br />
...
:::warning
Un partitionnement personnalisé, bien que valide et accepté par l'<ApiLink>API OVHcloud</ApiLink>, peut malgré tout empêcher le serveur de démarrer correctement (par exemple une combinaison inhabituelle de système de fichiers et de chargeur d'amorçage, ou un problème d'assemblage RAID/LVM/ZFS propre à votre configuration). Dans ce cas, aucune intervention du support ne sera automatiquement créée sur le serveur : un message explicite sera affiché au client après le dernier redémarrage. Consultez la section [Les erreurs clients fréquentes](#errors) pour plus de détails.
...
### Effacement des données <a name="data-erasure"></a>

Par défaut, les disques de chaque grappe de disques du serveur sont effacés lors de l'installation de l'OS, que leur grappe soit déclarée ou non dans la requête `storage`. Définir `erase` à `false` sur une grappe de disques est le seul moyen de conserver ses données.

Les grappes de disques autres que celle sur laquelle l'OS est installé peuvent également être déclarées, mais uniquement pour contrôler si leurs données sont effacées (`erase: true`) ou conservées (`erase: false`).

L'OS est installé sur la grappe de disques portant les attributs d'installation (`partitioning`, `hardwareRaid`) ; les autres grappes déclarées ne portent que l'attribut `erase`. Si aucune d'elles ne porte d'attribut d'installation, l'OS est installé sur la grappe de disques par défaut, comme décrit dans la section [Grappes de disques](#disk-group).

Exemple avec un serveur disposant de 2 grappes de disques : une simple installation de l'OS sur le `diskGroupId` 1 (dont les disques sont implicitement effacés), et le `diskGroupId` 2 déclaré uniquement pour conserver ses données existantes :
...
:::warning
Les restrictions suivantes s'appliquent à la liste `storage` :

- Vous ne pouvez pas définir `erase: false` pour la grappe de disques sur laquelle l'OS est installé.
- Vous ne pouvez pas déclarer plusieurs fois le même `diskGroupId`.
- Vous ne pouvez pas déclarer plus d'une grappe de disques sur laquelle installer l'OS. Pour toute grappe de disques supplémentaire, seul l'attribut `erase` peut être défini — sans installation d'OS, ni partitionnement, ni configuration de RAID matériel.

:::

:::warning
Définir `erase` à `false` conserve toutes les données de cette grappe de disques, mais OVHcloud ne peut pas garantir que l'installation de l'OS réussira : des métadonnées d'amorçage résiduelles, des superblocs `mdadm`, des signatures LVM ou des étiquettes ZFS provenant d'une installation précédente sur cette grappe de disques (sur n'importe lequel de ses disques) peuvent entrer en conflit avec la nouvelle installation.

Si l'installation échoue pour l'une de ces raisons, aucun ticket de support n'est créé automatiquement. À la place, un message explicite est affiché au client, soit pendant le partitionnement, soit au premier démarrage sur l'OS installé. Consultez la section [Les erreurs clients fréquentes](#errors) pour plus de détails.
...
Celui-ci est visible via la barre de progression depuis <ManagerLink to="/#/dedicated-servers/server">l'espace client OVHcloud</ManagerLink>.
Depuis l'<ApiLink>API OVHcloud</ApiLink>, cet état peut être obtenu avec l'appel API suivant :
...
|Partitioning failed. Leftover partitions from a previous install (on disk groups excluded from erasure) are likely conflicting with the new layout.|Des métadonnées résiduelles (table de partitions, superblocs `mdadm`, signatures LVM ou ZFS) sur une grappe de disques déclarée avec `erase: false` entrent en conflit avec le partitionnement demandé.|- Définissez `erase: true` sur cette grappe de disques si vous n'avez plus besoin d'en conserver les données<br />- Effacez manuellement la grappe de disques avant de réinstaller|
|The server does not seem to have booted properly into the newly installed operating system. This is likely related to leftover partitioning from a previous installation on disk group(s) excluded from erasure; please check the KVM, if available.|Des métadonnées résiduelles sur une grappe de disques déclarée avec `erase: false` perturbent le démarrage du système fraîchement installé.|- Consultez le KVM pour identifier le problème de démarrage exact<br />- Définissez `erase: true` sur cette grappe de disques si vous n'avez plus besoin d'en conserver les données<br />- Effacez manuellement la grappe de disques avant de réinstaller|
|The server does not seem to have booted properly into the newly installed operating system. This is likely related to the custom partitioning you requested; please check the KVM, if available.|Le partitionnement demandé est accepté par l'<ApiLink>API OVHcloud</ApiLink> mais produit un assemblage chargeur d'amorçage/système de fichiers/RAID à partir duquel le serveur ne peut pas démarrer.|- Consultez le KVM pour identifier le problème de démarrage exact<br />- Ajustez votre partitionnement (par exemple le choix du système de fichiers, le niveau de RAID, la configuration de `/boot`) et réinstallez|
