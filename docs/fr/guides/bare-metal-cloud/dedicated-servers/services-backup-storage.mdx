...
Rendez-vous sur la <ApiLink>console API OVHcloud</ApiLink> et utilisez l'appel suivant :
...
Assurez-vous d'abord d’avoir autorisé vos blocs d’IP à accéder au stockage et à utiliser le protocole NFS. Selon votre système d'exploitation Linux, il est possible que vous deviez installer le client NFS et démarrer les services NFS et `portmap`.
...
En cas de problème de connexion, vérifiez également que les ports UDP et TCP nécessaires à NFSv3 sont ouverts dans le pare-feu, en entrée comme en sortie.
Le port 111 UDP/TCP doit être ouvert dans les deux sens, de même que le port 2049/TCP (NFS) et la plage de ports dynamiques (de 32768 à 60999 UDP/TCP). Ces ports sont utilisés par RPC pour les programmes `mountd`, `lockd` et `statd`.

Enfin, autorisez toutes les connexions sortantes vers l'adresse IP du serveur Backup Storage.
