...
Pour de nombreux cas d'usage, il peut s'avérer intéressant d'automatiser l'installation ou la réinstallation d'un système d'exploitation de vos [serveurs dédiés](/links/bare-metal/bare-metal) à l'aide de l'<ApiLink>API OVHcloud</ApiLink>.
...
Ouvrez l'<ApiLink>API OVHcloud</ApiLink> puis rendez-vous dans la section <code className="action">/dedicated/server</code>.
...
Exemple de valeurs pour la clef <code className="action">inputs</code> pour Debian 13 (Trixie) :
...
Exemple de valeurs pour la clef <code className="action">inputs</code> pour Windows Server 2025 Standard (Core) :
...
:::info
Les réponses doivent utiliser le type JSON déclaré par la question :

- `boolean` : utilisez un booléen JSON et non une chaîne de caractères, c'est-à-dire `true` et non `"true"`. Notez que, dans la description de la question ci-dessus, `default` est toujours retourné sous forme de chaîne de caractères.
- `keyValue` : utilisez un objet JSON dont les valeurs sont des chaînes de caractères, par exemple `{"foo": "bar"}`.

:::

Concernant la question `enableLacpBonding`, consultez le guide « [Comprendre l'agrégation de liens sur les serveurs dédiés](/guides/bare-metal-cloud/dedicated-servers/link-aggregation-overview) » pour savoir quand elle s'applique et ce qu'elle configure.
...
Ce payload permet d'installer Debian 13 (Trixie) avec une clé SSH publique, un script bash de post-installation et l'agrégation LACP activée :
...
Ce payload permet d'installer Windows Server 2025 Standard (Core) en français avec un script PowerShell de post-installation :
