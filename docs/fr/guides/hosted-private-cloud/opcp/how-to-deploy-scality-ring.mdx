...
description: "Découvrez comment déployer un cluster Scality RING sur du bare metal OPCP d'OVHcloud avec Terraform, de la préparation des nœuds à la vérification"
lastUpdated: 2026-09-25
---

## Objectif

[Scality RING](https://www.scality.com/products/ring/) est une plateforme de stockage logicielle qui transforme un ensemble de serveurs en un système de stockage objet (S3<sup>1</sup>) et/ou fichier (NFS/CIFS) unique, avec une protection des données assurée soit par erasure coding (ARC), soit par réplication (COS), sans nécessiter de contrôleur RAID matériel.
...
- Un projet OVHcloud OPCP disposant de suffisamment de nœuds Bare Metal Pool à l'état Ironic `available` pour votre topologie cible. Le cluster minimal pris en charge par RING sur cette plateforme est **1 nœud superviseur et 3 nœuds de stockage**, les 3 nœuds de stockage devant être soit des modèles au catalogue **High Capacity Storage** (write cache pas encore disponible), soit des modèles **High Performance Storage**.
- Un **identifiant d'application** OpenStack (application credential) pour ce projet, utilisable à la fois par la CLI OpenStack et par Terraform. Consultez l'étape *Création d'une Application Credential depuis Horizon* du guide OVHcloud « [Comment utiliser Terraform](/guides/hosted-private-cloud/opcp/use-terraform) » pour OPCP.
- Les outils suivants installés sur le poste de travail depuis lequel vous lancez le déploiement :
  - la [CLI OpenStack](https://docs.openstack.org/python-openstackclient/latest/) (`openstack`), pour importer les images, configurer LACP/RAID (étape 3) et lancer les commandes de vérification (étape 8) ;
  ...
  - [`jq`](https://jqlang.org/), pour analyser la sortie JSON de plusieurs commandes `openstack` utilisées ci-dessous ;
  - un client SSH, utile si vous devez accéder directement au superviseur plutôt que par sa seule interface web.
- Une image qcow2 **superviseur** Scality RING et une ou plusieurs images qcow2 **stockage**, basées sur Rocky Linux ou RHEL — les seuls systèmes d'exploitation pris en charge par RING sur OPCP — prêtes à être importées dans Glance. L'image superviseur doit déjà embarquer le bundle d'installation RING, car le superviseur n'a aucun moyen garanti de le récupérer après son démarrage sur cette plateforme.
  :::info
  Vous n'avez pas à construire ces images vous-même : demandez les images qcow2 superviseur et stockage (ainsi que le bundle d'installation RING, s'il n'est pas déjà intégré à l'image superviseur) à Scality, via votre interlocuteur commercial ou le [support Scality](https://support.scality.com). La construction ou la personnalisation des images RING n'est pas couverte par ce guide — consultez le [portail de documentation Scality](https://documentation.scality.com) pour cette procédure, ou le guide OVHcloud sur la construction d'une [image CentOS Stream personnalisée](/guides/hosted-private-cloud/opcp/how-to-create-centos-image) pour comprendre les exigences sous-jacentes d'Ironic/Nova en matière d'images.
  :::
- Une **licence** Scality RING valide. RING ne s'installe et ne fonctionne sans licence que dans un état d'évaluation limité et borné dans le temps ; demandez une licence auprès du [support Scality](https://support.scality.com) avant toute mise en production.
- Un export de l'outil de dimensionnement RING (un fichier `cluster.csv` décrivant les rôles de vos nœuds, la disposition des disques et les paramètres de dimensionnement RING), généralement fourni par votre interlocuteur commercial ou support Scality.
- Le réseau privé backend déjà provisionné par le Bare Metal Pool pour votre projet (utilisé pour le plan interne RING) ; éventuellement, un second réseau pour le trafic client S3/NFS, que Terraform peut créer pour vous.
...
:::tip
Dans certaines régions OPCP, le catalogue de services annonce des endpoints qui ne sont pas joignables depuis votre réseau. Si les commandes `openstack` échouent à se connecter après l'émission réussie d'un jeton, ajoutez des surcharges d'endpoint explicites par service (`OS_COMPUTE_ENDPOINT_OVERRIDE`, `OS_IMAGE_ENDPOINT_OVERRIDE`, `OS_NETWORK_ENDPOINT_OVERRIDE`, `OS_BAREMETAL_ENDPOINT_OVERRIDE`, `OS_VOLUME_ENDPOINT_OVERRIDE`) pointant vers les adresses fournies pour votre projet. Le provider OpenStack de Terraform accepte les mêmes surcharges dans son bloc provider.
:::
...
:::warning
Les noms d'image doivent être uniques dans le projet. Importer une image sous un nom déjà existant dans Glance fait échouer votre plan Terraform plutôt que de réutiliser silencieusement l'ancienne image ; choisissez donc un nouveau nom à chaque import d'une image reconstruite (par exemple en ajoutant un suffixe de version).
:::
...
Deux réglages au niveau des nœuds se situent en dehors de Terraform, car le driver Ironic OpenStack standard ne dispose d'aucune ressource Terraform pour eux. Configurez les deux **avant** le déploiement des nœuds — ils ne peuvent être modifiés que sur des nœuds sans instance, et refaire cette opération plus tard nécessite d'abord de détruire l'instance.

**Bonding réseau (LACP).** RING attend que chaque nœud présente son ou ses réseaux sous forme d'interface agrégée (`bond0` pour le plan backend et, si vous utilisez un réseau frontend, `bond1`), pour que la perte d'une seule carte réseau ou d'un switch top-of-rack ne mette pas le nœud hors service. Configurez un groupe de ports 802.3ad (LACP) par plan directement dans OpenStack Ironic, en suivant le guide OVHcloud « [Comment configurer LACP sur un nœud](/guides/hosted-private-cloud/opcp/how-to-setup-lacp-on-node) ». Utilisez la politique de hachage `layer3+4` pour que le trafic se répartisse sur les 2 membres du bond (la valeur par défaut du noyau, `layer2`, attache chaque flux à un seul lien).
...
RING ne fournit pas de module Terraform public pour OPCP ; cette configuration est donc construite directement à partir des providers OpenStack, `tls` et `random`. Elle nécessite, dans l'ordre : une IP backend fixe par nœud, votre CSV de dimensionnement complété avec ces adresses, les instances bare metal elles-mêmes, une paire de clés SSH de déploiement dédiée, et suffisamment de `cloud-init` pour que le superviseur pilote l'installation une fois tous les nœuds démarrés.
...
Corriger un écart à ce stade coûte bien moins cher que de le découvrir après qu'une instance bare metal a déjà été provisionnée.
...
:::warning
Sur du bare metal, un *replace* Terraform d'une instance correspond à un redéploiement Ironic complet de la machine physique — le nœud est effacé et reprovisionné à partir de zéro. Recherchez spécifiquement `must be replaced` ou `must be destroyed` sur toute instance bare metal, port réseau ou paire de clés de déploiement, et, si vous en voyez un inattendu, vérifiez qu'il est voulu avant d'appliquer.
:::
...
- Terraform génère une paire de clés de déploiement dédiée. La partie publique est ajoutée aux `authorized_keys` root de chaque nœud ; la partie privée n'est présente que dans le `cloud-init` du superviseur, pour que seul le superviseur puisse accéder en root à chaque nœud de stockage.
- Au premier démarrage, le superviseur écrit sur le disque local le CSV d'installation complété et la configuration de l'installeur, puis sonde le SSH root de chaque nœud de stockage (l'ordre de démarrage des nœuds bare metal n'étant pas garanti, cela s'exécute généralement avec un délai d'attente généreux, de l'ordre d'une heure).
...
:::info
Une fois lancé, l'installeur ne se relance pas s'il trouve un marqueur `done` ou un processus encore en cours d'exécution ; redémarrer un nœud en cours d'installation ne relance donc pas une installation terminée ou en cours.
:::
