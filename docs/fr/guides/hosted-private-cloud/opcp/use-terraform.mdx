...
Le bloc `lifecycle` de l'exemple ci-dessus (`ignore_changes = [image_name]`) protège de ce scénario en figeant l'image de l'instance. Si vous référencez aussi l'ID de l'image, ajoutez `image_id` à la liste. Si vous souhaitez volontairement changer l'image d'une instance, prenez d'abord un snapshot ou une sauvegarde de celle-ci.
:::
