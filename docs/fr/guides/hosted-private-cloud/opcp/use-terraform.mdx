...
:::warning
**Modifier l'image d'une instance existante déclenche une reconstruction (*rebuild*) destructive.** Si l'image de l'instance (`image_id` ou `image_name`) doit changer au prochain `terraform plan` / `tofu plan` — par exemple après la suppression puis la recréation d'une image portant le même nom — l'`apply` reconstruit l'instance : celle-ci conserve son ID et ses adresses IP, mais **le contenu de son disque est entièrement effacé** et remplacé par la nouvelle image.

Le plan présente cette opération comme une simple mise à jour sur place (`will be updated in-place`, `0 to destroy`) : elle est pourtant bien destructive.

Le bloc `lifecycle` de l'exemple ci-dessus (`ignore_changes = [image_name]`) protège de ce scénario en figeant l'image de l'instance. Si vous référencez également l'ID de l'image, ajoutez `image_id` à la liste. Si vous souhaitez délibérément changer l'image d'une instance, effectuez d'abord un snapshot ou une sauvegarde de celle-ci.
:::
