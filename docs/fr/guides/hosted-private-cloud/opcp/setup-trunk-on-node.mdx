...
:::warning
Le réseau d'un sous-port doit être **différent** de celui du port parent. Le sous-port hérite de l'adresse MAC du port parent, tout comme la sous-interface vlan de l'OS invité, ce qui est nécessaire au DHCP. Deux ports d'un même réseau ne pouvant pas partager une adresse MAC, un sous-port créé sur le réseau du port parent fait échouer le déploiement de l'instance.
:::
