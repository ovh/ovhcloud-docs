---
title: "OPCP - Fonctionnement de la synchronisation Ironic vers NetBox"
...
Sur une **OVHcloud On-Prem Cloud Platform (OPCP)**, l'inventaire physique et logique affiché dans NetBox n'est pas maintenu manuellement : il est alimenté automatiquement depuis OpenStack Ironic, qui fait foi pour les nœuds bare-metal de votre plateforme.
...
NetBox étant maintenu synchronisé avec Ironic, il reflète la même réalité que la vue OpenStack/Ironic de vos nœuds. Les statuts décrits dans le guide « [Cycle de vie d'un nœud OPCP](/guides/hosted-private-cloud/opcp/node-lifecycle) » sont donc répercutés dans NetBox.
...
- **Le type et le fabricant de l'équipement** : déduits du modèle matériel et du fournisseur du nœud rapportés par Ironic.
...
- **Le câblage** : les câbles entre chaque interface de nœud et le port de switch correspondant, d'après les informations de liaison rapportées par Ironic.
...
La façon dont la position est déterminée, les cas où le placement automatique s'applique et ceux où le positionnement manuel est conservé comme mécanisme de repli sont expliqués en détail dans le guide « [OPCP - Gérer l'élévation de rack dans NetBox](/guides/hosted-private-cloud/opcp/how-to-handle-rack-elevation) ».
...
- [OPCP - Gérer l'élévation de rack dans NetBox](/guides/hosted-private-cloud/opcp/how-to-handle-rack-elevation)
