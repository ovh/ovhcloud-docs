...
description: "Comprenez comment les nœuds OPCP sont importés depuis Ironic vers NetBox et ce que la synchronisation maintient à jour."
...
NetBox étant maintenu synchronisé avec Ironic, il reflète la même réalité que la vue OpenStack/Ironic de vos nœuds. Les statuts décrits dans [Cycle de vie d'un nœud OPCP](/guides/hosted-private-cloud/opcp/node-lifecycle) sont donc répercutés dans NetBox.
...
La façon dont la position est déterminée, les cas où le placement automatique s'applique et ceux où le positionnement manuel est conservé comme mécanisme de repli sont expliqués en détail dans :
