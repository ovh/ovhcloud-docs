...
Il existe trois façons de déclencher le déverrouillage, selon les accès dont vous disposez sur le serveur cible :
...
- Un service [OPCP](https://www.ovhcloud.com/fr/hosted-private-cloud/onprem-cloud-platform/) actif avec des disques SED configurés.
...
```bash
ssh -i opcp-admin -p 2222 <controller_ip>
```

Remplacez `<controller_ip>` par l'adresse IP du contrôleur.

Une fois connecté, vous pouvez déclencher le déverrouillage depuis le shell PBA.

### Option 3 : Accès direct au serveur (clavier)

Si vous disposez d'un accès physique ou par KVM/console au serveur, l'invite PBA peut être déverrouillée directement en saisissant le mot de passe SED au clavier sur l'écran de connexion PBA, sans passer par le réseau.
