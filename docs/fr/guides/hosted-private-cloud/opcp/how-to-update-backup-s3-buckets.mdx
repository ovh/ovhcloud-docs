...
- Une infrastructure [OPCP](https://www.ovhcloud.com/fr/hosted-private-cloud/onprem-cloud-platform/) opérationnelle.
- Un accès administrateur au contrôleur OPCP avec les outils `opcp-cli`, `opcp-diag`, `kubectl`, `flux`, `tfctl` et `velero` installés et configurés.
- Les informations de connexion du stockage objet S3 **cible** : URL de l’endpoint, région et noms des buckets à utiliser pour les sauvegardes.
- Une fenêtre de maintenance, le plan de contrôle étant redéployé pendant l’opération.
...
Attendez que ces commandes ne signalent plus aucune ressource en attente ou en échec. Si une ressource reste bloquée, il peut être parfois nécessaire de redémarrer les pods concernés — soyez patient, la réconciliation peut prendre un certain temps.
