...
Vous devez avoir un accès SSH au nœud contrôleur ainsi que les droits `sudo`. Vérifiez ceci en exécutant :
...
Vous aurez besoin de ces droits sur tous les nœuds contrôleurs.
...
Cela vous donnera la liste de tous les disques utilisés comme OSD Ceph. Conservez cette liste, car vous en aurez besoin plus tard lors de l'effacement des disques.
...
Une fois ceci fait, effacez sur tous les nœuds contrôleurs toutes les signatures (table de partition, système de fichiers, LVM, RAID, etc.) des disques recensés [précédemment](#obtenir-les-informations-ceph) :
...
Vous pouvez consulter les mots de passe des commutateurs dans `opcp-cli secrets passwords --edit`. Les clés à rechercher sont `network_devices_admin_arista_secret`, `network_devices_admin_cisco_secret` et `network_devices_enable_secret`.
:::
