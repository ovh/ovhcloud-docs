...
- Avoir parcouru le guide « [Intégration réseau et connexion à la plateforme](/guides/hosted-private-cloud/opcp/opcp-network-architecture) », qui décrit l'architecture réseau dans laquelle s'inscrivent ces prérequis.
...
:::info
**Transmission des éléments sensibles.** Plusieurs prérequis impliquent des secrets : clé pré-partagée IPsec, clés d'accès S3<sup>1</sup>, clés privées de certificats, etc. Ces éléments **ne doivent pas être transmis par un canal non sécurisé**. Convenez avec votre point de contact OVHcloud d'un canal de transmission approprié pour chaque secret.
:::
...
- [Intégration réseau et connexion à la plateforme](/guides/hosted-private-cloud/opcp/opcp-network-architecture)
