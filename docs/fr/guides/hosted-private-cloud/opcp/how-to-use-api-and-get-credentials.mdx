...
> **Note**
> Depuis la version **3.0.5** d’OPCP, il est nécessaire d’ajouter un mapper d’audience au client afin que les tokens émis incluent l’audience `keystone`. Sans ce mapper, l’authentification auprès de Keystone échoue.

- Depuis l’onglet `Client Scopes`, sélectionnez de nouveau la portée `[votre-client-id]-dedicated`.
- Cliquez sur <code className="action">Configurer un nouveau mapper</code> et choisissez le type **Audience**.
- Configurez les champs suivants :
