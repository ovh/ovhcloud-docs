...
- Accéder aux ressources des projets OpenStack en mode `reader`, `member` ou `admin`, sous réserve d'attributs de projet configurés dans Keycloak
...
:::info
Le `dc_operator` n'a aucun droit d'administration des utilisateurs Keycloak. Comme le `reader`, il ne dispose d'aucun droit OpenStack par défaut : ceux-ci nécessitent des attributs de projet sur l'utilisateur ou son groupe. Contrairement au `reader`, il n'a pas d'iframe OpenStack dans le Dashboard et doit utiliser l'interface **Horizon**.
:::
...
:::info
L'accès OpenStack des rôles Keycloak `reader` et `dc_operator` est conditionné par la présence d'**attributs de projet** sur l'utilisateur ou son groupe dans Keycloak. Sans attribut de projet configuré, aucune ressource OpenStack n'est accessible. N'importe quel rôle OpenStack peut être attribué ainsi, y compris `admin`.
:::
...
:::info
Pour les rôles `reader` et `dc_operator`, n'importe quel rôle OpenStack peut être attribué via les attributs de projet, y compris `admin`. Seuls les rôles `it_admin` et `master_admin` disposent de droits OpenStack sans attribut de projet.
:::
