...
Profil supervision et audit. Ce rôle donne par défaut un accès en lecture seule, sans capacité d'administration.

Il permet de :

- Consulter ses paramètres de compte dans Keycloak
- Accéder aux ressources des projets OpenStack en mode **reader** ou **member**, sous réserve d'attributs de projet configurés dans Keycloak

:::info
Le rôle Keycloak `reader` peut recevoir le rôle OpenStack `reader` (lecture seule) ou `member` (lecture-écriture), jamais `admin`. Les deux sont conditionnés par la présence d'attributs de projet sur le compte ou le groupe Keycloak. Sans attribut de projet, aucune ressource OpenStack n'est visible.
:::
...
:::info
Le `dc_operator` n'a aucun droit d'administration des utilisateurs Keycloak. Son accès à OpenStack est identique à celui du rôle `reader` : il n'est pas accordé par défaut et nécessite des attributs de projet sur l'utilisateur ou son groupe.
:::
...
**Légende** : ✅ Autorisé | ❌ Non autorisé | 🟡 Configuration requise (selon les attributs de projet)
...
:::info
L'accès OpenStack des rôles Keycloak `reader` et `dc_operator` est conditionné par la présence d'**attributs de projet** sur l'utilisateur ou son groupe dans Keycloak. Sans attribut de projet configuré, aucune ressource OpenStack n'est accessible. Ces deux rôles accèdent à OpenStack via l'interface **Horizon**, et non via l'iframe OpenStack du Dashboard.
:::
...
:::info
Pour les rôles `reader` et `dc_operator`, les rôles OpenStack `reader` et `member` peuvent être attribués via les attributs de projet. Le rôle `admin` reste réservé aux rôles `it_admin` et `master_admin`.
:::
