...
Une allocation est donc une **réservation**, et non un déploiement. Le nœud reste dans l'état de provisionnement `available` et voit simplement ses champs `instance_uuid` et `allocation_uuid` renseignés. L'installation d'un système d'exploitation demeure une opération distincte, réalisée par la création d'une instance, comme décrit dans le guide « [Installer une instance avec le CLI OpenStack](/guides/hosted-private-cloud/opcp/how-to-setup-instance-from-cli) ».
