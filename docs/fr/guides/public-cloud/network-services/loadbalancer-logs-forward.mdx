...
- **loadBalancerId** : il s'agit de l'identifiant du Load Balancer, que vous pouvez retrouver dans la page de détails de votre Load Balancer dans votre espace client OVHcloud ou en utilisant <ApiLink section="/cloud" method="GET" route={"/cloud/project/\{serviceName\}/region/\{regionName\}/loadbalancing/loadbalancer"}>l'appel API dédié</ApiLink>.
- **regionName** : la région OpenStack, par exemple, `GRA11`.
- **serviceName** : il s’agit de l’identifiant du projet Public Cloud, que vous pouvez retrouver dans votre espace client OVHcloud sous le nom de votre projet ou en utilisant <ApiLink section="/cloud" method="GET" route={"/cloud/project"}>l’appel API dédié</ApiLink>.

La requête POST a une charge utile qui nécessite les informations suivantes :

- `kind` : le type de journal que vous voulez transférer. Notez que la seule valeur actuellement prise en charge pour Public Cloud Load Balancer est « haproxy » (vous pouvez trouver les types disponibles en utilisant <ApiLink section="/cloud" method="GET" route={"/cloud/project/\{serviceName\}/region/\{regionName\}/loadbalancing/log/kind"}>l'appel API dédié</ApiLink>).
