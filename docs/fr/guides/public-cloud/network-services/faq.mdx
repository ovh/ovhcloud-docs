...
Le Load Balancer est proposé en différentes tailles (S/M/L/XL) pour répondre au mieux aux besoins de nos clients. Ces différentes tailles sont définies par des flavors. Vous pouvez redimensionner votre Load Balancer directement, sans en créer un nouveau ni reconnecter l'adresse Floating IP, via l'API OpenStack/Horizon ou via l'API OVHcloud :
...
Oui. Managed Kubernetes Service (MKS) prend en charge l'intégration avec le Load Balancer Public Cloud OVHcloud (basé sur OpenStack Octavia). Cette fonctionnalité est désormais en disponibilité générale (GA). Pour plus d'informations, consultez notre guide sur [l'exposition de vos applications à l'aide du Load Balancer Public Cloud OVHcloud](/guides/public-cloud/containers-orchestration/managed-kubernetes/expose-applications-using-load-balancer).
