...
Une [Gateway](/links/public-cloud/gateway) OVHcloud fournit une connectivité sortante sécurisée (SNAT) pour les instances d'un sous-réseau privé. Elle est également requise sur ce sous-réseau pour exposer publiquement ces ressources privées via les [Floating IPs](/links/public-cloud/floating-ip).
...
<Tabs>
  ...
  <Tab label="API OVHcloud">
    Rendez-vous sur l'<ApiLink>API OVHcloud</ApiLink> et utilisez l'appel suivant pour créer une Gateway ainsi qu'un nouveau réseau privé et un sous-réseau en une seule opération :
...
<Tabs>
  <Tab label="Espace client OVHcloud">
    Depuis la liste <code className="action">Gateway</code>, cliquez sur le menu <code className="action">...</code> à côté de votre Gateway, puis sélectionnez <code className="action">Modifier votre Gateway</code>.
