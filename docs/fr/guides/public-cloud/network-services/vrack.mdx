...
**Ordre des interfaces après un attachement/détachement à chaud** <a name="interface-order-after-reboot"></a>

:::warning
Attacher ou détacher des interfaces réseau (ports) alors que l'instance est en cours d'exécution (à chaud) peut modifier l'ordre dans lequel le système d'exploitation invité les détecte lors du prochain redémarrage. L'interface identifiée comme `eth0` (ou toute autre interface attendue, y compris l'interface publique) peut alors changer, ce qui peut entraîner des problèmes réseau inattendus si la nouvelle configuration n'est pas validée au préalable.

Si vous devez réorganiser plusieurs interfaces sur une instance déjà en production :
1. Détachez toutes les interfaces que vous souhaitez modifier.
2. Rattachez-les une par une, dans l'ordre dans lequel vous souhaitez qu'elles apparaissent dans le système invité.
3. Effectuez un **hard reboot** de l'instance afin de valider que l'ordre des interfaces est bien celui attendu avant de vous y fier en production.
:::
...
Si vous prévoyez de rattacher des interfaces par la suite, consultez [Ordre des interfaces après un attachement/détachement à chaud](#interface-order-after-reboot) afin d'éviter tout changement d'ordre des interfaces après un redémarrage.
