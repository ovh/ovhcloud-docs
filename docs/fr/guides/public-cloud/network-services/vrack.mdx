...
**Ajout d'une interface réseau privée** <a id="adding-a-private-interface"></a>
...
### Ordre des interfaces après un attachement/détachement à chaud <a id="interface-order-after-reboot"></a>

:::warning
Attacher ou détacher des interfaces réseau (ports) alors que l'instance est en cours d'exécution (à chaud) peut modifier l'ordre dans lequel le système d'exploitation invité les détecte lors du prochain redémarrage. L'interface identifiée comme `eth0` (ou toute autre interface attendue, y compris l'interface publique) peut alors changer. Vérifiez la nouvelle configuration pour éviter tout problème réseau inattendu.

Si vous devez réorganiser plusieurs interfaces sur une instance en cours d'exécution :

1. Détachez toutes les interfaces que vous souhaitez modifier. Détacher l'interface « Ext-Net » relâche définitivement son adresse IP publique, comme indiqué dans la section « [Détacher une interface réseau](#removing-a-private-interface) ».
2. Rattachez-les une par une, dans l'ordre dans lequel vous souhaitez qu'elles apparaissent dans le système invité.
3. Effectuez un *hard reboot* de l'instance pour vérifier que l'ordre des interfaces correspond à celui attendu avant de vous y fier en production.

:::

### Détacher une interface réseau <a id="removing-a-private-interface"></a>
