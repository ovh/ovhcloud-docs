...
Cliquez sur votre service Enterprise File Storage, puis sur l'onglet `Volumes`. Cliquez sur le bouton `Créer un volume` et renseignez le formulaire de création du volume.
...
Cliquez sur `Add access`, puis saisissez soit les IP des nœuds une par une, soit l'IP publique de la Gateway selon votre configuration, et confirmez. Répétez cette opération pour chaque IP à autoriser.

Une fois cette opération terminée, la liste des ACL affichera toutes les adresses IP autorisées à accéder au volume.
