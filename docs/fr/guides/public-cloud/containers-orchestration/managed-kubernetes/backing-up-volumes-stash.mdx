...
### Un exemple : serveur Nginx avec journaux persistants

Dans ce guide, nous allons utiliser un exemple simple : un petit serveur web Nginx avec un `PersistentVolume` pour stocker les journaux d'accès.
...
Et effectuez quelques appels vers l'URL pour générer des journaux d'accès :
...
Nous devons maintenant nous connecter au pod pour lire le fichier de journal et vérifier que nos journaux sont bien écrits.
...
Puis connectez-vous à celui-ci et consultez vos journaux d'accès :
