...
Cliquez sur votre NAS, puis sur l'onglet <code className="action">Partitions</code>, puis sur le bouton <code className="action">Créer une partition</code>. Renseignez le nom de la partition, définissez la taille souhaitée et sélectionnez **NFS** comme protocole, puis confirmez pour créer la partition.
...
Cliquez sur le menu <code className="action">Manage Access</code> de votre partition nouvellement créée. Ajoutez soit les IP des nœuds une par une, soit l'IP publique de la Gateway selon votre configuration. Pour chaque entrée, cliquez sur <code className="action">Add an access</code>, saisissez l'adresse IP, puis confirmez.
