...
<Tabs>
  <Tab label="Via l'espace client OVHcloud">
    ...
    Vous pouvez intégrer votre cluster Kubernetes à un réseau privé via le vRack OVHcloud. Sélectionnez un réseau vRack existant dans la liste déroulante, ou ignorez cette étape pour configurer le réseau privé ultérieurement. Pour plus d'informations, consultez notre guide [Utiliser le vRack](/guides/public-cloud/containers-orchestration/managed-kubernetes/using-vrack).

    Configurez le pool de nœuds par défaut. Un pool de nœuds est un groupe de nœuds partageant la même configuration. Vous pouvez ajouter un pool de nœuds dès maintenant ou ignorer cette étape pour en ajouter un plus tard. Consultez le guide [Gérer les pools de nœuds](/guides/public-cloud/containers-orchestration/managed-kubernetes/managing-nodes) pour plus d'informations.

    Définissez la taille du pool de nœuds par défaut : saisissez un nom pour le pool, choisissez une flavor d'instance (type d'instance) et indiquez le nombre de nœuds.
    ...
    Saisissez un nom pour votre cluster, vérifiez le récapitulatif et cliquez sur <code className="action">Envoyer</code>.
  ...
  <Tab label="Via Terraform">
    ...
    L'identifiant de votre projet Public Cloud correspond au `service_name`. Le nom de service de votre cluster (son identifiant unique) apparaît dans l'URL de l'espace client OVHcloud lorsque vous êtes sur la page de détails du cluster, ainsi que dans l'onglet Aperçu du cluster.
