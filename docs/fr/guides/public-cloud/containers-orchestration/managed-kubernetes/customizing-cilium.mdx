...
:::warning
Assurez-vous de lire les limites décrites plus bas lorsque vous définissez les valeurs dans un objet `CiliumNodeConfig`.
:::


### Limite d'utilisation de CiliumNodeConfig

En raison de la façon dont les clusters Managed Kubernetes sont déployés, certaines configurations ne sont pas prises en charge bien que les objets `CiliumNodeConfig` permettent de les utiliser.

#### Remplacement de kube-proxy

À la création du cluster, le composant kube-proxy est remplacé par Cilium et bien qu'un objet `CiliumNodeConfig` puisse définir le paramètre `kube-proxy-replacement: "false"`, cela ne fonctionnera pas correctement et aura des conséquences imprévues.

### Exemple de personnalisation

Lors de la personnalisation de la configuration de Cilium à l'aide d'un objet `CiliumNodeConfig`, il est nécessaire de redémarrer le DaemonSet `cilium` :
...
:::warning
Les exemples suivants peuvent avoir divers effets en fonction de votre environnement et peuvent interrompre le fonctionnement de votre cluster.
