...
Les Floating IP pour OVHcloud Managed Kubernetes Service vous permettent d'assigner automatiquement une adresse Floating IP publique à chaque nœud worker d'un pool de nœuds, y compris les nœuds gérés par l'autoscaler.

### Avantages clés

- **Accès public direct** : Chaque nœud obtient sa propre adresse IP publique, permettant une connectivité externe directe.
- **Utilisation de la bande passante du nœud** : Exploite la bande passante publique du nœud (1 Gbps, 10 Gbps ou plus selon la flavor d'instance) au lieu de celle de la Managed Gateway.
...
- **Désactivation de la fonctionnalité** : Toutes les Floating IP du pool de nœuds sont supprimées lorsque la fonctionnalité est désactivée.
...
:::info
Les Floating IP sont facturées par adresse IP pour leur durée exacte d'assignation à un nœud, avec une durée minimale de facturation d'une minute. Elles sont automatiquement supprimées (et la facturation s'arrête) lors de la suppression des nœuds.
:::


## Comment activer les Floating IP

Vous pouvez activer les Floating IP sur un pool de nœuds de trois manières :

1. Lors de la création d'un nouveau cluster avec un pool de nœuds.
2. Lors de la création d'un nouveau pool de nœuds dans un cluster existant.
3. En mettant à jour un pool de nœuds existant.

### Méthode 1 : Activer les Floating IP lors de la création d'un cluster

Lors de la création d'un nouveau cluster MKS Standard via l'API OVHcloud, vous pouvez activer les Floating IP directement dans la configuration du pool de nœuds initial.
...
**Exemple de payload JSON** (partiel, focus sur le pool de nœuds avec Floating IP) :
...
- `attachFloatingIps.enabled` : Définir à `true` pour activer les Floating IP sur le pool de nœuds.

### Méthode 2 : Activer les Floating IP lors de la création d'un nouveau pool de nœuds

Pour activer les Floating IP sur un nouveau pool de nœuds dans un cluster existant, utilisez l'endpoint API suivant.
...
### Méthode 3 : Activer / désactiver les Floating IP sur un pool de nœuds existant

Vous pouvez activer ou désactiver les Floating IP sur un pool de nœuds existant en utilisant l'endpoint PUT.
...
Lorsque vous désactivez les Floating IP sur un pool de nœuds, **toutes les Floating IP attachées aux nœuds de ce pool seront immédiatement supprimées**. Cela interrompra toutes les connexions dépendant de ces adresses IP publiques.
:::
...
**Exemple : Créer un pool de nœuds avec les Floating IP activées.**
...
**Exemple : Créer un pool de nœuds avec Floating IP en utilisant un fichier de configuration.**
...
Puis créez le pool de nœuds :

```bash
ovhcloud cloud kube nodepool create <cluster_id> --from-file ./nodepool-params.json
```

Pour plus d'informations sur la CLI OVHcloud, consultez la documentation suivante : [CLI OVHcloud - Créer un pool de nœuds](https://github.com/ovh/ovhcloud-cli/blob/main/doc/ovhcloud_cloud_kube_nodepool_create.md).
...
Vous pouvez également interroger la configuration du pool de nœuds via l'API OVHcloud pour vérifier que les Floating IP sont activées :
...
- **Sélection de la flavor d'instance** : Choisissez des flavors d'instance avec une bande passante adéquate pour votre charge de travail (les flavors B3, C3 et R3 offrent 10 Gbps de bande passante).
...
- **Ciblage de la fonctionnalité** : N'activez les Floating IP que sur les pools de nœuds qui nécessitent réellement un accès public direct.
- **Révision régulière** : Révisez périodiquement l'utilisation des Floating IP et désactivez la fonctionnalité sur les pools de nœuds qui n'en ont plus besoin.
