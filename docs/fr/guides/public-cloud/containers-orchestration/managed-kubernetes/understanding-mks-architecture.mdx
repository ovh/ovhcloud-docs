...
- **Scheduler** : Assigne les pods aux nœuds en fonction des besoins en ressources et des contraintes
...
| Versions Kubernetes | Disponibilité des nouvelles versions mineures, correctifs de sécurité (définis par l'utilisateur via le paramètre Security Policy) |
...
Les worker nodes sont les machines où vos applications conteneurisées s'exécutent. Contrairement au control plane, vous avez un contrôle direct sur la configuration des nœuds.

### Comment les nœuds sont provisionnés

Les worker nodes sont basés sur des instances OVHcloud Public Cloud. Quand vous créez un pool de nœuds :
...
3. Les nœuds s'enregistrent auprès du control plane via Konnectivity
...
### Concept des pools de nœuds

Les nœuds sont organisés en pools de nœuds - des groupes de nœuds partageant la même configuration :

- **Flavor** : Type d'instance (b3-8, b3-16, t1-45 pour GPU, etc.)
- **Paramètres d'autoscaling** : Min/max nodes, seuils de scale-down
- **Anti-affinité** : Distribuer les nœuds sur différents hyperviseurs
...
- **Installing** : Le nœud est en cours de provisionnement et de configuration
- **Ready** : Le nœud est sain et peut recevoir des pods
- **NotReady** : Le nœud a des problèmes (réseau, ressources, etc.)
- **Draining** : Le nœud est en cours d'évacuation (graceful, respecte les PDBs pendant 10 min max)
- **Terminated** : Le nœud est supprimé
...
OVHcloud surveille la santé des nœuds. Si un nœud reste en état `NotReady` pendant plus de 10 minutes, l'auto-healing est déclenché :

- **Plan Free** : Le nœud est réinstallé in-place
- **Plan Standard** : Le nœud est supprimé et un nouveau est créé

Cela garantit la stabilité du cluster mais signifie :

- Ne stockez pas de données importantes directement sur les nœuds
- Utilisez toujours des Persistent Volumes pour les workloads stateful
- Concevez vos applications pour être résilientes aux pannes de nœuds
...
|  (Free uniquement)                        (Standard uniquement)               |
|                                                                               |
...
1. MKS met le nœud en **cordon** (le marque comme non-schedulable)
2. MKS **draine** le nœud (évacue tous les pods, en respectant les PodDisruptionBudgets)
3. Les composants Kubernetes sont **mis à jour** sur la même instance
4. Le nœud redevient **Ready**
5. Le processus se **répète** pour le nœud suivant (strictement un par un)

**Caractéristiques :**

- Pas d'instances supplémentaires requises
- Préserve l'identité de l'instance (mêmes IPs, même facturation)
- Processus plus lent (séquentiel, un nœud à la fois)
- Réduction temporaire de capacité pendant la mise à jour de chaque nœud

:::warning
Les mises à jour in-place peuvent créer une pression sur les ressources si votre cluster n'a pas assez de capacité disponible pour accueillir les pods évacués du nœud en cours de mise à jour. Assurez-vous que vos nœuds restants peuvent gérer la charge supplémentaire.
:::
...
Avec les rolling upgrades (disponibles sur le plan Standard), de nouveaux worker nodes sont créés avec la version Kubernetes cible :

1. MKS **crée** un nouveau nœud avec la version cible
2. MKS met en **cordon et draine** un ancien nœud
3. Les workloads **migrent** vers le nouveau nœud
4. L'ancien nœud est **supprimé**
5. Le processus se **répète** jusqu'à ce que tous les nœuds soient mis à jour
...
- État propre des nœuds (instances fraîches)
...
:::info
Dans la roadmap future, les rolling upgrades supporteront les paramètres Kubernetes `maxSurge` et `maxUnavailable` pour contrôler combien de nœuds peuvent être ajoutés ou mis hors ligne simultanément.
:::
...
- Quand un état propre des nœuds est préféré

#### Comparaison

| Aspect | In-place | Rolling |
|--------|----------|---------|
| Disponibilité | Free uniquement | Standard uniquement |
...
| **Floating IPs sur les nœuds** | Entrant & Sortant | Accès direct aux nœuds, préserver l'IP source |
| **Gateway (SNAT)** | Sortant uniquement | Sortie par défaut, IP partagée pour tous les nœuds |
...
- Prend en charge le load balancing L4 (TCP/UDP)

**Option 2 - Floating IPs sur les nœuds :**

- Chaque nœud peut avoir sa propre Floating IP attachée
...
:::warning
Pour attacher une Floating IP à un nœud, une Gateway (routeur OpenStack) doit être configurée sur le subnet du réseau privé. Sans Gateway, les Floating IPs ne peuvent pas être associées aux instances de ce subnet.
:::
...
- Tous les nœuds partagent la même IP sortante (IP de la Gateway)
...
:::warning
Avec un réseau privé (avec ou sans vRack), vous verrez toujours une IPv4 publique sur les worker nodes. Cette IP n'est pas joignable depuis Internet et est utilisée exclusivement pour l'administration des nœuds et la communication avec le control plane.
:::
...
|-------------|---------|-------------|
| ReadWriteOnce (RWO) | Pris en charge | Lecture-écriture sur un seul nœud |
| ReadOnlyMany (ROX) | Non pris en charge* | Lecture seule multi-nœuds |
| ReadWriteMany (RWX) | Non pris en charge* | Lecture-écriture multi-nœuds |
...
- Isolation des nœuds via les security groups
