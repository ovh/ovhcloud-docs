...
Dans l'[espace client OVHcloud](https://www.ovh.com/auth/?action=gotomanager&from=https://www.ovh.co.uk/), rendez-vous dans la section **Public Cloud**, sélectionnez votre projet, puis cliquez sur **Managed Kubernetes Service** dans le menu de gauche, sous **Containers & Orchestration**.

Cliquez sur le nom de votre cluster pour ouvrir sa page de détails.

Sur la page de détails de votre cluster, cliquez sur l'onglet **OIDC** pour accéder à la configuration du fournisseur OIDC. Cliquez sur le bouton `...` dans la section `OIDC provider`.
...
Dans la section Public Cloud, vous pouvez récupérer l'identifiant de votre service grâce au bouton <code className="action">Copier dans le presse-papier</code>. Le nom de service de votre cluster (son identifiant unique) apparaît dans l'URL de l'espace client OVHcloud lorsque vous êtes sur la page de détails du cluster, ainsi que dans l'onglet Overview du cluster.
