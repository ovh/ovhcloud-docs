...
- Avoir créé une [instance Public Cloud OVHcloud](/guides/public-cloud/compute/getting-started) au forfait **horaire**.
...
**Facturation mensuelle pour une instance suspendue (*shelved*) :** une instance suspendue (*shelved*) pendant l’intégralité du mois n’est pas facturée (à l’exception du snapshot). Dès que l’instance est réactivée (*unshelved*), même brièvement, la facturation du mois complet s’applique.

**Attention :** tant que l’instance n’est pas supprimée, toute opération continuera d’entraîner la facturation de l’instance.

:::

Le tableau ci-dessous vous permet de différencier les options disponibles sur vos instances. Cliquez sur l'option de votre choix pour poursuivre la lecture de ce guide. Nous mettons entre parenthèses la terminologie utilisée dans l'interface de Horizon.
...
<Tabs>
  ...
  <Tab label="Depuis l’interface Horizon">
    Pour utiliser cette méthode, connectez-vous à l’[interface Horizon](https://horizon.cloud.ovh.net/auth/login/) :
