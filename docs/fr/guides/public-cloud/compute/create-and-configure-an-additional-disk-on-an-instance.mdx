...
:::info
Chaque type de volume est également disponible en version chiffrée (**LUKS**). Ces volumes assurent la confidentialité des données. Ils sont disponibles depuis l'espace client OVHcloud ainsi qu’avec les outils présentés dans la section suivante, en spécifiant le type `<volume_type>-luks`.

:::

:::warning
**Impact du chiffrement (LUKS) sur les performances.** Le chiffrement LUKS ajoute une surcharge mesurable sur le débit. Sur les volumes High Speed Gen2 de plus de 600 Go, le débit maximal est ainsi plafonné à 470 Mo/s au lieu des 512 Mo/s nominaux (une réduction d'environ 8 %). Anticipez cet impact pour les charges de travail sensibles au débit.
