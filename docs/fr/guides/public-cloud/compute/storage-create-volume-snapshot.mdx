...
La création d'une sauvegarde de volume nécessite que le volume soit détaché de l'instance. Pour plus d’informations sur cette option, nous vous invitons à consulter le guide [Créer une sauvegarde d'un volume](/guides/public-cloud/compute/volume-backup).
...
Cliquez sur le bouton <code className="action">...</code> pour <code className="action">Supprimer</code> un snapshot ou <code className="action">Créer un volume</code> à partir du snapshot correspondant. Retrouvez plus d’informations sur le guide [Créer un volume à partir d'une sauvegarde](/guides/public-cloud/compute/storage-create-volume-from-backup).
