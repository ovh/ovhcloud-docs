...
:::info
Un fichier OpenRC est associé à un seul utilisateur et une seule région — il ne peut pas être utilisé pour gérer plusieurs régions à la fois. Conservez séparément les deux fichiers téléchargés, et chargez celui qui correspond à la région sur laquelle vous travaillez.
...
Chargez le fichier OpenRC de la région 1-AZ ou 3-AZ cible, puis téléversez l'image :
...
Une fois chargé le fichier OpenRC de la région 1-AZ ou 3-AZ cible :
...
Le format du disque doit être `raw` ; selon la taille de l'image, le téléversement peut prendre un certain temps.
...
Attendez que l'instance atteigne l'état `SHUTOFF`, puis :
