...
:::info
La liste des flavors disponibles dépend de la région sur laquelle vous êtes connecté (`ovhai config set <région>`). Par exemple, les GPU au-delà du NVIDIA Tesla V100S ne sont actuellement disponibles que sur la région GRA — relancez `ovhai capabilities flavor list` après avoir changé de région pour vérifier.
:::
