...
La facturation est à la minute : elle débute dès que l'image Docker de votre job commence à être récupérée (deuxième étape de la phase `PENDING`) et se poursuit pendant `RUNNING`, `INTERRUPTING` et `TIMEOUT`, jusqu'à ce que le job entre en `FINALIZING`. Chaque minute entamée est facturée intégralement. Les jobs qui n'atteignent pas l'étape de récupération de l'image de la phase `PENDING` ne sont pas facturés.
