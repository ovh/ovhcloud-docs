...
- La [facturation](/guides/public-cloud/ai-machine-learning/ai-training-billing-lifecycle) des **jobs** se fait à la minute. La facturation commence une fois que l'image Docker du job commence à être téléchargée (pendant la phase `PENDING`) et se poursuit pendant `RUNNING`, `INTERRUPTING` et `TIMEOUT`, jusqu'à ce que le job entre en `FINALIZING`. Chaque minute commencée est intégralement facturée.
...
:::info
La facturation commence une fois que l’image Docker du job commence à être téléchargée (pendant la phase `PENDING`) et se poursuit pendant `RUNNING`, `INTERRUPTING` et `TIMEOUT`, jusqu’à ce que le job entre en `FINALIZING`. Pour plus d’informations sur la facturation des jobs, consultez cette [documentation](/guides/public-cloud/ai-machine-learning/ai-training-billing-lifecycle).
:::
