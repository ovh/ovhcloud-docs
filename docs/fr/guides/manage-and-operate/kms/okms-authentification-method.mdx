...
description: Configurez des jetons d'accès personnels, des comptes de service ou des certificats d'accès pour vous authentifier sur l'API REST régionale de votre domaine OKMS
...
<Tabs>
  <Tab label="Jeton d'accès personnel (PAT)">
    ...
    Dans l'[interface Swagger](#use-the-okms-swagger-ui), utilisez le schéma **personalAccessToken** dans la boîte de dialogue <code className="action">Autoriser</code>.
  ...
  <Tab label="Compte de service">
    ...
    Dans l'[interface Swagger](#use-the-okms-swagger-ui), utilisez le schéma **oAuth2ClientCredentials** dans la boîte de dialogue <code className="action">Autoriser</code>.
  </Tab>
...
1. Cliquez sur le bouton <code className="action">Autoriser</code> (icône cadenas) dans l'interface Swagger.
2. Sous **personalAccessToken** (HTTP Bearer, format JWT), collez le PAT que vous avez créé pour votre utilisateur local.
3. Cliquez sur <code className="action">Autoriser</code>, puis sur <code className="action">Fermer</code>.

Swagger envoie votre jeton dans un en-tête `Authorization: Bearer <token>` pour chaque requête que vous exécutez.

#### Compte de service

1. Cliquez sur le bouton <code className="action">Autoriser</code> dans l'interface Swagger.
...
L'authentification par certificat d'accès utilise TLS mutuel (mTLS) au niveau transport. Le certificat est présenté par votre navigateur lors de la connexion à l'URL Swagger — il ne se configure pas via la boîte de dialogue <code className="action">Autoriser</code>.
...
Parcourez la liste complète des actions dans la [console API OVHcloud](https://api.eu.ovhcloud.com/console/) sous le type de produit **Key Management System (KMS)**, ou dans les [politiques IAM](/guides/account-and-service-management/account-information/iam-policy-ui) lors de la création d'une politique.
