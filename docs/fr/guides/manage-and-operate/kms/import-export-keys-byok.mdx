...
Après encapsulation avec la clé publique de destination, envoyez le `ciphertext` JWE Compact Serialization obtenu dans la requête d'import. Pour un exemple en Go, consultez la section « [Exemple complet en Go](#exemple-complet-en-go) ».
...
Les exemples suivants déroulent les deux cérémonies avec une clé RSA, en utilisant le [SDK Go](https://pkg.go.dev/github.com/ovh/okms-sdk-go) pour les appels à OVHcloud KMS et [go-jose](https://pkg.go.dev/github.com/go-jose/go-jose/v4) pour les opérations JWE de l'autre côté. Les deux exemples encodent le matériel de clé en PKCS#8 (DER), ce qui correspond au `keyFormatType` `PKCS8`.

#### Importer une clé générée localement

La bibliothèque standard Go génère la clé, que le programme encapsule localement avec la clé de transport publique, puis importe. Son texte en clair n'atteint jamais OVHcloud KMS.

```go
// 1. Créer la clé de transport sur OVHcloud KMS et récupérer sa partie publique.
...
// 2. Générer la clé à importer et l'encoder en PKCS#8 (DER).
...
// 3. Encapsuler le matériel de clé sous forme de JWE : encapsulation de clé RSA-OAEP-256, chiffrement du contenu A256GCM.
...
// 4. Importer la clé encapsulée. Le KMS la désencapsule et déduit le type et la taille à partir de la clé.
...
Le chiffrement du contenu JWE (`enc`) doit être `A256GCM`.

#### Exporter une clé et déchiffrer le JWE

La destination (ici, le programme local) génère la paire de clés de transport et n'importe que sa partie publique dans OVHcloud KMS. La destination déchiffre ensuite le JWE exporté avec la partie privée.

```go
// 1. Générer la paire de clés de transport sur la destination.
destKey, err := rsa.GenerateKey(rand.Reader, 4096)
if err != nil {
  return err
}

// 2. Importer uniquement sa partie publique dans OVHcloud KMS, avec l'usage wrapKey.
...
// 3. Rendre la clé extractible, puis l'exporter encapsulée par la clé publique de destination.
extractable := true
...
// 4. Déchiffrement du JWE avec la clé privée de destination et analyse de la clé PKCS#8.
