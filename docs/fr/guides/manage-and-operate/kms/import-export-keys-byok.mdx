...
- Disposer d'une politique IAM autorisant les actions BYOK décrites dans la section « [Configurer IAM pour le BYOK](#configurer-iam-pour-le-byok) ».
...
Appliquez une politique à la ressource de votre domaine OKMS (`urn:v1:<region>:resource:okms:<okmsId>`). Remplacez `<identity_urn>` et `<okms_urn>` par vos valeurs. Créez les politiques via l'[espace client OVHcloud](/guides/account-and-service-management/account-information/iam-policy-ui) ou l'[API OVHcloud](/guides/account-and-service-management/account-information/iam-policies-api).
...
Créez une clé RSA dédiée à l'encapsulation. Définissez toujours `wrapKey` et `unwrapKey`. `wrapKey` et `unwrapKey` sont mutuellement exclusives avec `sign` et `verify`.
...
`KEY` est la chaîne JWE Compact Serialization. Passez-la sous la forme `@path/to/wrapped.jwe`, directement en argument, ou utilisez `-` pour la lire depuis stdin.
...
Appelez `GET /v1/servicekey/{keyId}` avec les paramètres de requête d'encapsulation ci-dessous. Lorsque ces paramètres sont présents, l'API renvoie `wrappedKeys` au lieu de `keys` en clair.
