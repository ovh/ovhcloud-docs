---
pageType: overview
title: "Service de gestion de clés (KMS)"
description: "Générez, stockez et utilisez les clés de chiffrement de vos services OVHcloud avec Key Management Service - configuration KMS, intégration et rotation."
text: "Gérez les clés de chiffrement pour vos services OVHcloud"
essentialsTitle: "Les guides essentiels pour commencer"
essentials:
  - title: "Méthodes d'authentification OKMS"
    link: /guides/manage-and-operate/kms/okms-authentication-methods
  - title: "Utiliser votre OVHcloud Key Management Service (KMS)"
    link: /guides/manage-and-operate/kms/kms-usage
  - title: "Importer et exporter des clés sur OVHcloud KMS avec BYOK"
    link: /guides/manage-and-operate/kms/import-export-keys-byok
  - title: "Générer un certificat d'accès OKMS"
    link: /guides/manage-and-operate/kms/okms-certificate-management
  - title: "OKMS - Aperçu de l'architecture"
    link: /guides/manage-and-operate/kms/architecture-overview
  - title: "Comment connecter un produit compatible en utilisant le protocole KMIP"
    link: /guides/manage-and-operate/kms/kms-kmip
  - title: "Comment chiffrer ETCD de Kubernetes avec OVHcloud KMS"
    link: /guides/manage-and-operate/kms/kms-etcd
  - title: "Chiffrer un volume Block Storage avec LUKS"
    link: /guides/public-cloud/compute/encrypting-a-block-storage-volume-with-luks
goFurther:
  title: "Aller plus loin"
  items:
    - title: "Rejoindre la communauté OVHcloud"
      link: https://community.ovhcloud.com/
    - title: "Voir la roadmap & changelog"
      link: https://www.ovhcloud.com/fr/roadmap-changelog/
    - title: "Rejoindre Discord"
      link: https://discord.gg/ovhcloud
cta:
  title: "Des questions ? Accédez au support OVHcloud"
  description: "Trouvez la réponse à vos questions dans notre centre d'aide."
  actions:
    - text: "Accéder au support"
      link: https://help.ovhcloud.com/csm/fr-home
      theme: brand
---
