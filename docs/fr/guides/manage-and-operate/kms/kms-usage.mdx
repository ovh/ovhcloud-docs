...
| --- | --- | --- |
| `extractable` | Oui (création / PATCH) | Si `true`, le matériel de clé peut être exporté (en clair ou encapsulé, selon la classe de clé). Par défaut : `false` ; les clés publiques seules passent à `true` par défaut. |
| `never_extractable` | Non | `true` si la clé n'a jamais été extractible depuis sa création dans le KMS. Les clés [importées avec BYOK](/guides/manage-and-operate/kms/import-key-byok) ou via un import en clair ont `never_extractable` défini à `false`, car le matériel existait en dehors du KMS avant l'import. Activer l'extraction efface également cet indicateur de façon permanente. |
| `sensitive` | Non | `true` lorsque la clé ne peut être extraite qu'après encapsulation (ce qui nécessite également que `extractable` soit `true`). Les clés privées (AES/`oct` et matériel RSA/EC privé) sont toujours considérées comme sensibles. |
| `always_sensitive` | Non | `true` si la clé a toujours été sensible depuis sa création. |

Exemple de forme de réponse GET (champs abrégés) :
...
Pour la cérémonie d'extraction sécurisée (définir `extractable` uniquement pour l'opération d'export, puis exporter avec encapsulation RSA), consultez « [Exporter une clé encapsulée](/guides/manage-and-operate/kms/import-key-byok#exporter-une-clé-encapsulée) ».
