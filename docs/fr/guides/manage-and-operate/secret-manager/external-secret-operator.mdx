...
- Avoir [configuré une méthode d'authentification](/guides/manage-and-operate/kms/okms-authentication-methods) pour le plan de données OKMS (ce guide utilise un jeton d'accès personnel).
...
Créez un [utilisateur local IAM](/guides/account-and-service-management/account-information/ovhcloud-users-management) avec les droits d'accès à votre domaine, ou utilisez un [compte de service](/guides/manage-and-operate/api/manage-service-account). Consultez [Méthodes d'authentification OKMS](/guides/manage-and-operate/kms/okms-authentication-methods) pour la configuration complète et les modèles de politiques IAM.
...
Vous aurez également besoin de la `region` et de l'`okms-id` du domaine OKMS que vous souhaitez utiliser. Cet ID et cette région peuvent être trouvés sur le <ManagerLink to="/#/okms/key-management-service">dashboard de votre domaine OKMS</ManagerLink> ou via l'[OVHcloud CLI](https://github.com/ovh/ovhcloud-cli) :
...
#### Installation de l'External Secrets Operator (ESO) sur votre cluster Kubernetes
...
Le champ `stringData` accepte le jeton brut — Kubernetes l'encode en base64 lors du stockage du secret. Si vous préférez utiliser le champ `data`, vous devez encoder le jeton en base64 au préalable (`echo -n "<token>" | base64`).

Appliquez-le avec la commande `kubectl apply -f secret.yaml`.

Ou, si le `user_pat` est stocké dans la variable d'environnement `PAT_TOKEN`, créez le secret directement avec kubectl, qui se charge également de l'encodage :
...
Lorsque vous inspectez le secret avec `kubectl get secret ovhcloud-vault-token -n external-secrets -o yaml`, la valeur apparaît sous `data`, encodée en base64.

#### Configuration de l'External Secrets Operator
...
:::info
Cette intégration fonctionne également avec un `SecretStore`.
...
#### Utilisation de l'External Secrets Operator
...
Pour plus d'informations sur la gestion de l'External Secrets Operator, veuillez consulter la documentation dédiée, en utilisant le fournisseur HashiCorp Vault : [https://external-secrets.io/latest/](https://external-secrets.io/latest/).
