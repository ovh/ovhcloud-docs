...
Les API disponibles sur <ApiLink>API OVHcloud</ApiLink> vous permettent d'acheter, gérer, mettre à jour et configurer des produits OVHcloud sans utiliser une interface graphique comme l'espace client.

**Découvrez comment explorer les API OVHcloud à travers la console API.**
...
Vous pouvez parcourir la console API sans compte. Pour exécuter des appels sur vos propres produits, connectez-vous avec vos identifiants OVHcloud via l'entrée <code className="action">Authentication</code> en haut à gauche — les étapes de connexion sont détaillées dans le guide [Premiers pas avec les API OVHcloud](/guides/manage-and-operate/api/first-steps).
...
- **V1** : l'API disponible à l'URL `https://api.eu.ovhcloud.com/v1`
- **V2** : la nouvelle API OVHcloud, disponible à l'URL `https://api.eu.ovhcloud.com/v2`
