...
Les API disponibles sur le <ApiLink>portail API OVHcloud</ApiLink> vous permettent d'acheter, gérer, mettre à jour et configurer des produits OVHcloud sans utiliser une interface graphique comme l'espace client.
...
- Être sur la page web des <ApiLink>API OVHcloud</ApiLink>.
...
Sur la page des <ApiLink>API OVHcloud</ApiLink>, cliquez sur <code className="action">Explore the OVH API</code> pour afficher la liste des API. 
...
Pour ce faire, cliquez sur le lien suivant : <CreateToken>page de création de token OVHcloud</CreateToken>.
...
Le token **CK** peut être utilisé pour de la délégation de droits. Consultez le guide suivant pour en savoir plus : [Comment gérer le compte d'un client OVHcloud via les API](/guides/manage-and-operate/api/api-right-delegation) (guide en anglais).
...
Pour lister et révoquer les clés, il est possible d'utiliser le <ApiLink>portail API</ApiLink> ou l'<ManagerLink to="/">espace client OVHcloud</ManagerLink> :
...
[Comment gérer le compte d'un client OVHcloud via les API](/guides/manage-and-operate/api/api-right-delegation) (guide en anglais)
