...
| **Import/Export massif d’objets** | La migration de données par lots à l’aide de commandes S3 (`aws s3 cp/sync`, etc.). | S3 (objets, dossiers) | **Entrant** : copie des données depuis une autre solution S3 vers le bucket Object Storage d’OVHcloud. <br/> **Sortant** : export massif via les mêmes outils vers une autre plateforme S3 ou compatible. | [Documentation de l’offre Cold Archive](/guides/storage-and-backup/object-storage/overview) |
