...
| **Fonction** | **Description** | **Formats disponibles** | **Modèle de migration** | **Documentation disponible** |
| --- | --- | --- | --- | --- |
| Interface Web Openstack Horizon | Gestion d’une infrastructure OpenStack offrant des tableaux de bord visuels pour interagir avec les différents services OpenStack. | HTML | **Entrante** : NA. Interface utilisateur disponible par défaut. <br/>**Sortante** : Pas de ressource à exporter, les dashboards sont statiques  | [Présentation de Horizon](/guides/public-cloud/cross-functional/introducing-horizon) |
...
Aujourd’hui, la connectivité publique se fait via un produit supplémentaire Managed Dedicated Cloud - SecNumCloud qui est une solution temporaire le temps de mettre en place une connectivité publique dédiée.
...
À la fin du service, OVHcloud opère un effacement sécurisé. Cet effacement fera l’objet d’un préavis de 21 jours calendaires.
