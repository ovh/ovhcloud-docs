...
| --- | --- | --- | --- | --- |
| Virtualisation | Gestion des VM via vSphere, vCenter, vMotion, et support de formats standards VMware | OVA, OVF | **Entrante** : import de VM, disques, snapshots via l’interface vSphere ou les outils spécialisés (exemple Veeam, Zerto, etc …)  <br/>**Sortante** : export des VM, disques via vSphere, et réutilisation sur tout environnement VMware ou compatible. Des outils spécialisés peuvent être utilisés (exemple Veeam, Zerto, PowerCLI, etc …) | [Se connecter à l’interface web vSphere](/guides/hosted-private-cloud/powered-by-vmware/vsphere-interface-connection)<br/><br/>[Déploiement d’une machine virtuelle](/guides/hosted-private-cloud/powered-by-vmware/deploy-virtual-machine)<br/><br/>[Comment connecter une image ISO à une VM](/guides/hosted-private-cloud/powered-by-vmware/connect-iso-to-vm) <br/><br/>[Création de cluster et activation EVC](/guides/hosted-private-cloud/powered-by-vmware/create-cluster-enable-evc) |
...
Le produit repose sur une solution VMware dédiée, incluant des briques comme vSphere, vCenter, stockage NFS, vROps, des options avancées de sécurité (chiffrement, MFA) et options NSX (SDN) et/ou vSAN à activer. Les ressources (compute, stockage, réseau) sont isolées physiquement et logiquement, avec une gestion fine des droits (IAM), et l'intégration de services de réseaux privés. L’architecture est opérée dans les datacentres OVHcloud.
