...
Les fonctionnalités décrites dans les tableaux sont disponibles sans coûts et frais sauf mentions contraires, et sont librement utilisables par le client. Par défaut, aucune surfacturation n’est appliquée suite à la migration des données. La facturation s’arrête dès la résiliation du service.
