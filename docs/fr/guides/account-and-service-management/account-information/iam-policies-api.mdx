...
<ApiLink section="/iam" method="GET" route={"/iam/policy"} version="v2">`GET /iam/policy`</ApiLink>
...
- **permissions**:
    ...
    - **deny**: Ensemble des actions explicitement interdites pour les identités concernant les ressources. Une action interdite sera refusée quelles que soient les actions autorisées dans d'autres politiques.
...
Les conditions sont ajoutées dans une politique d'accès sous la forme suivante :
...
L'exemple ci-dessus illustre une politique avec une condition valable si les ressources ciblées sont de type **dnsZone** avec un tag **"environment:prod"**, sauf les **Samedi et Dimanche** sur le fuseau horaire de Paris.
...
| :--------------------: | :--------------------------------------------------------: | :------------: | :--------------------------------: | :-------------------------------------------: |
|  date(location).Date   |               EQ <br/>BEFORE <br/>AFTER <br/>IN               |   YYYY-MM-DD   |  Filtre sur les jours calendaires  |  "date(America/New_York).Date.EQ": "2024-12-25"  |
...
| resource.Tag(tag_key)  |              EQ <br/>STARTS_WITH <br/>ENDS_WITH              |     string     |        Filtre sur les tags         |       "resource.Tag(environment).EQ": "dev"       |
|     resource.Name      |          EQ <br/>IN <br/>STARTS_WITH <br/>ENDS_WITH           |     string     | Filtre sur les noms de ressources  |      "resource.Name.STARTS_WITH": "vps-"       |
...
- **STARTS_WITH** : La valeur doit commencer par celle indiquée
- **ENDS_WITH** : La valeur doit terminer par celle indiquée
...
Le compte destinataire de cette politique pourra gérer les droits ainsi reçus dans ses propres politiques d'accès, mais ne pourra jamais outrepasser les droits tels que définis dans la politique d'accès.
...
<ApiLink section="/me">section `/me` de la console API</ApiLink>
...
<ApiLink section="/iam" method="GET" route={"/iam/resource"} version="v2">`GET /iam/resource`</ApiLink>
...
<ApiLink section="/iam" method="GET" route={"/iam/resourceGroup"} version="v2">`GET /iam/resourceGroup`</ApiLink>
...
L'exemple suivant crée un groupe de ressources "*Test\_Environment*" contenant 2 ressources:
...
<ApiLink section="/iam" method="GET" route={"/iam/reference/action"} version="v2">`GET /iam/reference/action`</ApiLink>
