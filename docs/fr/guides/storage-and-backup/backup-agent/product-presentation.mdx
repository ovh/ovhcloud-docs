...
Découvrez comment naviguer dans la VSPC via [ce guide](/guides/storage-and-backup/backup-agent/vspc-presentation).
...
Une fois que l'agent obtient les informations, il envoie directement les données vers le point de stockage sans jamais transiter par l'infrastructure VSPC.

Une fois que l'agent a effectué des sauvegardes, vous pouvez restaurer vos données de deux façons : restauration par fichier (FLR) ou restauration du serveur.

## Points clés

Plusieurs points forts sont présents dans cette offre :

- Première politique de sauvegarde automatique avec une rétention de 30 jours.
- Possibilité d'étendre la rétention jusqu'à 365 jours.
