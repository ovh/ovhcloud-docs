---
title: "Object Storage - Téléversement de fichiers depuis le navigateur via HTTP POST"
description: "Découvrez comment permettre aux utilisateurs de téléverser des fichiers directement depuis leur navigateur vers un bucket OVHcloud Object Storage via un formulaire HTML signé côté serveur"
lastUpdated: 2026-07-23
---

## Objectif

**Ce guide explique comment implémenter des téléversements de fichiers directs depuis le navigateur vers OVHcloud Object Storage en utilisant la méthode HTTP POST, avec génération d'une POST Policy côté serveur et authentification SigV4.**
...
- Avoir [installé et configuré AWS CLI](/guides/storage-and-backup/object-storage/s3-getting-started-with-object-storage)

## En pratique

### Présentation

Les téléversements POST depuis le navigateur permettent aux applications web de laisser les utilisateurs finaux téléverser des fichiers directement dans un bucket OVHcloud Object Storage, sans faire transiter les données par votre serveur applicatif. Votre backend génère une politique et une signature, l'utilisateur sélectionne un fichier, et le navigateur envoie les données directement au bucket.
...
|---|---|---|
| `acl` | ACL S3<sup>1</sup> à appliquer à l'objet. Valeurs : `private`, `public-read`, `public-read-write`, `authenticated-read`, `bucket-owner-read`, `bucket-owner-full-control` | Correspondance exacte, commence par |
...
| `success_action_redirect` | URL vers laquelle le navigateur est redirigé après un téléversement réussi. | Correspondance exacte, commence par |
...
| `success_action_redirect` | URL HTTPS absolue vers laquelle rediriger le navigateur après un téléversement réussi |
...
| `x-amz-server-side-encryption-customer-key` | Votre clé AES 256 bits (32 octets), encodée en base64 (44 caractères avec remplissage) |
...
| Versioning activé | OVHcloud génère un identifiant de version unique par téléversement. La réponse inclut l'en-tête `x-amz-version-id` avec une valeur non nulle |

Pour plus d'informations, consultez le guide « [Object Storage - Premiers pas avec la gestion de versions](/guides/storage-and-backup/object-storage/s3-versioning) ».
