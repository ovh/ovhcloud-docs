...
| AbortIncompleteMultipartUpload                      | non       | Une action de lifecycle qui applique une opération de suppression sur les parties d'un téléversement multi-parties incomplet. <br/><br/> ⚠️ Cet attribut s'exclut mutuellement avec Filter.Tag, Filter.ObjectSizeGreaterThan et Filter.ObjectSizeLessThan, c'est-à-dire que si vous filtrez les objets par tag et/ou par taille, vous devez configurer une règle distincte pour supprimer les téléversements multi-parties incomplets. |
...
Dans la section <code className="action">Object Storage</code> de votre projet Public Cloud, cliquez sur le nom du bucket cible, accédez à l'onglet <code className="action">Lifecycle</code> et cliquez sur <code className="action">Créer une règle</code>.
...
- **Suppression des marqueurs de suppression expirés** : supprime les marqueurs de suppression qui n'ont plus aucune version non courante. Aucun champ supplémentaire requis.
    ...
    :::warning
    Cette action doit être configurée dans une règle distincte si un délai d'expiration en jours est également défini pour la version courante, ou si un filtre par tag a été défini à l'étape 3.
    :::
...
- **Suppression des uploads multipart incomplets** : arrête et supprime les parties des uploads multipart incomplets.
    ...
    :::warning
    Cette action doit être configurée dans une règle distincte si des filtres par tag et/ou par taille d'objet ont été définis à l'étape 3.
    :::
