...
description: Apprenez à chiffrer vos objets Object Storage au repos avec des clés gérées par le client (SSE-C) ou par OVHcloud (SSE-OMK)
...
OVHcloud Object Storage propose deux méthodes de chiffrement côté serveur (SSE) pour protéger vos objets au repos :
...
- **SSE-OMK**<sup>1</sup> (Server-Side Encryption with OVHcloud-Managed Keys) : OVHcloud génère et gère automatiquement une clé de chiffrement unique par objet. Aucune gestion de clés n'est requise.
- Une troisième option est le **chiffrement côté client (CSE)**, elle vous permet de chiffrer vos données avant de les envoyer vers Object Storage. Ce guide inclut le CSE dans le tableau comparatif ci-dessous, mais n'en couvre pas la mise en œuvre en détail.
...
Une fois activé, tous les uploads suivants sont automatiquement chiffrés. Les objets existants ne sont pas affectés — uploadez-les à nouveau pour les chiffrer rétroactivement.
...
- **Surcharge de chiffrement** : SSE-OMK introduit une légère surcharge de latence, négligeable pour la plupart des charges de travail. Pour la minimiser, utilisez un bucket dans une région géographiquement proche de votre application.
