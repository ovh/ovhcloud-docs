---
pageType: overview
title: Cloud Disk Array
description: "Commandez, attachez et exploitez le stockage Cloud Disk Array sur OVHcloud - stockage bloc partagé Ceph pour vos services Dedicated et Public Cloud."
text: "Découvrez les guides Cloud Disk Array"
essentialsTitle: "Les guides essentiels pour commencer"
essentials:
  - title: Cloud Disk Array - Comment créer des utilisateurs
    link: /guides/storage-and-backup/block-storage/cloud-disk-array/ceph-create-a-user
  - title: Cloud Disk Array - Comment créer un pool
    link: /guides/storage-and-backup/block-storage/cloud-disk-array/ceph-create-a-pool
  - title: "Cloud Disk Array - Comment créer une ACL IP"
    link: /guides/storage-and-backup/block-storage/cloud-disk-array/ceph-create-an-ip-acl
tutorials:
  - title: Utiliser Ceph avec Proxmox (EN)
    link: /guides/storage-and-backup/block-storage/cloud-disk-array/ceph-use-ceph-with-proxmox
goFurther:
  title: Aller plus loin
  items:
    - title: Consulter la communauté OVHcloud
      link: https://community.ovh.com/
    - title: Consulter la roadmap & le changelog
      link: https://www.ovhcloud.com/fr/roadmap-changelog/
    - title: Rejoindre Discord
      link: https://discord.gg/ovhcloud
cta:
  title: Des questions ? Accédez au support OVHcloud
  description: "Trouvez la réponse à vos questions dans notre centre d'aide."
  actions:
    - text: Accéder au support
      link: https://help.ovhcloud.com/
      theme: brand
---
