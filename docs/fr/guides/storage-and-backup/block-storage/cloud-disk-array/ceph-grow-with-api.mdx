...
[[cpnav:storage-cloud-disk-array]]
...
Utilisez le nom du service (UUID) de l'étape 1 pour obtenir l'ID de service correspondant, en filtrant sur le paramètre `resourceName` :
...
Ouvrez la <ApiLink>console API</ApiLink>.

**Étape 2 : Envoyer l'ordre de mise à niveau**

Utilisez la route API suivante :

<Api version="v1" section="/order" method="POST" route={"/order/upgrade/cephaas/\{serviceName\}/\{planCode\}"} />
