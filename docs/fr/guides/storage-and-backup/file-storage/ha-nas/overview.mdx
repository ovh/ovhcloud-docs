---
pageType: overview
title: NAS-HA
description: "Commandez, configurez et exploitez vos systèmes de fichiers partagés HA-NAS sur OVHcloud - stockage NFS et CIFS avec haute disponibilité, snapshots et contrôle d'accès."
text: "Découvrez les guides NAS-HA"
essentialsTitle: "Les guides essentiels pour commencer"
essentials:
  - title: Premiers pas avec un NAS-HA
    link: /guides/storage-and-backup/file-storage/ha-nas/nas-get-started
  - title: NAS-HA - Premiers pas avec les API
    link: /guides/storage-and-backup/file-storage/ha-nas/nas-quick-api
goFurther:
  title: Aller plus loin
  items:
    - title: Consulter la communauté OVHcloud
      link: https://community.ovh.com/
    - title: Consulter la roadmap & le changelog
      link: https://www.ovhcloud.com/fr/roadmap-changelog/
    - title: Rejoindre Discord
      link: https://discord.gg/ovhcloud
cta:
  title: Des questions ? Accédez au support OVHcloud
  description: "Trouvez la réponse à vos questions dans notre centre d'aide."
  actions:
    - text: Accéder au support
      link: https://help.ovhcloud.com/
      theme: brand
---
