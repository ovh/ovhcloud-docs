...
Il est accessible via les API OVHcloud, la CLI OVHcloud, OpenStack CLI et API, Manila CSI, l'espace client OVHcloud et Terraform.
...
Les shares File Storage d'OVHcloud peuvent être chiffrés au repos grâce aux clés gérées par OVHcloud (OMK), le même mécanisme que pour les [volumes Block Storage chiffrés](/guides/storage-and-backup/block-storage/block-storage-the-right-storage-class) (LUKS).

Pour créer un share chiffré, utilisez le type `standard-1az-luks` à la place de `standard-1az` avec l'API OVHcloud, la CLI OpenStack ou Manila CSI. Avec Terraform, ajoutez plutôt un bloc `encryption` (voir l'onglet Terraform).

:::info
La création d'un share `-luks` génère automatiquement une clé de chiffrement dédiée. Le chiffrement et le déchiffrement sont gérés de manière transparente par l'infrastructure OVHcloud — aucune configuration supplémentaire n'est requise côté client.
:::

:::info
Aucun impact sur les performances n'a été observé à ce jour sur les shares File Storage chiffrés.
:::
...
<Tabs>
  <Tab label="Via l'espace client OVHcloud">
    ...
    **1\. Créer un share**
    ...
    Définissez le niveau de performance et la capacité (en Go), associez un réseau privé, puis cliquez sur <code className="action">Valider</code> pour finaliser la création du share.
  ...
  <Tab label="Via l’API OVHcloud">
    ...
    **Note :** Sélectionnez uniquement un réseau privé.
    ...
    Créez un share NFS de 150 Go connecté à votre réseau privé :
    ...
    **Note :** Remplacez `<my-share-name>` par le nom de share que vous avez choisi.
    ...
    :::info
    Pour créer un share chiffré, définissez `encryption.encrypted` sur `true` dans le corps de la requête. Le type `standard-1az-luks` est alors appliqué automatiquement et une clé de chiffrement dédiée est générée — ne la modifiez pas et ne la supprimez pas, sous peine de rendre les données du share définitivement irrécupérables.
    :::
    ...
    **Note :** L'identifiant du share doit avoir le format `abc12345-def6-4abc-8def-123456abcdef`.
    ...
    <Api version="v1" section="/cloud" method="GET" route={"/cloud/project/\{serviceName\}/region/\{regionName\}/share/\{shareId\}"} />
    ...
    **2\. Autoriser une machine virtuelle cliente**
    ...
    <Api version="v1" section="/cloud" method="POST" route={"/cloud/project/\{serviceName\}/region/\{regionName\}/share/\{shareId\}/acl"} />
    ...
    <Api version="v1" section="/cloud" method="GET" route={"/cloud/project/\{serviceName\}/region/\{regionName\}/share/\{shareId\}/acl/\{aclId\}"} />
  ...
  <Tab label="Via la CLI OVHcloud">
    **Prérequis supplémentaires**
    ...
    - La [CLI OVHcloud](https://github.com/ovh/ovhcloud-cli) doit être installée et configurée.
    
    Installez la CLI OVHcloud :
    ...
    **1\. Identifier votre réseau privé et votre sous-réseau**
    
    Listez les réseaux privés disponibles dans votre projet :
    
    ```bash
    ovhcloud cloud network private vrack list --cloud-project <PROJECT_ID>
    ```
    
    Récupérez les sous-réseaux associés à votre réseau privé :
    
    ```bash
    ovhcloud cloud network private vrack subnet list <NETWORK_ID> --cloud-project <PROJECT_ID>
    ```
    
    :::info
    Les IDs du réseau et du sous-réseau ressemblent à : **abc12345-def6-4abc-8def-123456abcdef**
    :::

    
    **2\. Créer un réseau partagé**
    
    Créez un réseau partagé rattaché à votre réseau privé et à votre sous-réseau :
    
    ```bash
    ovhcloud cloud storage file network create <REGION_NAME> \
    ...
    :::info
    Notez l'ID du réseau partagé renvoyé par la commande. Il est nécessaire pour créer le share.
    :::

    
    **3\. Créer un share NFS**
    
    Créez un share NFS de 150 Go connecté à votre réseau privé :
    
    ```bash
    ovhcloud cloud storage file share create <REGION_NAME> \
      --cloud-project <PROJECT_ID> \
      --name <my-share-name> \
      --size 150 \
      --share-type STANDARD_1AZ \
      --share-network-id <SHARE_NETWORK_ID>
    ```
    
    :::info
    Remplacez `<my-share-name>` par le nom souhaité pour votre share.
    :::

    
    **4\. Vérifier le statut du share**
    
    Listez vos shares et attendez que le nouveau share apparaisse avec le statut `READY` :
    
    ```bash
    ovhcloud cloud storage file share list --cloud-project <PROJECT_ID> --region <REGION_NAME>
    ```
    
    Récupérez les détails du share, notamment son chemin d'export :
    
    ```bash
    ovhcloud cloud storage file share get <SHARE_ID> --cloud-project <PROJECT_ID> --region <REGION_NAME> -o json
    ```
    
    :::info
    Notez le chemin d'export (`currentState.exportLocations[0].path`) qui ressemble à `10.1.0.12:/shares/share-abc12345-def6-4abc-8def-123456abcdef`.
    :::

    
    **5\. Autoriser une machine virtuelle cliente (ACL)**
    ...
    Accordez l'accès au share à l'aide de l'adresse IP privée de la machine virtuelle (par exemple, 10.1.0.123) :
    ...
    ovhcloud cloud storage file share acl create <SHARE_ID> \
      ...
      --access-to 10.1.0.123
    ```
    
    Vérifiez les règles d'accès :
    
    ```bash
    ovhcloud cloud storage file share acl list <SHARE_ID> --cloud-project <PROJECT_ID> --region <REGION_NAME>
    ```
    
    Pour supprimer une règle d'accès, récupérez son ID depuis la commande précédente, puis exécutez :
    
    ```bash
    ovhcloud cloud storage file share acl delete <SHARE_ID> <ACL_ID> --cloud-project <PROJECT_ID> --region <REGION_NAME>
    ```
    
    **6\. Monter le share sur votre machine virtuelle cliente**
    
    Connectez-vous à votre machine virtuelle cliente et installez les utilitaires NFS :
    ...
    **7\. Gérer le cycle de vie du share**
    ...
    ```bash
    ovhcloud cloud storage file share edit <SHARE_ID> \
      --cloud-project <PROJECT_ID> \
      --region <REGION_NAME> \
      --new-size 200
    ```
    ...
    ```bash
    ovhcloud cloud storage file share delete <SHARE_ID> --cloud-project <PROJECT_ID> --region <REGION_NAME>
    ```
  </Tab>
  <Tab label="Via la CLI OpenStack avec le plugin Manila">
    ...
    **2\. Vérifier les types de shares disponibles**
    ...
    :::info
    Une variante chiffrée est également disponible : `standard-1az-luks`. La création d'un share `-luks` génère automatiquement une clé de chiffrement dédiée — ne la modifiez pas et ne la supprimez pas, sous peine de rendre les données du share définitivement irrécupérables.
    :::
    ...
    **3\. Créer un réseau partagé**
    ...
    Créez un réseau partagé lié à votre réseau privé :
    ...
    :::info
    Remarque : Remplacez `<my-share-network-name>` par le nom que vous souhaitez donner à votre réseau partagé.
    :::

    
    Vérifiez le réseau partagé :
    ...
    **4\. Créer un share NFS**
    ...
    :::info
    Remplacez `standard-1az` par `standard-1az-luks` pour créer un share chiffré.
    :::
  ...
  <Tab label="Via Manila CSI dans l'environnement K8s">
    ...
    **11\. Montage d'un share Manila existant comme volume dans les pods**
    ...
    - `SHARE_SIZE` est la taille à allouer à votre volume NFS (en GB).
  ...
  <Tab label="Via Terraform">
    ...
    **2\. Déclarer le fournisseur OVHcloud**
    
    Ajoutez la configuration suivante à votre `main.tf` pour spécifier le fournisseur Terraform requis :
    ...
    Ce bloc garantit que Terraform utilise le fournisseur OVHcloud, avec une version minimale prenant en charge le chiffrement File Storage (OMK).
    ...
    **3\. Récupérer les informations de votre réseau privé**
    ...
    ```bash
    data "ovh_cloud_network_private_vrack" "private_network" {
      service_name = "<YOUR_PROJECT_ID>"
      id           = "<YOUR_PRIVATE_NETWORK_ID>"
    }
    
    data "ovh_cloud_network_private_vrack_subnets" "private_subnets" {
      service_name = "<YOUR_PROJECT_ID>"
      network_id   = "<YOUR_PRIVATE_NETWORK_ID>"
    }
    ```
    
    :::info
    Remplacez `<YOUR_PRIVATE_NETWORK_ID>` par l'ID de votre réseau privé. Vous pouvez le récupérer via l'API ou la CLI OVHcloud, comme indiqué dans les onglets précédents.
    :::

    
    Ces blocs de données permettent à Terraform d'interroger l'API OVHcloud et de récupérer les détails nécessaires de votre réseau privé et de votre sous-réseau, requis pour configurer le service File Storage.
    ...
    **4\. Créer un réseau partagé**
    ...
    resource "ovh_cloud_storage_file_share_network" "sharenetwork" {
      ...
      network_id   = data.ovh_cloud_network_private_vrack.private_network.id
      subnet_id    = data.ovh_cloud_network_private_vrack_subnets.private_subnets.subnets[0].id
    }
    ```
    
    Cette ressource associe votre réseau privé et votre sous-réseau existants à un réseau partagé. Elle est nécessaire pour provisionner et gérer des shares File Storage.
    
    :::info
    Si votre réseau privé possède plusieurs sous-réseaux, remplacez `subnets[0]` par l'index (ou une expression filtrée) correspondant au sous-réseau de votre région cible.
    :::
    ...
    **5\. Créer un share NFS**
    ...
    resource "ovh_cloud_storage_file_share" "share" {
      ...
      size             = 150
      share_network_id = ovh_cloud_storage_file_share_network.sharenetwork.id
    }
    ```
    
    Cette ressource provisionne un share NFS rattaché au réseau partagé précédemment créé. Ajustez la `size` selon vos besoins.
    
    :::info
    Pour créer un share chiffré (OMK), ajoutez un bloc `encryption` :
    ...
    Cela chiffre automatiquement le share au repos avec LUKS et génère une clé de chiffrement dédiée. Le chiffrement ne peut être défini qu'à la création — le modifier par la suite recrée le share. Nécessite `terraform-provider-ovh >= 2.20.0`.
    :::
    ...
    **6\. Autoriser une machine virtuelle cliente**
    ...
    ```bash
    resource "ovh_cloud_storage_file_share_acl" "share_access" {
    ...
    Cette ressource autorise l'adresse IP cliente spécifiée à accéder au share NFS avec des permissions en lecture / écriture (`access_level` accepte aussi `READ_ONLY`).
    ...
    **7\. Récupérer le chemin d'exportation**
    ...
    ```bash
    output "export_path" {
      value = ovh_cloud_storage_file_share.share.current_state.export_locations[0].path
    }
    ```
