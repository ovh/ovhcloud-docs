...
Les shares File Storage d'OVHcloud peuvent être chiffrés au repos grâce aux clés gérées par OVHcloud (OMK), le même mécanisme utilisé pour les [volumes Block Storage chiffrés](/guides/storage-and-backup/block-storage/block-storage-the-right-storage-class) (LUKS).

Pour créer un share chiffré, utilisez le type `standard-1az-luks` à la place de `standard-1az` lors de la création de votre share, quelle que soit l'interface utilisée (API OVHcloud, CLI OVHcloud, CLI OpenStack, Manila CSI ou Terraform).
...
<Tabs>
  ...
  <Tab label="Via l’API OVHcloud">
    ...
    **Note :** Remplacez `<my-share-name>` par le nom de share que vous avez choisi.
    ...
    :::info
    Pour créer un share chiffré, définissez `encryption.encrypted` sur `true` dans le corps de la requête. Cela dérive automatiquement le type `standard-1az-luks` et génère une clé de chiffrement dédiée — ne la modifiez pas et ne la supprimez pas, sous peine de rendre les données du share définitivement irrécupérables.
    :::
  ...
  <Tab label="Via la CLI OVHcloud">
    ...
    **1\. Identifier votre réseau privé et votre sous-réseau**
    ...
    ```bash
    ovhcloud cloud network private vrack list --cloud-project <PROJECT_ID> --region <REGION_NAME>
    ```
    
    Récupérez les sous-réseaux associés à votre réseau privé :
    
    ```bash
    ovhcloud cloud network private vrack subnet list <NETWORK_ID> --cloud-project <PROJECT_ID> --region <REGION_NAME>
    ```
    ...
    **3\. Vérifier le statut du share**
    ...
    ```bash
    ovhcloud cloud storage file share get <SHARE_ID> --cloud-project <PROJECT_ID> --region <REGION_NAME>
    ```
    
    :::info
    Notez le chemin d'export (`exportLocations[0].path`) qui ressemble à `10.1.0.12:/shares/share-abc12345-def6-4abc-8def-123456abcdef`.
    :::

    
    **4\. Autoriser une machine virtuelle cliente (ACL)**
    ...
    **5\. Monter le share sur votre machine virtuelle cliente**
    ...
    **6\. Gérer le cycle de vie du share**
  ...
  <Tab label="Via Terraform">
    ...
    **2\. Déclarer le fournisseur OVHcloud**
    ...
    Ce bloc garantit que Terraform utilise le fournisseur OVHcloud, avec une version minimale supportant le chiffrement File Storage (OMK).
    ...
    **3\. Récupérer les informations de votre réseau privé**
    ...
    Ces blocs de données permettent à Terraform d'interroger l'API OVHcloud et de récupérer les détails nécessaires de votre réseau privé et sous-réseau, requis pour configurer le service File Storage.
    ...
    **4\. Créer un réseau partagé**
    ...
    Cette ressource associe votre réseau privé et sous-réseau existants à un réseau partagé. Elle est nécessaire pour provisionner et gérer des shares File Storage.
    ...
    **6\. Autoriser une machine virtuelle cliente**
    ...
    Cette ressource autorise l'IP cliente spécifiée à accéder au share NFS avec des permissions en lecture / écriture (`access_level` accepte aussi `READ_ONLY`).
