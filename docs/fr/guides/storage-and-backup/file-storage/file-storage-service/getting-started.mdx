...
<Tabs>
  ...
  <Tab label="Via l’API OVHcloud">
    ...
    **Note :** Remplacez `<my-share-name>` par le nom de share que vous avez choisi.
    ...
    :::info
    Pour créer un share chiffré, utilisez le type `standard-1az-luks` à la place de `standard-1az`. Cela génère automatiquement une clé de chiffrement dédiée — ne la modifiez pas et ne la supprimez pas, sous peine de rendre les données du share définitivement irrécupérables.
    :::
