...
<Tabs>
  ...
  <Tab label="Via Terraform">
    ...
    **3\. Récupérer les informations de votre réseau privé**
    ...
    data "ovh_cloud_project_network_private_subnets" "private_subnets" {
      service_name = "<YOUR_PROJECT_ID>"
      network_id   = data.ovh_cloud_project_network_private.private_network.network_id
    }
    ```
    
    :::info
    Remplacez `<YOUR_PRIVATE_NETWORK_ID>` par l'ID OVHcloud de votre réseau privé, renvoyé par <ApiLink section="/cloud" method="GET" route={"/cloud/project/\{serviceName\}/network/private"}>la liste de vos réseaux privés</ApiLink>. Il diffère de l'ID réseau OpenStack régional indiqué dans les onglets précédents.
    :::
    ...
    **4\. Créer un réseau partagé**
    ...
    resource "ovh_cloud_storage_file_share_network" "sharenetwork" {
      ...
      network_id   = data.ovh_cloud_project_network_private.private_network.regions_openstack_ids["<YOUR_REGION_NAME>"]
      subnet_id    = data.ovh_cloud_project_network_private_subnets.private_subnets.subnets[0].id
    }
    ```
