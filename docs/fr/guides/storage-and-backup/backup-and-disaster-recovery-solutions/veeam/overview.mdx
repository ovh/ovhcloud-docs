---
pageType: overview
title: Veeam Enterprise Plus
description: "Configurez Veeam Enterprise Plus sur OVHcloud Hosted Private Cloud - onboarding, dépôts, jobs de sauvegarde et de réplication, et workflows de restauration."
text: "Découvrez les guides Veeam Enterprise Plus"
essentialsTitle: "Les guides essentiels pour commencer"
essentials:
  - title: "Installer Veeam Backup & Replication"
    link: /guides/storage-and-backup/backup-and-disaster-recovery-solutions/veeam/veeam-veeam-backup-replication
  - title: "Préparer une sauvegarde de serveur Bare Metal avec Veeam Enterprise"
    link: /guides/bare-metal-cloud/dedicated-servers/veeam-enterprise-server-backup-preparation
  - title: "Sauvegarder un serveur Bare Metal Linux avec Veeam Enterprise"
    link: /guides/bare-metal-cloud/dedicated-servers/veeam-enterprise-server-backup-linux
  - title: "Sauvegarder un serveur Bare Metal Windows via Veeam Agent pour Windows"
    link: /guides/bare-metal-cloud/dedicated-servers/veeam-enterprise-server-backup-windows-agent
  - title: "Restaurer un serveur Bare Metal avec Veeam Enterprise"
    link: /guides/bare-metal-cloud/dedicated-servers/veeam-enterprise-server-restore
goFurther:
  title: Aller plus loin
  items:
    - title: Consulter la communauté OVHcloud
      link: https://community.ovhcloud.com/
    - title: Consulter la roadmap & le changelog
      link: https://www.ovhcloud.com/fr/roadmap-changelog/
    - title: Rejoindre Discord
      link: https://discord.gg/ovhcloud
cta:
  title: Des questions ? Accédez au support OVHcloud
  description: "Trouvez la réponse à vos questions dans notre centre d'aide."
  actions:
    - text: Accéder au support
      link: https://help.ovhcloud.com/
      theme: brand
---
