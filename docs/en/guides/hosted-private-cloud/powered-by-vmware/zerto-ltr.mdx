...
- Have S3 credentials associated with this bucket (access key and secret key).

:::info
LTR configuration is done from the Zerto section of your datacenter in Managed VMware vSphere.
:::
...
For immutability, enable the `enableImmutability` option, then choose a policy:
...
If you choose `Period`, set `numberOfDays` between **30 and 90 days**.
...
- A **Order an OVHcloud bucket** button redirecting to OVHcloud Object Storage ordering.
  - If you have a Public Cloud project, the redirect opens the Object Storage section of the Manager.
  - If you do not have a Public Cloud project, the redirect opens the Object Storage page on the OVHcloud website.
- A secondary **Add existing bucket** button opening the add window.

### Step 7: Manage existing buckets

For each configured bucket, the available actions are:
