...
Today, the position of each node in the rack elevation is set **manually**. This is the standard way of working: an administrator sets a device's position by editing the device in NetBox and assigning it a rack and a rack unit (U). A position set this way is persistent: it is not changed automatically, so it stays until you change it yourself.
