...
There are backups of the controller data in OPCP. These allow you to recover in case of physical or logical issues. The backups mentioned here don't cover the rest, like all the data present on the OPCP servers.

Backups of OPCP controller data are automatically scheduled hourly for databases, PVs, KMS autounsealer shares and tfstate. Database backups are done with the kubernetes resource `ScheduledBackups`, PVs, KMS autounsealer shares and tfstate backups are done using [velero](https://velero.io/).

The backups of `tfstate` contain the state of [opentofu](https://opentofu.org/) within the setup of OPCP. It is used for example to set up the IAM.
...
Verify db backups are done by typing `kubectl get backup -A`. All available database backups are listed.
...
Adjust the values as needed. Then you can do a `s3cmd ls s3://bucket-name` to see if there is any data in there.
