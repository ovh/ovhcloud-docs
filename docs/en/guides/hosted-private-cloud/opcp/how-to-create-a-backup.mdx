...
There are backups of the controller data in OPCP. These allow you to recover in case of pysical or logical issues. The backups mentioned here don't cover the rest, like all the data present on the OPCP servers.

Backups of OPCP controller data are automatically scheduled hourly for databases, PVs, KMS autounsealer shares and tfstate. Database backups are done with the kubernetes resource `ScheduledBackups`, PVs, KMS autounsealer shares and tfstate is done using [velero](https://velero.io/).

The backups of `tfstate` are containing the state of [opentofu](https://opentofu.org/) within the setup of OPCP. It is used for example to set up the IAM.

PVs mean the kubernetes Persisten Volume resources. This is data with state stored in the controllers kubernetes cluster.
