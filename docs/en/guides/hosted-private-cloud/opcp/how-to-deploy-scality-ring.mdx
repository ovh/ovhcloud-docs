...
description: "Find out how to deploy a Scality RING storage cluster on OVHcloud OPCP bare metal with Terraform, from node preparation to verification"
...
- The following tools installed on the workstation you will run the deployment from:
  - the [OpenStack CLI](https://docs.openstack.org/python-openstackclient/latest/) (`openstack`), to upload images, set up LACP/RAID (step 3) and run the verification commands (step 8);
  ...
  - [`jq`](https://jqlang.org/), to parse the JSON output of several `openstack` commands used below;
  - an SSH client, useful if you need to reach the supervisor directly rather than only through its web UI.
- A Scality RING **supervisor** qcow2 image and one or more **storage** qcow2 images, based on Rocky Linux or RHEL — the only operating systems RING supports on OPCP — ready to upload to Glance. The supervisor image must already carry the RING installer bundle, since the supervisor has no guaranteed way to fetch it after boot on this platform.
  :::info
  These images are not something you build yourself: request the supervisor and storage qcow2 images (and the RING installer bundle, if not already embedded in the supervisor image) from Scality, through your account team or [Scality support](https://support.scality.com). Building or customising RING images is not covered by this guide — see the [Scality documentation portal](https://documentation.scality.com) for that procedure, or OVHcloud's guide on building a [custom CentOS Stream image](/guides/hosted-private-cloud/opcp/how-to-create-centos-image) for background on the underlying Ironic/Nova image requirements.
  :::
...
:::tip
In some OPCP regions, the service catalog advertises endpoints that are not reachable from your network. If `openstack` commands fail to connect after a successful token issue, add explicit per-service endpoint overrides (`OS_COMPUTE_ENDPOINT_OVERRIDE`, `OS_IMAGE_ENDPOINT_OVERRIDE`, `OS_NETWORK_ENDPOINT_OVERRIDE`, `OS_BAREMETAL_ENDPOINT_OVERRIDE`, `OS_VOLUME_ENDPOINT_OVERRIDE`) pointing at the addresses your project was given. Terraform's OpenStack provider accepts the same overrides in its provider block.
:::
...
Two node-level settings live outside Terraform, because the standard OpenStack Ironic driver has no Terraform resource for them. Configure both **before** the nodes are deployed — they can only be changed on nodes that carry no instance, and redoing them later requires tearing the instance down first.
...
- If `lacp_enabled` is set, every node you expect Terraform to pick already carries its LACP port groups (step 3) — Nova can select any `available` node, and the installer CSV bakes in bond names at plan time, so a node without bonding will boot mismatched.
...
:::warning
On bare metal, a Terraform *replace* of an instance is a full Ironic redeploy of the physical machine — the node is wiped and reprovisioned from scratch. Look specifically for `must be replaced` or `must be destroyed` on any bare-metal instance, network port, or the deployment keypair, and stop to double-check intent before applying if you see one you did not expect.
:::
...
:::info
Once launched, the installer skips itself if it finds a `done` marker or a still-running process, so rebooting a node mid-install will not restart a finished or in-progress install.
:::
