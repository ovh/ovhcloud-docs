...
:::info
The `dc_operator` has no Keycloak user administration rights. Its OpenStack access is the same as the `reader` role's: not granted by default, it requires project attributes to be configured on the user or their group.
:::
