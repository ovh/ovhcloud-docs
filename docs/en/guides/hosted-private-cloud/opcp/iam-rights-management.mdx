...
Supervision and audit profile. This role provides read-only access by default, with no administration capability.

It allows you to:

- View your own account settings in Keycloak
- Access OpenStack project resources in **reader** or **member** mode, subject to project attributes configured in Keycloak

:::info
The `reader` role can be granted the OpenStack `reader` (read-only) or `member` (read-write) role, never `admin`. Both require project attributes to be configured on the Keycloak account or group. Without a project attribute, no OpenStack resource is visible.
:::
...
:::info
The `dc_operator` has no Keycloak user administration rights. Its OpenStack access is the same as the `reader` role's: it is not granted by default and requires project attributes to be configured on the user or their group.
:::
...
:::info
The `reader` and `dc_operator` roles' OpenStack access requires **project attributes** to be configured on the user or their group in Keycloak. Without a configured project attribute, no OpenStack resource is accessible. Both roles reach OpenStack through the **Horizon** interface, not through the Dashboard's OpenStack iframe.
:::
...
:::info
For the `reader` and `dc_operator` roles, the OpenStack `reader` and `member` roles can be assigned via project attributes. The `admin` role remains reserved for `it_admin` and `master_admin`.
:::
