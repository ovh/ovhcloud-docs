...
:::warning
This chapter reflects OPCP version **3.0.6 and newer**.
:::


#### The `reader` role

Supervision and audit profile. This role provides read-only access to the platform's basic applications.
...
**Legend**: ✅ Allowed | ❌ Not allowed | 🟡 Need Configuration (subject to project attributes)
...
:::info
The `reader` and `dc_operator` roles' OpenStack access requires **project attributes** to be configured on the user or their group in Keycloak. Without a configured project attribute, no OpenStack resource is accessible.
:::
