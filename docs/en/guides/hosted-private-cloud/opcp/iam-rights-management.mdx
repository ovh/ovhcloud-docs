...
- Access OpenStack project resources in `reader`, `member` or `admin` mode, subject to project attributes configured in Keycloak
...
:::info
The `dc_operator` has no Keycloak user administration rights. Like the `reader`, it holds no OpenStack rights by default: they require project attributes on the user or their group. Unlike the `reader`, it has no OpenStack iframe in the Dashboard and must use the **Horizon** interface.
:::
...
:::info
The `reader` and `dc_operator` roles' OpenStack access requires **project attributes** to be configured on the user or their group in Keycloak. Without a configured project attribute, no OpenStack resource is accessible. Any OpenStack role can be granted this way, including `admin`.
:::
...
:::info
For the `reader` and `dc_operator` roles, any OpenStack role can be assigned via project attributes, including `admin`. Only `it_admin` and `master_admin` hold OpenStack rights without a project attribute.
:::
