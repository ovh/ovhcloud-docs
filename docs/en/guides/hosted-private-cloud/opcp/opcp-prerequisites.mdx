...
- Having read the [Network integration and platform connectivity](/guides/hosted-private-cloud/opcp/opcp-network-architecture) guide, which describes the network architecture in which these prerequisites take place.
...
:::info
**Transmission of sensitive items.** Several prerequisites involve secrets: IPsec pre-shared key, S3<sup>1</sup> access keys, certificate private keys, and so on. These items **must not be transmitted through an insecure channel**. Agree with your OVHcloud point of contact on an appropriate transmission channel for each secret.
:::
...
- [Network integration and platform connectivity](/guides/hosted-private-cloud/opcp/opcp-network-architecture)
