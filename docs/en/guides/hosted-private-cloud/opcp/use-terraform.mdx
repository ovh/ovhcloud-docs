...
:::warning
**Modifying the image of an existing instance triggers a destructive rebuild.** If the instance's image (`image_id` or `image_name`) changes at the next `terraform plan` / `tofu plan` — for example after the image was deleted and re-created with the same name — the `apply` rebuilds the instance: it keeps its ID and IP addresses, but **the content of its disk is entirely erased** and replaced by the new image.

The plan presents this operation as a simple in-place update (`will be updated in-place`, `0 to destroy`): it is nevertheless destructive.

The `lifecycle` block in the example above (`ignore_changes = [image_name]`) protects against this scenario by pinning the instance's image. If you also reference the image ID, add `image_id` to the list. If you deliberately want to change the image of an instance, take a snapshot or a backup of it first.
:::
