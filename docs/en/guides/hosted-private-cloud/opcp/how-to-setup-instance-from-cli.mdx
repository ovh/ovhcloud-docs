...
This guide details the steps to install an OPCP node by creating an instance through the OpenStack CLI. 
...
To check the current state of a node and retrieve its ID, see the [OPCP Node Lifecycle](/guides/hosted-private-cloud/opcp/node-lifecycle) guide.
...
If a node remains stuck in an intermediate state after a shutdown performed from the OS, you can unblock it by forcing it off and then on again with the commands `openstack baremetal node power off <node>` and `openstack baremetal node power on <node>`.
:::
