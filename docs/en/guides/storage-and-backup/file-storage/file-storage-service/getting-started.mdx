...
It can be accessed via OVHcloud API, OVHcloud CLI, OpenStack CLI and API, Manila CSI, OVHcloud Control Panel, and Terraform.
...
OVHcloud File Storage shares can be encrypted at rest using OVHcloud Managed Keys (OMK), the same mechanism used for [encrypted Block Storage volumes](/guides/storage-and-backup/block-storage/block-storage-the-right-storage-class) (LUKS).

To create an encrypted share, use the `standard-1az-luks` type instead of `standard-1az` with the OVHcloud API, the OpenStack CLI or Manila CSI. With Terraform, add an `encryption` block instead (see the Terraform tab).

:::info
Creating a `-luks` share automatically generates a dedicated encryption key. Encryption and decryption are handled transparently by the OVHcloud infrastructure — no additional configuration is required on the client side.
:::

:::info
No performance impact has been observed on encrypted File Storage shares so far.
:::
...
<Tabs>
  <Tab label="Via the OVHcloud Control Panel">
    ...
    **1\. Create a share**
    ...
    Set the performance level and size (in GB), associate a private network, and click <code className="action">Confirm</code> to complete the share creation.
  ...
  <Tab label="Via the OVHcloud API">
    ...
    **NOTE:** Only select a private network.
    ...
    Create a 150 GB NFS share attached to your private network:
    ...
    **NOTE:** Replace `<my-share-name>` with your chosen share name.
    ...
    :::info
    To create an encrypted share, set `encryption.encrypted` to `true` in the request body. This automatically derives the `standard-1az-luks` type and generates a dedicated encryption key — do not modify or delete it, as doing so would make the share's data permanently unrecoverable.
    :::
  ...
  <Tab label="Via the OVHcloud CLI">
    **Additional requirements**
    ...
    - The [OVHcloud CLI](https://github.com/ovh/ovhcloud-cli) must be installed and configured.
    ...
    **1\. Identify your private network and subnet**
    
    List the private networks available in your project:
    
    ```bash
    ovhcloud cloud network private vrack list --cloud-project <PROJECT_ID>
    ```
    
    Retrieve the subnets associated with your private network:
    ...
    **2\. Create a share network**
    ...
    Create a share network attached to your private network and subnet:
    ...
    :::info
    Note the share network ID returned by the command. You need it to create the share.
    :::
    ...
    **3\. Create an NFS share**
    ...
    :::info
    Replace `<my-share-name>` with your chosen share name.
    :::

    
    **4\. Check the share status**
    
    List your shares and wait until the new share appears with the status `READY`:
    
    ```bash
    ovhcloud cloud storage file share list --cloud-project <PROJECT_ID> --region <REGION_NAME>
    ```
    
    Retrieve the share details, including its export path:
    ...
    :::info
    Note the export path (`currentState.exportLocations[0].path`), which looks like `10.1.0.12:/shares/share-abc12345-def6-4abc-8def-123456abcdef`.
    :::

    
    **5\. Authorize a client VM (ACL)**
    ...
    To remove an access rule, retrieve its ID from the previous command, then run:
    
    ```bash
    ovhcloud cloud storage file share acl delete <SHARE_ID> <ACL_ID> --cloud-project <PROJECT_ID> --region <REGION_NAME>
    ```
    
    **6\. Mount the share on your client VM**
    
    Connect to your client VM and install the NFS utilities:
  ...
  <Tab label="Via the OpenStack CLI with the Manila plugin">
    ...
    **2\. Check available share types**
    ...
    :::info
    An encrypted variant is also available: `standard-1az-luks`. Creating a `-luks` share automatically generates a dedicated encryption key — do not modify or delete it, as doing so would make the share's data permanently unrecoverable.
    :::
    ...
    **4\. Create an NFS share**
    ...
    :::info
    Replace `standard-1az` with `standard-1az-luks` to create an encrypted share.
    :::
  ...
  <Tab label="Via Manila CSI in Kubernetes environment">
    ...
    **11\. Mounting an existing Manila share as a volume in Pods**
    ...
    - `SHARE_SIZE` is the size to allocate to your NFS volume (in GB).
  ...
  <Tab label="Via Terraform">
    ...
    **2\. Declare the OVHcloud provider**
    
    Add the following configuration to your `main.tf` to specify the required Terraform provider:
    ...
    This block ensures that Terraform uses the OVHcloud provider, with a minimum version that supports File Storage encryption (OMK).
    ...
    **3\. Retrieve information about your private network**
    ...
    :::info
    Replace `<YOUR_PRIVATE_NETWORK_ID>` with your private network ID. You can retrieve it via the OVHcloud API or CLI, as shown in the previous tabs.
    :::

    
    These data blocks allow Terraform to query the OVHcloud API and retrieve the necessary details of your private network and subnet, which are required for configuring the File Storage service.
    ...
    **4\. Create a share network**
    ...
    This resource binds your existing private network and subnet to a share network. It is required to provision and manage File Storage shares.
    
    :::info
    If your private network has several subnets, replace `subnets[0]` with the index (or a filtered expression) matching the subnet for your target region.
    :::
    ...
    **5\. Create an NFS share**
    ...
    This resource provisions an NFS share attached to the previously created share network. Adjust the `size` according to your requirements.
    
    :::info
    To create an encrypted share (OMK), add an `encryption` block:
    ...
    This automatically encrypts the share at rest with LUKS and generates a dedicated encryption key. Encryption can only be set at creation — changing it afterwards recreates the share. Requires `terraform-provider-ovh >= 2.20.0`.
    :::
    ...
    **6\. Authorize a client VM**
    ...
    This resource authorizes the specified client IP to access the NFS share with read/write permissions (`access_level` also accepts `READ_ONLY`).
