...
To create an encrypted share, use the `standard-1az-luks` type instead of `standard-1az` when creating your share, whichever interface you use (OVHcloud API, OVHcloud CLI, OpenStack CLI, Manila CSI, or Terraform).
...
<Tabs>
  ...
  <Tab label="Via the OVHcloud CLI">
    ...
    **3\. Check the share status**
    ...
    List your shares and wait until the new share appears with the status `available`:
    ...
    :::info
    Note the export path (`exportLocations[0].path`), which looks like `10.1.0.12:/shares/share-abc12345-def6-4abc-8def-123456abcdef`.
    :::

    
    **4\. Authorize a client VM (ACL)**
    ...
    **5\. Mount the share on your client VM**
