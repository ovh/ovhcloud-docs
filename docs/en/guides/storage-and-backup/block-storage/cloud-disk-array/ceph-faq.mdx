...
You can enable CephFS using API to have fully managed distributed filesystem.
...
In order to maximize performance and minimize latency for block devices (RBD) and filesystem (CephFS) it is only possible to use it inside OVH network - from other OVH services. If you need external access please contact us.
