...
- **Delete expired delete markers**: remove delete markers that have no remaining non-current versions. No additional fields required.<br/>
:::reminder
Must be specified in another rule if an expiration delay in days is specified or if tags are specified in Step 2.
:::
...
- **Delete incomplete multipart uploads**: stop and remove orphan upload parts.
    - **Days after initiation**: number of days after the upload was initiated before its parts are deleted (must be greater than 0).<br/>
:::reminder
Must be specified in another rule if tags and/or size filters are specified in Step 2.
:::
