...
| AbortIncompleteMultipartUpload                      | no       | A lifecycle action that applies a delete operation on parts of an incomplete multipart upload. <br/><br/> ⚠️ This attribute is mutually exclusive with Filter.Tag, Filter.ObjectSizeGreaterThan and Filter.ObjectSizeLessThan: if you filter objects by tags and/or size, you must configure a separate rule to delete incomplete multipart uploads. |
...
In the <code className="action">Object Storage</code> section of your Public Cloud project, click the name of the target bucket, go to the <code className="action">Lifecycle</code> tab, and click <code className="action">Create rule</code>.
...
- **Delete expired delete markers**: remove delete markers that have no remaining non-current versions. No additional fields required.
    ...
    :::warning
    This action must be configured in a separate rule if an expiration in days is also set for the current version, or if a tag filter was defined in Step 3.
    :::
...
- **Delete incomplete multipart uploads**: stop and remove orphan upload parts.
    ...
    :::warning
    This action must be configured in a separate rule if tag and/or object size filters were defined in Step 3.
    :::
