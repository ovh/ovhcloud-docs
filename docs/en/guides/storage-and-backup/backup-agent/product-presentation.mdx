...
The VSPC allows you to deploy the backup policies to the agents stored on them, and allows you to give each agent the storage and credentials information when starting the backup.
...
Once the agent has performed backups, you can restore your data in two ways: file-level restore (FLR) or server restore.

## Key aspects

There are several key advantages to this solution:

- First automatic backup policy with 30 days of retention.
- Possibility to extend retention up to 365 days.
...
- Storage is based on [OVHcloud Object Storage](https://www.ovhcloud.com/en-gb/public-cloud/object-storage/) technology, which is hosted in OVHcloud datacentres.
