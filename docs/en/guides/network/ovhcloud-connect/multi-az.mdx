...
2. **Configure BGP on both links** with appropriate route priorities (using AS-path prepending on your own devices for path selection) so that traffic prefers one path but can fall back to the other.
...
- **MED** — **Not supported** on OVHcloud Connect. Active/passive path selection relies on AS-path prepending, which is customer-side only.
