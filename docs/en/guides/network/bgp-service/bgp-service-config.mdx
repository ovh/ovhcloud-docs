...
- At least one [Bare Metal dedicated server](https://www.ovhcloud.com/en-gb/bare-metal/) from one of the following ranges: High Grade, Scale, Advance Gen3. All servers that will take part in the BGP peering must be in the same 1-AZ region.
- Being logged in to your <ManagerLink to="/">OVHcloud Control Panel</ManagerLink>.
- If you use [Bring Your Own IP (BYOIP)](https://www.ovhcloud.com/en-gb/network/byoip/): the IP prefixes you own and are allowed to announce.
- A [vRack private network](https://www.ovhcloud.com/en-gb/network/vrack/).
- Knowledge of IP networking and the BGP routing protocol.
- Knowledge of Linux network settings.
