...
```mermaid
flowchart LR
    Client[Client] -->|HTTPS port 443| WAF[OWAF proxy<br/>Request inspection]
    WAF -->|Allowed| Backend[Backend<br/>Upstream URL]
    WAF -->|Matched in Blocking mode| Blocked[403 Forbidden]
    Operator[Operator] -->|HTTPS + Bearer token<br/>port 8443| AdminUI[OWAF Admin UI]
...
| Changes are not applied | The form was not saved. | Verify the <code className="action">Save Changes</code> button has been clicked on the page you edited. Configuration is applied immediately on save without a restart. |
