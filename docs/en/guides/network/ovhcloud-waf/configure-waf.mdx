...
description: Configure the OVHcloud Web Application Firewall to inspect, filter, and block malicious HTTP/HTTPS traffic before it reaches your backend applications
...
- The **OWAF instance URL** provided by the OVHcloud team (typically `http://<your-instance-ip>:8443/control-plane`)
...
- Network access from your client to the OWAF instance on port `8443` (Admin UI) and from your clients to port `443` (WAF data path, HTTPS)
...
Client traffic is sent to the WAF instance on port `443` (HTTPS), where each request is inspected against the active rule set and scored against the anomaly threshold. Requests that pass the inspection are forwarded to the backend defined as the **Upstream URL** on the **Proxy** page; requests that exceed the threshold in **Blocking** mode are rejected with a `403 Forbidden` response. Operators configure the engine and view live metrics through the OWAF Admin UI on port `8443`, which is fully independent from the data path.
...
| :--- | :--- |
| **Protect a public API** | Block SQLi, XSS, and RCE attempts targeting API endpoints. <br/>Use path-prefix rules to apply stricter policies to sensitive routes such as `/admin/`. |
...
| **Pre-production security testing** | Run in Detection mode to see what would be blocked without impacting users. <br/>Review the **Stats** dashboard to identify noisy rules before going live. |
| **Credential and token rotation** | Use the **Proxy** request header rules to rotate service tokens transparently. <br/>Strip the client's `Authorization` header and inject a backend service token. |
...
- **OWASP Core Rule Set coverage** with built-in rules detecting SQLi, XSS, RCE, LFI, SSTI, scanner detection, and other common attack categories.
...
| Customer SSL certificate | Not yet supported (SSL certificates are fully managed by OVHcloud in alpha phase) |
...
- The **OWAF instance URL** (for example, `http://<your-instance-ip>:8443/control-plane`)
...
### Step 2: Point your domain to the OWAF instance

Before signing in to the Admin UI or sending traffic through the WAF, you need to route your domain to the IP address provided by OVHcloud during onboarding. This is required so that TLS certificates can be issued via Let's Encrypt (see [TLS management](#tls-management)) and so that clients reach the WAF data path.

1. Log in to your DNS provider's management console (for example, the OVHcloud Control Panel, Cloudflare, Route 53).
2. Locate the DNS zone for the domain you submitted during the alpha onboarding.
3. Create or update an `A` record pointing your domain (or subdomain) to the **OWAF instance IP** provided by the OVHcloud team:

    | Field | Value |
    | :--- | :--- |
    | **Type** | `A` |
    | **Name** | The hostname you want to protect (for example, `www` or `@` for the apex domain) |
    | **Target** | The OWAF instance IP address provided by OVHcloud |
    | **TTL** | `3600` (or your provider's default) |

4. Save the record and wait for DNS propagation. You can verify with `dig +short your-domain.example.com` or an equivalent tool; the response must return the OWAF instance IP.

:::info
DNS propagation can take from a few minutes up to 24 hours depending on the TTL of any previous record. Wait until the new IP is returned globally before sending production traffic, as Let's Encrypt certificate issuance relies on the domain resolving to the OWAF instance.
:::

### Step 3: Sign in to the OWAF Admin UI
...
1. In the sidebar, click <code className="action">Proxy</code>.
...
4. Click <code className="action">Save Changes</code>.
...
### Step 5: Choose the WAF mode and Paranoia Level
...
1. In the sidebar, click <code className="action">Configuration</code>.
2. Under **WAF Mode**, click the mode you want to apply:

    | Mode | Behaviour |
    | :--- | :--- |
    | **Blocking** | Requests that match rules and exceed the anomaly threshold receive a `403 Forbidden`. <br/>Recommended for production. |
    | **Detection** | Rules are evaluated and events are logged, but all requests are allowed through. <br/>Recommended for testing new rules without impact on users. |
    | **Disabled** | The WAF is bypassed completely. All traffic is forwarded to the backend without inspection. <br/>Use only for debugging. |

3. Under **Paranoia Level**, move the slider to the desired level:

    | Level | Name | Description |
    | :--- | :--- | :--- |
    | **1** | Basic | Only the most common and reliable attack patterns. Lowest false positive rate. <br/>Recommended for most deployments. |
    | **2** | Standard | Broader detection coverage. May produce some false positives. |
    | **3** | Strict | Aggressive detection. Expect more false positives. <br/>Recommended for high-security applications. |
    | **4** | Paranoid | Maximum detection. Will produce many false positives. <br/>Suitable for security-critical environments with careful tuning. |
...
5. Click <code className="action">Save Changes</code>.
...
After Steps 2 to 5, validate that the WAF is processing traffic as expected:

1. From a client, send a normal request to the WAF data path (port `443` over HTTPS) and confirm the response is served by your backend.
...
:::warning
The OWAF is currently single-tenant per instance and may does not persist rule changes across reboots in the alpha release. Do not rely on it as the sole protection layer for a production workload until general availability.
:::

## TLS management

TLS is **fully managed by OVHcloud**. You do not need to provision certificates, configure a reverse proxy, or handle renewals. The WAF instance is deployed where an nginx sidecar handles TLS termination in front of the WAF engine, and certificates are automatically issued and renewed via **cert-manager** with **Let's Encrypt**.

### How it works

The nginx sidecar runs alongside the WAF engine in the same pod and:

- Listens on port `443` (HTTPS) for WAF data-path traffic and proxies it to the WAF engine internally.
- Listens on port `8443` (HTTP) for Admin UI traffic and proxies it to the Admin UI internally.
- Listens on port `80` (HTTP) solely to serve Let's Encrypt HTTP-01 ACME challenges and redirects all other traffic to HTTPS.
- Enforces TLSv1.2 and TLSv1.3 only.

### What this means for you

- Configurate your domain to route in the OWAF IP provided
- Send your traffic to the WAF instance over **HTTPS on port `443`**. Plain HTTP on port `80` is redirected to HTTPS automatically.
- Access the Admin UI over **HTTP on port `8443`**.
- **No TLS configuration is required on your side.** Certificate provisioning, renewal, and nginx configuration are handled entirely by OVHcloud.

:::info
The real client IP is preserved and forwarded to the WAF engine via the `X-Real-IP` and `X-Forwarded-For` headers set by the nginx sidecar. IP-based custom rules using the `ipmatch` operator on `REMOTE_ADDR` work correctly without any additional configuration.
:::
...
- Click <code className="action">Refresh</code> to reload the current rule set.

### Disabling and enabling a rule

1. Locate the rule in the table.
2. Click <code className="action">Disable</code> in the **ACTIONS** column to deactivate it, or <code className="action">Enable</code> to reactivate it.

Disabled rules appear in the **Disabled Rules** read-only list on the **Configuration** page. To re-enable a rule from there, return to the **Rules** page and click <code className="action">Enable</code>.
...
1. On the **Rules** page, click <code className="action">+ Add Rule</code> in the top-right corner of the page.
2. Fill in the rule form:
    ...
    | **Targets** | What part of the request to inspect. Select one or more: <br/>`ARGS` (query/body parameters), `REQUEST_PATH`, `REQUEST_URI`, `REQUEST_HEADERS`, `REQUEST_COOKIES`, `REQUEST_BODY`, `REMOTE_ADDR`. <br/>Use **Select all** to check every target. |
...
3. Click <code className="action">Save Rule</code> to create the rule, or <code className="action">Cancel</code> to discard.

{/* DRAFT: To screenshot — capture the Add Rule form with the fields populated for a sample bot-block rule */}

### Edit a custom rule

1. Click <code className="action">Edit</code> on the row of a custom rule.
2. Modify the fields as needed. The **Rule ID** cannot be changed.
3. Click <code className="action">Save Rule</code>.

### Delete a custom rule

1. Click <code className="action">Delete</code> on the row of a custom rule.
...
## Transforming request headers on the proxy
...
2. Click <code className="action">+ Add rule</code> to append an empty rule row.
3. Fill in:
...
5. Click <code className="action">Save Changes</code> to apply.
...
| **Access-Control-Allow-Headers** | Custom allowed headers for preflight responses. Leave empty to use the built-in default. <br/>Example: `authorization, content-type, x-requested-with`. |
| **OPTIONS preflight passthrough** | When checked, the WAF answers `OPTIONS` preflight requests immediately with `204 No Content`, without WAF inspection or forwarding to the backend. <br/>This prevents preflight requests from being accidentally blocked by WAF rules. |

Click <code className="action">Save Changes</code> to apply the new CORS settings.
...
- Click <code className="action">Refresh</code> to refresh on demand.
...
| :--- | :--- | :--- |
| Cannot reach the OWAF Admin UI on port `8443` | Network access is blocked between your client and the WAF instance. | Verify your firewall allows outbound traffic to the instance on TCP `8443`. <br/>Confirm the instance URL provided by OVHcloud is the latest one. |
| Sign-in fails with the supplied token | The Bearer token is malformed or has been revoked. | Paste the token again exactly as received, with or without the `Bearer ` prefix. <br/>If the issue persists, contact your OVHcloud contact for a new token. |
| Legitimate requests are blocked | A built-in rule is firing on benign input, <br/>or the Paranoia Level is too high. | Identify the rule on the **Stats** and **Rules** pages, then either disable it, <br/>change its action to `log`, or lower the global Paranoia Level. <br/>See the [Reduce false positives](#reduce-false-positives) recipe. |
| The WAF is not forwarding traffic to the backend | The **Upstream URL** is missing or incorrect, <br/>or the backend is unreachable from the WAF. | On the **Proxy** page, confirm the Upstream URL is set with an `http://` or `https://` <br/>scheme. Verify the backend is reachable from the WAF instance. |
| A custom rule does not match expected traffic | The wrong target or operator is selected, <br/>or the pattern is incorrect. | Open the rule on the **Rules** page and verify the **Targets**, **Operator**, and **Pattern**. <br/>Test the regular expression against sample input outside the WAF. |
...
| Changes are not applied | The form was not saved. | Verify the <code className="action">Save Changes</code> button has been clicked on the page you edited. <br/>Configuration is applied immediately on save without a restart. |
...
| **Built-in Rule** | A rule shipped with the WAF based on the OWASP Core Rule Set. <br/>Cannot be edited or deleted, but can be disabled or have its action and Paranoia Level changed. |
