...
All API calls that are made in the section */ipLoadbalancing* are available on <ApiLink>this page</ApiLink>.
...
#### List the zones available to order for the OVHcloud Load Balancer
...
|-----------|----------|-------------|
| `ovhSubsidiary` | Yes | The code of the OVHcloud subsidiary (country or region) whose catalogue you want to consult, for example: `FR` (String of characters) |

The accepted values, and the currency of the prices returned for each one, depend on your commercial zone:

<Tabs noSync>
  <Tab label="Europe / Africa">

| Value | Country or region | Currency |
|-------|-------------------|----------|
...
:::info
The API schema lists `EU` among the values of this parameter, but this call does not accept it.
:::

:::info
The `CZ`, `FI` and `LT` values return the catalogue of the Irish subsidiary (`IE`), which offers fewer zones.
:::

  </Tab>
  <Tab label="Canada / Asia-Pacific">
...
| `QC` | Quebec | CAD |
| `SG` | Singapore | SGD |
| `WE` | Rest of the world (English) | USD |
| `WS` | Rest of the world (Spanish) | USD |

  </Tab>
</Tabs>

The zones available to order are listed in the `zone` add-on family of each plan (`plans[].addonFamilies`), for example `iplb-zone-lb1-gra`.
...
#### List the zones of a OVHcloud Load Balancer service
...
#### Return the global status of an OVHcloud Load Balancer service
