...
Now we can provide the output configuration that will send logs to Logs Data Platform in GELF format. Add it under the same `config:` key as the `filters:` block above.
...
You can now open the stream interface to check that your logs are correctly structured.

<img className="thumbnail" alt="Graylog stream showing a Kubernetes log entry with its kubernetes metadata as separate fields" src="/images/manage-and-operate/observability/logs-data-platform/ingestion-kubernetes-fluent-bit/kube_graylog.png" loading="lazy" />

Your Kubernetes activity is now fully logged in one place.
