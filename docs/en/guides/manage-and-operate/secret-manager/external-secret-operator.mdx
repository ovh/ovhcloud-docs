...
The `stringData` field accepts the raw token — Kubernetes base64-encodes it when storing the secret. If you prefer to use the `data` field instead, you must base64-encode the token yourself first (`echo -n "<token>" | base64`).
