...
A Hardware Security Module (HSM) is a specialised physical device designed to securely generate, store, and use cryptographic keys. HSM-backed keys never leave the hardware: cryptographic operations run on dedicated, tamper-resistant equipment, which strengthens key confidentiality and integrity.

OVHcloud uses Thales Luna HSMs, certified to FIPS 140-3 Level 3 and Common Criteria EAL4+.

Keys created with the HSM protection level are replicated across three different physical HSMs. Their exact location depends on the region selected for the OKMS.

#### 3-AZ regions

On 3-AZ regions, the three HSMs are spread across all Availability Zones.

#### 1-AZ regions

On 1-AZ regions, two HSMs are located in the region and a third remote HSM is deployed in a distinct nearby region.

Remote HSM locations:

- **EU-WEST-GRA** (coming soon): EU-WEST-RBX
- **EU-WEST-RBX** (coming soon): EU-WEST-SBG
- **EU-WEST-SBG** (coming soon): EU-WEST-GRA
