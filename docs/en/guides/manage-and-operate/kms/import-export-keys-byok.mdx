...
- **Operator:** entity with the permissions to access both the OKMS domain and the source of the key to import. Can be a human using the OKMS CLI or a service performing using the API.
...
5. On the destination, import the wrapped key with `POST /v1/servicekey` and a `wrappedKeys` payload. The KMS unwraps the ciphertext with the private transport key and stores the resulting service key.
