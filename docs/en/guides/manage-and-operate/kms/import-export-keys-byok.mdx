...
After wrapping with the destination public key, send the resulting JWE Compact Serialization `ciphertext` in the import request. For a Go example, see [Full example in Go](#full-example-in-go).
...
The following examples run both ceremonies with an RSA key, using the [Go SDK](https://pkg.go.dev/github.com/ovh/okms-sdk-go) for the OVHcloud KMS calls and [go-jose](https://pkg.go.dev/github.com/go-jose/go-jose/v4) for the JWE operations on the other side. Both examples encode the key material in PKCS#8 (DER), which matches `keyFormatType` `PKCS8`.

#### Import a locally generated key

The Go standard library generates the key, which the program wraps locally with the public transport key and then imports. Its plaintext never reaches OVHcloud KMS.

```go
// 1. Create the transport key on OVHcloud KMS and fetch its public part.
...
// 2. Generate the key to import, and encode it in PKCS#8 (DER).
...
// 3. Wrap the key material as a JWE: RSA-OAEP-256 key wrapping, A256GCM content encryption.
...
// 4. Import the wrapped key. The KMS unwraps it and infers the type and size from the material.
...
The JWE content encryption (`enc`) must be `A256GCM`.

#### Export a key and decrypt the JWE

The destination (here, the local program) generates the transport key pair and imports only its public part into OVHcloud KMS. The destination then decrypts the exported JWE with the private part.

```go
// 1. Generate the transport key pair on the destination.
destKey, err := rsa.GenerateKey(rand.Reader, 4096)
if err != nil {
  return err
}

// 2. Import only its public part into OVHcloud KMS, with the wrapKey usage.
...
// 3. Make the key extractable, then export it wrapped by the destination public key.
extractable := true
...
// 4. Decrypt the JWE with the destination private key, and parse the PKCS#8 material.
