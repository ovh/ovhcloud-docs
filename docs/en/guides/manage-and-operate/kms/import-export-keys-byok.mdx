...
Apply a policy to your OKMS domain resource (`urn:v1:<region>:resource:okms:<okmsId>`). Replace `<identity_urn>` and `<okms_urn>` with your values. Create policies via the [OVHcloud Control Panel](/guides/account-and-service-management/account-information/iam-policy-ui) or the [OVHcloud API](/guides/account-and-service-management/account-information/iam-policies-api).
...
Create an RSA key dedicated to wrapping. Always set both `wrapKey` and `unwrapKey`. `wrapKey` and `unwrapKey` are mutually exclusive with `sign` and `verify`.
...
Secure export is import in reverse: OVHcloud KMS encrypts the key material with an RSA transport key and returns only a JWE ciphertext. The private transport key must live on the **destination** environment so only that environment can unwrap the material.
...
Call `GET /v1/servicekey/{keyId}` with the wrapping query parameters below. When these parameters are present, the API returns `wrappedKeys` instead of plaintext `keys`.
