...
description: Securely import and export encrypted key material with OVHcloud KMS using asymmetric RSA key wrapping (BYOK), without sending plaintext in transit
...
Export is import in reverse: OVHcloud KMS encrypts the key material with an RSA transport key and returns only a JWE ciphertext. The private transport key must live on the **destination** environment so that only that environment can unwrap the material.
...
The transport key referenced by `wrappingKeyId` must be a SOFTWARE RSA key in the same OKMS domain with the `wrapKey` usage. Sensitive keys (AES/`oct` and private RSA/EC material) can only be extracted **after wrapping** — plaintext extraction is not supported for those keys.
