...
description: Configure Personal Access Tokens, service accounts, or access certificates to authenticate to your OKMS domain regional REST API
...
You can also open it from the Swagger link on your <ManagerLink to="/#/dedicated/security/kms">OKMS domain dashboard</ManagerLink>.
...
Browse the full action list in the [OVHcloud API console](https://api.eu.ovhcloud.com/console/) under product type **Key Management System (KMS)**, or in [IAM policies](/guides/account-and-service-management/account-information/iam-policy-ui) when creating a policy.

#### Suggested IAM policies

Apply these templates to your OKMS domain resource (`urn:v1:<region>:resource:okms:<okmsId>`). Replace `<identity_urn>` and `<okms_urn>` with your values. Create policies via the [Control Panel](/guides/account-and-service-management/account-information/iam-policy-ui) or the [IAM API](/guides/account-and-service-management/account-information/iam-policies-api).
