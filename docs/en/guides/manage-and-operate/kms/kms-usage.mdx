...
:::warning
Importing key material in plaintext is not recommended for keys that must remain trusted. Prefer [secure BYOK with asymmetric RSA wrapping](/guides/manage-and-operate/kms/import-key-byok).
:::
...
| --- | --- | --- |
| `extractable` | Yes (create / PATCH) | If `true`, the key material can be exported (plain or wrapped, depending on key class). Default: `false`; public-only keys default to `true`. |
| `never_extractable` | No | `true` if the key has never been extractable since creation inside the KMS. Keys [imported with BYOK](/guides/manage-and-operate/kms/import-key-byok) or plaintext import have `never_extractable` set to `false`, because the material existed outside the KMS before import. Enabling extraction also clears this flag permanently. |
| `sensitive` | No | `true` when the key can only be extracted after being wrapped (which also requires `extractable` to be `true`). Private keys (AES/`oct` and private RSA/EC material) are always considered sensitive. |
...
For the secure extraction ceremony (set `extractable` only for the export operation, then export with RSA wrapping), see [Export a wrapped key](/guides/manage-and-operate/kms/import-key-byok#export-a-wrapped-key).
...
[Import and export keys on OVHcloud KMS using BYOK](/guides/manage-and-operate/kms/import-key-byok)
