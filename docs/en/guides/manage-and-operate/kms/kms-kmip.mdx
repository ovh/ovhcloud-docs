...
- An [access certificate created](/guides/manage-and-operate/kms/okms-certificate-management) and [IAM policy configured](/guides/manage-and-operate/kms/okms-authentication-methods) for KMIP access. KMIP only supports certificate-based authentication — see [OKMS authentication methods](/guides/manage-and-operate/kms/okms-authentication-methods).
...
The configuration depends on the product to integrate, but does not need any specific configuration on the OVHcloud KMS other than [generating an access certificate](/guides/manage-and-operate/kms/okms-certificate-management) and granting the required [KMIP IAM rights](#iam-rights). KMIP does not support PAT or service account authentication.
