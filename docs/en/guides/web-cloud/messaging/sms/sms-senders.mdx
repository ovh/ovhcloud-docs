...
Some countries also require a regulatory declaration of the alphanumeric sender (Sender ID) with their regulator before it can be used:

| Country | Declaration required | Channel |
|------|----------------------|-------|
| Spain | Yes | [Declaring a Sender ID for Spain](/guides/web-cloud/messaging/sms/declaration-sender-id-espagne) |
...
It allows you to receive a reply via <code className="action">Message and campaign</code> > <code className="action">SMS management</code> > <code className="action">SMS received</code>.
...
:::info
For some countries, an additional regulatory declaration is required before you can use an alphanumeric sender — see the country status table at the beginning of this guide.
:::
