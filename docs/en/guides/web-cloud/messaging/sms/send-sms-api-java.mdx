...
Create your script credentials (all keys at once) on this page: <CreateToken rights="GET=/sms/&GET=/sms/*/jobs&POST=/sms/*/jobs">OVHcloud token creation page</CreateToken> (this URL will automatically give you the correct rights for the steps described in this guide).
...
With the <ApiLink section="/sms" regions={["eu"]}>API console</ApiLink>, you can explore other methods for using SMS services, such as: SMS messages with the ability to send replies (for OVHcloud accounts in France only), mass-sending a CSV file, advertising mail, acknowledgements of receipt, etc.
