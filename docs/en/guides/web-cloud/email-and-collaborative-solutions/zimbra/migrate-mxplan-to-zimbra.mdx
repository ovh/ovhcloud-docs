...
<Tabs>
  ...
  <Tab label="Step 2">
    **Log in to the project and create the migration**
    ...
    - **Source account**:
        - **Account type**: Select `OVHcloud`, then choose `MX Plan` or `Auto-detection`. Click <code className="action">Log in</code> to authenticate with your OVHcloud account and automatically select the service and address to migrate (e.g. `contact@mydomain.ovh`). Then enter the password for this email account.
    - **Destination account**:
        - **Account type**: Select `OVHcloud`, then choose `Zimbra`. Click <code className="action">Log in</code> to authenticate with your OVHcloud account and select the Zimbra service and destination address (e.g. `contact2@mydomain.ovh`). Then enter the password for this email account.
