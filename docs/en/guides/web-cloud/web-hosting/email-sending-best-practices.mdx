...
Both methods send emails from your website. They differ in where the email leaves from, how it is limited, and how you can follow it.
...
| **Sender identity** | The `From:` address is declared by the script. Make sure your SPF record authorises OVHcloud servers. | The `From:` address must match the authenticated account. The email is signed with DKIM if DKIM is enabled for your domain. |
