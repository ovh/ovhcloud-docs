...
Domain Names are fully reversible by design. DNS Zones may be backed up in txt files and deployed on any new DNS server.

| **Activity** | **Customer** | **OVHcloud** |
| --- | --- | --- |
| Know the services that depend on some DNS registrations | RA |  |
...
| Deploy the configuration on the DNS zone with the help of OVHcloud automation tools | RI | AI |
...
| Perform the DNS modifications for services deployed via OVHcloud automation tools | RI | RA |
...
| **Activity** | **Customer** | **OVHcloud** |
| --- | --- | --- |
| Document the services that depend on some DNS registrations | RA |  |
