...
As for the holder, it has to be John. In case of dispute with the web agency, the fact that he is the holder will be legally acknowledged and will allow him to recover management of his domain name.
...
By default if not set otherwise, a Nic admin is Nic tech and Nic billing of a service as well.
...
In the domain names context, we strongly recommend the exclusive use of `/domain/contacts` routes.
...
It retrieves the final purchase order, without generating it (it is a "dry-run").
...
<Api version="v1" section="/domain" method="GET" route={"/domain/\{serviceName\}/task"} />
