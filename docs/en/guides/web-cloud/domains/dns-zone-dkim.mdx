...
- **To access your services:** <code className="action">Web Cloud</code> > <code className="action">Domain names</code> > select your domain name > <code className="action">DNS zone</code> tab
...
If your domain name is registered with OVHcloud, you can check if it is using the OVHcloud configuration in your <ManagerLink to="/">Control Panel</ManagerLink>. To do this, open the <code className="action">Domain names</code> section, select the domain concerned, and check its <code className="action">DNS servers</code> tab.
:::
...
<ZoneTabs>
  <Tab label="MX Plan">
    ...
    In the **General information** box, `DKIM` is displayed in red in the column **Diagnostic**.
...
Open the <code className="action">DNS zone</code> tab, then click <code className="action">Add an entry</code> above the table. In the form that opens, select the `CNAME` type and complete the fields according to the values you have noted.
...
To configure DKIM, go to the <ApiLink>OVHcloud API page</ApiLink> and log in:
...
<Region zones={['eu']}>
Go to the API section `/email/domain/` (MX Plan and Zimbra offers), `/email/exchange` (Exchange offer) or `/email/pro` (Email Pro offer) and enter "dkim" in the `Filter` field to display only the API functions related to DKIM.
</Region>
...
Go to the <code className="action">DNS zone</code> tab, then click <code className="action">Add an entry</code> above the table. In the form that opens, select the `CNAME` type and complete the fields according to the values you have noted.
    ...
    Once you have entered the values, click <code className="action">Add</code>.
...
<Tabs>
  <Tab label="1. List of selectors">
    ...
    - `exchangeService`: Type the name of your Exchange service in the form "hosted-zz111111-1" or "private-zz111111-1". <br/>
    - `organisationName`: Type the name of your Exchange service in the form "hosted-zz111111-1" or "private-zz111111-1". <br/>
  ...
  <Tab label="2. Create a selector">
    ...
    - `exchangeService`: Type the name of your Exchange service in the form "hosted-zz111111-1" or "private-zz111111-1".
    - `organisationName`: Type the name of your Exchange service in the form "hosted-zz111111-1" or "private-zz111111-1".
...
<ZoneTabs>
  <Tab label="MX Plan and Zimbra" availableIn={['eu']}>
    ...
    - `error`: The installation process encountered an error. Please open a [support ticket](/links/support-contact) with the domain name concerned.<br/>
...
From the <ManagerLink to="/">OVHcloud Control Panel</ManagerLink>, open the <code className="action">Web Cloud</code> section, click <code className="action">Domain names</code>, then select the relevant domain name.

Open the <code className="action">DNS zone</code> tab, then click <code className="action">Add an entry</code> above the table. There are 3 ways to add a DKIM record to your DNS zone:
...
<img className="thumbnail" alt="Adding a DKIM record from the DNS zone" src="/images/assets/screens/control-panel/product-selection/web-cloud/domain-dns/dns-zone/nx-dkim-form.png" loading="lazy" />

- **Selector** (Subdomain): enter the DKIM selector name provided by your email service. The `._domainkey` suffix is added automatically, and your domain name is appended at the end.
...
- **Public key (base64)**: paste the public key provided by your email service. This is the key that recipients use to verify your DKIM signature.
- **Key type**: the signature algorithm of the key — `RSA` (default) or `ed25519`.
- **Test mode**: leave it on `Disabled` in production. When enabled, recipients treat DKIM-signed test messages like unsigned ones: a failed check therefore does not affect delivery while you validate your setup.
...
<ZoneTabs>
  <Tab label="Exchange" availableIn={['eu', 'ca']}>
    ...
    - `organizationName`: Enter the name of your Exchange service, which is in the form of "hosted-zz111111-1" or "private-zz111111-1". <br/>
    - `exchangeService`: Enter the name of your Exchange service, which is in the form of "hosted-zz111111-1" or "private-zz111111-1". <br/>
  </Tab>
  <Tab label="Email Pro" availableIn={['eu']}>
    To switch to the second selector, use the following API call:
...
<Tabs>
  ...
  <Tab label="502">
    ...
    An error has occurred configuring the DKIM. After 24 hours, if your configuration is still in this state, please open a [support ticket](/links/support-contact).
  </Tab>
  <Tab label="503">
...
<ZoneTabs>
  ...
  <Tab label="MX Plan and Zimbra" availableIn={['eu']}>
    ...
    - `error`: The installation process encountered an error. Please open a [support ticket](/links/support-contact), specifying the domain name concerned.
