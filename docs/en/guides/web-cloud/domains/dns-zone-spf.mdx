...
To find the row for the OVHcloud SPF record, a display filter can be applied to the table. Since the record can appear in two different places, select both the <code className="action">TXT</code> and the <code className="action">SPF</code> filter. You will receive one of the following results.
