...
<Tabs>
  <Tab label="Step 1">
    In the `Web Cloud` section, click <ManagerLink to="/#/web-domains/domain">Domain names</ManagerLink>, select the domain name you used to create the hosts in [Part 3](#step3), then open the **DNS zone** tab.
  ...
  <Tab label="Step 3">
    In the form that opens, select the *A* or *AAAA* DNS record type depending on the type of associated IP you want to add.
  </Tab>
  <Tab label="Step 4">
    Enter the *subdomain* and the *IPv4* (A) or *IPv6* (AAAA) address, then click <code className="action">Add</code>. If necessary, refer to the instructions in our guide "[How to edit an OVHcloud DNS zone](/guides/web-cloud/domains/dns-zone-edit)".
  </Tab>
</Tabs>
...
<Tabs>
  <Tab label="Step 1">
    In the `Web Cloud` section, click <ManagerLink to="/#/web-domains/domain">Domain names</ManagerLink>, select the domain name for which you have customised the DNS servers, then open the **DNS zone** tab.
  </Tab>
  <Tab label="Step 2">
    ...
    The DNS zone appears in *text* mode:
  ...
  <Tab label="Step 3">
    In this window, replace **only in the *NS* type records** the DNS server names with your own custom DNS server names, **making sure** to increment the first numeric value of the *SOA* line by "1". Once you have made your changes, follow the on-screen steps to confirm.
