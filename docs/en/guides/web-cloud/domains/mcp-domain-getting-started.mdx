...
| United States | `https://mcp.us.ovhcloud.com/mcp/{section}` |
...
The button uses the endpoint of the hub you are browsing. From the European hub it therefore configures the extension against `https://mcp.eu.ovhcloud.com`: for an account in Canada or the United States, open your region's hub or follow the manual setup below.
...
### 4 — Understanding tool names

Two naming conventions coexist on the server:

| Convention | Example | Notes |
|------------|---------|-------|
| **Short alias** | `get-domain-list` | Given to the most common operations |
| **Generated name** | `mcp__v1_domain__GET__v1_domain_zone__zoneName__status` | Built from the API method and route |

These conventions change nothing in the way you phrase your requests: you never have to remember or type these names. Describe what you want in plain language (“list my domain names”, “show the status of the my-domain.com zone”) and the agent picks the matching tool itself. Knowing them only helps you understand which tool the agent announces before calling it, and point it in the right direction when it cannot find an operation.

### 5 — Display information about a domain name
...
```text
Show me the details of my-domain.com: expiry date, name servers and whether the transfer lock is on.
```

### 6 — Order a domain name
...
```text
Order my-domain.com for one year.
```
...
```text
Simulate the order of my-domain.com for one year.
```
...
```text
Change the A record of www.my-domain.com to 203.0.113.42.
```
...
Make www.my-domain.com point to it.
```
...
### 8 — Create a redirection

A redirection sends visitors from your domain to another address. All redirection tools use the generated naming convention.

Example request:

```text
Redirect blog.my-domain.com to https://my-domain.com/news, keeping the address visible in the browser.
```

### 9 — What else is covered
