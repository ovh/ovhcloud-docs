...
:::info
The `gpt-oss-120b` model supports tool use (function calling), which is essential for an agent. You can browse the [AI Endpoints catalog](/links/public-cloud/ai-endpoints-catalog) to pick another compatible model; if so, check its context window (64k minimum) and its function calling support.
:::
...
:::info
Each run consumes AI Endpoints tokens (pay-as-you-go billing). A weekly summary task typically represents a few hundred thousand tokens per month, i.e. a few cents with `gpt-oss-120b`. Monitor your usage from the <code className="action">AI Endpoints</code> section of your <ManagerLink to="/#/public-cloud/pci/projects">Public Cloud project</ManagerLink>. For pricing details and the models lifecycle, refer to our guide "[AI Endpoints - Billing and lifecycle](/guides/public-cloud/ai-machine-learning/ai-endpoints-billing)".
:::
