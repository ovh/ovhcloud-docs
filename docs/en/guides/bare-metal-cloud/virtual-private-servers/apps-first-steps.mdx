...
:::info
If the link has expired already, please connect to your VPS via SSH using your login details and execute the `sudo whmlogin` command to generate a new one.
:::
...
WHM uses the root user for authentication by default. The one-time link in the email provides access to complete the initial setup and set your root password. On subsequent logins, enter `root` as the username and the password you defined here.
