...
If emails are not sending or SMTP is not responding, check whether SMTP ports are blocked. Port 25 is not blocked by default on OVHcloud VPS, but we still recommend using port 587 (STARTTLS) for outbound mail as a best practice.

The exception is VPS deployed in a [Local Zone](https://www.ovhcloud.com/en-gb/lp/vps-localzone/): these cannot contact SMTP servers at all, so port 25 — like every other SMTP port — is simply inaccessible, with no option to unblock it.
