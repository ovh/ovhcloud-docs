...
If emails aren't being sent or if the SMTP protocol isn't responding, check whether the SMTP ports are blocked. Port 25 is not blocked by default on OVHcloud VPS servers (with the exception of VPS deployed in Local Zones, which cannot contact SMTP servers under any circumstances; port 25, like all other SMTP ports, is therefore simply inaccessible, with no option to unblock it), but we still recommend using port 587 (STARTTLS) for sending emails, in accordance with best practices.
...
The best option is to open our <ApiLink>API console</ApiLink> and check if your license can be moved to a different VPS. Find the basics in our guide on [how to get started with the OVHcloud API](/guides/manage-and-operate/api/first-steps).
...
Open the <ApiLink>OVHcloud API console</ApiLink> and use the following call:
