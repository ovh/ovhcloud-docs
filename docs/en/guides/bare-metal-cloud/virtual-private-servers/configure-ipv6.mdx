...
<Tabs>
  ...
  <Tab label="Via the OVHcloud API">
    Go to the <ApiLink>OVHcloud console API</ApiLink> page:
...
- [Persistent application on Red Hat and its derivatives (Rocky Linux, AlmaLinux, etc.)](#persistentredhat)
- [Persistent application on Fedora 43 and later](#persistentfedora)
...
There are 2 ways to configure your network depending on the operating system installed on your server:
...
#### Persistent application on Red Hat and its derivatives (Rocky Linux, AlmaLinux, etc.) <a name="persistentredhat"></a>

There are 2 ways to configure your network depending on the operating system installed on your server:

- **For Rocky Linux & AlmaLinux (8/9)**: use the method based on *network-scripts* files.
...
#### Persistent application on Fedora 43 and later <a name="persistentfedora"></a>
...
Cloud-init is a package installed by default on a VPS. It functions as a framework to run a script after creating your server or rebooting it. The mechanics in place allow the underlying OpenStack infrastructure to inject scripts into the Cloud-init environment and therefore the server configuration.
...
In the case of newer distributions, the default configuration of Cloud-init may automatically reset the network configuration when the server starts up.

This is relevant for specific use cases in which you should disable automatic network management in Cloud-init to avoid the reset. To do this, use the following command to create a file `/etc/cloud/cloud.cfg.d/98-disable-network-config.cfg` with the value `network: {config: disabled}`:
