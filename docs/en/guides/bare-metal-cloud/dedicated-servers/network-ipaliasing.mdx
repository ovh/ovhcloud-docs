...
The following sections cover the distributions we currently offer and the most commonly used operating systems. The first step is always to log in to your server via SSH or a GUI login session (RDP for a Windows server). 
...
In the examples below, we use the `nano` text editor. If it isn't installed on your OS, you will be prompted to install it, but you can use any text editor you prefer.
...
<Tabs>
  <Tab label="Fedora 43+ / AlmaLinux (10) / Rocky Linux (10)">
    ...
    **Step 2: Edit the config file**
    ...
    Run one of the following commands to find the name of your network interface before editing the appropriate network file:
  ...
  <Tab label="Windows Servers">
    ...
    Otherwise, first switch from a network-level DHCP configuration to a fixed IP configuration.
  ...
  <Tab label="Plesk">
    ...
    **Step 1: Access the Plesk IP management section**
    ...
    <img className="thumbnail" alt="access to the ip addresses management" src="/images/bare-metal-cloud/dedicated-servers/network-ipaliasing/pleskip1.png" loading="lazy" />
    
    Click on <code className="action">IP Addresses</code> under **Tools & Resources**.
    
    **Step 2: Add the Additional IP information**
    
    In this section, click <code className="action">Add IP Address</code>.
...
To test the connection, ping your Additional IP from outside. If it responds in rescue mode, this indicates a configuration error. If, however, the IP is still not working, please open a ticket with the support team via the [OVHcloud Help Center](/links/support-contact) with the following information:
