...
By default, outgoing **port 25** is blocked on Public Cloud instances to prevent spam abuse.

Port 25 is not blocked by default on dedicated servers or VPS. However, our anti-spam system may block it at the network level if spam is detected on your IP — see [OVHcloud AntiSpam - Best Practices and Unblocking an IP](/guides/bare-metal-cloud/dedicated-servers/antispam-best-practices) for details.

The exception is VPS and Public Cloud instances in Local Zones: these cannot reach SMTP services at all, so port 25 is unusable there and cannot be unblocked.

To resolve this:

- Use port 587 (STARTTLS) for outbound mail submission.
- If your use case requires port 25, verify whether it is available for your infrastructure (see above). If so, request that it be unblocked by [contacting our support team](/links/support-contact).
