...
The backup storage is only compatible with NFSv3. First, ensure that you have authorised your IP ranges to access the storage and use the NFS protocol. Depending on your Linux operating system, you might have to install the **NFS** client and start the NFS and `portmap` services.
...
If you experience connection problems, also make sure that the UDP and TCP ports required for NFSv3 are open in the firewall for both inbound and outbound traffic.
Open port 111 (UDP/TCP), port 2049 (TCP, NFS) and the dynamic port range 32768–60999 (UDP/TCP) in both directions. These ports are used by RPC for the `mountd`, `lockd`, and `statd` programs.

Finally, allow all outgoing connections to the IP address of your Backup Storage.
