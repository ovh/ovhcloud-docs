...
- **UEFI boot servers**: iPXE uses [`sanboot`](https://ipxe.org/cmd/sanboot) to boot the EFI bootloader path (`efiBootloaderPath`) specified during [OS installation](/guides/bare-metal-cloud/dedicated-servers/getting-started-with-dedicated-server#install) (e.g. `\efi\debian\grubx64.efi`).
    ...
    The `efiBootloaderPath` can be changed via the <ApiLink>OVHcloud API</ApiLink>:
...
For advanced use cases, you can configure a custom iPXE script via the <ApiLink>OVHcloud API</ApiLink>. For more information, see [Configuring a custom iPXE script](/guides/bare-metal-cloud/dedicated-servers/ipxe-scripts).
