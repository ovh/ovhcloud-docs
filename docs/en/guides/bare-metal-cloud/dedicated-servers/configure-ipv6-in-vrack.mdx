...
Enter your vRack ID (e.g. `pn-9000000`) as the `serviceName` path parameter and set the IPv6 block in the request body:

```json
{
  "block": "2001:41d0:abcd:ef00::/56"
}
```
...
```json
{
  "bridgedSubrange": "2001:41d0:abcd:ef00::/64",
  "gateway": "2001:41d0:abcd:ef00::1",
  "slaac": "disabled"
}
```
...
Enable SLAAC in the request body:
...
Use the following call to define such a subnet:

<Api version="v1" section="/vrack" method="POST" route={"/vrack/\{serviceName\}/ipv6/\{ipv6\}/routedSubrange"} />
