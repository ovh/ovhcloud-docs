...
With [OVHcloud Dedicated Servers](/links/bare-metal/bare-metal), you can configure Disks, [hardware RAID](/guides/bare-metal-cloud/dedicated-servers/raid-hard), Partitions, [software RAID](/guides/bare-metal-cloud/dedicated-servers/raid-soft), LVM, ZFS, etc. during [OS reinstallation](/guides/bare-metal-cloud/dedicated-servers/getting-started-with-dedicated-server) from the [OVHcloud API](/links/api) as well as the <ManagerLink to="/">OVHcloud Control Panel</ManagerLink>. In this article, we will focus on the [OVHcloud API](/links/api). This will give us more details about the engine that is running in the background in order to create the storage customisation on the dedicated server from the input data passed on to the OVHcloud API.
