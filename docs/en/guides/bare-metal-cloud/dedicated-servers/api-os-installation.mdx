...
Fully automating OS installation or reinstallation on a [dedicated server](/links/bare-metal/bare-metal) with the [OVHcloud API](/guides/manage-and-operate/api/first-steps) can be interesting in various situations.
...
For the `enableLacpBonding` question, see [Understanding link aggregation on dedicated servers](/guides/bare-metal-cloud/dedicated-servers/link-aggregation-overview) to find out when it applies and what it configures.
