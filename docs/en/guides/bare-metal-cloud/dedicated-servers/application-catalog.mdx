...
description: Deploy a ready-to-use application on your bare metal server through automatic post-installation with the application catalogue
...
- Access to the <ApiLink>OVHcloud API</ApiLink> (for the "[Deploying via the OVHcloud API](#viaapi)" section of this guide)
- Access to the email address of your OVHcloud account contacts, to receive the link to the configuration interface
...
<Tabs>
  <Tab label="Via the OVHcloud Control Panel">
    ...
    #### From the OVHcloud Control Panel <a name="viacontrolpanel"></a>

    1. Log in to the <ManagerLink to="/#/dedicated-servers/server">OVHcloud Control Panel</ManagerLink> and select your server under <code className="action">Bare Metal Cloud</code> > <code className="action">Dedicated servers</code>.
    2. In the <code className="action">General information</code> tab, next to <code className="action">Operating system (OS)</code>, click the <code className="action">...</code> button and select the installation option.
    ...
    3. In the <code className="action">Installation from an OVHcloud template</code> window, set <code className="action">Type of OS</code> to <code className="action">Ready-to-use (graphical user interface)</code>, then select the **Ubuntu Server 24.04 "Noble Numbat" LTS with Application Installer** template. Choose your target disk group and click <code className="action">Next</code>.
    ...
    5. At the <code className="action">Application to install</code> step, select the **single application** you want to deploy from the drop-down list, then click <code className="action">Confirm</code> to launch the installation.
  ...
  <Tab label="Via the OVHcloud API">
    ...
    You can trigger the same installation through the <ApiLink>OVHcloud API</ApiLink>, using the following API call:

    <Api version="v1" section="/dedicated/server" method="POST" route={"/dedicated/server/\{serviceName\}/reinstall"} />

    Use an application catalogue operating system and set the application to install in the `appToInstall` customization:
    ...
    To list the operating systems available for your server, and confirm the exact name of the application catalogue templates, use the following API call:
    ...
    Follow the installation task to completion as described in [OVHcloud API and OS installation](/guides/bare-metal-cloud/dedicated-servers/api-os-installation), then continue with the "[Configuring and accessing your application](#configure)" section below.
...
Open that link in your web browser. The token it carries authenticates you and stores a secure, session-only cookie, so you can then navigate the configurator without the token in the address bar. Opening the configurator without a valid token returns an `Unauthorized` page.
...
- **Applications with no fields** — Coolify, Dokploy, Easypanel, FastPanel, HestiaCP, Discourse, Matomo and n8n — generate any credentials they need and display them at the end. Just click <code className="action">Deploy</code>.
...
Each field is validated before the deployment starts (for example an email field must be a valid email address, and passwords have a minimum length). Fix any highlighted field and submit again.
...
#### Step 4: Retrieve your access details
...
- your server runs only your application, with no leftover setup tool or administration plane.
...
|---|---|---|
| The configurator page does not load at `https://<your-server>:10143/` | The installation is not finished yet, or inbound access to port `10143` is blocked. | Wait for the installation to complete, then retry. Make sure any firewall (including the OVHcloud [Network Firewall](/guides/bare-metal-cloud/dedicated-servers/firewall-network)) allows inbound traffic to your server on port `10143`. |
