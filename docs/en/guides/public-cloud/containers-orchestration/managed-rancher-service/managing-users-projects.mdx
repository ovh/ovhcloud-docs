...
In Kubernetes, features like Role-Based Access Control (RBAC) or cluster resources are assigned to individual namespaces. A project allows you to save time by giving an individual or a team access to multiple namespaces simultaneously.
...
Click on the name of your chosen Kubernetes cluster.
...
Select the project then click the <code className="action">Move</code> button to move the `falco` namespace in your project.
...
Follow the official documentation from Rancher to know more about:
