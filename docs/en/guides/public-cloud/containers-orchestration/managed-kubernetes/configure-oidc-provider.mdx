...
In the [OVHcloud Control Panel](https://www.ovh.com/auth/?action=gotomanager&from=https://www.ovh.co.uk/), navigate to the **Public Cloud** section, select your project, then click **Managed Kubernetes Service** in the left sidebar under **Containers & Orchestration**.

Click your cluster name to open its details page.

In your cluster details page, click the **OIDC** tab to access OIDC provider configuration. Click on the `...` button in the `OIDC provider` section.
...
Go to https://api.ovh.com/ and click "Login". Authenticate with your OVHcloud credentials.

If you go to the [Kubernetes section](https://api.ovh.com/console/#/cloud/project/%7bservicename%7d/kube~get) of the API Explorer, you will see the available endpoints. Once logged in, expand the /cloud section and navigate to /cloud/project/{serviceName}/kube to find the Managed Kubernetes endpoints.
...
Specifically, you have to generate these credentials via the [OVHcloud token generation page](https://api.ovh.com/createToken/?GET=/*&POST=/*&PUT=/*&DELETE=/*) with the following rights. Go to https://api.ovh.com/ and click "Login". Authenticate with your OVHcloud credentials. You need an API token (application key, application secret, consumer key) with rights on /cloud/project endpoints. Follow the on-screen instructions to generate one.
...
In the Public Cloud section, you can retrieve your service name ID thanks to the <code className="action">Copy to clipboard</code> button. Your cluster service name (its unique identifier) appears in the OVHcloud Control Panel URL when you are on the cluster details page, and in the cluster Overview tab.
