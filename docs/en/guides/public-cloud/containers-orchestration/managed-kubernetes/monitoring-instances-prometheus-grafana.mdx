...
1. Log in to the [OVHcloud Control Panel](https://www.ovh.com/auth/?action=gotomanager&from=https://www.ovh.co.uk/), go to the **Public Cloud** section, and select your project.
2. In the left sidebar, click **Users & Roles**.
3. Click **Add user**, fill in a username and optional description, then click **Confirm**.
4. Once the user is created, click the **...** button next to it and select **Edit roles**.
5. Enable the **Infrastructure Supervisor** role and save.
...
To download the OpenRC file, go to **Users & Roles** in your Public Cloud project, click the **...** button next to the user, and select **Download OpenStack RC file**. In the dialog, choose the region you want to scrape (this example uses GRA11) and confirm the download.
...
Create **dedicated script credentials** (as [described here](https://github.com/ovh/go-ovh#supported-apis)):

1. Go to [https://api.ovh.com/](https://api.ovh.com/) and click **Login**. Authenticate with your OVHcloud credentials.
2. Follow the on-screen instructions to generate a token (application key, application secret, consumer key).
3. Note all three values — you will need them in the next step.
