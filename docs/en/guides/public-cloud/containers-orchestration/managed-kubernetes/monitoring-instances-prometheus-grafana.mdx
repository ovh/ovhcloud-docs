...
1. Log in to the <ManagerLink to="/#/pci/projects">OVHcloud Control Panel</ManagerLink>, go to the **Public Cloud** section, and select your project.
...
Create **dedicated script credentials** (as [described here](https://github.com/ovh/go-ovh#supported-apis)). Follow our [First steps with the OVHcloud APIs](/guides/manage-and-operate/api/first-steps) tutorial to generate them via the <CreateToken>OVHcloud token generation page</CreateToken>, then note the application key, application secret, and consumer key — you will need them in the next step.
