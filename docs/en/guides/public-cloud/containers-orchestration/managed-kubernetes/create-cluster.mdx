...
<Tabs>
  <Tab label="Via OVHcloud Control Panel">
    ...
    Optionally, integrate your cluster into a private network using OVHcloud vRack. Select an existing vRack network from the dropdown, or skip this step to configure private networking later. For more information, read our guide [Using the vRack](/guides/public-cloud/containers-orchestration/managed-kubernetes/using-vrack).

    Configure the default node pool. A node pool is a group of nodes sharing the same configuration. You can add a node pool now or skip this step to add one later. Read the [Managing node pools](/guides/public-cloud/containers-orchestration/managed-kubernetes/managing-nodes) guide for more information.

    Define the size of the default node pool: enter a name for the pool, choose an instance flavor (instance type), and set the number of nodes.
    ...
    Enter a name for your cluster, review the summary, and click <code className="action">Send</code>.
  ...
  <Tab label="Via Terraform">
    ...
    Your Public Cloud project ID is the `service_name`. Your cluster service name (its unique identifier) appears in the OVHcloud Control Panel URL when you are on the cluster details page, and in the cluster Overview tab.
