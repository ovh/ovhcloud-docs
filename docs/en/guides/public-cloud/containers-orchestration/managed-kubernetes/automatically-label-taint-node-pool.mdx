...
Go to https://api.ovh.com/ and click "Login". Authenticate with your OVHcloud credentials. You need an API token (application key, application secret, consumer key) with rights on /cloud/project endpoints. Follow the on-screen instructions to generate one.
...
Your cluster service name (its unique identifier) appears in the OVHcloud Control Panel URL when you are on the cluster details page, and in the cluster Overview tab.
...
with the following information: set `serviceName` to your Public Cloud project ID, `kubeId` to your cluster ID, and `nodePoolId` to the ID of the node pool you want to delete. In the [API Explorer](https://api.ovh.com/), go to https://api.ovh.com/ and click "Login", then navigate to the `DELETE /cloud/project/{serviceName}/kube/{kubeId}/nodepool/{nodePoolId}` endpoint, fill in the required parameters, and execute the call.
