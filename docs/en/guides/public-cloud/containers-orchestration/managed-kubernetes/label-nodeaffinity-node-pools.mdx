...
Or if you already have a cluster, the UI is a little bit different, you need to click on the <code className="action">Create a Kubernetes cluster</code> button.
...
In the next step, define the size of the default node pool: enter a name for the pool and set the number of nodes.
...
Now, your Kubernetes cluster is up and running. Click your cluster name to open its details page, then click the <code className="action">Node pools</code> tab.
