...
On OVHcloud, you can add the record directly in the DNS zone: in the <ManagerLink to="/#/web-domains/domain">OVHcloud Control Panel</ManagerLink>, open your domain, go to the **DNS zone** tab, click **Add an entry**, select the record type (e.g., A or CNAME), enter the subdomain and target value, then click **Confirm**.
