...
Click on your NAS, then on the <code className="action">Partitions</code> tab, then on the <code className="action">Create a partition</code> button. Fill in the partition name, set the desired size, and select **NFS** as the protocol, then confirm to create the partition.
...
Click on the <code className="action">Manage Access</code> menu of our newly created partition. Add either the nodes' IPs one by one or the Gateway's Public IP depending on your configuration. For each entry, click <code className="action">Add an access</code>, enter the IP address, and confirm.

Once access rules are saved, note the **zpool IP** (`ZPOOL_IP`), the **zpool name** (`ZPOOL_NAME`), and the **partition name** (`PARTITION_NAME`) displayed on the partition details page.
