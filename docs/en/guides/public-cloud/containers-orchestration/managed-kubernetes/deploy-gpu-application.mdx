...
GPU node pools group nodes of the same GPU flavor and can be configured with autoscaling to handle variable workloads efficiently.
...
In the node pool creation form, enter a name for your GPU node pool (for example, `gpu-node-pool`).
...
On the flavor selection step, click the **GPU** tab and select a GPU flavor such as **T1-45**.
...
Set the node count and, if desired, enable **Autoscaling** and define the minimum and maximum pool size (for example, 3 minimum and 10 maximum).

Then, choose `Hourly` billing mode for this GPU node pool.

Select **Hourly** as the billing mode for this GPU node pool.
...
Once the node pool status shows `OK` in the OVHcloud Control Panel, you can confirm the GPU nodes are correctly labelled using the command above.
...
:::info
We recommend using the latest driver version released by NVIDIA with the `*-5.15.0-*-generic-ubuntu22.04` pattern, available in the [NVIDIA NGC driver catalog](https://catalog.ngc.nvidia.com/orgs/nvidia/-/containers/driver/-/tags).

Example: `595-5.15.0-185-generic-ubuntu22.04`. Set it using `driver.version` in the [gpu-operator chart values](https://github.com/NVIDIA/gpu-operator/blob/c37a3850b40db6dbd850aa75119dd479f75935ec/deployments/gpu-operator/values.yaml).
