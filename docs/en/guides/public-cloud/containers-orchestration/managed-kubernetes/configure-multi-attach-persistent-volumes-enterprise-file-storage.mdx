...
Click your Enterprise File Storage service, then click the `Volumes` tab. Click the `Create a volume` button and fill in the volume creation form.
...
Click `Add access`, then enter either the nodes' IPs one by one or the Gateway's public IP depending on your configuration, and confirm. Repeat for each IP you need to authorize.

Once done, the ACL list will display all the authorized IP addresses that can access the volume.
