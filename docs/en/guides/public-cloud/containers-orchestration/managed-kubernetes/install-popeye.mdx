...
First, you need to have an Object Storage container, if you don't already had one, you can follow the [Getting started with Object Storage Swift](/guides/storage-and-backup/object-storage/pcs-create-container) guide.

For this guide, our Object Storage container is named `popeye` and its region is `GRA`. You can verify and configure the container name in the [OVHcloud Control Panel](https://www.ovh.com/auth/?action=gotomanager&from=https://www.ovh.co.uk/) under **Public Cloud** > your project > **Object Storage**.
...
1. In the [OVHcloud Control Panel](https://www.ovh.com/auth/?action=gotomanager&from=https://www.ovh.co.uk/), navigate to **Public Cloud** > your project, then click **Users & Roles** in the left-hand menu.
2. Click <code className="action">+ Add user</code>.
3. Enter a description for the user, for example `Popeye`, then click <code className="action">Next</code>.
4. Check the `ObjectStore operator` role and click <code className="action">Confirm</code>.
5. Wait for the user to be created. The username and password will be automatically generated and displayed in the Control Panel — make sure to save the password, as it will only be shown once.
...
1. In the **Users & Roles** section, click the `...` menu next to your newly created user and select <code className="action">Download OpenStack's RC file</code>.
2. Select the region of your Object Storage container (`GRA` for this guide) and click <code className="action">Download</code>.
