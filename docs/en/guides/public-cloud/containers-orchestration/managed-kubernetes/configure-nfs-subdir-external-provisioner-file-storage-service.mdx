---
title: Getting started with NFS Subdir External Provisioner on OVHcloud Managed Kubernetes
description: Learn how to deploy the NFS Subdir External Provisioner to dynamically provision persistent volumes from an OVHcloud File Storage Service share on your Managed Kubernetes cluster.
lastUpdated: 2026-06-09
---

## Objective

This guide explains how to deploy the [NFS Subdir External Provisioner](https://github.com/kubernetes-sigs/nfs-subdir-external-provisioner) on an [OVHcloud Managed Kubernetes](https://www.ovhcloud.com/en-gb/public-cloud/kubernetes/) cluster to dynamically provision persistent volumes from an [OVHcloud File Storage Service](/guides/storage-and-backup/file-storage/file-storage-service/getting-started) NFS share.
...
- An [OVHcloud Managed Kubernetes](https://www.ovhcloud.com/en-gb/public-cloud/kubernetes/) cluster whose nodes are deployed on a private network
- An [OVHcloud File Storage Service](/guides/storage-and-backup/file-storage/file-storage-service/getting-started) NFS share in the same region, attached to the same private network as your cluster
...
### Step 1 - Prepare your File Storage Service share
...
Once the share is in `available` status, retrieve its NFS export path. In the OVHcloud Control Panel, go to <code className="action">Storage & backup</code> > <code className="action">File Storage</code>, click your share, and find the `Mount Path` on the <code className="action">General information</code> tab.
...
Split it into its two components and note them for the following steps:
...
The provisioner pod and every node that will mount NFS volumes must be allowed to reach the share. Add an ACL entry covering your MKS cluster's private network subnet.

In the OVHcloud Control Panel, click your share, go to the <code className="action">Access Control List (ACL)</code> tab, then click <code className="action">Add a new access</code>. Enter the CIDR range of your MKS private network (for example `10.1.0.0/24`) and select the `Read and write` permission.

:::info
The File Storage Service is only accessible from OVHcloud IPs (Public Cloud, Managed Kubernetes Service). A single CIDR rule for your MKS subnet covers all nodes without listing individual IPs.
:::
...
Deploy two Nginx replicas that mount the same PVC to verify `ReadWriteMany` access:
...
- **Use `pathPattern` for readable directory names**: Set `storageClass.pathPattern` to `${.PVC.namespace}/${.PVC.name}` to generate human-readable subdirectory structures. See the [NFS Subdir External Provisioner documentation](https://github.com/kubernetes-sigs/nfs-subdir-external-provisioner#dynamic-provisioning) for the full list of template variables.
...
- **Match NFS mount options to FSaaS capabilities**: OVHcloud File Storage Service supports NFS v4. If you need to override mount options (e.g. `nfsvers=4,hard,timeo=600`), set them via `storageClass.mountOptions` in the Helm values.
