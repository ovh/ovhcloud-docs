...
description: Learn how to use the NFS Subdir External Provisioner to provision Kubernetes persistent volumes from an OVHcloud File Storage Service NFS share.
...
Split it into two components and note them for the following steps:
...
Click your share, go to the <code className="action">Access Control List (ACL)</code> tab, then click <code className="action">Add a new access</code>. Enter the CIDR range of your cluster's private network (for example `10.1.0.0/24`) and select the `Read and write` permission.
