...
In this guide, we’ll walk you through deploying a Hello World application on your OVHcloud Managed Kubernetes Service. You can manage your cluster either via the <ManagerLink to="/">OVHcloud Control Panel</ManagerLink> or the <ApiLink>OVHcloud API</ApiLink>.
...
:::warning
For API users, this guide assumes you are familiar with the <ApiLink>OVHcloud API</ApiLink>. If you have never used it, you can find the basics here: [First steps with the OVHcloud API](/guides/manage-and-operate/api/first-steps).
...
<Tabs>
  <Tab label="Via the OVHcloud Control Panel">
    In the [OVHcloud Control Panel](https://www.ovh.com/auth/?action=gotomanager&from=https://www.ovh.co.uk/), navigate to the **Public Cloud** section, select your project, then click **Managed Kubernetes Service** in the left sidebar under **Containers & Orchestration**. Your clusters are listed on this page.
...
<Tabs>
  <Tab label="Via the OVHcloud Control Panel">
    ...
    Enter a node pool name in the **Name** field.
...
Define the billing method: monthly or hourly, then confirm the creation.
  </Tab>
  <Tab label="Via the OVHcloud API">
...
<Tabs>
  <Tab label="Via the OVHcloud Control Panel">
    In the **Node pools** tab, you can inspect the state of your node pools. The node pool you have just created should now be in an *Installing* or *Deploying* state. The node pool installation can take a minute, so feel free to take a short break, then try again until it's ready.

    After its creation, click on the node pool name to list its node details.
  </Tab>
  <Tab label="Via the OVHcloud API">
