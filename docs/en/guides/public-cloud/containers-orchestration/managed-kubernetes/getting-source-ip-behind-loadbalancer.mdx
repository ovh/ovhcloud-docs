...
The Public Cloud Load Balancer service is hourly charged and will appear in your Public Cloud project. For more information, please refer to the following documentation: [Network Load Balancer price](/links/public-cloud/prices-network)
:::
...
After applying the patch, run the following commands to confirm the proxy protocol is correctly configured.

Check the service annotations and external traffic policy:
...
- The annotation `service.beta.kubernetes.io/ovh-loadbalancer-proxy-protocol: v2`
- The annotation `loadbalancer.openstack.org/proxy-protocol: v2`
- `ExternalTrafficPolicy: Local`

Check the controller configmap:

```bash
kubectl -n ingress-nginx describe configmap ingress-nginx-controller
```

Verify that the `Data` section contains:

- `use-proxy-protocol: "true"`
- `real-ip-header: proxy_protocol`
- `proxy-real-ip-cidr` set to your egress IPs or subnet range
