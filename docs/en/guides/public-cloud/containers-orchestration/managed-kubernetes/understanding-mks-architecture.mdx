...
With rolling upgrades (available on Standard plan), new worker nodes are created with the target Kubernetes version:
...
|--------|----------|---------|
| Availability | Free only | Standard only |
