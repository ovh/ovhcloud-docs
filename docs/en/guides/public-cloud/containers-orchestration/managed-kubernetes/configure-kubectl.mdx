...
In the [OVHcloud Control Panel](https://www.ovh.com/auth/?action=gotomanager&from=https://www.ovh.co.uk/), navigate to the **Public Cloud** section, select your project, then click **Managed Kubernetes Service** in the left sidebar under **Containers & Orchestration**. Click your cluster name to open its details page.

On the cluster **Overview** tab, click <code className="action">kubeconfig</code> to download the `kubectl` configuration file.
...
3. In the window that opens, add a new variable, `KUBECONFIG`, with the path to the `kube-config` file as its value.
