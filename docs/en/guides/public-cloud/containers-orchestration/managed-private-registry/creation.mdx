...
2. Click on <code className="action">Create a private registry</code>.
...
3. Choose a region, then click on <code className="action">Next</code>.
...
4. Enter a name for your registry, then click on <code className="action">Next</code>.
...
5. Choose a plan and click on <code className="action">Next</code>.
...
:::info
The `M` and `L` plans include a managed vulnerability scanner: [Trivy](https://aquasecurity.github.io/trivy/).
:::
...
[Terraform](https://www.terraform.io/) is an open-source Infrastructure as Code (IaC) tool. OVHcloud provides an official [Terraform provider](https://registry.terraform.io/providers/ovh/ovh/latest) to manage your resources programmatically.

**Requirements**

- [Terraform CLI](https://www.terraform.io/downloads.html) 0.12.x or later
...
:::info
If you encounter a `Provider is missing a required configuration key` error, make sure all OVH environment variables are exported before running `pulumi up`.
:::
