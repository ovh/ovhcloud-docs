...
This system is similar to the one used by certain telephone operators, where they bill at the end of the month for the number of minutes used. Here, OVHcloud bills for the time a server or another element of the service is used — for most compute resources, down to the second. Some services follow their own billing granularity (storage resources, for example, are billed by the GB-hour, except Object Storage, which uses the GiB-hour); see the dedicated sections below for details.
...
For each time-based resource, a usage counter (in seconds) starts when the resource is created, and stops when the resource is removed. Every second of use is billed, beyond an initial 1-minute minimum — usage is never rounded up to the next hour.
...
At the end of the month, each counter is converted into the corresponding cost for the resource. To get the overall bill total, add up all of the counters charged for. Prices are usually displayed as an hourly rate for readability, but these resources are billed per second.
...
Because instance usage is metered to the second (beyond an initial 1-minute minimum), shorter usage periods are billed exactly: for example, if you use an instance for only 20 minutes, you are billed only for those 20 minutes — never rounded up to a full hour.
...
With hourly billing, the service is billed by the exact amount of time (down to the second) during which it is used.
