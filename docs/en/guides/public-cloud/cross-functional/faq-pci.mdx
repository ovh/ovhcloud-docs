...
If emails are not sending or SMTP is not responding, check whether SMTP ports are blocked. Port 25 is blocked by default on OVHcloud Public Cloud instances to prevent abuse (with the exception of instances deployed in [Local Zones](/guides/public-cloud/compute/local-zones-capabilities-limitations), which cannot contact SMTP servers under any circumstances; port 25, like all other SMTP ports, is therefore simply inaccessible, with no option to unblock it). Use port 587 (STARTTLS) for outbound mail instead or, if required, request port 25 to be unblocked by [contacting our support team](https://help.ovhcloud.com/csm?id=csm_get_help).
