...
- **Fixes:**
  - the cookie preferences control now stays at the bottom of the page.
  - project tiles now show a rounded DPU total instead of a long decimal.
  - the message shown when a quota update is below current usage now explains the problem clearly.
  - sign-in and permission checks are more stable and respond faster under load.

**Data Processing Engine**

- **Logs from your own packages:** the logs emitted by the Python packages you install are no longer dropped, in Spark jobs, notebooks and Python jobs alike.
  - Warnings and errors from your packages now reach the job logs, while known platform libraries stay quiet.
  - To see more, add `lib_log_level` to the action's parameters in advanced mode and set it to `info` or `debug`.
- **Faster environments:** the Python packages of an environment now install in parallel, so an environment is ready sooner.
- **Fixes:**
  - loading data into a partitioned table with a [Load](/guides/public-cloud/data-platform/dpe-actions-load) action no longer fails.
...
- **[Row filters](/guides/public-cloud/data-platform/lakehouse-manager-row-filters):** restrict which rows of a table a user or group can see, without duplicating the table. The table keeps one copy of its data, and each person queries it through their own filters.
  - Write the condition in the default builder mode (column, operator, value, combined with *Match ALL (AND)* or *Match ANY (OR)*), or switch to SQL mode for conditions the builder cannot express.
...
- **[Custom action](/guides/public-cloud/data-platform/dpe-actions-custom) logs:** `print()` and standard error output from action code now show up in the job logs, and a missing action produces a clearer error message.
- **[Spark UI and History](/guides/public-cloud/data-platform/dpe-spark-history-server):** Spark applications now carry the job's name instead of a generic one.
- **Fixes:**
...
- **[SQL editor](/guides/public-cloud/data-platform/analytics-manager-queries-sql-editor) autocomplete:** the editor now suggests SQL keywords, ready-made snippets, and every available Trino function with its description.
- **Fixes:**
  - the runtime shown in the [query history](/guides/public-cloud/data-platform/analytics-manager-history) is now formatted correctly.

**Control Center**

- **[Audit trail](/guides/public-cloud/data-platform/control-center-auditing) search:** a new full-text search finds an event quickly, and the layout is clearer.

**Apps and API**

- **App resources:** each DPU allocated to an app now gives it 1 vCPU and 4 GB of RAM, matching the [DPU definition](/guides/public-cloud/data-platform/manage-pricing#billing-units). Apps previously received half of that. The change applies on the app's next restart.
- **Fixes:**
...
- **SDK Update:** Added a new [`create_from_physical`](/guides/public-cloud/data-platform/developers-python-sdk-connect-dataset#create_from_physicaltable-datasetdefault_dataset-kwargs) method to create logical objects from existing physical tables.
