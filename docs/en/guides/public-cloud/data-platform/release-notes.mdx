...
- **Fixes:**
  ...
  - project tiles now show a rounded DPU total instead of a long decimal value.
  - the message shown when a new quota value is below current usage now explains the problem clearly.
  - sign-in and permission checks are more stable and faster under load.
...
- **Logs from your own packages:** the logs emitted by the Python packages you install are no longer dropped in Spark jobs, notebooks and Python jobs alike.
  - Warnings and errors from your packages now reach the job logs, while the platform's own libraries stay quiet.
  - For more detail, set `lib_log_level` to `info` or `debug` in the action's parameters, in advanced mode.
- **Faster environments:** the Python packages of an environment now install in parallel, so it is ready sooner.
- **Fixes:**
  - loading data into a partitioned table with a [Load action](/guides/public-cloud/data-platform/dpe-actions-load) no longer fails.
