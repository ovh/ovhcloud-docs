...
AI Training is a pay-per-use solution. Billing starts once your job's Docker image begins pulling (partway through the `PENDING` phase) and continues through `RUNNING`, `INTERRUPTING` and `TIMEOUT`, until the job enters `FINALIZING`.
