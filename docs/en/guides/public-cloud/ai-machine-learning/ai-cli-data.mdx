...
:::info
When you use `--store-credentials-locally`, the data store is only registered in your local `ovhai` CLI configuration for the region context (`ovhai config set <region>`) you were using when you ran the command. It is not shared across regions. If you want to use this data store from notebooks, jobs or apps running in another region, run the same `ovhai datastore add` command again after switching to that region's context.
:::
