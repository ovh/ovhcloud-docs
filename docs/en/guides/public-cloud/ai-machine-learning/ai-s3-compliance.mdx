...
To create your first **S3 compatible Object Storage bucket**, click <ManagerLink to="/#/public-cloud/pci/projects">this link</ManagerLink> to access your Public Cloud project, then click the <code className="action">Object Storage</code> button in the left-hand side bar, in the <code className="action">Storage</code> section:
...
:::info
Since you used `--store-credentials-locally`, this datastore is only registered in your local `ovhai` CLI configuration for the region context (`ovhai config set <region>`) you were using when you ran the command. It is not shared across regions. If you want to use this datastore from notebooks, jobs or apps running in another region, run the same `ovhai datastore add` command again after switching to that region's context.
:::
