...
- CPU: 12 per app.
- GPU: 4 per app for V100S, A10, L4, L40s and H100; 2 for A100; and H200 is only available in bundles of 4 NVLinked GPUs (the `--gpu` count must be a multiple of 4 for this flavor).
...
You can attach data volumes from Public Cloud Object Storage. For best performance, we recommend keeping the Object Storage bucket in the same region as your AI Deploy app. Mounting a bucket from a different region also works, but adds cross-region network latency.
