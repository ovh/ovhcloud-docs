...
- Specify the version of the conda framework to use.
...
Before training an AI model inside the notebook, you must install all the dependencies of Rasa. Rasa Open Source only supports Python up to 3.10, so you first need to create and activate a dedicated conda environment for it. To do this, open a terminal by clicking the menu icon > Terminal > New Terminal, and launch the following commands:
...
Accept the default path when asked where the project should be created (press `Enter`), then answer `Yes` to train an initial model and `Yes` again to speak with the trained assistant. It will create some folders with basic data for the Rasa chatbot, train a model, then open an interactive chat session in the terminal. Type `/stop` (or press `Ctrl+C`) to end the conversation. Here is a small example of a conversation with the chatbot. 

<img className="thumbnail" alt="Terminal session showing a sample conversation with the Rasa chatbot" src="/images/public-cloud/ai-machine-learning/notebook-tuto-10-create-chatbot/conversation.png" loading="lazy" />
