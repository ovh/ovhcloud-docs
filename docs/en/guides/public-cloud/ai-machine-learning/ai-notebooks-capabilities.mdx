...
- CPU: 12 per notebook.
- GPU: 4 per notebook for V100S, A10, L4, L40s and H100; 2 for A100; and H200 is only available in bundles of 4 NVLinked GPUs (the `--gpu` count must be a multiple of 4 for this flavor).

##### **Available hardware for AI Notebooks**

Hardware availability depends on the datacenter region:
...
Run `ovhai capabilities flavor list` after logging into the region you plan to use (`ovhai config set <region>`) to get the exact, up-to-date list.

Pricing is available on the [OVHcloud Public Cloud prices page](/links/public-cloud/prices-ai).
...
:::info
For best performance, we recommend keeping the Object Storage bucket in the **same region** as your AI Notebook. Mounting a bucket from a different region also works, but adds cross-region network latency. If you use an external S3<sup>1</sup> datastore registered via the `ovhai` CLI with `--store-credentials-locally`, keep in mind that the datastore is only registered for the CLI region context (`ovhai config set <region>`) it was created under, so you need to register it again under each region you use it from.
