...
:::info
Your Object Storage bucket does not have to be in the same region as the **AI Training job**, **AI Notebook** or **AI Deploy app** using it, though cross-region mounting adds network latency — for best performance, keep them in the same region. If you use an external S3<sup>1</sup> datastore, keep in mind that it is only registered for the `ovhai` CLI region context it was created under, see [manage your data with the CLI](/guides/public-cloud/ai-machine-learning/ai-cli-data).
:::
