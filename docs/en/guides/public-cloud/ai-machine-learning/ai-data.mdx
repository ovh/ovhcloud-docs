...
:::info
Your Object Storage bucket does not have to be in the same region as the **AI Training job**, **AI Notebook** or **AI Deploy app** using it. Mounting a bucket from a different region works, but adds cross-region network latency, so we recommend keeping them in the same region for best performance. If you use an external S3 datastore, keep in mind that it is only registered for the `ovhai` CLI region context it was created under, see [manage your data with the CLI](/guides/public-cloud/ai-machine-learning/ai-cli-data).
:::
