...
You can create a DB instance via the <ApiLink>OVHcloud API</ApiLink> or the OVHcloud Control Panel. In both cases, you are able to select various parameters such as DBMS type and DBMS versions.
...
If all the conditions are validated, you can perform the update directly via the <ApiLink>OVHcloud API</ApiLink> or the OVHcloud Control Panel.
...
Public Cloud Databases are billed with a "pay-as-you-go" model. The entire Public Cloud ecosystem allows you to create projects, then attach a payment method to the project (credit card, PayPal, etc.) and finally launch various products and services inside this project.

You don't pay before using the service. At the end of the month, we summarise your spending in one bill per project. Public Cloud Databases are billed per second, for the service range and flavor selected during DB instance creation, with a minimum billing period of 1 minute.
...
When possible, we store your encrypted backups outside the region of your service, for sustainability reasons. The location of the backup is available with all the other information of the backup in the <ApiLink>OVHcloud API</ApiLink>.
...
Yes, you can, via the OVHcloud Control Panel, Terraform, or the <ApiLink>OVHcloud API</ApiLink>.
...
- **European data sovereignty**: Public Cloud Databases are part of OVHcloud, a European company. It guarantees data sovereignty to our customers. [Read more](/links/about-us/data-sovereignty)

### Who can access my database instance initially?

Once you start a new database instance, the default values are:

- Zero access to IPs or IP ranges. Nothing is authorised (ACL).
...
### How can I manage authorised accesses to my database instance?

Once your database instance is created, you can manage authorised IPs or IP blocks either in the OVHcloud Control Panel or via Terraform or the <ApiLink>OVHcloud API</ApiLink>. You need at least 1 authorised IP to access your service.
...
Once your database instance is created, you can manage database users either in the OVHcloud Control Panel or via Terraform or the <ApiLink>OVHcloud API</ApiLink>. You need at least 1 created and configured user. Please read our [Getting started guide](/guides/public-cloud/databases/getting-started) to get a step-by-step documentation.
...
For healthcare compliance such as HIPAA/HDS, please refer to our [official page](/links/hosted-private-cloud/certification-hds). For certification overview, please refer to our [official page](/links/compliance).

## Network

### Can I connect to my database instance via public network?

Yes, all database instances can connect to the public network. It has to be specified and selected during database instance creation. You can later switch to a private network if needed: see the [Update the network configuration](/guides/public-cloud/databases/update-cluster-network) guide, which covers the service rebuild.

### Can I connect to my database instance via private network (vRack)?

Yes, based on the service plan, database instances can connect to private network (vRack). It has to be specified and selected during database instance creation. You can later switch to a public network if needed: see the [Update the network configuration](/guides/public-cloud/databases/update-cluster-network) guide, which covers the service rebuild and its effect on your IP restrictions.

### Can I move my instance from public to private network or vice versa?

Yes. You can switch your database service between public and private networks via the <ApiLink>OVHcloud API</ApiLink>. The service must be in `READY` status, and the operation triggers a service rebuild, during which the database is temporarily unavailable. Switching to the public network also clears the existing IP restrictions: reconfigure them before any client can connect again. For detailed instructions, see the [Update the network configuration](/guides/public-cloud/databases/update-cluster-network) guide.
...
- Have at least 1 authorised IP. It has to be the IP which will access your database (your computer, your dedicated server, etc.). For troubleshooting purposes only, you can also add the "0.0.0.0" IP so that it will allow "any" connection. Remove it after your tests are done.
...
- Your data schema. The organisation of your data will strongly impact your performances.
