...
Yes, all database instances have the ability to connect to the public network. It has to be specified and selected during database instance creation. You can later switch to a private network if needed — see the [Update the network configuration](/guides/public-cloud/databases/update-cluster-network) guide.

### Can I connect to my database instance via private network (vRack)?

Yes, based on the service plan, database instances have the ability to connect to private network (vRack). It has to be specified and selected during database instance creation. You can later switch to a public network if needed — see the [Update the network configuration](/guides/public-cloud/databases/update-cluster-network) guide.

### Can I move my instance from public to private network or vice versa?

Yes. You can switch your database service between public and private networks via the OVHcloud API. The operation triggers a service rebuild, during which the database is temporarily unavailable. For detailed instructions, see the [Update the network configuration](/guides/public-cloud/databases/update-cluster-network) guide.
