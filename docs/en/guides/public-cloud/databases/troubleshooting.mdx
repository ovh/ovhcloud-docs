...
2. **Check OVHcloud health statuses:** when major outages occur, they are listed on [https://www.status-ovhcloud.com/](https://www.status-ovhcloud.com/).
...
- A modification on the cluster is requested. Depending on the chosen DBMS and service plan, the reboot can be required even if we try to avoid it as much as we can. When a modification is required, we inform customers about it via the Control Panel.
