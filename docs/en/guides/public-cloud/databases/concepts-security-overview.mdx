...
In addition to [the responsibility model for Public Cloud Database services](/guides/public-cloud/databases/responsibility-model), this security fact sheet aims to describe the security features and functions associated with the service. It also describes best practices that customers can adopt to secure their databases.
...
Once you have followed [these first steps to subscribe your service](/guides/public-cloud/databases/getting-started) and reset the default password communicated to access the service, you must filter connections by using iptables.<br/>
You can also activate a private connection by using the vRack option.

### 2.2 Vulnerability scans

You are authorized to perform vulnerability scans on the service you have subscribed to. OVHcloud does not need to be informed beforehand.<br/>
...
You can import and export your data following the recommendations provided by the vendors for each Database engine technology. Here are some examples:
