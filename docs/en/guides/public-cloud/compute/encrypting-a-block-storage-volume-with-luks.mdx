...
OVHcloud Block Storage volumes can be encrypted at rest using <Tooltip term="luks">LUKS</Tooltip>. The encryption key — called a Service Key — is generated and stored in [OVHcloud Key Management Service (OKMS)](/guides/manage-and-operate/kms/overview).

By default, this Service Key is entirely generated and managed by OVHcloud: you simply order an encrypted (`-luks`) volume type and OVHcloud takes care of the rest. Optionally, you can instead use a **customer-managed key (CMK)**: a Service Key that you create and control yourself in your own OKMS domain, including its rotation, suspension, and deletion.
...
To encrypt a Block Storage volume with the default, OVHcloud-managed key, order the encrypted variant of any volume type (`classic-luks`, `high-speed-luks`, `high-speed-gen2-luks`) — from the Control Panel or with the tools described in the [How to create and configure an additional disk on an instance](/guides/public-cloud/compute/create-and-configure-an-additional-disk-on-an-instance#the-different-types-of-volumes) guide. OVHcloud automatically generates and manages the Service Key in OKMS: there is no OKMS domain to create, no Service Key to manage, and no IAM policy to configure.

:::info
Once attached to your instance, an encrypted volume behaves like a standard volume — all encryption and unlocking are handled automatically by the OVHcloud infrastructure. See the [Migrating a Block Storage volume to an encrypted LUKS volume](/guides/public-cloud/compute/migrating-non-encrypted-to-encrypted-volume) guide if you need to encrypt an existing, non-encrypted volume.
:::
...
- API access with valid credentials — a Bearer token (OAuth2 via an IAM service account) or the classic application key / signature authentication. See the [First Steps with the OVHcloud APIs](/guides/manage-and-operate/api/first-steps) guide.
- Optionally, the OpenStack CLI installed and configured for the target region, as an alternative to the OVHcloud API for creating the volume. See the [Preparing an environment for using the OpenStack API](/guides/public-cloud/cross-functional/compute-prepare-openstack-api-environment) guide.
...
### Step 1: Create an OKMS domain <a id="step1"></a>
...
<Tabs>
  <Tab label="OVHcloud Control Panel">
    To order an OKMS domain, follow the "Ordering your KMS" section of the [Getting started with OVHcloud Key Management Service (KMS)](/guides/manage-and-operate/kms/quick-start#ordering-your-kms) guide.
  </Tab>
...
### Step 2: Create a Service Key <a id="step2"></a>
...
<Tabs>
  <Tab label="OVHcloud Control Panel">
    To create the Service Key, follow the "Create an encryption key" section of the [Getting started with OVHcloud Key Management Service (KMS)](/guides/manage-and-operate/kms/quick-start#create-an-encryption-key) guide, and set the following parameters:
...
:::warning
If your region is not listed here, contact [OVHcloud support](/links/support) to obtain the correct NIC.
:::
...
<Tabs>
  <Tab label="OVHcloud Control Panel">
    Create a new IAM policy in the Control Panel, as described in the [How to use IAM policies using the OVHcloud Control Panel](/guides/account-and-service-management/account-information/iam-policy-ui) guide, and configure it with the following values:
...
#### 3.3 Actions to authorize <a id="actions33"></a>

Under <code className="action">Key Management Service (OKMS) / Service Keys</code>, in the <code className="action">OPERATE</code> category:
...
### Step 4: Create the LUKS volume <a id="step4"></a>
...
Once created, a CMK LUKS encrypted volume behaves like a standard OVHcloud Block Storage volume: you [attach](/guides/public-cloud/compute/create-and-configure-an-additional-disk-on-an-instance#attaching-a-new-volume) and [detach](/guides/public-cloud/compute/create-and-configure-an-additional-disk-on-an-instance#detach-a-volume) it the same way. Deleting the volume does not delete your Service Key.
