...
:::info
All volume types are also available in an encrypted version (LUKS). These volumes ensure data confidentiality. They are available through the OVHcloud Control Panel as well as via the tools presented in the next section, by specifying the type `<volume_type>-luks`.

:::

:::warning
**Performance overhead of encrypted (LUKS) volumes.** The LUKS encryption path adds a measurable overhead to data-plane throughput. On High Speed Gen2 volumes above 600 GB, this caps the maximum throughput at 470 MB/s instead of the nominal 512 MB/s (a reduction of about 8%). Plan capacity accordingly for throughput-sensitive workloads.
