...
:::warning
Attaching or detaching interfaces while the instance is running can change the interface order seen by the guest OS after a reboot. If you reorganise several interfaces, see [Interface order after hot-attaching or hot-detaching ports](/guides/public-cloud/network-services/vrack#interface-order-after-reboot) and validate the result with a [hard reboot](#reboot-an-instance) before relying on it in production.
:::
