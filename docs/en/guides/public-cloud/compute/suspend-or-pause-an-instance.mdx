...
**Monthly billing for a suspended (shelved) instance:** An instance that remains shelved for the entire month will not be billed (except for the snapshot). As soon as the instance is unshelved, even briefly, the full month's billing applies.
