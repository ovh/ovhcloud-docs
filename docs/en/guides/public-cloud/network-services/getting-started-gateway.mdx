...
An OVHcloud [Gateway](/links/public-cloud/gateway) provides secure outbound connectivity (SNAT) for instances on a private network subnet. It is also required on that subnet to use [Floating IPs](/links/public-cloud/floating-ip) for public exposure of those private resources.
...
<Tabs>
  <Tab label="OVHcloud Control Panel">
    ...
    3. **Name and attach a network.** Enter a name for your gateway and select an existing private network from the drop-down list, or click <code className="action">Add private network</code> to create one inline.
...
<Tabs>
  <Tab label="OVHcloud Control Panel">
    From the <code className="action">Gateway</code> list, click the <code className="action">...</code> menu next to your gateway, then select <code className="action">Modify Gateway</code>.
