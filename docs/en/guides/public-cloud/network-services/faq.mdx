...
Load Balancer is offered in different sizes (S/M/L/XL) to best fit our customer needs. These different sizes are defined via flavours. You can resize your Load Balancer in place, without needing to spawn a new one or reconnect the Floating IP, either through the OpenStack API/Horizon or through the OVHcloud API (`PUT /cloud/project/{serviceName}/loadbalancer/{loadBalancerId}`).
...
Yes. Managed Kubernetes Service (MKS) supports integration with the OVHcloud Public Cloud Load Balancer (based on OpenStack Octavia), and this feature is now Generally Available. For more information, please refer to our guide on [exposing applications using OVHcloud Public Cloud Load Balancer](/guides/public-cloud/containers-orchestration/managed-kubernetes/expose-applications-using-load-balancer).
