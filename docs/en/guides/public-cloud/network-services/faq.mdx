...
Load Balancer is offered in different sizes (S/M/L/XL) to best fit our customer needs. These different sizes are defined via flavours. You can resize your Load Balancer in place, without spawning a new one or reconnecting the Floating IP, either through the OpenStack API/Horizon or through the OVHcloud API:

<Api version="v1" section="/cloud" method="PUT" route={"/cloud/project/\{serviceName\}/region/\{regionName\}/loadbalancing/loadbalancer/\{loadBalancerId\}"} />
...
Yes. Managed Kubernetes Service (MKS) supports integration with the OVHcloud Public Cloud Load Balancer (based on OpenStack Octavia). This feature is now Generally Available. For more information, please refer to our guide on [exposing applications using OVHcloud Public Cloud Load Balancer](/guides/public-cloud/containers-orchestration/managed-kubernetes/expose-applications-using-load-balancer).
