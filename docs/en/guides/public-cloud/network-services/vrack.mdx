...
**Interface order after hot-attaching or hot-detaching ports** <a name="interface-order-after-reboot"></a>

:::warning
Attaching or detaching network interfaces (ports) while the instance is running (hot-plug) can change the order in which the guest operating system sees them on the next reboot. As a result, the interface your OS identifies as `eth0` (or another expected interface, including the public one) may change, which can lead to unexpected network issues if the new configuration is not validated beforehand.

If you need to reorganise several interfaces on a running instance:
1. Detach all the interfaces you want to change.
2. Reattach them one by one, in the order you want them to appear in the guest OS.
3. Perform a **hard reboot** of the instance to confirm the interface order is applied as expected before relying on it in production.
:::
...
If you plan to reattach interfaces afterwards, see [Interface order after hot-attaching or hot-detaching ports](#interface-order-after-reboot) to avoid interface reordering after a reboot.
