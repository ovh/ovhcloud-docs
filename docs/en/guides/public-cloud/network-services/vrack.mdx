...
**The sections below describe the available actions for each interface/method.** <a id="horizon"></a>
...
Every action available in your OVHcloud Control Panel can also be performed via the <ApiLink>OVHcloud APIv6</ApiLink>.
...
You can access it from <ApiLink>our web page</ApiLink> but also use it to create your PHP or Python scripts.
...
The [OVHcloud CLI](https://github.com/ovh/ovhcloud-cli) lets you manage Public Cloud resources from your terminal, including private networks and instance interfaces.
...
### Step 1: Activating and managing a vRack <a id="activation"></a>
...
You will be redirected to another page to validate the order, it will take a few minutes for the vRack to be setup in your account.
    ...
    To continue configuring your vRack from the OVHcloud Control Panel, go to [Step 2: Create a private network in the vRack](#create-pn-in-vrack), under the **OVHcloud Control Panel** tab.
  </Tab>
  <Tab label="OVHcloud APIv6">
    **Step 1: Activating and managing a vRack**
    
    Open the OVHcloud APIv6 interface according to the relevant guide ([First steps with the OVHcloud API](/guides/manage-and-operate/api/first-steps)) and follow these steps:
    ...
    **Step 2: Adding your Public Cloud project to the vRack**
    ...
    Open the OVHcloud APIv6 interface according to the relevant guide: [First steps with the OVHcloud API](/guides/manage-and-operate/api/first-steps).
...
### Step 2: Creating a private network in the vRack <a id="create-pn-in-vrack"></a>
...
In the **Private Network Name** field, set a name for your private network.
    ...
    **Network Gateway Options**
    ...
    - **Assign a Gateway and connect to the private network**: Select this option if you intend to create instances with a private network only. For more information, please consult the following guides: [Creating a private network with Gateway](/guides/public-cloud/network-services/create-private-network-gateway) and [How to create a Public Cloud instance and connect to it](/guides/public-cloud/compute/getting-started).
  ...
  <Tab label="OVHcloud APIv6">
    Open the <ApiLink>OVHcloud APIv6 interface</ApiLink>, then follow these steps:
...
### Step 3: Integrating an instance into vRack <a id="instance-integration"></a>
...
<Tabs>
  <Tab label="OVHcloud Control Panel">
    For help, see [Creating an instance in the OVHcloud Control Panel](/guides/public-cloud/compute/getting-started). When creating an instance, choose a network mode in Step 5, then a private network. 
...
**Adding a private interface** <a id="adding-a-private-interface"></a>
...
Select your interface and confirm.
  ...
  <Tab label="OpenStack CLI">
    ...
    **List available networks**
    ...
    Note the network IDs of interest:
...
### Interface order after hot-attaching or hot-detaching ports <a id="interface-order-after-reboot"></a>

:::warning
Attaching or detaching network interfaces (ports) while the instance is running (hot-plug) can change the order in which the guest operating system sees them on the next reboot. As a result, the interface the guest OS identifies as `eth0` (or another expected interface, including the public one) may change. Check the new configuration to avoid unexpected network issues.

If you need to reorganise several interfaces on a running instance:

1. Detach all the interfaces you want to change. Detaching the "Ext-Net" interface releases its public IP address permanently, as explained in [Removing a network interface](#removing-a-private-interface).
2. Reattach them one by one, in the order you want them to appear in the guest OS.
3. Perform a *hard reboot* of the instance to confirm the interface order is the one you expect before relying on it in production.

:::

### Removing a network interface <a id="removing-a-private-interface"></a>
...
To detach an interface, first identify the port attached to your instance:
