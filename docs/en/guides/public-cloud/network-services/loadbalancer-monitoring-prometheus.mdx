...
<Tabs>
  <Tab label="OVHcloud Control Panel">
    ...
    :::warning
    The `allowed_cidrs` option is not yet available in the OVHcloud Control Panel. To restrict access to the metrics endpoint, use the OpenStack CLI or Horizon instead.
    :::
  </Tab>
  <Tab label="Horizon">
  ...
  <Tab label="OpenStack CLI">
    ...
    To restrict access to the metrics endpoint to a specific subnet, add the `--allowed-cidr` option:
