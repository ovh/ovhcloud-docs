...
### Assign API access rights to your service account

First, your service account must be able to access the required APIs. Each API call requires specific rights, which you can look up in the <ApiLink>OVHcloud API console</ApiLink>: the name of the required action is shown on the **IAM actions** line.
For example, for the `/v1/hosting/web` call, the required action is `webHosting:apiovh:get`, as you can see in the following example:
...
Use the token endpoint that matches your API region (the example above uses the EU endpoint):
