...
`<ManagerLink>` also takes a `brands` prop — a list of brands, same shape as `regions` (default `['ovh']`) — same meaning as `<Api>`/`<ApiLink>` below. Whenever `brands` includes `'ks'`/`'sys'`, `to`/`authFlow` (and `urls`/`regionMeta`) have no effect: the Kimsufi/So you Start manager login URLs are built in-component, on a different domain/scheme than OVH's manager:

A Kimsufi / So you Start manager link — no `to` needed, the picker offers all 4 (Kimsufi EU/CA + So you Start EU/CA):

<ManagerLink brands={['sys', 'ks']}>the Kimsufi/So you Start manager</ManagerLink>

Every OVH region *and* every Kimsufi/So you Start endpoint in one picker:
...
GET call on a Kimsufi / So you Start route — `brands`, and no `version` (these brands have no versioned API like OVHcloud's); default offers all 4: Kimsufi EU/CA + So you Start EU/CA:

<Api brands={['sys', 'ks']} section="/dedicated/server" method="GET" route={"/dedicated/server/\{serviceName\}/install/compatibleTemplates"} />

Same route, restricted to SoYouStart only via `brands`, further restricted to EU only via `regions`:
...
`'ovh'`, `'ks'` and `'sys'` are independently selectable — mixing all three shows every OVH region *and* every Kimsufi/So you Start endpoint in one picker (`regions` filters both families at once, since they share the same eu/ca set); `version` defaults to `'v1'` for the OVH keys if omitted:
...
A Kimsufi / So you Start operation deep link — same `brands`/`regions` props as `<Api>`; `section` alone (no `route`/`method`) also works here, same three levels (gateway / section / operation) as the `ovh` brand:
...
Restricted to SoYouStart EU only, same as the `<Api>` example above:

See <ApiLink brands={['sys']} regions={["eu"]} method="GET" route={"/dedicated/server/\{serviceName\}"}>this endpoint</ApiLink>.

Every OVH region *and* every Kimsufi/So you Start endpoint in one picker — `brands` works the same way across `<Api>`, `<ApiLink>` and `<ManagerLink>`:

See <ApiLink brands={['ovh', 'sys', 'ks']} method="GET" route={"/dedicated/server/\{serviceName\}"}>this endpoint on any dedicated server brand</ApiLink>.
