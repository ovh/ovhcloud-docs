...
Cree una clave RSA dedicada al encapsulado. Defina siempre tanto `wrapKey` como `unwrapKey`. `wrapKey` y `unwrapKey` son mutuamente excluyentes con `sign` y `verify`.
...
`KEY` es la cadena JWE Compact Serialization. Pásela en la forma `@path/to/wrapped.jwe`, directamente como argumento, o `-` para leerla desde stdin.
...
Llame a `GET /v1/servicekey/{keyId}` con los parámetros de consulta de encapsulado siguientes. Cuando estos parámetros están presentes, la API devuelve `wrappedKeys` en lugar de `keys` en claro.
