...
Para comunicarse con su dominio OKMS, necesita un certificado de acceso. Este es uno de los tres [métodos de autenticación OKMS](/guides/manage-and-operate/kms/okms-authentification-method); los certificados son **obligatorios para KMIP** y se recomiendan cuando se necesita autenticación de cliente mTLS. Para el uso de la API REST, un Personal Access Token (PAT) o una cuenta de servicio suele ser más sencillo de configurar.
