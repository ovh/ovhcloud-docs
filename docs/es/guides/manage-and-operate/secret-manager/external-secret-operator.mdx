...
El campo `stringData` acepta el token sin codificar — Kubernetes lo codifica en base64 al almacenar el secreto. Si prefiere utilizar el campo `data`, deberá codificar antes el token en base64 usted mismo (`echo -n "<token>" | base64`).

Aplíquelo con el comando `kubectl apply -f secret.yaml`.

O, si el `user_pat` está almacenado en la variable de entorno `PAT_TOKEN`, cree el secreto directamente con kubectl, que también se encarga de la codificación:
...
Al inspeccionar el secreto con `kubectl get secret ovhcloud-vault-token -n external-secrets -o yaml`, el valor aparece bajo `data`, codificado en base64.
