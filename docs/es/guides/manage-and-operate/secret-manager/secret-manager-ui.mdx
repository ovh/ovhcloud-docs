...
- Para acceder a los secretos a través de la API REST o de la API compatible con HashiCorp Vault KV2, configure un [método de autenticación](/guides/manage-and-operate/kms/okms-authentification-method) para su dominio OKMS (Personal Access Token, cuenta de servicio o certificado de acceso).
...
A user with the necessary IAM rights can access the content of a secret. Configure la autenticación y las políticas IAM según se describe en [Métodos de autenticación OKMS](/guides/manage-and-operate/kms/okms-authentification-method).
...
El Secret Manager es accesible a través de dos conjuntos de API diferentes. Ambos operan sobre los mismos objetos: un secreto creado con un método es accesible con el otro.

Antes de llamar a cualquiera de las API, configure un [método de autenticación](/guides/manage-and-operate/kms/okms-authentification-method) y la [política IAM](/guides/manage-and-operate/kms/okms-authentification-method#iam-rights-for-okms) requerida en su dominio OKMS. Para el uso de la API REST, se recomienda un Personal Access Token o una cuenta de servicio.
