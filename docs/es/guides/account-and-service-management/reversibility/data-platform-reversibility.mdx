...
OVHcloud uses pay-as-you-go pricing, with no egress fee. Billing is interrupted immediately when the service is cancelled, allowing flexible cost control.

## Data retention after service termination

Once the service is cancelled, all data are irreversibly deleted. It is the client's responsibility to carry out a full backup or migration before termination.
