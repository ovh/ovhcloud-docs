...
Acceda a la <ApiLink>API de OVHcloud</ApiLink> y utilice las siguientes llamadas a la API para modificar el anuncio del bloque de IP:

<Api version="v1" section="/vrack" method="GET" route={"/vrack"} />

Esta llamada a la API permite mostrar la lista de vRacks. Si el nombre mostrado en la API no le permite identificar el vRack, puede consultarlo en el <ManagerLink to="/">área de cliente de OVHcloud</ManagerLink>.
