...
Acceda a la <ApiLink>consola API de OVHcloud</ApiLink> y utilice la siguiente llamada:
...
El backup storage solo es compatible con NFSv3. En primer lugar, asegúrese de haber autorizado a sus bloques de IP a acceder al espacio de backup y a utilizar el protocolo NFS. Según el sistema operativo Linux, es posible que necesite instalar el cliente NFS e iniciar los servicios NFS y `portmap`.
...
En caso de problemas de conexión, compruebe también que los puertos UDP y TCP necesarios para NFSv3 están abiertos en el firewall, tanto de entrada como de salida.
El puerto 111 UDP/TCP debe estar abierto en ambos sentidos, al igual que el puerto 2049/TCP (NFS) y el rango de puertos dinámicos (de 32768 a 60999 UDP/TCP). Estos puertos son utilizados por RPC para los programas `mountd`, `lockd` y `statd`.

Por último, autorice todas las conexiones salientes hacia la dirección IP del servidor Backup Storage.
