---
pageType: overview
title: Servidores dedicados
description: "Configure, proteja y opere sus servidores dedicados de OVHcloud - instalación, red, RAID, copias de seguridad y administración avanzada del servidor."
text: "Descubra las guías de Servidores dedicados"
essentialsTitle: "Las guías esenciales para empezar"
essentials:
  - title: "Primeros pasos con un servidor dedicado"
    link: /guides/bare-metal-cloud/dedicated-servers/getting-started-with-dedicated-server
  - title: "Primeros pasos con un servidor dedicado Kimsufi, So you Start o Rise"
    link: /guides/bare-metal-cloud/dedicated-servers/getting-started-with-dedicated-server-eco
  - title: "Cómo empezar con buen pie con las conexiones SSH"
    link: /guides/bare-metal-cloud/dedicated-servers/ssh-introduction
  - title: "Cómo crear y utilizar claves de autenticación para las conexiones SSH a los servidores de OVHcloud"
    link: /guides/bare-metal-cloud/dedicated-servers/creating-ssh-keys
  - title: "Cómo almacenar las llaves SSH en su área de cliente"
    link: /guides/bare-metal-cloud/dedicated-servers/import-keys-control-panel
  - title: "Cómo utilizar la consola IPMI con un servidor dedicado"
    link: /guides/bare-metal-cloud/dedicated-servers/ipmi
tutorials:
  - title: "Cómo utilizar PuTTY para conexiones SSH y autenticación"
    link: /guides/web-cloud/web-hosting/ssh-using-putty-on-windows
  - title: Tutorial - Configuring pfSense network bridge
    link: /guides/bare-metal-cloud/dedicated-servers/pfsense-bridging
  - title: "Tutorial - Instalar un servidor web (LAMP) en Debian o Ubuntu"
    link: /guides/bare-metal-cloud/dedicated-servers/installing-lamp-debian9-ubuntu18
goFurther:
  title: "Más información"
  items:
    - title: Visitar la comunidad OVHcloud
      link: https://community.ovhcloud.com/
    - title: "Consultar la hoja de ruta y el registro de cambios"
      link: https://www.ovhcloud.com/es/roadmap-changelog/
    - title: Unirse a Discord
      link: https://discord.gg/ovhcloud
cta:
  title: "¿Tiene preguntas? Acceda al soporte de OVHcloud"
  description: "Encuentre la respuesta a sus preguntas en nuestro centro de ayuda."
  actions:
    - text: Acceder al soporte
      link: https://help.ovhcloud.com/
      theme: brand
---
