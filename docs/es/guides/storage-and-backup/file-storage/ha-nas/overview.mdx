---
pageType: overview
title: NAS-HA
description: "Solicite, configure y opere los sistemas de archivos compartidos NAS-HA en OVHcloud - almacenamiento NFS y CIFS con alta disponibilidad, snapshots y control de accesos."
text: "Descubra las guías de NAS-HA"
essentialsTitle: "Las guías esenciales para empezar"
essentials:
  - title: Primeros pasos con un NAS-HA
    link: /guides/storage-and-backup/file-storage/ha-nas/nas-get-started
  - title: NAS-HA - Primeros pasos con las API
    link: /guides/storage-and-backup/file-storage/ha-nas/nas-quick-api
goFurther:
  title: Más información
  items:
    - title: Visitar la comunidad OVHcloud
      link: https://community.ovhcloud.com/
    - title: Consultar la hoja de ruta y el registro de cambios
      link: https://www.ovhcloud.com/es/roadmap-changelog/
    - title: Unirse a Discord
      link: https://discord.gg/ovhcloud
cta:
  title: "¿Tiene preguntas? Acceda al soporte de OVHcloud"
  description: "Encuentre la respuesta a sus preguntas en nuestro centro de ayuda."
  actions:
    - text: Acceder al soporte
      link: https://help.ovhcloud.com/
      theme: brand
---
