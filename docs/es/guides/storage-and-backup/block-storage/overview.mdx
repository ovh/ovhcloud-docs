---
pageType: overview
title: Block Storage
description: "Solicite, asocie, redimensione y haga snapshots de volúmenes Block Storage en OVHcloud Public Cloud - almacenamiento por bloques de alto rendimiento, classic y compartido."
text: Descubra las guías de Block Storage
essentialsTitle: Las guías esenciales para empezar
essentials:
  - title: Crear y configurar un disco adicional en una instancia
    link: /guides/public-cloud/compute/create-and-configure-an-additional-disk-on-an-instance
  - title: Crear una copia de seguridad de un volumen
    link: /guides/public-cloud/compute/volume-backup
  - title: Crear un snapshot de un volumen
    link: /guides/public-cloud/compute/storage-create-volume-snapshot
goFurther:
  title: Más información
  items:
    - title: Visitar la comunidad OVHcloud
      link: https://community.ovh.com/
    - title: Consultar la hoja de ruta y el registro de cambios
      link: https://www.ovhcloud.com/es/roadmap-changelog/
    - title: Unirse a Discord
      link: https://discord.gg/ovhcloud
cta:
  title: "¿Tiene preguntas? Acceda al soporte de OVHcloud"
  description: Encuentre la respuesta a sus preguntas en nuestro centro de ayuda.
  actions:
    - text: Acceder al soporte
      link: https://help.ovhcloud.com/
      theme: brand
---
