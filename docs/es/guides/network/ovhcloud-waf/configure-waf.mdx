...
| The WAF is not forwarding traffic to the backend | The **Upstream URL** is missing or incorrect, <br/>or the backend is unreachable from the WAF. | On the **Proxy** page, confirm the Upstream URL is set with an `http://` or `https://` scheme. Verify the backend is reachable from the WAF instance. |
