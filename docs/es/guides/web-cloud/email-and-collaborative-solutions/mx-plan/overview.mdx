---
pageType: overview
title: MX Plan
description: "Empiece a utilizar la solución de correo MX Plan de OVHcloud - creación de buzones, configuración del cliente de correo, redirecciones, listas de distribución y filtrado de spam."
text: "Cómo empezar a utilizar la solución de correo electrónico MX Plan"
essentialsTitle: "Las guías esenciales para empezar"
essentials:
  - title: "Primeros pasos con la solución MX Plan"
    link: /guides/web-cloud/email-and-collaborative-solutions/mx-plan/email-generalities
  - title: "Crear una dirección de correo electrónico en un MX Plan"
    link: /guides/web-cloud/email-and-collaborative-solutions/mx-plan/email-creation
  - title: "Cambiar la contraseña de una dirección de correo"
    link: /guides/web-cloud/email-and-collaborative-solutions/mx-plan/email-change-password
  - title: "Webmail: Guía de uso de Roundcube"
    link: /guides/web-cloud/email-and-collaborative-solutions/mx-plan/email-roundcube
  - title: "FAQ soluciones de correo electrónico de OVHcloud"
    link: /guides/web-cloud/email-and-collaborative-solutions/mx-plan/faq-emails
  - title: "Utilizar el webmail Zimbra"
    link: /guides/web-cloud/email-and-collaborative-solutions/mx-plan/email-zimbra
goFurther:
  title: Más información
  items:
    - title: Visitar la comunidad OVHcloud
      link: https://community.ovhcloud.com/
    - title: Consultar la hoja de ruta y el registro de cambios
      link: https://www.ovhcloud.com/es/roadmap-changelog/
    - title: Unirse a Discord
      link: https://discord.gg/ovhcloud
cta:
  title: "¿Tiene preguntas? Acceda al soporte de OVHcloud"
  description: "Encuentre la respuesta a sus preguntas en nuestro centro de ayuda."
  actions:
    - text: Acceder al soporte
      link: https://help.ovhcloud.com/
      theme: brand
availableIn: [eu, ca, apac]
---
