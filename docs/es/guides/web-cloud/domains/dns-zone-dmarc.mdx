...
Haga clic en <code className="action">Añadir una entrada</code> encima de la tabla y, a continuación, seleccione el tipo **DMARC**. El formulario le permite configurar cada parte del registro:
...
- **Versión**: campo obligatorio que determina la versión del protocolo DMARC (`DMARC1`).
...
- **URI de creación de informes globales (rua=)**: direcciones a las que se enviarán los informes (lista de texto sin formato con comas). Se puede especificar cualquier URI válido. La mención "mailto:" debe preceder al destinatario de correo (por ejemplo: `mailto:address@example.com`).
...
<img className="thumbnail" alt="Añadir un registro DMARC desde la zona DNS" src="/images/assets/screens/control-panel/product-selection/web-cloud/domain-dns/dns-zone/nx-dmarc-form.png" loading="lazy" />
...
Haga clic en <code className="action">Añadir una entrada</code> encima de la tabla y, a continuación, seleccione el tipo **TXT**.
...
Para ilustrar este primer ejemplo, hemos utilizado el [registro DMARC](#dmarc-record) en la zona DNS y le hemos aplicado los siguientes parámetros. Obtenemos el siguiente resultado:
...
- **pct=100**: La política DMARC se aplica al 100% de los mensajes procedentes del flujo de correo del titular del nombre de dominio.
