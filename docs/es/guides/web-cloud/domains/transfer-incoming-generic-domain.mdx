...
Una vez desbloqueado el nombre de dominio, y con el código obtenido, puede solicitar la transferencia a OVHcloud. Esta etapa se realiza en el sitio web de OVHcloud, y no en el área de cliente de OVHcloud: solicite la transferencia desde [nuestro sitio web](/links/web/domains-transfer) y haga clic en <code className="action">Transferir un dominio</code> para abrir el formulario de pedido. A continuación, introduzca el nombre de dominio en el campo de registro de un nombre de dominio y continúe con el pedido.

<img className="thumbnail" alt="domain" src="/images/assets/screens/website/order/domain-transfer-order.png" loading="lazy" />

Cuando se le pida el código de transferencia, introdúzcalo en el campo situado junto al nombre de dominio. El código es necesario para autorizar la transferencia, por lo que debe disponer de él antes de iniciar el pedido. En algunos nombres de dominio, el pedido indica que no se requiere ningún código de autorización. Tenga en cuenta que la transferencia no se iniciará hasta que se haya proporcionado un código válido.
...
- **configuración de los servidores DNS para su nombre de dominio.** Si está utilizando su nombre de dominio para mantener un sitio web o un servicio de correo electrónico en línea, elija mantener los servidores DNS existentes o indicar los suyos para evitar interrupciones del servicio.
...
|Opción|Descripción|
|---|---|
|<code className="action">Servidor DNS por defecto</code>|El nombre de dominio se asocia a los servidores DNS de OVHcloud y se crea una nueva zona DNS. Su configuración DNS actual no se conserva: en ese caso, puede ser necesario [modificar manualmente la zona DNS](/guides/web-cloud/domains/dns-zone-edit) para restablecer sus servicios.|
|<code className="action">Mantener los servidores DNS actuales</code>|Los servidores DNS declarados actualmente en el nombre de dominio en su agente registrador se mantienen sin cambios tras la transferencia. Sus servicios siguen siendo accesibles sin ninguna intervención por su parte, siempre que esos servidores DNS sigan activos.|
|<code className="action">Personalizar los servidores DNS</code>|Usted mismo introduce los nombres de los servidores DNS que desea asociar al nombre de dominio. Utilice esta opción cuando esos servidores sean distintos de los declarados actualmente en su agente registrador.|

:::info
Si actualmente utiliza su nombre de dominio para un sitio web o un servicio de correo electrónico, elija <code className="action">Mantener los servidores DNS actuales</code> o <code className="action">Personalizar los servidores DNS</code> para evitar cualquier interrupción del servicio. La opción <code className="action">Servidor DNS por defecto</code> debe reservarse a los nombres de dominio cuyos servicios estén (o vayan a estar) alojados en OVHcloud.

:::

:::warning
Si elige <code className="action">Mantener los servidores DNS actuales</code>, su anterior agente registrador puede desactivar sus propios servidores DNS después de la transferencia, lo que haría que su nombre de dominio dejara de ser accesible. Asegúrese de que sus servidores DNS no estén alojados en su anterior agente registrador.
