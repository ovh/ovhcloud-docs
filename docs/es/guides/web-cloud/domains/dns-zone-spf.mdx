...
Se mostrará una tabla que muestra la <Tooltip term="dnsZone">zona DNS</Tooltip> de su nombre de dominio en OVHcloud. Está formada por varios registros, todos ellos simbolizados por una línea de la tabla.
...
Para una solución de correo de OVHcloud, el registro SPF mínimo es simplemente la casilla **Incluir los servidores de correo de OVH** marcada, lo que produce `v=spf1 include:mx.ovh.ca ~all` (consulte [Configuración SPF de OVHcloud para los servicios de correo en alojamiento compartido](#ovhcloudspfvalue)).
...
- Solo <Tooltip term="mxPlan">MX Plan</Tooltip> o incluido en un plan de [hosting de OVHcloud](/links/web/hosting).
<Region zones={['eu']}>
...
Acceda a la sección <code className="action">Dominios</code> de la columna izquierda y seleccione el nombre de dominio asociado a su plataforma Private Exchange. A continuación, abra la pestaña de la columna izquierda y haga clic en <code className="action">Zona DNS</code>. Obtenga las direcciones IPv4 (registro `A`) y IPv6 (registro `AAAA`) correspondientes a la URL del webmail.
