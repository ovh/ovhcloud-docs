...
1. **Cree claves API** desde la <CreateToken>página de creación de tokens de la API de OVHcloud</CreateToken> autorizando los endpoints `/sms/*`.
...
La API devuelve un identificador de job que permite hacer seguimiento del estado del envío. También hay bibliotecas disponibles en Python, Node.js, Java y C#. La documentación completa de la API SMS está disponible en la <ApiLink section="/sms">consola API de OVHcloud</ApiLink>.
...
La documentación interactiva completa está disponible en la <ApiLink section="/sms">consola API de OVHcloud</ApiLink>. La autenticación se realiza a través de claves API (Application Key, Application Secret, Consumer Key).
