...
En la página <code className="action">Acceso rápido</code>, haga clic en <code className="action">Crear una instancia</code>. A continuación, elija un modelo de instancia GPU compatible correspondiente a las instancias de tipo **Cloud GPU**, para beneficiarse de recursos adaptados a las cargas de trabajo gráficas o de cálculo intensivo.

Siga los pasos restantes como se detalla en [esta guía](/guides/public-cloud/compute/getting-started#step-4-create-the-instance). Este proceso puede tardar unos minutos.

<Tabs>
  <Tab label="En Linux">
    Todas las imágenes que ofrecemos pueden utilizarse en una instancia GPU.

    En el paso de selección de imagen, abra la pestaña <code className="action">Distribuciones Unix</code> y, a continuación, elija una imagen UNIX que se adapte a sus necesidades.
  ...
  <Tab label="En Windows">
    ...
    En el paso de selección de imagen, abra la pestaña <code className="action">Distribuciones Windows</code> y, a continuación, seleccione una imagen Windows compatible con el modelo de instancia elegido.
    ...
    **En Windows**
    ...
    Indique la dirección IPv4 de su instancia y el usuario "Administrator" y, a continuación, introduzca su contraseña. Al tratarse de un certificado desconocido, es probable que aparezca un mensaje de aviso pidiéndole que confirme la conexión. Haga clic en <code className="action">Sí</code> para conectarse a la instancia.
