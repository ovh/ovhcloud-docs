---
pageType: overview
title: Block Storage
description: "Zamawiaj, podłączaj, skaluj i twórz snapshoty woluminów block storage w OVHcloud Public Cloud - high-performance, classic i shared block storage."
text: Odkryj przewodniki Block Storage
essentialsTitle: Najważniejsze przewodniki na start
essentials:
  - title: Zarządzanie wolumenem instancji Public Cloud
    link: /guides/public-cloud/compute/create-and-configure-an-additional-disk-on-an-instance
  - title: Tworzenie kopii zapasowej wolumenu
    link: /guides/public-cloud/compute/volume-backup
  - title: Snapshot wolumenu instancji Public Cloud
    link: /guides/public-cloud/compute/storage-create-volume-snapshot
goFurther:
  title: Sprawdź również
  items:
    - title: Odwiedź społeczność OVHcloud
      link: https://community.ovhcloud.com/
    - title: Zobacz roadmapę i changelog
      link: https://www.ovhcloud.com/pl/roadmap-changelog/
    - title: Dołącz do Discord
      link: https://discord.gg/ovhcloud
cta:
  title: "Masz pytania? Skontaktuj się z pomocą OVHcloud"
  description: Znajdź odpowiedzi na swoje pytania w naszym centrum pomocy.
  actions:
    - text: Przejdź do pomocy
      link: https://help.ovhcloud.com/
      theme: brand
---
