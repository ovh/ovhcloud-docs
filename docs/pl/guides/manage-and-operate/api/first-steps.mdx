...
API dostępne na <ApiLink>portalu API OVHcloud</ApiLink> pozwalają na zakup, zarządzanie i konfigurowanie produktów OVHcloud bez konieczności korzystania z interfejsu graficznego, takiego jak Panel klienta.
...
- Bycie na stronie WWW <ApiLink>API OVHcloud</ApiLink>.
...
Na stronie <ApiLink>API OVHcloud</ApiLink> kliknij <code className="action">Explore the OVH API</code>, aby wyświetlić listę API. 
...
W tym celu kliknij link: <CreateToken>strona tworzenia tokenów OVHcloud</CreateToken>.
...
Token **CK** może być wykorzystywany do przekazywania uprawnień. Więcej informacji znajdziesz w przewodniku: [Jak zarządzać kontem klienta OVHcloud za pomocą API](/guides/manage-and-operate/api/api-right-delegation) (przewodnik po języku angielskim).
...
Do wyświetlania i odwoływania kluczy możesz użyć <ApiLink>portalu API</ApiLink> lub <ManagerLink to="/">panelu klienta OVHcloud</ManagerLink>:
...
[Jak zarządzać kontem klienta OVHcloud za pomocą API](/guides/manage-and-operate/api/api-right-delegation) (przewodnik po angielsku)
