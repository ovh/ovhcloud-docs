...
Utwórz klucz RSA przeznaczony do opakowywania. Zawsze ustawiaj zarówno `wrapKey`, jak i `unwrapKey`. Operacje `wrapKey` i `unwrapKey` wzajemnie wykluczają się z `sign` i `verify`.
...
`KEY` to ciąg JWE Compact Serialization. Przekaż go jako `@path/to/wrapped.jwe`, bezpośrednio w poleceniu lub `-`, aby odczytać ze stdin.
...
Wywołaj `GET /v1/servicekey/{keyId}` z poniższymi parametrami zapytania opakowywania. Gdy te parametry są obecne, API zwraca `wrappedKeys` zamiast jawnych `keys`.
