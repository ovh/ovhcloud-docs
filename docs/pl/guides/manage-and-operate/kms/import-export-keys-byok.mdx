...
Po opakowaniu publicznym kluczem miejsca docelowego prześlij uzyskaną wartość `ciphertext` w formacie JWE Compact Serialization w żądaniu importu. Przykład w Go znajdziesz w sekcji [Pełny przykład w Go](#pełny-przykład-w-go).
...
Poniższe przykłady przeprowadzają obie ceremonie z kluczem RSA, używając [SDK Go](https://pkg.go.dev/github.com/ovh/okms-sdk-go) do wywołań OVHcloud KMS oraz [go-jose](https://pkg.go.dev/github.com/go-jose/go-jose/v4) do operacji JWE po drugiej stronie. Oba przykłady kodują materiał klucza w formacie PKCS#8 (DER), co odpowiada `keyFormatType` `PKCS8`.

#### Import klucza wygenerowanego lokalnie

Klucz jest generowany przez standardową bibliotekę Go, a program opakowuje go lokalnie publicznym kluczem transportowym, a następnie importuje. Jego postać jawna nigdy nie trafia do OVHcloud KMS.

```go
// 1. Utwórz klucz transportowy w OVHcloud KMS i pobierz jego część publiczną.
...
// 2. Wygeneruj klucz do zaimportowania i zakoduj go w formacie PKCS#8 (DER).
...
// 3. Opakuj materiał klucza jako JWE: opakowanie klucza RSA-OAEP-256, szyfrowanie treści A256GCM.
...
// 4. Zaimportuj opakowany klucz. KMS rozpakowuje go i wnioskuje typ oraz rozmiar z materiału.
...
Szyfrowanie treści JWE (`enc`) musi mieć wartość `A256GCM`.

#### Eksport klucza i odszyfrowanie JWE

Miejsce docelowe (tutaj lokalny program) generuje parę kluczy transportowych i importuje do OVHcloud KMS tylko jej część publiczną. Następnie miejsce docelowe odszyfrowuje wyeksportowany JWE częścią prywatną.

```go
// 1. Wygeneruj parę kluczy transportowych w miejscu docelowym.
destKey, err := rsa.GenerateKey(rand.Reader, 4096)
if err != nil {
  return err
}

// 2. Zaimportuj do OVHcloud KMS tylko jej część publiczną, z użyciem wrapKey.
...
// 3. Ustaw klucz jako eksportowalny, a następnie wyeksportuj go opakowany publicznym kluczem miejsca docelowego.
extractable := true
...
// 4. Odszyfruj JWE prywatnym kluczem miejsca docelowego i przeanalizuj materiał PKCS#8.
