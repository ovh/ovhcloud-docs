...
Aby komunikować się z domeną OKMS, potrzebujesz certyfikatu dostępu. Jest to jedna z trzech [metod uwierzytelniania OKMS](/guides/manage-and-operate/kms/okms-authentification-method); certyfikaty są **wymagane dla KMIP** i zalecane, gdy potrzebne jest uwierzytelnianie klienta mTLS. W przypadku korzystania z REST API prostsze w konfiguracji jest zazwyczaj użycie Personal Access Token (PAT) lub konta usługi (service account).

Certyfikat dostępu jest ważny tylko dla domeny, dla której został wygenerowany.
