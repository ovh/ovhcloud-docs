...
| Scalability | Cluster mode to distribute keys across multiple database instances (shards) | No special format required, it depends on the customer choice | **Incoming** : import of resharding keys to adapt to service topology via the CLI <br />**Outgoing** CLI : export and re-import of keys into the target infrastructure via the CLI | [Valkey - Capabilities and limits](/guides/public-cloud/databases/valkey-capabilities) |
