...
Besides the API end-point, the required keys are the `application_key`, the `application_secret`, and the `consumer_key`. These keys can be generated via the <CreateToken rights="GET=/*&POST=/*&PUT=/*&DELETE=/*">OVHcloud token generation page</CreateToken>.
