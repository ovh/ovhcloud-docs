...
<Tabs>
  <Tab label="W systemie Windows">
    ...
    Wyświetla to wolną przestrzeń i całkowity rozmiar każdego dysku.
    ...
    Wyświetla to wykorzystaną i dostępną przestrzeń dyskową.
  ...
  <Tab label="W systemie Linux">
    ...
    Wyświetla to wykorzystanie dysku w formacie czytelnym dla człowieka.
...
<Tabs>
  <Tab label="Dla partycji do 2 TB">
    Sprawdź bieżący układ partycji:
    ...
    :::warning
    Zwróć uwagę na wartość sektora `Start` (w tym przykładzie `2048`). Musisz ponownie wprowadzić tę dokładną wartość podczas odtwarzania partycji poniżej. Użycie innego sektora początkowego spowoduje niewłaściwe wyrównanie systemu plików i może doprowadzić do uszkodzenia lub utraty danych.
    :::
  ...
  <Tab label="Dla partycji powyżej 2 TB">
    Uruchom następujące polecenie, aby wyświetlić informacje o partycjach:
    ...
    :::info
    Sprawdź pole `Partition Table` w powyższym wyniku: jeśli widnieje tam `gpt`, kontynuuj zgodnie z opisem. Jeśli zamiast tego widnieje `msdos`, najpierw przekonwertuj tablicę, uruchamiając następujące polecenie:

    ```bash
    admin@server:~$ sudo parted /dev/vdb mklabel gpt
    ```

    Ta operacja nadpisuje tablicę partycji, więc przed kontynuowaniem utwórz migawkę swojego woluminu.

    Następnie ponownie uruchom polecenie `print`, aby to potwierdzić.
    :::
    ...
    Podobnie jak w poprzedniej metodzie, usunięcie i ponowne utworzenie partycji nadpisuje jedynie tablicę partycji — Twoje dane zostają zachowane, pod warunkiem że przy tworzeniu nowej partycji podasz tę samą wartość `Start`, którą zanotowałeś wcześniej.
    ...
    Ponowne utworzenie partycji nadaje jej nowy GUID partycji. Jeśli `/etc/fstab` (lub inna konfiguracja montowania) odwołuje się do tej partycji za pomocą `PARTUUID=`, zaktualizuj tę wartość.

    Najpierw uruchom następujące polecenie, aby znaleźć nową wartość:
    ...
    Następnie otwórz `/etc/fstab`, aby go zaktualizować:

    ```bash
    admin@server:~$ sudo nano /etc/fstab
    ```

    Punkty montowania odwołujące się do partycji za pomocą UUID systemu plików (wartość domyślna w większości obrazów chmurowych) lub nazwy urządzenia nie są tym dotknięte.
  </Tab>
</Tabs>
