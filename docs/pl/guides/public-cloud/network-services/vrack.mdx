...
**Poniższe sekcje opisują działania dostępne dla każdego interfejsu/metody.** <a id="horizon"></a>

<details>

<summary>Panel klienta OVHcloud</summary>
...
Każde działanie dostępne w Panelu klienta OVHcloud można również wykonać za pomocą <ApiLink>APIv6 OVHcloud</ApiLink>.
...
Możesz uzyskać do niego dostęp z <ApiLink>naszej strony internetowej</ApiLink>, ale także używać go do tworzenia swoich skryptów PHP lub Python.
...
[CLI OVHcloud](https://github.com/ovh/ovhcloud-cli) umożliwia zarządzanie zasobami Public Cloud z poziomu terminala, w tym prywatnymi sieciami i interfejsami instancji.

Przed rozpoczęciem [zainstaluj i skonfiguruj CLI OVHcloud](https://github.com/ovh/ovhcloud-cli#installation). Ustaw swój projekt cloud za pomocą `--cloud-project <project_id>` lub skonfiguruj go w swoim profilu.
...
### Krok 1: Aktywowanie i zarządzanie vRackiem <a id="activation"></a>
...
Zostaniesz przekierowany na inną stronę, aby potwierdzić zamówienie. Ustawienie vRacka w Twoim koncie zajmie kilka minut.
    ...
    Aby kontynuować konfigurowanie vRacka z Panelu klienta OVHcloud, przejdź do [Kroku 2: Utwórz prywatną sieć w vRacku](#create-pn-in-vrack), w zakładce **Panel klienta OVHcloud**.
  </Tab>
  <Tab label="APIv6 OVHcloud">
    **Krok 1: Aktywowanie i zarządzanie vRackiem**
    
    Otwórz interfejs APIv6 OVHcloud zgodnie z przewodnikiem ([Pierwsze kroki z API OVHcloud](/guides/manage-and-operate/api/first-steps)) i wykonaj następujące kroki:
    ...
    **Krok 2: Dodawanie projektu Public Cloud do vRacka**
    ...
    Otwórz interfejs APIv6 OVHcloud zgodnie z przewodnikiem: [Pierwsze kroki z API OVHcloud](/guides/manage-and-operate/api/first-steps).
...
### Krok 2: Tworzenie prywatnej sieci w vRacku <a id="create-pn-in-vrack"></a>
...
W polu **Nazwa prywatnej sieci** ustaw nazwę swojej prywatnej sieci.
  ...
  <Tab label="APIv6 OVHcloud">
    W <ApiLink>interfejsu APIv6 OVHcloud</ApiLink> wykonaj poniższe kroki:
...
### Krok 3: Integracja instancji z vRackiem <a id="instance-integration"></a>
...
**Dodawanie prywatnego interfejsu** <a id="adding-a-private-interface"></a>
...
Wybierz swój interfejs i potwierdź.
  ...
  <Tab label="CLI OVHcloud">
    Użyj [CLI OVHcloud](https://github.com/ovh/ovhcloud-cli), aby zarządzać interfejsami sieciowymi istniejącej instancji. Ustaw swój projekt cloud za pomocą `--cloud-project <project_id>` lub skonfiguruj go w swoim profilu.
...
### Kolejność interfejsów po podłączeniu lub odłączeniu portów na gorąco <a id="interface-order-after-reboot"></a>

:::warning
Podłączanie lub odłączanie interfejsów sieciowych (portów) w trakcie działania instancji (na gorąco) może zmienić kolejność, w jakiej system operacyjny gościa wykrywa je przy następnym ponownym uruchomieniu. W rezultacie interfejs rozpoznawany przez system gościa jako `eth0` (lub inny oczekiwany interfejs, w tym publiczny) może się zmienić. Sprawdź nową konfigurację, aby uniknąć nieoczekiwanych problemów z siecią.

Jeśli musisz zreorganizować kilka interfejsów w działającej instancji:

1. Odłącz wszystkie interfejsy, które chcesz zmienić. Odłączenie interfejsu "Ext-Net" trwale zwalnia jego publiczny adres IP, zgodnie z opisem w sekcji [Usuwanie interfejsu sieciowego](#removing-a-private-interface).
2. Podłącz je ponownie jeden po drugim, w kolejności, w jakiej mają się pojawić w systemie gościa.
3. Wykonaj *hard reboot* instancji, aby potwierdzić, że kolejność interfejsów jest zgodna z oczekiwaniami, zanim zaczniesz na niej polegać w środowisku produkcyjnym.

:::

### Usuwanie interfejsu sieciowego <a id="removing-a-private-interface"></a>
