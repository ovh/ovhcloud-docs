...
<Tabs>
  <Tab label="Panel klienta OVHcloud">
    ...
    3. **Nazwij i przypisz sieć.** Wpisz nazwę Gateway i wybierz istniejącą sieć prywatną z listy rozwijanej lub kliknij <code className="action">Dodaj prywatną sieć</code>, aby utworzyć ją bezpośrednio.
  ...
  <Tab label="API OVHcloud">
    Połącz się z <ApiLink>API OVHcloud</ApiLink> i użyj poniższego wywołania, aby utworzyć Gateway wraz z nową siecią prywatną i podsiecią w jednej operacji:
...
<Tabs>
  <Tab label="Panel klienta OVHcloud">
    Na liście <code className="action">Gateway</code> kliknij menu <code className="action">...</code> obok Twojej Gateway, a następnie wybierz <code className="action">Zmień Gateway</code>.
