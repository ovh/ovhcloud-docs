---
pageType: overview
title: MX Plan
description: "Rozpocznij pracę z rozwiązaniem pocztowym OVHcloud MX Plan - tworzenie skrzynek, konfiguracja klienta poczty, przekierowania, listy mailingowe i filtr antyspamowy."
text: "Dowiedz się, jak rozpocząć korzystanie z usługi MX Plan"
essentialsTitle: "Najważniejsze przewodniki na start"
essentials:
  - title: "Pierwsze kroki z usługą MX Plan"
    link: /guides/web-cloud/email-and-collaborative-solutions/mx-plan/email-generalities
  - title: "Tworzenie konta e-mail w ramach usługi MX Plan"
    link: /guides/web-cloud/email-and-collaborative-solutions/mx-plan/email-creation
  - title: "Zmiana hasła do konta e-mail"
    link: /guides/web-cloud/email-and-collaborative-solutions/mx-plan/email-change-password
  - title: "Korzystanie z konta e-mail w interfejsie Webmail Roundcube"
    link: /guides/web-cloud/email-and-collaborative-solutions/mx-plan/email-roundcube
  - title: FAQ e-mail OVHcloud
    link: /guides/web-cloud/email-and-collaborative-solutions/mx-plan/faq-emails
  - title: "Korzystaj z interfejsu Webmail Zimbra"
    link: /guides/web-cloud/email-and-collaborative-solutions/mx-plan/email-zimbra
goFurther:
  title: Sprawdź również
  items:
    - title: Odwiedź społeczność OVHcloud
      link: https://community.ovh.com/
    - title: Zobacz roadmapę i changelog
      link: https://www.ovhcloud.com/pl/roadmap-changelog/
    - title: Dołącz do Discord
      link: https://discord.gg/ovhcloud
cta:
  title: Masz pytania? Skontaktuj się z pomocą OVHcloud
  description: "Znajdź odpowiedzi na swoje pytania w naszym centrum pomocy."
  actions:
    - text: Przejdź do pomocy
      link: https://help.ovhcloud.com/
      theme: brand
availableIn: [eu, ca, apac]
---
