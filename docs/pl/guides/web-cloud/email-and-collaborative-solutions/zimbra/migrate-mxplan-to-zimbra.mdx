...
<Tabs>
  ...
  <Tab label="Krok 2">
    **Logowanie do projektu i tworzenie migracji**
    ...
    - **Konto źródłowe**:
        - **Typ konta**: Wybierz `OVHcloud`, a następnie `MX Plan` lub `Autodetekcja`. Kliknij <code className="action">Zaloguj się</code>, aby zalogować się na swoje konto OVHcloud i automatycznie wybrać usługę oraz adres do migracji (np.: `contact@mydomain.ovh`). Następnie wpisz hasło do tego konta e-mail.
    - **Konto docelowe**:
        - **Typ konta**: Wybierz `OVHcloud`, a następnie `Zimbra`. Kliknij <code className="action">Zaloguj się</code>, aby zalogować się na swoje konto OVHcloud i wybrać usługę Zimbra oraz adres docelowy (np.: `contact2@mydomain.ovh`). Następnie wpisz hasło do tego konta e-mail.
