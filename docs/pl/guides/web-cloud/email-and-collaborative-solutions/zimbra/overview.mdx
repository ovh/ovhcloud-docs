---
pageType: overview
title: Zimbra
description: "Konfiguruj i obsługuj rozwiązanie do pracy zespołowej OVHcloud Zimbra - konfiguracja skrzynki, synchronizacja mobilna, foldery współdzielone i administracja."
text: "Dowiedz się, jak zarządzać rozwiązaniem e-mail Zimbra"
essentialsTitle: "Najważniejsze przewodniki na start"
essentials:
  - title: "Pierwsze kroki z ofertą Zimbra"
    link: /guides/web-cloud/email-and-collaborative-solutions/zimbra/getting-started-zimbra
  - title: "Korzystaj z interfejsu Webmail Zimbra"
    link: /guides/web-cloud/email-and-collaborative-solutions/mx-plan/email-zimbra
  - title: "FAQ dotyczący rozwiązania Zimbra OVHcloud"
    link: /guides/web-cloud/email-and-collaborative-solutions/mx-plan/faq-zimbra
  - title: "Konfiguracja konta e-mail Zimbra w programie pocztowym"
    link: /guides/web-cloud/email-and-collaborative-solutions/zimbra/mail-apps
  - title: "Migracja adresu e-mail MX Plan na konto Zimbra OVHcloud"
    link: /guides/web-cloud/email-and-collaborative-solutions/zimbra/migrate-mxplan-to-zimbra
  - title: "Zimbra - Konfiguracja konta e-mail w aplikacji mobilnej Zimbra"
    link: /guides/web-cloud/email-and-collaborative-solutions/zimbra/mail-app-zimbra-for-android-ios
goFurther:
  title: Sprawdź również
  items:
    - title: Odwiedź społeczność OVHcloud
      link: https://community.ovhcloud.com/
    - title: "Zobacz roadmapę i changelog"
      link: https://www.ovhcloud.com/pl/roadmap-changelog/
    - title: "Dołącz do Discord"
      link: https://discord.gg/ovhcloud
cta:
  title: "Masz pytania? Skontaktuj się z pomocą OVHcloud"
  description: "Znajdź odpowiedzi na swoje pytania w naszym centrum pomocy."
  actions:
    - text: "Przejdź do pomocy"
      link: https://help.ovhcloud.com/
      theme: brand
availableIn: [eu]
---
