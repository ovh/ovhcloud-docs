...
Wysyłasz e-mail na adres `john.smith@mydomain.ovh`.
...
Pojawi się tabela. Strona pokaże <Tooltip term="dnsZone">strefę DNS</Tooltip> Twojej nazwy domeny w OVHcloud. Składają się one z kilku rekordów, które są symbolizowane przez wiersz w tabeli.
...
W przypadku rozwiązania e-mail OVHcloud minimalny rekord SPF to po prostu zaznaczone pole **Uwzględnij serwery e-mail OVH** — daje to `v=spf1 include:mx.ovh.ca ~all` (patrz [Konfiguracja SPF OVHcloud w przypadku ofert e-mail na hostingu](#ovhcloudspfvalue)).
...
- Usługa <Tooltip term="mxPlan">MX Plan</Tooltip> w monoterapii lub w [cenie hostingu](/links/web/hosting).
<Region zones={['eu']}>
