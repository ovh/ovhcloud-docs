...
Jeśli Twoja domena jest zarejestrowana w OVHcloud, możesz sprawdzić, czy używa ona konfiguracji OVHcloud w <ManagerLink to="/">Panelu klienta</ManagerLink>. W tym celu przejdź do sekcji <code className="action">Domeny</code>, wybierz odpowiednią domenę i sprawdź jej zakładkę <code className="action">Serwery DNS</code>.
:::
...
<ZoneTabs>
  ...
  <Tab label="Zimbra" availableIn={['eu']}>
    ...
    **Szybki sposób na utworzenie wpisu CNAME**
    ...
    Otwórz zakładkę <code className="action">Strefa DNS</code>, a następnie kliknij <code className="action">Dodaj wpis</code> nad tabelą. W otwartym formularzu wybierz typ `CNAME` i uzupełnij pola zgodnie z zanotowanymi wartościami.
    :::
  </Tab>
</ZoneTabs>
...
<Region zones={['eu']}>
W przypadku platformy Exchange lub Email Pro należy najpierw pobrać numer referencyjny platformy, aby skonfigurować DKIM.
</Region>
...
Aby skonfigurować DKIM, przejdź na <ApiLink>stronę API OVHcloud</ApiLink> i zaloguj się:
...
<Region zones={['eu']}>
Przejdź do sekcji `/email/domain/` (oferty MX Plan i Zimbra), `/email/exchange` (oferta Exchange) lub `/email/pro` (oferta Email Pro) API i wpisz "dkim" w polu `Filter`, aby wyświetlić tylko funkcje API związane z DKIM.
</Region>
...
<Tabs>
  ...
  <Tab label="4. Skonfiguruj rekord DNS">
    ...
    Przejdź do zakładki <code className="action">Strefa DNS</code>, a następnie kliknij <code className="action">Dodaj wpis</code> nad tabelą. W otwartym formularzu wybierz typ `CNAME` i uzupełnij pola zgodnie z zanotowanymi wartościami.
    ...
    <img className="thumbnail" alt="Dodawanie rekordu CNAME w strefie DNS" src="/images/assets/screens/control-panel/product-selection/web-cloud/domain-dns/dns-zone/nx-cname-form.png" loading="lazy" />

    Po wpisaniu wartości kliknij <code className="action">Dodaj</code>.
...
<Tabs>
  ...
  <Tab label="4. Konfiguracja rekordu DNS">
    Z <ManagerLink to="/">strefy klienta OVHcloud</ManagerLink>, gdzie nazwa Twojej domeny Exchange, w zakładce <code className="action">Web Cloud</code> kliknij <code className="action">Domeny</code> w lewej kolumnie i wybierz odpowiednią nazwę domeny.<br/>
    Przejdź do zakładki <code className="action">Strefa DNS</code>, a następnie kliknij <code className="action">Dodaj wpis</code> nad tabelą. W otwartym formularzu wybierz typ `CNAME` i uzupełnij pola zgodnie z zanotowanymi wartościami.

    Jeśli weźmiemy wartości z przykładu na etapie "**3. Pobierz rekord DNS**":

    - `customerRecord: "ovhex123456-selector1._domainkey.mydomain.ovh"` odpowiada subdomenie rekordu CNAME. Otrzymujemy tylko `ovhex123456-selector1._domainkey`, ponieważ `.mydomain.ovh` jest już wstępnie wypełniony. <br/>
...
##### **Dla Email Pro** <a name="confemp"></a>

Postępuj zgodnie z **5 krokami** poniżej, klikając na każdą z zakładek.

<Tabs>
  <Tab label="1. Lista sektorów">
    Przed utworzeniem jednego z wybranych parametrów Twojej domeny, pobierz nazwę nadaną im automatycznie przez platformę Email Pro.<br/>
    <br/>
    W tym celu użyj następującego wywołania API:<br/>
    ...
    - `service`: wpisz nazwę Twojej platformy Email Pro w postaci "emailpro-zz111111-1". <br/>
    - `domainName`: wprowadź nazwę domeny przypisanej do Twojej platformy Email Pro, którą chcesz aktywować DKIM. <br/>
  ...
  <Tab label="2. Utwórz wyłącznik">
    ...
    - `domainName`: wprowadź nazwę domeny przypisanej do Twojej platformy Email Pro, dla której chcesz włączyć DKIM.
    - `service`: wpisz nazwę Twojej platformy Email Pro w postaci "emailpro-zz111111-1". <br/>
  ...
  <Tab label="3. Pobierz rekord DNS">
    ...
    - Twoja platforma Email Pro jest powiązana z domeną zarządzaną na innym koncie klienta OVHcloud;<br/>
    - Twoja platforma Email Pro jest powiązana z domeną zarządzaną w innym urzędzie rejestracji;<br/>
    ...
    - `service`: wpisz nazwę Twojej platformy Email Pro w postaci "emailpro-zz111111-1".
    - `domainName`: wprowadź nazwę domeny przypisanej do Twojej platformy Email Pro, dla której chcesz skonfigurować DKIM.
    - `selectorName`: wprowadź nazwę wybranego przełącznika, który utworzyłeś na poprzednim etapie.
  ...
  <Tab label="4. Konfiguracja rekordu DNS">
    Z <ManagerLink to="/">strefy klienta OVHcloud</ManagerLink>, gdzie nazwa Twojej domeny Platforma Email Pro, w zakładce <code className="action">Web Cloud</code> kliknij <code className="action">Domeny</code> w lewej kolumnie i wybierz odpowiednią nazwę domeny.<br/>
    Przejdź do zakładki <code className="action">Strefa DNS</code>, a następnie kliknij <code className="action">Dodaj wpis</code> nad tabelą. W otwartym formularzu wybierz typ `CNAME` i uzupełnij pola zgodnie z zanotowanymi wartościami.
...
<ZoneTabs>
  <Tab label="MX Plan i Zimbra" availableIn={['eu']}>
    ...
    - `error`: Podczas procesu instalacji wystąpił błąd. Otwórz [zgłoszenie w dziale obsługi klienta](/links/support-contact) i podaj nazwę wybranej domeny.<br/>
  ...
  <Tab label="Email Pro" availableIn={['eu']}>
    Podczas wykonywania operacji na DKIM na Twojej platformie Email Pro użyj poniższego wywołania API, aby sprawdzić aktualny status DKIM.

    <Api version="v1" section="/email/pro" method="GET" route={"/email/pro/\\{service\\}/domain/\\{domainName\\}/dkim/\\{selectorName\\}"} />

    - `service`: wpisz nazwę Twojej platformy Email Pro w postaci "emailpro-zz111111-1". <br/>
    - `domainName`: wprowadź nazwę domeny przypisanej do Twojej platformy Email Pro, na której ma być zainstalowany DKIM. <br/>
...
:::warning
**W przypadku oferty Exchange i Email Pro** <br/>
...
<ZoneTabs>
  ...
  <Tab label="Email Pro" availableIn={['eu']}>
    ...
    - `domainName`: wprowadź nazwę domeny przypisanej do Twojej platformy Email Pro. <br/>
    - `selectorName`: wprowadź nazwę wybranego przełącznika, który chcesz wyłączyć. <br/>
    - `service`: wpisz nazwę Twojej platformy Email Pro w postaci "emailpro-zz111111-1". <br/>
...
W <ManagerLink to="/">Panelu klienta OVHcloud</ManagerLink> przejdź do sekcji <code className="action">Web Cloud</code>, kliknij <code className="action">Domeny</code>, a następnie wybierz odpowiednią nazwę domeny.

Otwórz zakładkę <code className="action">Strefa DNS</code>, a następnie kliknij <code className="action">Dodaj wpis</code> nad tabelą. Istnieją 3 sposoby dodania rekordu do konfiguracji DKIM w strefie DNS:
...
<img className="thumbnail" alt="Dodawanie rekordu DKIM ze strefy DNS" src="/images/assets/screens/control-panel/product-selection/web-cloud/domain-dns/dns-zone/nx-dkim-form.png" loading="lazy" />

- **Selektor** (Subdomena): wprowadź nazwę selektora DKIM dostarczoną przez Twoją usługę e-mail. Sufiks `._domainkey` jest dodawany automatycznie, a Twoja nazwa domeny jest dołączana na końcu.
...
- **Klucz publiczny (base64)**: wklej klucz publiczny dostarczony przez Twoją usługę e-mail. Jest to klucz, którego odbiorcy używają do weryfikacji Twojego podpisu DKIM.
- **Typ klucza**: algorytm podpisu klucza — `RSA` (domyślnie) lub `ed25519`.
- **Tryb testowy**: w środowisku produkcyjnym pozostaw `Wyłączona`. Po włączeniu odbiorcy traktują testowe wiadomości podpisane przez DKIM jak wiadomości niepodpisane, więc nieudana weryfikacja nie wpływa na dostarczanie, gdy sprawdzasz swoją konfigurację.
...
:::warning
To pytanie dotyczy tylko ofert Exchange i Email Pro.
:::
...
<ZoneTabs>
  ...
  <Tab label="Email Pro" availableIn={['eu']}>
    ...
    - `domainName`: wprowadź nazwę domeny przypisanej do Twojej platformy Email Pro, na której DKIM ma być obecny.<br/>
    - `selectorName`: wprowadź nazwę przełącznika wyboru. <br/>
    - `service`: wprowadź nazwę Twojej platformy Email Pro w formie "emailpro-zz111111-1". <br/>
  </Tab>
</ZoneTabs>
...
<Tabs>
  <Tab label="501">
    ...
    - Po wybraniu selektora do konfiguracji postępuj zgodnie z instrukcjami zawartymi w tej sekcji "[API - Kompletna konfiguracja DKIM](#firststep)" w zależności od wybranej oferty (Exchange lub Email Pro) i zastosuj ją tylko do wybranego selektora.
    Odczekaj maksymalnie 24 godziny od skonfigurowania selektora.
  </Tab>
  <Tab label="502">
    "**One DKIM configuration task is in error**"<br/><br/>
    Wystąpił błąd podczas konfigurowania DKIM. Po upływie 24 godzin, jeśli Twoja konfiguracja nadal jest w tym stanie, prosimy o otwarcie [zgłoszenia w dziale obsługi klienta](/links/support-contact).
  </Tab>
  <Tab label="503">
...
<ZoneTabs>
  <Tab label="Exchange i Email Pro" availableIn={['eu']}>
  ...
  <Tab label="E-maile (MX Plan) i Zimbra" availableIn={['eu']}>
    ...
    - `error`: Podczas procesu instalacji wystąpił błąd. Otwórz [zgłoszenie w dziale obsługi klienta](/links/support-contact) i podaj nazwę wybranej domeny.
