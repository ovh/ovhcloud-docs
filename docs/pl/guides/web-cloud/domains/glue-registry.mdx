...
<Tabs>
  <Tab label="Krok 1">
    W sekcji `Web Cloud` kliknij <ManagerLink to="/#/web-domains/domain">Domeny</ManagerLink>, wybierz nazwę domeny, której użyłeś do utworzenia hostów w [kroku 3](#step3), następnie otwórz zakładkę **Strefa DNS**.
  ...
  <Tab label="Krok 3">
    W formularzu, który się otworzy, wybierz typ rekordu DNS *A* lub *AAAA* w zależności od typu powiązanego adresu IP, który chcesz dodać.
  </Tab>
  <Tab label="Krok 4">
    Wprowadź *subdomenę* oraz adres *IPv4* (A) lub *IPv6* (AAAA), a następnie kliknij <code className="action">Dodaj</code>. W razie potrzeby skorzystaj z instrukcji zawartych w przewodniku "[Jak edytować strefę DNS OVHcloud](/guides/web-cloud/domains/dns-zone-edit)".
  </Tab>
</Tabs>
...
<Tabs>
  <Tab label="Krok 1">
    W sekcji `Web Cloud` kliknij <ManagerLink to="/#/web-domains/domain">Domeny</ManagerLink>, wybierz nazwę domeny, dla której spersonalizowałeś serwery DNS, następnie otwórz zakładkę **Strefa DNS**.
  </Tab>
  <Tab label="Krok 2">
    ...
    Strefa DNS pojawia się w trybie *tekstowym*:
  ...
  <Tab label="Krok 3">
    W tym oknie zamień **tylko w rekordach typu *NS*** nazwy serwerów DNS na Twoje własne spersonalizowane nazwy serwerów DNS, **pamiętając o** zwiększeniu o "1" pierwszej wartości liczbowej wiersza *SOA*. Po dokonaniu zmian postępuj zgodnie z instrukcjami wyświetlanymi na ekranie, aby potwierdzić.
