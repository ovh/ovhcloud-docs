...
Rekord DMARC możesz dodać do strefy DNS w Panelu klienta OVHcloud. W tym celu zaloguj się do <ManagerLink to="/">Panelu klienta OVHcloud</ManagerLink> i przejdź do sekcji <code className="action">Web Cloud</code>. W lewej kolumnie wybierz w sekcji <code className="action">Domeny</code> odpowiednią nazwę domeny, następnie kliknij na zakładkę <code className="action">Strefa DNS</code>, aby przejść do strefy DNS.

Kliknij <code className="action">Dodaj wpis</code> nad tabelą, a następnie wybierz typ **DMARC**. Formularz pozwala ustawić każdą część rekordu:

- **Subdomena**: ten wpis **musi zaczynać się od** `_dmarc`. Jeśli stosujesz DMARC do całej nazwy domeny, nie wpisuj w tym polu niczego innego niż `_dmarc`. Jeśli ustawisz DMARC na subdomenę swojej nazwy domeny głównej, dodaj subdomenę po `_dmarc`. Na przykład, jeśli chcesz zastosować DMARC do subdomeny *subdomain.mydomain.ovh*, musisz wpisać `_dmarc.subdomain` w polu "subdomena" dla nazwy domeny *mydomain.ovh*.

Poniżej znajduje się wyczerpujący opis tagów używanych do **rejestracji DMARC** OVHcloud:

- **Wersja**: obowiązkowe pole określające wersję protokołu DMARC (`DMARC1`).

- **Reguła dla domeny (p=)**: zasady do przyjęcia przez adresata na wniosek abonenta nazwy domeny nadawcy. Polityka dotyczy badanej nazwy domeny i subdomen, chyba że znacznik subdomeny **sp=** wskazuje na inne instrukcje. Możliwe wartości to:
...
- **URI do globalnych raportów (rua=)**: adresy, do których mają być wysyłane raporty (zwykły tekst rozdzielany przecinkami). Można określić dowolny prawidłowy identyfikator URI. Wpis "mailto:" musi poprzedzać adres e-mail odbiorcy (na przykład: `mailto:address@example.com`).
...
- **Tryb dopasowania dla SPF (aspf=)** (domyślnie `r`): wskazuje tryb wyrównania SPF. Wartości są następujące:
...
<img className="thumbnail" alt="Dodawanie rekordu DMARC ze strefy DNS" src="/images/assets/screens/control-panel/product-selection/web-cloud/domain-dns/dns-zone/nx-dmarc-form.png" loading="lazy" />
...
Kliknij <code className="action">Dodaj wpis</code> nad tabelą, a następnie wybierz typ **TXT**.
...
<img className="thumbnail" alt="Dodawanie rekordu TXT ze strefy DNS" src="/images/assets/screens/control-panel/product-selection/web-cloud/domain-dns/dns-zone/nx-txt-form.png" loading="lazy" />
