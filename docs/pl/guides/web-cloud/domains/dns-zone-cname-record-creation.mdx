...
Kliknij <code className="action">Dodaj wpis</code> nad tabelą i wybierz typ **CNAME**. W polu **Subdomena** wpisz subdomenę (na przykład: `www` dla `www.domain.tld`); w polu **Cel** wpisz nazwę domeny lub subdomenę, na którą chcesz wskazywać (na przykład: `domain.tld`). Następnie kliknij <code className="action">Dodaj</code>.
