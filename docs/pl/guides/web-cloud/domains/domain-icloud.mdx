...
W tym przewodniku dowiesz się, jak używać nazwy domeny zarejestrowanej w OVHcloud z usługą iCloud Mail firmy Apple. Dzięki temu możesz tworzyć spersonalizowane adresy e-mail (`john.smith@mydomain.ovh`), w pełni zarządzane i hostowane przez Apple.
...
Aby aktywować nazwę domeny w usłudze iCloud, postępuj zgodnie z instrukcjami na stronie "Add a domain you own to iCloud Mail on iCloud.com" w [oficjalnej dokumentacji Apple](https://support.apple.com/pl-pl/guide/icloud/mma473945269/icloud). Zwróć szczególną uwagę na sekcję "Step 3: Update the records with your domain registrar", aby poznać rekordy DNS, które należy dodać do Twojej <Tooltip term="dnsZone">strefy DNS</Tooltip> OVHcloud.
...
<Tabs>
  <Tab label="Krok 1">
    Na liście <code className="action">Strefy DNS</code> wybierz odpowiednią nazwę domeny.
  ...
  <Tab label="Krok 2">
    ...
    - **CNAME**: dla kluczy <Tooltip term="dkim">DKIM</Tooltip> (`sig1._domainkey...`, `sig2._domainkey...`).
    - <Tooltip term="txtRecord">**TXT**</Tooltip>: dla <Tooltip term="spf">SPF</Tooltip> (scal rekordy, jeśli rekord SPF już istnieje).
...
Aby poprawić dostarczalność wiadomości e-mail i zapobiec ich trafianiu do spamu, dodaj rekord <Tooltip term="dmarc">**DMARC**</Tooltip>.
...
Postępuj zgodnie z instrukcjami na stronie "Add a domain you own to iCloud Mail on iCloud.com" w [oficjalnej dokumentacji Apple](https://support.apple.com/pl-pl/guide/icloud/mma473945269/icloud). Zwróć szczególną uwagę na sekcję "Step 4: Finish setting up the domain".
...
:::warning
Weryfikacja może się nie powieść, jeśli rekordy DNS nie zostały jeszcze rozpropagowane. <Tooltip term="dnsPropagation">Propagacja</Tooltip> może potrwać kilka godzin.
...
- Dla jednej nazwy domeny może istnieć tylko **jeden rekord SPF**. Jeśli rekord SPF OVHcloud już istnieje (np.: `v=spf1 include:mx.ovh.ca ~all`), scal go z rekordem dostarczonym przez Apple (np.: `v=spf1 include:mx.ovh.ca include:icloud.com ~all`).

</Region>

#### Niekompletny DKIM

- Apple prosi o dodanie do Twojej strefy DNS kilku kluczy DKIM (`sig1`, `sig2` itp.): dodaj je wszystkie.
