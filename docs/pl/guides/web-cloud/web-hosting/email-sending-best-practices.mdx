...
| **Tożsamość nadawcy** | Adres `From:` jest deklarowany przez skrypt. Jego domena musi być przypisana do Twojego hostingu (multisite), w przeciwnym razie wiadomość e-mail nie może zostać uwierzytelniona. | Adres `From:` musi być zgodny z uwierzytelnionym kontem. Wiadomość e-mail jest podpisywana <Tooltip term="dkim">DKIM</Tooltip>, jeżeli DKIM jest włączony dla Twojej domeny. |
...
Domena ta nie musi być zarządzana w Twoim koncie OVHcloud: możesz użyć domeny zarejestrowanej u innego rejestratora, o ile jest przypisana do Twojego hostingu WWW. To właśnie to przypisanie potwierdza, że masz prawo wysyłać wiadomości e-mail w imieniu tej domeny.

:::tip
Jeśli Twoja domena nie jest jeszcze przypisana do hostingu, dodaj ją jako multisite w Panelu klienta OVHcloud, a następnie sprawdź powiązaną konfigurację DNS. Zapoznaj się z przewodnikiem [Instalacja kilku stron WWW na jednym hostingu](/guides/web-cloud/web-hosting/multisites-configure-multisite).
:::

:::warning
Wiadomość wysłana dla domeny, która nie jest przypisana do Twojego hostingu, nie może zostać uwierzytelniona: ma cechy wysyłki nieuwierzytelnionej, dlatego zostanie odrzucona i zarejestrowana jako błąd.
:::
