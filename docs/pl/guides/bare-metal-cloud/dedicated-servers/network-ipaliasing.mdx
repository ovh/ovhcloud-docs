...
<Tabs>
  ...
  <Tab label="AlmaLinux / Rocky Linux">
    ...
    Dla każdego dodatkowego adresu IP, który ma zostać skonfigurowany, utwórz oddzielny plik konfiguracyjny z następującymi parametrami: `ifcfg-NETWORK_INTERFACE:ID`. Gdzie `NETWORK_INTERFACE` reprezentuje interfejs fizyczny, a `ID` jest wirtualnym interfejsem sieciowym lub aliasem ethernetowym rozpoczynającym się od wartości 0. Na przykład w przypadku interfejsu o nazwie `eth0` pierwszy alias to `eth0:0`, drugi alias to `eth0:1`, etc.
...
Następnie wystarczy skierować ping z Additional IP na zewnątrz. Jeśli to działa, prawdopodobnie oznacza to, że błąd konfiguracji wymaga naprawy. Jeśli adres IP nadal nie działa, otwórz zgłoszenie dla zespołu pomocy technicznej w [Centrum pomocy OVHcloud](/links/support-contact), podając następujące informacje:
