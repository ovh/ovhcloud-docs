...
Otwórz <ApiLink>konsoli API OVHcloud</ApiLink> i wykonaj następujące wywołanie: 
...
Backup storage jest kompatybilny wyłącznie z NFSv3. Upewnij się, że zezwalasz blokom IP na dostęp do przestrzeni dyskowej oraz używania protokołu NFS. W zależności od dystrybucji Linux, jaką posiadasz, być może powinieneś zainstalować klienta NFS i uruchomić usługi NFS i `portmap`.
...
W przypadku problemów z połączeniem upewnij się również, że porty UDP i TCP wymagane przez NFSv3 są otwarte w zaporze sieciowej zarówno dla ruchu przychodzącego, jak i wychodzącego.
Otwórz port 111 (UDP/TCP), port 2049 (TCP, NFS) oraz zakres portów dynamicznych 32768–60999 (UDP/TCP) w obu kierunkach. Porty te są używane przez RPC dla programów `mountd`, `lockd` i `statd`.

Na koniec zezwól na wszystkie połączenia wychodzące do adresu IP Twojej usługi Backup Storage.
