...
To żądanie API jest używany do pobierania listy usług vRack. Jeśli nie możesz odnaleźć odpowiedniej usługi za pomocą wyświetlonych nazw, skorzystaj z <ManagerLink to="/">Panelu klienta</ManagerLink>, aby ją odszukać. W tym celu przejdź do sekcji <code className="action">Bare Metal Cloud</code>, następnie <code className="action">Sieć</code> i następnie "vRack". 
