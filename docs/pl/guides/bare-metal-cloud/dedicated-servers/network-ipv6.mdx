...
Ponieważ ifcfg jest obecnie przestarzały, NetworkManager nie tworzy już domyślnie nowych profili w tym formacie. Plik konfiguracyjny można teraz znaleźć w `/etc/NetworkManager/system-connections/`.
  ...
  <Tab label="Windows Server 2019 i nowsze wersje">
    ...
    **Krok 2: otwarcie konfiguracji sieci dla serwera**
    ...
    Otwórz menu Start systemu Windows, wpisz `ncpa.cpl`, a następnie kliknij sugerowany wynik wyszukiwania.

    <img className="thumbnail" alt="Okno Połączenia sieciowe w panelu sterowania systemu Windows" src="/images/bare-metal-cloud/dedicated-servers/network-ipv6/ipv6_network_connections.png" loading="lazy" />
...
W każdym przypadku nie wahaj się [skontaktować się z naszym zespołem pomocy technicznej](/links/support-contact), aby poprosić o sprawdzenie konfiguracji. Będziesz musiał podać:
