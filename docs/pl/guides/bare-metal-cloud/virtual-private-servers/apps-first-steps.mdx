...
:::info
Jeśli link już wygasł, połącz się z VPS przez SSH za pomocą nazwy użytkownika i wykonaj polecenie `sudo whmlogin`, aby wygenerować nowy.
:::
...
WHM używa domyślnie użytkownika root do uwierzytelniania. Unikalny link zawarty w wiadomości e-mail umożliwia dostęp do konfiguracji początkowej i ustawienie hasła root. Przy kolejnych logowaniach do WHM wpisz `root` jako nazwę użytkownika oraz ustalone wcześniej hasło.
