...
O ficheiro está em imparidade. Por predefinição, o NetworkManager não cria novos perfis neste formato. O ficheiro de configuração encontra-se agora em `/etc/NetworkManager/system-connections/`.
  ...
  <Tab label="Windows Server 2019 e versões posteriores">
    ...
    **Passo 2: Abrir a configuração da rede do servidor**
    ...
    Abra o menu Iniciar do Windows, introduza `ncpa.cpl` e, de seguida, clique no resultado de pesquisa sugerido.

    <img className="thumbnail" alt="Janela Ligações de rede no painel de controlo do Windows" src="/images/bare-metal-cloud/dedicated-servers/network-ipv6/ipv6_network_connections.png" loading="lazy" />
...
De qualquer forma, não hesite em [contactar a nossa equipa de suporte](/links/support-contact) para pedir para examinar as suas configurações. Será necessário fornecer:
