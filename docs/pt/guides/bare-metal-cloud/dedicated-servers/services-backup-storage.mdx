...
Aceda à <ApiLink>consola API OVHcloud</ApiLink> com os identificadores da sua conta de cliente e utilize a seguinte chamada:
...
O backup storage é compatível apenas com NFSv3. Em primeiro lugar, certifique-se de que autorizou o acesso dos seus blocos de IP ao armazenamento e que estes podem utilizar o protocolo NFS. Dependendo do seu sistema operativo Linux, é possível que tenha de instalar o cliente NFS e iniciar os serviços NFS e `portmap`.
...
- **HostName**: o nome do seu Backup Storage.
- **ServiceName**: o nome do seu servidor (exemplo: "`ns1111111.ip-203-0-113.eu`").
...
Em caso de problemas de ligação, verifique também se as portas UDP e TCP necessárias para o NFSv3 estão abertas na firewall, tanto de entrada como de saída.
A porta 111 UDP/TCP deve estar aberta nos dois sentidos, tal como a porta 2049/TCP (NFS) e o intervalo de portas dinâmicas (de 32768 a 60999 UDP/TCP). Estas portas são utilizadas pelo RPC para os programas `mountd`, `lockd` e `statd`.

Por fim, autorize todas as ligações de saída para o endereço IP do servidor Backup Storage.
