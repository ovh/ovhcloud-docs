...
:::info
Se a ligação já expirou, ligue-se ao seu VPS via SSH com o nome de utilizador e execute o comando `sudo whmlogin` para gerar uma nova.
:::
...
O WHM utiliza de forma padrão o utilizador root para a autenticação. O link de utilização única presente no e-mail permite aceder à configuração inicial e definir a sua palavra-passe root. Nos acessos seguintes ao WHM, introduza `root` como nome de utilizador e a palavra-passe que definiu.
