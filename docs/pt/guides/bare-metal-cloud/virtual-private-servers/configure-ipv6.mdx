...
- Acesso à <ApiLink>API OVHcloud</ApiLink> (opcional).
...
<Tabs>
  ...
  <Tab label="Através das API OVHcloud">
    Aceda à página <ApiLink>API OVHcloud</ApiLink>:
...
- [Aplicação persistente em Red Hat e derivados (Rocky Linux, AlmaLinux, etc.)](#persistentredhat).
- [Aplicação persistente em Fedora 43 e versões posteriores](#persistentfedora).
...
#### Aplicação persistente em Red Hat e seus derivados (Rocky Linux, AlmaLinux, etc.) <a name="persistentredhat"></a>

Existem dois métodos para configurar a sua rede de acordo com o sistema operativo instalado no seu servidor:

- **para Rocky Linux e AlmaLinux (8/9)**: utilize o método baseado no ficheiro *network-scripts*;
...
#### Aplicação persistente no Fedora 43 e versões posteriores <a name="persistentfedora"></a>
