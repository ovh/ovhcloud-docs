---
pageType: overview
title: Servidores privados virtuais
description: "Implemente, configure e faça a gestão do seu VPS OVHcloud - instalação, rede, backups e administração diária dos seus servidores virtuais privados."
text: "Descubra os guias Servidores privados virtuais"
essentialsTitle: "Os guias essenciais para começar"
essentials:
  - title: Primeiros passos com um VPS
    link: /guides/bare-metal-cloud/virtual-private-servers/starting-with-a-vps
  - title: "VPS - Gestão a partir da Área de Cliente OVHcloud"
    link: /guides/bare-metal-cloud/virtual-private-servers/understand-vps-control-panel
  - title: OVHcloud VPS Perguntas frequentes
    link: /guides/bare-metal-cloud/virtual-private-servers/vps-faq
  - title: "Configuração das contas utilizadores e do acesso root num servidor"
    link: /guides/bare-metal-cloud/dedicated-servers/changing-root-password-linux-ds
  - title: "Como começar com as ligações SSH"
    link: /guides/bare-metal-cloud/dedicated-servers/ssh-introduction
  - title: "Como criar e utilizar chaves de autenticação para as ligações SSH aos servidores OVHcloud"
    link: /guides/bare-metal-cloud/dedicated-servers/creating-ssh-keys
tutorials:
  - title: "Tutorial - Como usar o PuTTY para conexões SSH e autenticação"
    link: /guides/web-cloud/web-hosting/ssh-using-putty-on-windows
  - title: "Como utilizar o SFTP para transferir ficheiros"
    link: /guides/bare-metal-cloud/dedicated-servers/transfer-data-via-sftp
  - title: "Recuperação das bases de dados em modo rescue"
    link: /guides/bare-metal-cloud/dedicated-servers/restore-bdd-rescue
goFurther:
  title: Saiba mais
  items:
    - title: Visitar a comunidade OVHcloud
      link: https://community.ovhcloud.com/
    - title: "Consultar o roadmap e o changelog"
      link: https://www.ovhcloud.com/pt/roadmap-changelog/
    - title: Juntar-se ao Discord
      link: https://discord.gg/ovhcloud
cta:
  title: "Tem questões? Aceda ao suporte OVHcloud"
  description: "Encontre a resposta às suas questões no nosso centro de ajuda."
  actions:
    - text: Aceder ao suporte
      link: https://help.ovhcloud.com/
      theme: brand
---
