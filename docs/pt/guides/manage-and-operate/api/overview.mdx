---
pageType: overview
title: API
description: "Autentique, consulte e automatize os seus serviços OVHcloud com a API OVHcloud - chaves de aplicação, tokens, OAuth2 e boas práticas de chamadas."
text: "Explore os guias API"
essentialsTitle: "Guias essenciais para começar"
essentials:
  - title: "Primeiros passos com as API OVHcloud"
    link: /guides/manage-and-operate/api/first-steps
  - title: "Exploring the OVHcloud APIs"
    link: /guides/manage-and-operate/api/console-preview
  - title: "OVHcloud API v2 - Principles of operation"
    link: /guides/manage-and-operate/api/apiv2
  - title: Manage services
    link: /guides/manage-and-operate/api/services
  - title: "Managing OVHcloud service accounts"
    link: /guides/manage-and-operate/api/manage-service-account
  - title: "How to use service accounts to connect to OVHcloud APIs"
    link: /guides/account-and-service-management/account-information/authenticate-api-with-service-account
goFurther:
  title: "Saiba mais"
  items:
    - title: Visite a comunidade OVHcloud
      link: https://community.ovhcloud.com/
    - title: "Consulte a roadmap & changelog"
      link: https://www.ovhcloud.com/pt/roadmap-changelog/
    - title: "Junte-se ao Discord"
      link: https://discord.gg/ovhcloud
cta:
  title: "Questões? Aceda ao suporte OVHcloud"
  description: "Encontre a resposta às suas questões no nosso centro de ajuda."
  actions:
    - text: Aceder ao suporte
      link: https://help.ovhcloud.com/csm/pt-home
      theme: brand
---
