...
- Para aceder aos segredos através da API REST ou da API compatível com HashiCorp Vault KV2, configure um [método de autenticação](/guides/manage-and-operate/kms/okms-authentification-method) para o seu domínio OKMS (Personal Access Token, conta de serviço ou certificado de acesso).
...
A user with the necessary IAM rights can access the content of a secret. Configure a autenticação e as políticas IAM conforme descrito em [Métodos de autenticação OKMS](/guides/manage-and-operate/kms/okms-authentification-method).
...
O Secret Manager é acessível através de dois conjuntos de API diferentes. Ambos operam sobre os mesmos objetos: um segredo criado por um método é acessível pelo outro.

Antes de chamar qualquer uma das API, configure um [método de autenticação](/guides/manage-and-operate/kms/okms-authentification-method) e a [política IAM](/guides/manage-and-operate/kms/okms-authentification-method#iam-rights-for-okms) necessária para o seu domínio OKMS. Para utilizar a API REST, recomenda-se um Personal Access Token ou uma conta de serviço.
