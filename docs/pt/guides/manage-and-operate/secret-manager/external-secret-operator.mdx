...
O campo `stringData` aceita o token sem codificação — o Kubernetes codifica-o em base64 ao armazenar o segredo. Se preferir utilizar o campo `data`, tem de codificar previamente o token em base64 (`echo -n "<token>" | base64`).

Aplique-o com o comando `kubectl apply -f secret.yaml`.

Ou, se o `user_pat` estiver armazenado na variável de ambiente `PAT_TOKEN`, crie o segredo diretamente com kubectl, que também trata da codificação:
...
Ao inspecionar o segredo com `kubectl get secret ovhcloud-vault-token -n external-secrets -o yaml`, o valor aparece em `data`, codificado em base64.
