...
description: Descubra como configurar o External Secret Operator para armazenar os segredos Kubernetes no Secret Manager da OVHcloud
...
Este manual explica como configurar o External Secret Operator do Kubernetes para utilizar o Secret Manager da OVHcloud como fornecedor.
...
Irá também precisar da `region` e do `okms-id` do domínio OKMS que pretende utilizar. Este ID e esta região podem ser encontrados no <ManagerLink to="/#/dedicated/security/kms">dashboard do seu domínio OKMS</ManagerLink> ou através da [CLI OVHcloud](https://github.com/ovh/ovhcloud-cli):
...
#### Instalação do External Secret Operator (ESO) no seu cluster Kubernetes
...
#### Configuração do External Secret Operator
...
#### Utilização do External Secret Operator
...
Para mais informações sobre a gestão do External Secret Operator, consulte a documentação dedicada, utilizando o fornecedor HashiCorp Vault: [https://external-secrets.io/latest/](https://external-secrets.io/latest/).
