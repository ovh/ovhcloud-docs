...
description: Importe e exporte em segurança material de chave encriptado com o OVHcloud KMS através de encapsulamento RSA assimétrico
...
Bring Your Own Key (BYOK) permite-lhe importar e exportar material criptográfico com o OVHcloud Key Management Service (KMS) sem o enviar em texto claro. Um par de chaves RSA de transporte protege o material em trânsito: a chave pública encapsula a carga útil; apenas o detentor da chave privada a pode desencapsular.
...
- Dispor de uma política IAM que autorize as ações BYOK descritas em [Configurar o IAM para o BYOK](#configurar-o-iam-para-o-byok).
- Opcional: a [CLI OKMS](https://github.com/ovh/okms-cli) ou o [SDK Go](https://pkg.go.dev/github.com/ovh/okms-sdk-go) instalado(s) para os exemplos nos separadores.

## Instruções

### Compreender a cerimónia de encapsulamento RSA

<img className="thumbnail" alt="Fluxo de importação de chave BYOK RSA da origem para o KMS de destino" src="/images/manage-and-operate/kms/import-key-byok/byok-rsa-import-ceremonie.png" loading="lazy" />

A cerimónia envolve dois ambientes:

- **Destino (Dst):** o seu domínio KMS OVHcloud, onde a chave importada será armazenada.
- **Origem (Src):** o sistema que detém atualmente o material de chave (outro KMS, uma aplicação ou uma ferramenta como o OpenSSL).

1. Gere um par de chaves RSA de transporte no KMS OVHcloud com as operações `wrapKey` e `unwrapKey`.
2. Exporte a chave de transporte **pública** e partilhe-a com o ambiente de origem.
3. Na origem, encapsule (encripte) o material de chave com a chave pública de destino utilizando `RSA-OAEP` ou `RSA-OAEP-256`.
4. No destino, importe a chave encapsulada com `POST /v1/servicekey` e uma carga útil `wrappedKeys`. O KMS desencapsula o ciphertext com a chave de transporte privada e armazena a chave de serviço resultante.
...
**Exportação BYOK** — importar uma chave de transporte pública, atualizar a extractibilidade e encapsular uma chave para exportação:
...
Crie uma chave RSA dedicada ao encapsulamento. Defina sempre `wrapKey` e `unwrapKey`. Estas operações são mutuamente exclusivas com `sign` e `verify`.

Tamanhos RSA suportados: `2048`, `3072`, `4096`.
...
`keyFormatType` descreve o formato do material de chave em **texto claro** antes do encapsulamento / após a desencapsulação. O ciphertext é sempre uma cadeia [JWE Compact Serialization](https://datatracker.ietf.org/doc/html/rfc7516) (o algoritmo de encapsulamento é transportado no cabeçalho JWE).
...
Após o encapsulamento com a chave pública de destino, dispõe de um `ciphertext` JWE Compact Serialization para enviar no pedido de importação.

### Importar a chave encapsulada

Invoque `POST /v1/servicekey` com um array `wrappedKeys` em vez de `keys` em texto claro.

|**Método**|**Caminho**|**Descrição**|
| :-: | :-: | :-: |
|POST|/v1/servicekey|Criar, importar ou importar de forma encapsulada uma chave de serviço|
...
`KEY` é a cadeia JWE Compact Serialization. Passe-a como `@path/to/wrapped.jwe`, em linha ou `-` para a ler a partir de stdin.
...
- O estado da chave está ativo (ou ative-a se o seu fluxo de trabalho o exigir).
...
Pode depois utilizar a chave para encriptar, desencriptar, assinar ou verificar, conforme documentado em [Utilizar o seu OVHcloud Key Management Service (KMS)](/guides/manage-and-operate/kms/kms-usage).

### Exportar uma chave encapsulada

A exportação segura é o inverso da importação: o OVHcloud KMS encripta o material de chave com uma chave de transporte RSA e devolve apenas um ciphertext JWE. A chave de transporte privada deve residir no ambiente de **destino**, para que apenas esse ambiente possa desencapsular o material.
...
:::info
Não gere o par de chaves de transporte de exportação no OVHcloud KMS. Se a chave de transporte privada permanecesse no OVHcloud KMS, o destino não conseguiria desencapsular o ciphertext. Apenas a metade pública é importada no OVHcloud KMS para o encapsulamento.
:::
...
Registe a chave pública de destino para que o OVHcloud KMS possa encapsular com ela. Importe-a como chave RSA apenas pública com `wrapKey` (sem material privado).
...
A chave que exporta deve ser extractível. Prefira definir `extractable` como `true` **apenas para a operação de exportação** (via PATCH) e voltar a defini-lo como `false` de seguida. Mantenha as chaves não extractíveis na criação sempre que possível. Tornar uma chave extractível define definitivamente `never_extractable` como `false`.
...
Após uma exportação encapsulada bem-sucedida, desative novamente a extração:
...
Invoque `GET /v1/servicekey/{keyId}` com os parâmetros de consulta de encapsulamento abaixo. Quando estão presentes, a resposta devolve `wrappedKeys` em vez de `keys` em texto claro.
...
|**Método**|**Caminho**|**Descrição**|
| :-: | :-: | :-: |
|GET|/v1/servicekey/{keyId}|Obter metadados ou exportar o material de chave sob forma encapsulada|
...
Outros algoritmos de encapsulamento: `types.RSAOAEP`, `types.RSAOAEP256` (recomendado).
...
- Se o destino for outro domínio KMS OVHcloud, importe aí o material encapsulado com `wrappedKeys` como em [Importar a chave encapsulada](#importar-a-chave-encapsulada), utilizando a chave de transporte privada desse domínio como `wrappingKeyId`.
- Se o destino for um sistema externo, utilize a respetiva API nativa de desencapsulação / importação com a chave de transporte privada e o mesmo `keyFormatType` / algoritmo de encapsulamento que na exportação.
