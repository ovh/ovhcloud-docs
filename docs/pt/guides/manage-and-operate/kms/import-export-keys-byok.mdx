...
Após o encapsulamento com a chave pública de destino, envie no pedido de importação o `ciphertext` JWE Compact Serialization resultante. Para um exemplo em Go, consulte [Exemplo completo em Go](#exemplo-completo-em-go).
...
### Exemplo completo em Go

Os exemplos seguintes executam as duas cerimónias com uma chave RSA, utilizando o [SDK Go](https://pkg.go.dev/github.com/ovh/okms-sdk-go) para as chamadas ao OVHcloud KMS e o [go-jose](https://pkg.go.dev/github.com/go-jose/go-jose/v4) para as operações JWE do outro lado. Ambos os exemplos codificam o material de chave em PKCS#8 (DER), o que corresponde ao `keyFormatType` `PKCS8`.

#### Importar uma chave gerada localmente

A biblioteca padrão de Go gera a chave, que o programa encapsula localmente com a chave de transporte pública e depois importa. O seu texto simples nunca chega ao OVHcloud KMS.

```go
// 1. Crie a chave de transporte no OVHcloud KMS e obtenha a sua parte pública.
...
// 2. Gere a chave a importar e codifique-a em PKCS#8 (DER).
...
// 3. Encapsule o material de chave como JWE: encapsulamento de chave RSA-OAEP-256, encriptação do conteúdo A256GCM.
...
// 4. Importe a chave encapsulada. O KMS desencapsula-a e deduz o tipo e o tamanho a partir do material.
...
A encriptação do conteúdo JWE (`enc`) deve ser `A256GCM`.

#### Exportar uma chave e desencriptar o JWE

O destino (aqui, o programa local) gera o par de chaves de transporte e importa apenas a sua parte pública no OVHcloud KMS. Em seguida, o destino desencripta o JWE exportado com a parte privada.

```go
// 1. Gere o par de chaves de transporte no destino.
destKey, err := rsa.GenerateKey(rand.Reader, 4096)
if err != nil {
  return err
}

// 2. Importe apenas a sua parte pública no OVHcloud KMS, com a utilização wrapKey.
...
// 3. Torne a chave extraível e, em seguida, exporte-a encapsulada pela chave pública de destino.
extractable := true
...
// 4. Desencripte o JWE com a chave privada de destino e analise o material PKCS#8.
