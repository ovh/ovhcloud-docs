...
Crie uma chave RSA dedicada ao encapsulamento. Defina sempre `wrapKey` e `unwrapKey`. `wrapKey` e `unwrapKey` são mutuamente exclusivas com `sign` e `verify`.
...
Invoque `GET /v1/servicekey/{keyId}` com os parâmetros de consulta de encapsulamento abaixo. Quando estes parâmetros estão presentes, a API devolve `wrappedKeys` em vez de `keys` em texto claro.
