...
| **Plano de controlo (API OVHcloud)** | <ApiLink section="/okms" version="v2">`/v2/okms`</ApiLink> | Encomendar domínios, gerir as credenciais, configurar o registo de logs |
...
:::info
O seu `okmsId` e o seu endpoint regional são ambos devolvidos pela seguinte chamada à API:

<Api version="v2" section="/okms" method="GET" route={"/okms/resource"} />

Ambos são também apresentados no separador **Informações gerais** do <ManagerLink to="/#/okms/key-management-service">dashboard do seu domínio OKMS</ManagerLink>.
:::
...
### Configurar a autenticação <a id="set-up-authentication"></a>
...
### Utilizar a interface Swagger OKMS <a id="use-the-okms-swagger-ui"></a>
...
Também pode abri-la a partir do link Swagger no <ManagerLink to="/#/okms/key-management-service">dashboard do seu domínio OKMS</ManagerLink>.
...
### Direitos IAM para OKMS <a id="iam-rights-for-okms"></a>
