...
No entanto, se duas zonas ou duas bases de dados deixarem de estar disponíveis em simultâneo, o serviço OKMS passa para o modo apenas de leitura: todas as operações de escrita (criação de chaves, gestão de secrets, atualização de metadados, etc.) falham. As chaves existentes permanecem disponíveis para as operações criptográficas e os secrets permanecem acessíveis.
