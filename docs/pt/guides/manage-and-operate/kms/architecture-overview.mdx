...
Um Hardware Security Module (HSM) é um dispositivo físico especializado concebido para gerar, armazenar e utilizar chaves criptográficas de forma segura. As chaves protegidas por HSM nunca saem do hardware: as operações criptográficas são executadas em equipamento dedicado e resistente a manipulações, o que reforça a confidencialidade e a integridade das chaves.

A OVHcloud utiliza HSM Thales Luna, certificados FIPS 140-3 Level 3 e Common Criteria EAL4+.

As chaves criadas com o nível de proteção HSM são replicadas em três HSM físicos distintos. A respetiva localização exata depende da região selecionada para o OKMS.

#### Regiões 3-AZ

Nas regiões 3-AZ, os três HSM estão repartidos por todas as zonas de disponibilidade.

#### Regiões 1-AZ

Nas regiões 1-AZ, dois HSM situam-se na região e um terceiro HSM remoto é implementado numa região vizinha distinta.

Localizações dos HSM remotos:

- **EU-WEST-GRA** (em breve): EU-WEST-RBX
- **EU-WEST-RBX** (em breve): EU-WEST-SBG
- **EU-WEST-SBG** (em breve): EU-WEST-GRA
