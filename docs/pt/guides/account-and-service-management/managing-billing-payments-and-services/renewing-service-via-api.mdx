...
- Acesso às <ApiLink>API OVHcloud</ApiLink>.
- Ter [criado os ID para a API OVHcloud](/guides/manage-and-operate/api/first-steps).

## Instruções

Para utilizar a <ApiLink section="/service" method="GET" route={"/service/\{serviceId\}/renew"}>API de renovação</ApiLink>, vai precisar do `serviceId`. Esta informação está disponível na API `serviceInfos`; por exemplo:
