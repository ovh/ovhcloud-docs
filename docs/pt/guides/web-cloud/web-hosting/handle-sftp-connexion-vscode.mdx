...
No Visual Studio Code, introduza novamente o comando `SFTP: Config`. Um ficheiro de configuração chamado "sftp.json" aparece no Visual Studio Code.
...
:::info
Lembre-se de que a configuração correta do ficheiro "sftp.json" é primordial. Se você encontrar um erro antes de fazer o download do seu projeto, isso geralmente é causado por uma falha de configuração do ficheiro "sftp.json". Para todas as questões, consulte [FAQ do plug-in](https://github.com/Natizyskunk/vscode-sftp/blob/HEAD/FAQ.md).
:::

### Efetuar alterações nos ficheiros

Agora que o projeto é baixado localmente em sua máquina, você pode facilmente editar, adicionar ou excluir ficheiros no Visual Studio Code.
...
[Utilizar o acesso SSH do alojamento web](/guides/web-cloud/web-hosting/ssh-on-webhosting). Lembre-se de que, para utilizar o SSH, deve dispor de um [plano de alojamento web](/links/web/hosting) **Pro**, **Performance**, **Agency**, **Agency Plus** ou **Agency Max**.
