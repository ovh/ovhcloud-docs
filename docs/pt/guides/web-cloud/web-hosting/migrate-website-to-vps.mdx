...
<Tabs>
  ...
  <Tab label="Drupal">
    ...
    - `/modules` : e `/themes` : para os módulos e temas personalizados.
...
Depois de ter transferido os ficheiros do seu website e, se for caso disso, importado a base de dados para o seu VPS, é importante que atualize os ficheiros de configuração do seu website para garantir o seu bom funcionamento. As variáveis principais a ajustar são muitas vezes as informações de ligação à base de dados, bem como os caminhos para as pastas. Estas são as configurações específicas a serem atualizadas para os principais CMS.
