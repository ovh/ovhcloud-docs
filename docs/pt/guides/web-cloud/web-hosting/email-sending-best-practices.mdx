...
description: Escolha entre a função PHP mail() e o envio SMTP autenticado a partir do seu alojamento web e configure o SMTP com a sua conta de e-mail OVHcloud
...
- Apenas para o envio SMTP autenticado: dispor de um endereço de e-mail associado ao seu nome de domínio (<Region zones={['eu']}>oferta MX Plan, E-mail Pro ou Exchange</Region><Region zones={['ca']}>oferta MX Plan ou Exchange</Region><Region zones={['apac']}>oferta MX Plan</Region>). Se ainda não tiver nenhum, consulte o guia [Criar um endereço de e-mail com a oferta MX Plan](/guides/web-cloud/email-and-collaborative-solutions/mx-plan/email-creation).
...
- os métodos de seguimento dos e-mails.

| | PHP `mail()` | SMTP autenticado |
|---|---|---|
| **Funcionamento** | O script entrega o e-mail aos servidores de envio do seu alojamento. Não é necessária qualquer configuração. | O script liga-se ao servidor de correio OVHcloud com o nome de utilizador e a palavra-passe de um endereço de e-mail associado ao seu domínio. |
| **Quota de envio** | Por alojamento e por hora, consoante a sua oferta. Consulte as [Especificidades técnicas relacionadas com os alojamentos web](/guides/web-cloud/web-hosting/hosting-technical-specificities). | Por endereço de e-mail e por hora (cerca de 200 e-mails para um endereço MX Plan), mais **300 e-mails por hora e por endereço IP de origem** no servidor SMTP OVHcloud. Num alojamento partilhado, o endereço IP de saída é partilhado com os outros sites do cluster. |
| **Seguimento** | A partir do separador <code className="action">Scripts e-mails</code> do seu alojamento na Área de Cliente OVHcloud: estatísticas, relatórios de erros, bloqueio e purga. Consulte o guia [Seguimento dos e-mails automatizados](/guides/web-cloud/web-hosting/mail-function-script-records). | Invisível na Área de Cliente OVHcloud. Siga os envios a partir dos logs da sua aplicação. |
| **Identidade do remetente** | O endereço `From:` é declarado pelo script. O seu domínio deve estar associado ao seu alojamento (multisite), caso contrário o e-mail não pode ser autenticado. | O endereço `From:` deve corresponder à conta autenticada. O e-mail é assinado com <Tooltip term="dkim">DKIM</Tooltip> se o DKIM estiver ativado para o seu domínio. |
| **Utilização típica** | Formulários de contacto, notificações, qualquer script cujo volume caiba na quota do alojamento. | CMS com vários plugins que enviam e-mails, aplicações que exigem um remetente fixo e assinado, ou endereço `From:` partilhado com a sua caixa de correio. |

Independentemente do método escolhido:

- Os servidores de destino (Gmail, Outlook, etc.) verificam se o domínio do `From:` autoriza o servidor de envio. Mantenha os seus registos [SPF](/guides/web-cloud/domains/dns-zone-spf), [DKIM](/guides/web-cloud/domains/dns-zone-dkim) e [DMARC](/guides/web-cloud/domains/dns-zone-dmarc) corretos na sua zona DNS.
- Um envio abusivo ou de spam a partir do seu alojamento ou do seu endereço de e-mail provoca um bloqueio automático. Se os seus e-mails já não são enviados, consulte o guia [Seguimento dos e-mails automatizados](/guides/web-cloud/web-hosting/mail-function-script-records).
...
- **SPF e DKIM**: o e-mail é emitido por um servidor autorizado para o seu domínio e assinado com DKIM se o [DKIM estiver ativado](/guides/web-cloud/domains/dns-zone-dkim) para o seu domínio.
...
Este domínio não precisa de ser gerido na sua conta OVHcloud: pode utilizar um domínio registado noutro agente de registo, desde que esteja associado ao seu alojamento web. É esta associação que atesta que está autorizado a enviar e-mails em nome desse domínio.

:::tip
Se o seu domínio ainda não estiver associado ao seu alojamento, adicione-o como multisite a partir da sua Área de Cliente OVHcloud e verifique depois a configuração DNS associada. Consulte o guia [Partilhar o alojamento entre vários sites](/guides/web-cloud/web-hosting/multisites-configure-multisite).
:::

:::warning
Um envio efetuado para um domínio que não está associado ao seu alojamento não pode ser autenticado: apresenta as características de um envio não autenticado, pelo que será rejeitado e registado como erro.
:::
...
A função `wp_mail()` do WordPress utiliza em segundo plano a função PHP `mail()`, com um endereço `From:` definido pelo WordPress ou por cada plugin. Os envios ficam, por isso, sujeitos à quota do seu alojamento e podem ser seguidos a partir da Área de Cliente OVHcloud.

Se pretender um remetente fixo e autenticado para todos os e-mails do seu site, instale um **plugin SMTP** que substitui `wp_mail()` por uma ligação SMTP autenticada para o seu servidor de correio OVHcloud.
...
O [FluentSMTP](https://pt.wordpress.org/plugins/fluent-smtp/) é uma alternativa leve e gratuita. A quota de envio do seu endereço de e-mail aplica-se da mesma forma.
...
[Especificidades técnicas relacionadas com os alojamentos web](/guides/web-cloud/web-hosting/hosting-technical-specificities)
