...
O [PuTTY](https://www.chiark.greenend.org.uk/~sgtatham/putty/latest.html) é um software cliente SSH open source com uma interface gráfica do utilizador. Ele foi desenvolvido para o Windows, mas também está disponível para outros sistemas operativos e inclui recursos úteis, como a gestão de chaves SSH.
...
- [Instância Public Cloud](/guides/public-cloud/compute/getting-started)
- [Servidor dedicado](/guides/bare-metal-cloud/dedicated-servers/getting-started-with-dedicated-server)
- [Servidor dedicado da gama **Eco**](/guides/bare-metal-cloud/dedicated-servers/getting-started-with-dedicated-server-eco)
- [VPS](/guides/bare-metal-cloud/virtual-private-servers/starting-with-a-vps)
...
Introduza um nome para o seu ficheiro e guarde-o. Agora você deve ter um novo ficheiro **private key** com a extensão `ppk` (*PuTTY private key*) em sua pasta.
...
Você também pode copiar a cadeia de chave pública e colá-la em um ficheiro de texto simples (sem interromper a cadeia de chave por quebras de linha).
...
<Tabs>
  <Tab label="Instance Public Cloud">
    Destaque e copie **toda a cadeia de chaves públicas** que criou [no passo anterior](#puttygen) a partir da janela PuTTYgen (abra primeiro o **ficheiro de chaves privadas** correspondente, se necessário). De seguida, utilize-a tal como indicado na secção correspondente do nosso [guia relativo à criação de uma instância Public Cloud na Área de Cliente OVHcloud](/guides/public-cloud/compute/getting-started)
...
Salve o ficheiro e saia do editor. Reinicie o seu servidor (`sudo reboot`) ou reinicie apenas o serviço OpenSSH com um dos seguintes comandos (o comando apropriado pode variar em função do seu sistema operativo):
...
Se você tiver seguido as instruções acima, poderá aceder ao seu host remoto usando a autenticação de chave. Embora a ligação em si não necessite de uma palavra-passe, o PuTTY irá sempre pedir a palavra-passe do ficheiro de chave privada correspondente.
...
- Você não precisa selecionar o ficheiro de chave privada para cada conexão no PuTTY.
- Você só precisa inserir a frase secreta para o ficheiro de chave privada uma vez, quando o ficheiro de chave é aberto por Pageant.
