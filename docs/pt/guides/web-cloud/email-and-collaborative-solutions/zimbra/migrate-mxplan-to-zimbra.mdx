...
<Tabs>
  ...
  <Tab label="Etapa 2">
    **Ligar-se ao projeto e criar a migração**
    ...
    - **Conta de origem**:
        - **Tipo de conta**: Na categoria `OVHcloud`, escolha `MX Plan` ou `Deteção automática`. Clique em <code className="action">Ligar-se</code> para se identificar com a sua conta OVHcloud e selecionar automaticamente o serviço e o endereço a migrar (exemplo: `contact@mydomain.ovh`). Introduza depois a palavra-passe desta conta de e-mail.
    - **Conta de destino**:
        - **Tipo de conta**: Na categoria `OVHcloud`, escolha `Zimbra`. Clique em <code className="action">Ligar-se</code> para se identificar com a sua conta OVHcloud e selecionar o serviço Zimbra e o endereço de destino (exemplo: `contact2@mydomain.ovh`). Introduza depois a palavra-passe desta conta de e-mail.
