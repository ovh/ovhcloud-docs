...
- [Gestão das pastas da sua conta de e-mail](#folders-management)
 ...
 - [Tratamento dos e-mails](#email-management)
    ...
    - [Recuperar e-mails eliminados](#restore)
...
:::info
Depois de introduzir o seu endereço de email e a sua palavra-passe na página de acesso ao webmail, é redirecionado para a janela de início de sessão do **Zimbra**, onde tem de introduzir novamente as suas credenciais. Nesta janela, o campo **Nome de utilizador** ("Username") corresponde ao seu **endereço de email completo** e não a um nome de utilizador distinto.
:::

<img className="thumbnail" alt="Janela de início de sessão do Zimbra" src="/images/web-cloud/email-and-collaborative-solutions/mx-plan/email-zimbra/zimbra-login-window.png" loading="lazy" />
...
- **Ficheiro**: pasta padrão para classificar os e-mails processados.
- **Spam**: pasta onde são armazenados os e-mails considerados indesejados.
- **Lixo**: Os e-mails eliminados são armazenados nessa pasta antes de serem eliminados permanentemente.
...
- 4.**Ficheiro**: mover o e-mail para a pasta "Ficheiro" da sua conta de e-mail.
...
#### Recuperar e-mails eliminados <a name="restore"></a>

Quando elimina e-mails, estes são colocados por padrão na lixeira.<br/>
Se eliminar e-mails da sua lixeira ou a esvaziar, estes serão colocados em retenção. Ainda é possível recuperá-los por 30 dias.

Vamos ver como recuperar um e-mail em retenção revisando os passos da sua eliminação e restauração:

1. **Eliminação de um e-mail**: Quando você visualiza um e-mail na caixa de entrada ou em uma das pastas e clica em <code className="action">excluir</code>, ele irá por padrão para a lixeira.
2. **Esvaziar a Lixeira**: Quando você clica em <code className="action">Esvaziar a Lixeira</code> ou elimina um e-mail dela, o e-mail não aparecerá mais na lixeira e entrará no período de retenção de 30 dias.
3. **Aceder à restauração**: Para recuperar um e-mail eliminado da lixeira há menos de 30 dias, clique com o botão direito em <code className="action">Lixeira</code>, depois clique em <code className="action">Recuperar e-mail excluído</code>.
...
<Tabs>
  ...
  <Tab label="Calendário e lembretes">
    ...
    **Definições gerais do calendário**
    ...
    - **Partilhar**: `Ativar a delegação para os clientes CALDav`. Esta opção permite-lhe gerir os seus calendários através de um software que suporte o protocolo CALdav.
...
- **Lixeira**: os contactos eliminados são armazenados nessa pasta antes de serem eliminados permanentemente.
...
Por predefinição, na lista `Os meus calendários`, tem um `Calendário` criado por predefinição. Este calendário padrão não pode ser eliminado, mas você verá que é possível criar seus próprios calendários no próximo parágrafo.
...
2. Na coluna central, você encontrará a lista de ficheiros presentes na pasta selecionada.

3. Na janela à direita, dependendo do tipo de ficheiro selecionado na coluna central, uma visão geral do conteúdo é exibida. A barra superior da pré-visualização exibe as ações possíveis para o(s) ficheiro(s) selecionado(s).
...
- **(2)** Este botão é usado como uma pega, permite mover o filtro para a lista para atribuir uma ordem de aplicação. Cada filtro é aplicado de acordo com a ordem indicada na lista.
...
:::info
No exemplo abaixo, decidimos redirecionar todos os e-mails recebidos para outro endereço de e-mail. Para entender o exemplo nas capturas de ecrã, estamos conectados ao endereço **zimbra@mydomain.ovh** e queremos redirecionar os e-mails deste conta para o endereço **address@example.com**.
:::


Para aceder os filtros e criar seu redirecionamento, siga as seguintes instruções:
...
<img className="thumbnail" alt="zimbra" src="/images/assets/screens/email-clients/zimbra/zimbra_redirection03.png" loading="lazy" />
