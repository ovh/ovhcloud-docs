---
pageType: overview
title: SMS
description: "Envie mensagens SMS e faça a gestão de campanhas de SMS com a OVHcloud - contas, destinatários, autenticação a dois fatores e integração com a API."
text: "Descubra os guias SMS"
essentialsTitle: "Os guias essenciais para começar"
essentials:
  - title: Launching your first SMS campaign
    link: /guides/web-cloud/messaging/sms/ma-premiere-campagne-sms
  - title: "Time2Chat, la messagerie conversationnelle par SMS pour les entreprises"
    link: /guides/web-cloud/messaging/sms/time-2-chat
goFurther:
  title: Saiba mais
  items:
    - title: "Visitar a comunidade OVHcloud"
      link: https://community.ovh.com/
    - title: Consultar o roadmap e o changelog
      link: https://www.ovhcloud.com/pt/roadmap-changelog/
    - title: Juntar-se ao Discord
      link: https://discord.gg/ovhcloud
cta:
  title: "Tem questões? Aceda ao suporte OVHcloud"
  description: "Encontre a resposta às suas questões no nosso centro de ajuda."
  actions:
    - text: Aceder ao suporte
      link: https://help.ovhcloud.com/
      theme: brand
---
