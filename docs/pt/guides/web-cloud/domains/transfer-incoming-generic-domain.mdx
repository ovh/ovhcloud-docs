...
Depois de desbloquear o seu nome de domínio e obter o código, pode encomendar a sua transferência para a OVHcloud. Esta etapa decorre no site da OVHcloud e não na Área de Cliente OVHcloud: encomende a transferência a partir do [nosso site](/links/web/domains-transfer) e clique em <code className="action">Transferir um nome de domínio</code> para abrir o formulário de encomenda. Introduza o seu nome de domínio e siga os passos da encomenda.
...
Quando for necessário indicar o código de transferência, introduza-o no campo ao lado do seu nome de domínio. O código é necessário para autorizar a transferência, pelo que deve tê-lo à sua disposição antes de iniciar a encomenda. Para alguns nomes de domínio, a encomenda indica que não é necessário qualquer código de autorização. Lembre-se de que a transferência não será iniciada até que um código válido seja fornecido.
...
- **configurar os servidores DNS para o seu nome de domínio.** Se atualmente utiliza o seu nome de domínio para manter um website ou um serviço de e-mail online, escolha manter os servidores DNS existentes ou indicar os seus para evitar qualquer interrupção do serviço.
...
|Opção|Descrição|
|---|---|
|<code className="action">Servidores DNS padrão</code>|O nome de domínio é associado aos servidores DNS da OVHcloud e é criada uma nova zona DNS. A sua configuração DNS atual não é mantida: a [alteração manual da zona DNS](/guides/web-cloud/domains/dns-zone-edit) pode ser necessária para repor os seus serviços.|
|<code className="action">Manter os servidores DNS atuais</code>|Os servidores DNS atualmente declarados no nome de domínio junto do seu agente de registo são mantidos tal como estão após a transferência. Os seus serviços continuam acessíveis sem qualquer intervenção da sua parte, desde que esses servidores DNS se mantenham ativos.|
|<code className="action">Personalizar os servidores DNS</code>|Introduz manualmente os nomes dos servidores DNS a associar ao nome de domínio. Utilize esta opção quando esses servidores forem diferentes dos atualmente declarados junto do seu agente de registo.|

:::info
Se utiliza atualmente o seu nome de domínio para um site ou um serviço de e-mail, escolha <code className="action">Manter os servidores DNS atuais</code> ou <code className="action">Personalizar os servidores DNS</code> para evitar qualquer interrupção de serviço. A opção <code className="action">Servidores DNS padrão</code> destina-se aos nomes de domínio cujos serviços estão (ou estarão) alojados na OVHcloud.

:::

:::warning
Se escolher <code className="action">Manter os servidores DNS atuais</code>, o seu anterior agente de registo pode desativar os respetivos servidores DNS após a transferência, o que tornaria o seu nome de domínio inacessível. Certifique-se de que os seus servidores DNS não estão alojados no seu anterior agente de registo.
