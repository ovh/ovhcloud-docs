...
:::warning
Se o nome de domínio não usar os servidores DNS da OVHcloud, os registos DKIM deverão ser alterados através da interface do agente responsável pela gestão da configuração do seu nome de domínio.

Se o seu nome de domínio estiver registado na OVHcloud, pode verificar se este utiliza a configuração OVHcloud na sua <ManagerLink to="/">Área de Cliente OVHcloud</ManagerLink>. Para isso, abra a secção <code className="action">Nomes de domínio</code>, selecione o domínio em questão e verifique o respetivo separador <code className="action">Servidores DNS</code>.
:::
...
<ZoneTabs>
  ...
  <Tab label="Zimbra" availableIn={['eu']}>
    ...
    **Dica para criar um registo CNAME**
    ...
    Abra o separador <code className="action">Zona DNS</code> e clique em <code className="action">Adicionar uma entrada</code> acima da tabela. No formulário que se abre, selecione o tipo `CNAME` e preencha os campos de acordo com os valores que registou.
    :::
  </Tab>
</ZoneTabs>
...
Para configurar o DKIM, dirija-se à <ApiLink>página das API OVHcloud</ApiLink> e inicie sessão:
...
<Tabs>
  ...
  <Tab label="4. Configurar o registo DNS">
    ...
    Depois de inserir os valores, clique em <code className="action">Adicionar</code>.
...
<Tabs>
  ...
  <Tab label="3. Recuperar o registo DNS">
    ...
    *Exemplo de resultado:*
    ...
    Obtenha os valores `customerRecord` e `targetRecord` em um ficheiro de texto. Vá para a próxima fase.
  ...
  <Tab label="4. Configurar o registo DNS">
    ...
    Dirija-se ao separador <code className="action">Zona DNS</code> e clique em <code className="action">Adicionar uma entrada</code> acima da tabela. No formulário que se abre, selecione o tipo `CNAME` e preencha os campos em função dos valores que tiver registado.
...
<ZoneTabs>
  <Tab label="MX Plan e Zimbra" availableIn={['eu']}>
    ...
    - `toConfigure`: a configuração do DKIM está a aguardar os parâmetros DNS do domínio. Os registos DNS devem ser preenchidos manualmente na zona do domínio. Para isso, clique em [etapa 4 de "a configuração completa do DKIM" para MX Plan e Zimbra](#confemail).<br/>
    - `enabled`: o DKIM está configurado e funcional.<br/>
    - `error`: O processo de instalação encontrou um erro. Sugerimos que abra um [ticket junto do suporte](/links/support-contact) especificando o domínio em causa.<br/>
...
Aceda ao separador <code className="action">Zona DNS</code> e clique em <code className="action">Adicionar uma entrada</code> acima da tabela. Existem 3 formas de adicionar um registo para configurar o DKIM na sua zona DNS:
...
Este registo é designado DKIM na interface, mas na verdade é um registo TXT na zona. O registo DKIM tem por objetivo facilitar a leitura dos diferentes parâmetros do DKIM, apresentando-os sob a forma de campos independentes.

<img className="thumbnail" alt="Adicionar um registo DKIM a partir da zona DNS" src="/images/assets/screens/control-panel/product-selection/web-cloud/domain-dns/dns-zone/nx-dkim-form.png" loading="lazy" />

- **Seletor** (Subdomínio): introduza o nome do seletor DKIM fornecido pelo seu serviço de e-mail. O sufixo `._domainkey` é adicionado automaticamente e o seu nome de domínio é acrescentado no final.
...
- **Chave pública (base64)**: cole a chave pública fornecida pelo seu serviço de e-mail. É a chave que os destinatários utilizam para verificar a sua assinatura DKIM.
- **Tipo de chave**: o algoritmo de assinatura da chave — `RSA` (por defeito) ou `ed25519`.
- **Modo de teste**: mantenha em `Desativado` em produção. Quando ativado, os destinatários tratam as mensagens de teste assinadas com DKIM como mensagens não assinadas, pelo que uma verificação falhada não afeta a entrega enquanto valida a sua configuração.
...
<Tabs>
  ...
  <Tab label="502">
    ...
    Ocorreu um erro aquando da configuração do DKIM. Se a sua configuração ainda estiver nesse estado, após 24 horas, convidamo-lo a abrir um [ticket junto do suporte](/links/support-contact).
  </Tab>
  <Tab label="503">
...
<ZoneTabs>
  ...
  <Tab label="MX Plan e Zimbra" availableIn={['eu']}>
    - `disabled`: O DKIM está desativado, ainda não foi configurado ou foi desativado pela API. <br/>
    - `modifying`: A configuração do DKIM está em curso, é necessário aguardar até ao fim do processo.<br/>
    - `toConfigure`: A configuração do DKIM está a aguardar os parâmetros DNS do domínio. Os registos DNS devem ser preenchidos manualmente na zona do domínio. Para isso, clique na etapa "[API - Configuração completa do DKIM](#confemail)" deste guia. <br/>
    - `error`: O processo de instalação encontrou um erro. Sugerimos que abra um [ticket junto do suporte](/links/support-contact) especificando o domínio em causa.
