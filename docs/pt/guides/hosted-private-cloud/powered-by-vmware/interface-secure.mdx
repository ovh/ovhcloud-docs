...
- [Validar uma operação com um token](#validar-uma-operação-com-um-token)
...
- uma ligação que permite validar a operação (atenção: se o seu telefone não estiver ligado a uma rede cujo [o endereço IP](/guides/hosted-private-cloud/powered-by-vmware/manager-ovh-private-cloud#segurança) não foi autorizado, a página não aparecerá).
...
De seguida, será enviado um token aos utilizadores com a autorização [token validator](#validar-uma-operação-com-um-token) para que possam **confirmar a operação**.
