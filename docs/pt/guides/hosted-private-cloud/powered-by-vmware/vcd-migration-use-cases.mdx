...
You can now reset the **Admin** password for a Public VCF as-a-Service organization using APIv2: <ApiLink section="/vmwareCloudDirector" method="POST" route={"/vmwareCloudDirector/organization/\{organizationId\}/password"} version="v2">Access APIv2</ApiLink>.
...
Passwords for these users may have been reset during the migration process. If needed, you can update them using the <ApiLink>dedicated OVHcloud API</ApiLink> with the following call:
