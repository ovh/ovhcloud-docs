---
pageType: overview
title: Valkey
description: "Aprovisione e opere um armazenamento Valkey em memória gerido no Public Cloud da OVHcloud - criação de clusters, escalabilidade, persistência e segurança."
text: Descubra os guias Valkey
essentialsTitle: Os guias essenciais para começar
essentials:
  - title: Getting started with Public Cloud Databases
    link: /guides/public-cloud/databases/getting-started
  - title: Getting started with Terraform for Public Cloud Databases
    link: /guides/public-cloud/databases/public-cloud-databases-order-terraform
  - title: Configure incoming connections of a Public Cloud Databases for Valkey service
    link: /guides/public-cloud/databases/valkey-prepare-for-incoming-connections
  - title: Connect using the CLI for Public Cloud Databases for Valkey
    link: /guides/public-cloud/databases/valkey-connect-cli
  - title: Connect using PHP for Public Cloud Databases for Valkey
    link: /guides/public-cloud/databases/valkey-connect-php
tutorials:
  - title: Boost your WordPress CMS with caching with Public Cloud Databases for Valkey
    link: /guides/public-cloud/databases/valkey-tuto-wordpress
goFurther:
  title: Saiba mais
  items:
    - title: Visitar a comunidade OVHcloud
      link: https://community.ovhcloud.com/
    - title: Consultar o roadmap e o changelog
      link: https://www.ovhcloud.com/pt/roadmap-changelog/
    - title: "Juntar-se ao Discord"
      link: https://discord.gg/ovhcloud
cta:
  title: "Tem questões? Aceda ao suporte OVHcloud"
  description: Encontre a resposta às suas questões no nosso centro de ajuda.
  actions:
    - text: Aceder ao suporte
      link: https://help.ovhcloud.com/
      theme: brand
---
