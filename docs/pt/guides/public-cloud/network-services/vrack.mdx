...
**Aqui está uma descrição rápida das ações possíveis consoante o método/interface escolhido:** <a id="horizon"></a>
...
Toda a ação que realiza na Área de Cliente OVHcloud recorre às <ApiLink>APIv6 OVHcloud</ApiLink>. 
...
Pode aceder às APIs a partir <ApiLink>da nossa página web</ApiLink> ou criar os seus scripts PHP ou Python para os chamar.
...
### Passo 1: Ativar e gerir um vRack <a id="activation"></a>
...
Será redirecionado para outra página para validar a encomenda; a operação demora alguns minutos.
  ...
  <Tab label="APIv6 OVHcloud">
    **Passo 1: Ativar e gerir um vRack**
    ...
    Aceda às APIv6 OVHcloud seguindo o guia "[Primeiros passos com as API OVHcloud](/guides/manage-and-operate/api/first-steps)".
...
### Passo 2: Criar uma rede privada no vRack <a id="create-pn-in-vrack"></a>
...
No campo **Nome da rede privada**, defina um nome para a sua rede privada.
  ...
  <Tab label="APIv6 OVHcloud">
    Após se ligar à <ApiLink>APIv6 OVHcloud</ApiLink>, execute os seguintes comandos por ordem.
...
<Tabs>
  ...
  <Tab label="APIv6 OVHcloud">
    Uma vez ligado à <ApiLink>APIv6 OVHcloud</ApiLink>, execute os seguintes comandos por ordem.
...
**Adição de uma interface de rede privada** <a id="adding-a-private-interface"></a>
...
### Ordem das interfaces após uma ligação/desligação a quente <a id="interface-order-after-reboot"></a>

:::warning
Ligar ou desligar interfaces de rede (ports) enquanto a instância está em execução (a quente) pode alterar a ordem pela qual o sistema operativo convidado as deteta no reinício seguinte. A interface identificada como `eth0` (ou qualquer outra interface esperada, incluindo a interface pública) pode então mudar. Verifique a nova configuração para evitar problemas de rede inesperados.

Se precisar de reorganizar várias interfaces numa instância em execução:

1. Desligue todas as interfaces que pretende modificar. Desligar a interface "Ext-Net" liberta definitivamente o seu endereço IP público, conforme indicado na secção [Desligar uma interface de rede](#removing-a-private-interface).
2. Volte a ligá-las uma a uma, pela ordem em que pretende que apareçam no sistema operativo convidado.
3. Efetue um *hard reboot* da instância para verificar se a ordem das interfaces corresponde à esperada antes de confiar nela em produção.

:::

### Desligar uma interface de rede <a id="removing-a-private-interface"></a>
