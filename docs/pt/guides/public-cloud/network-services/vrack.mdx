...
Será redirecionado para outra página para validar a encomenda; a operação demora alguns minutos.
    ...
    Para continuar a partir da área de cliente OVHcloud, prossiga no [passo 2: Criar uma rede privada no vRack](#create-pn-in-vrack), separador **A partir da Área de Cliente OVHcloud**.
  </Tab>
...
No campo **Nome da rede privada**, defina um nome para a sua rede privada.
    ...
    **Opções de gateway de rede**
    ...
    - **Atribuir um Gateway e ligar-se à rede privada**: Selecione esta opção se tiver a intenção de criar instâncias com rede privada apenas. Para mais informações, consulte os seguintes guias: [Criar uma rede privada com uma Gateway](/guides/public-cloud/network-services/create-private-network-gateway) e [Criar uma primeira instância Public Cloud e ligar-se a ela](/guides/public-cloud/compute/getting-started).
...
Pode preencher os campos da seguinte forma:
  ...
  <Tab label="A partir da CLI ovhcloud">
...
<Tabs>
  <Tab label="A partir da Área de Cliente OVHcloud">
    Consulte o guia "[Criar uma instância a partir do espaço cliente](/guides/public-cloud/compute/getting-started)". Durante a criação, escolha na etapa 5 um modo de rede e uma rede privada para a sua instância.
...
Selecione a interface a eliminar e valide:
  ...
  <Tab label="A partir da CLI OpenStack">
    ...
    **Adição de uma interface de rede privada**
    ...
    Para ligar uma nova interface, execute o seguinte comando:
...
Para desligar uma interface de rede, identifique primeiro o port Neutron criado.
...
Uma vez identificado o port, execute o seguinte comando:
