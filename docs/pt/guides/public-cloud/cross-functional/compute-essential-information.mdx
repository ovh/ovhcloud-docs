...
Com o seu método de pagamento registado, este servirá para debitar o montante calculado para a sua [faturação no final do mês](/guides/public-cloud/cross-functional/analyze-billing). Este cálculo é feito com base no tempo de utilização de cada recurso, em função do preço dos recursos. Para as instâncias, os preços são geralmente apresentados como uma taxa horária para maior legibilidade, mas a faturação é calculada ao segundo, com uma duração mínima de faturação de 1 minuto.
