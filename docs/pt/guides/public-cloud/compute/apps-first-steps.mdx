...
1. Aceda à <ApiLink>consola API</ApiLink>
2. A seguir, utilize <ApiLink section="/cloud" method="POST" route={"/cloud/project/\{serviceName\}/instance/\{instanceId\}/applicationAccess"}>esta chamada API</ApiLink>
...
|Parâmetro|Descrição|
|---|---|
|serviceName *|Trata-se do seu ID de projeto Public Cloud|
|instânciaId *|Trata-se da UID da sua instância|
...
1. Obtenha o URL de utilização única [seguindo estas etapas](#detalhes-de-ligação-à-aplicação).
...
1. Obtenha o URL de acesso à sua aplicação seguindo [estas etapas](#detalhes-de-ligação-à-aplicação).
2. Clique no URL reenviado pela API.
