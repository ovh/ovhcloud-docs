...
- Dispor de uma instância [Public Cloud](/guides/public-cloud/compute/getting-started) na sua conta OVHcloud.
...
:::info
Cada tipo de volume está igualmente disponível em versão encriptada (**LUKS**). Estes volumes asseguram a confidencialidade dos dados. Estão disponíveis a partir da Área de Cliente OVHcloud, bem como com as ferramentas apresentadas na secção seguinte, especificando o tipo `<volume_type>-luks`.

:::

:::warning
**Impacto da encriptação (LUKS) no desempenho.** Os volumes encriptados têm um impacto no desempenho de cerca de 10% em relação a volumes equivalentes não encriptados.
