...
export interface SanitizeOptions {
  /**
   * Locale prefix of intra-doc links to rewrite, e.g. `en`. Links like
   * `/en/guides/.../x.md` that point inside this product become bare anchors;
   * for v1 we just drop the `.md` and the locale/`/guides` prefix is left as a
   * site-relative URL (pandoc keeps it as an external link). Optional.
   */
  locale?: string;
}
...
export function sanitizeMd(md: string, _opts: SanitizeOptions = {}): string {
  let out = md;
