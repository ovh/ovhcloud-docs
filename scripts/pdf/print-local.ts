/**
 * Local product-PDF preview generator for macOS.
 *
 * Produces a real PDF of a whole product bundle WITHOUT Docker, Homebrew, pandoc
 * or weasyprint — it drives a local Chromium (Playwright-cached build, or an
 * installed Chromium/Chrome under /Applications). PREVIEW ONLY: CI renders the
 * shipped PDFs with pandoc + weasyprint (see build-pdfs.ts), a different engine —
 * use this to check content, order and images, not final styling.
 *
 * Steps:
 *   1. Build the assembled bundle HTML via preview-html.ts (theme-rendered guides
 *      + TOC + print CSS, images inlined as file:// URLs).
 *   2. Drive Chromium to print that HTML to a PDF.
 *
 * Usage:
 ...
 *
 * Bookmarks: the clickable PDF outline uses `playwright-core` (a devDependency;
 * driver only, it never downloads a browser). Without it, the script falls back
 * to Chromium's CLI print — a valid PDF, but no bookmarks and no images (headless
 * Chromium blocks file:// sub-resources outside the driver's HTTP serving mode).
 */
...
/**
 * Find a Chromium executable: cached Playwright build (newest first), then an
 * installed Chromium/Chrome under /Applications.
 */
function findChromium(): string | null {
  ...
  return installed.find((exe) => fs.existsSync(exe)) ?? null;
}
