...
description: "OVHcloud provides a File Storage Service powered by OpenStack Manila. This service offers managed NFS shares on private networks, supporting ReadWriteMany (RWX)"
zendesk_id: 47960500366099
zendesk_hash: ab5933f40a4b7519
content_hash: 014d577988d833cd
updated: 2026-06-18
---
...
**NOTE:** To see instructions for use with a dedicated server, scroll to the [bottom of this article](#to-connect-with-a-dedicated-server).

:::

- You already have a [private network](https://support.us.ovhcloud.com/hc/en-us/articles/360002093130) available in your project.
- A [Public Cloud instance](https://us.ovhcloud.com/public-cloud/) in your OVHcloud account
- An [OpenStack CLI ready environment](https://support.us.ovhcloud.com/hc/en-us/articles/1500009262762)
...
In the left sidebar, go to `Storage` > `File Storage`, then click `+ Create a share`.
...
Retrieve the [private IP address](https://support.us.ovhcloud.com/hc/en-us/articles/360002093130) of your client VM and ensure it is on the same private network as the share.
...
File Storage can only be used with OVHcloud Private IPs (vRack).
...
Retrieve the mount path (displayed as `Mount Path` in the `General information` tab of your share), and edit the path to have the following format: `\\server_ip\shares\share-<share_Id>`.

Create a mount point and mount the share by entering the following command in your client VM:

```
mount.exe <edited_mount_path>
```

On your Windows VM, open the `Server manager`, choose `Add roles and features`, and select your server.

Skip to the `Features` section, *check* `☑️` the **Client for NFS** option, and save your changes.

![windows\_activateNFSclient.PNG](/images/storage-and-backup/file-storage/file-storage-service-getting-started/52200013674899.png)

To verify the mount point, open `File Explorer` to see your share.
...
Retrieve the network ID and subnet ID of your private network:

Both the network ID and subnet ID should have the format: `abc12345-def6-4abc-8def-123456abcdef`.

<Api version="v1" section="/cloud" method="GET" route={"/cloud/project/\{serviceName\}/region/\{regionName\}/network"} regions={["us"]} />
...
<Api version="v1" section="/cloud" method="POST" route={"/cloud/project/\{serviceName\}/region/\{regionName\}/share"} regions={["us"]} />
...
```
{
 "description": "string",
 "name": "string",
 "networkId": "<private_network_networkId>",
 "size": 150,
 "subnetId": "private_network_subnetId",
 "type": "standard-1az"
}
```
...
```
{
    ...
    "createdAt": "2026-05-07T13:19:30.814655Z"
  }
```
...
**NOTE:** You will need the `path` in Step 3.

:::

<Api version="v1" section="/cloud" method="GET" route={"/cloud/project/\{serviceName\}/region/\{regionName\}/share/\{shareId\}"} regions={["us"]} />
...
<Api version="v1" section="/cloud" method="POST" route={"/cloud/project/\{serviceName\}/region/\{regionName\}/share/\{id\}/acl"} regions={["us"]} />
...
<Api version="v1" section="/cloud" method="GET" route={"/cloud/project/\{serviceName\}/region/\{regionName\}/share/\{id\}/acl/\{aclId\}"} regions={["us"]} />
...
**4. Verify the mount and check capacity and usage**
...
The type `standard-1az` uses `driver_handles_share_servers = True`, which means you must attach a share network when creating a share.
...
This export path is used to mount the share on your client VM.
...
This lets you monitor the storage capacity and usage of your NFS share.
...
Prepare your environment to use the OpenStack API by installing python-openstackclient, following [this guide](https://support.us.ovhcloud.com/hc/en-us/articles/1500009262762).
...
To verify RWX functionality, use `kubectl exec` to connect to one pod and create a file in the mounted directory (e.g., `/var/lib/www/`). Then, connect to the second pod and check that the file is visible. If it is, your Manila share exposed through NFS is functioning correctly.
...
At this time, only the share extension is supported — shrinking a volume is not possible.
...
Patch the PVC to request a new size (for example, 280Gi):

```
kubectl patch pvc nfs-share-pvc -p '{ "spec": { "resources": { "requests": { "storage": "280Gi" }}}}'
```
...
- You already have a working [Terraform environment](https://support.us.ovhcloud.com/hc/en-us/articles/15974268257299).
...
This output provides the NFS export path, which client VMs can use to mount the share.
...
### To connect with a dedicated server

To use File Storage Service with a dedicated server, you will follow a similar process; the difference will be that, rather than authorizing a Public Cloud instance to connect to the share, you will authorize your dedicated server.

You will need the following products in your OVHcloud account:
...
- While these steps are for a dedicated server, this process should work with other services, such as VMware.
- Ensure that there are no overlapping IP address used throughout this process.
- If a VLAN is configured, ensure that the same VLAN is used across the involved servers and shares.

:::

1. Add your dedicated server to your vRack (see [more information](https://support.us.ovhcloud.com/hc/en-us/articles/360001410984)).

2. In your Public Cloud project, create a [private network using the same vRack](https://support.us.ovhcloud.com/hc/en-us/articles/360002093130) that your server is on.

   **WARNING:** Your server and private network **must** be on the same vRack.

3. Get the **Network ID** and **Subnet ID** of that private network you just created.

4. Following the instructions [above](#instructions), create a File Storage share using that Network ID and Subnet ID.

5. Following the instructions [above](#instructions), add the IP address of your dedicated server to the share's ACL.
