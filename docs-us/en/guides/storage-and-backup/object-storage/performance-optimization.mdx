---
title: "Object Storage - Optimizing performance"
description: "OVHcloud Object Storage supports byte range fetch. This retrieves an object chunk by chunk, with each chunk defined by a byte range. The main advantage is that"
zendesk_id: 27885987068179
zendesk_hash: 131d60523d859dfb
content_hash: 25bc455a5c6b85ec
updated: 2026-04-27
---

# Object Storage - Optimizing performance

### Using byte range fetch

OVHcloud Object Storage supports byte range fetch. This retrieves an object chunk by chunk, with each chunk defined by a byte range. The main advantage is that you can parallelize GET requests to download an object, with each GET requesting a specific range of bytes: typical sizes for byte range requests are 8 MB or 16 MB, but you can specify any size.

![OS tutorials optimize performance - step 1](/images/storage-and-backup/object-storage/performance-optimization/os-tutorials-optimize-perf-01.png)
...
- An [OVHcloud bucket](/guides/storage-and-backup/object-storage/getting-started) created
...
**Did you know?** When you use a high-level command to upload an object using the `cp` command, the AWS CLI automatically does a multipart upload. To better optimize the default configuration values for multipart uploads (multipart\_threshold, multipart\_chunksize), you can consult [this article](/guides/storage-and-backup/object-storage/getting-started) and see the table explaining how to configure the AWS CLI.
...
**NOTE:** Part numbers can range from 1 to 10,000. You can check the technical limitations [here](/guides/storage-and-backup/object-storage/limitations).
...
```bash
aws s3api complete-multipart-upload --bucket <bucket_name> --key <object_key> --upload-id <upload-id> --multipart-upload file://mpu.json
```
...
The following list describes the options to perform multipart uploads using other tools. The list is **not** exhaustive, and you might want to check the relevant documentation of the tool you are using.

 

#### s3cmd

Size of each chunk of a multipart upload:

```bash
multipart-chunk-size-mb=<size_mb>
```

Files bigger than SIZE are automatically uploaded as multithreaded-multipart, smaller files are uploaded using the traditional method.\
SIZE is in megabytes (MB). The default chunk size is 15MB, with a minimum allowed chunk size of 5MB and a maximum of 5 GB.
...
```bash
rclone copy --s3-upload-concurrency 300 --s3-chunk-size 100M --s3-upload-cutoff 100M <filepath> s3:<bucket_name>
```
...
To customize the default value on the AWS CLI, consult [this guide](/guides/storage-and-backup/object-storage/optimise-the-sending-of-your-files).
...
It is also possible to significantly optimize performance by adopting good practices to distribute the I/Os as widely as possible in the object storage cluster, taking advantage of the sharding mechanism.
...
In OpenIO, a **container** is an internal logical entity that contains all the objects for a given bucket. Each container is associated with an internal metadata database that lists all the addresses in the cluster of the objects contained in it. By default, an S3 bucket is associated with one container, but this can change with the sharding mechanism.

Sharding is the mechanism by which a container is split into two new sub-containers (and thus its associated metadata database is also split into two) when it reaches **a critical number of objects** called **shards**.

Sharding enables:

- optimizing read/write operations by distributing them evenly over several servers (shards).
...
![OS tutorials optimize performance - step 2](/images/storage-and-backup/object-storage/performance-optimization/os-tutorials-optimize-perf-02.png)

 

### Prefix good practices

You can optimize the I/Os on the cluster by taking advantage of the sharding mechanisms described above.
...
#### Scenario 1: Bad practice by using the date as the prefix
...
This solution is not optimal because, since dates are incremental by nature, all new downloads will always be carried out on the second shard, which will be split again when it reaches a critical size. Thus, all future write operations will always be performed on the latest shard created and the previous shards will rarely be accessed. Furthermore, you may experience some throttling during the sharding process.

![OS tutorials optimize performance - step 3](/images/storage-and-backup/object-storage/performance-optimization/os-tutorials-optimize-perf-03.png)
...
Assuming a threshold of 100, after the 100th object is uploaded, the sharding is triggered to split the objects into two shards. This second scenario is optimal because all new uploads will be spread on the two shards.

![OS tutorials optimize performance - step 4](/images/storage-and-backup/object-storage/performance-optimization/os-tutorials-optimize-perf-04.png)
...
To avoid reduced performance (503 SLOWDOWN errors), we recommend that you optimize your uploads by spreading your requests over time. This spread does not have to be linear, but it does have to give us enough time to balance your workload.

A simple way to achieve that lies in better 503 slowdown errors management and error recovery: ramp up your uploads until you hit 503 errors, modulate your workload to accommodate the throttling until sharding is complete, and then ramp up again.
...
When applicable, we recommend that you increase the object/part size as much as possible to reduce the number of objects.
