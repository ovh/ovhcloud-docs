...
description: "By default, all resources (buckets, objects) and sub-resources (lifecycle configuration, website configuration, etc.) are private in Object Storage. Only the re"
zendesk_id: 23885460175123
zendesk_hash: 56a12ab7ccfc52b8
content_hash: 0465f2b4122c8ce4
updated: 2026-04-29
---
...
Access policies attached to a specific user are called user policies. A user policy is evaluated using Object Storage IAM permissions and applies only to the specific user attached to it.
...
ACLs can either be attached at the bucket level or the individual object level.
...
![Object storage tutorial acl - step 1](/images/storage-and-backup/object-storage/bucket-acl/object-storage-tutorial-acl-01.png)

:::warning

**NOTE:** Bucket policies are a feature that is not yet available for Object Storage.
...
- **log delivery group**: this group contains the applicative users used by OVHcloud services to write server access logs inside buckets (read our [Server access logging](/guides/storage-and-backup/object-storage/server-access-logging) guide for more information)
...
| READ\_ACP     | allows read of the bucket ACL                                  | allows read of the object ACL                           |
| WRITE\_ACP    | allows creation/deletion/overwrites of the bucket ACL          | allows creation/deletion/overwrites of the object ACL   |
| FULL\_CONTROL | shorthand for READ, WRITE, READ\_ACP, WRITE\_ACP on the bucket | shorthand for READ, READ\_ACP, WRITE\_ACP on the object |
...
| ------------------------- | --------------------------------------------------------- | ------ | ------ | ------------------------- |
| private                   | default behavior, owner has FULL\_CONTROL                 | x      | x      |                           |
| public-read               | owner has FULL\_CONTROL AllUsers group has READ           | x      | x      |                           |
| public-read-write         | owner has FULL\_CONTROL AllUsers group has READ, WRITE    | x      | x      |                           |
| authenticated-read        | owner has FULL\_CONTROL AuthenticatedUsers group has READ | x      | x      |                           |
| bucket-owner-read         | object owner has FULL\_CONTROL bucket owner has READ      |        | x      | currently not managed yet |
| bucket-owner-full-control | object owner and bucket owner both have FULL\_CONTROL     |        | x      | currently not managed yet |
| log-delivery-write        | LogDelivery group has WRITE, READ\_ACP                    | x      |        |                           |
...
You can set the ACL on a bucket at its creation or afterward, by calling the `put-bucket-acl` endpoint.
...
Here, we change the ACL to give the account user "user-yyyyyyyyyy" the permission to write in the bucket.

Again, to verify that ACLs are set correctly:
...
Similar to bucket level, you can set the ACL on an object at its creation or afterward.
...
In this example, we created an object named `<object_key>` and we gave `FULL_CONTROL` on that object to account user `<user_name>`.
...
Again, to verify that ACLs are set correctly, use the following command:
...
As mentioned before, ACLs can be attached at the object level.
...
We strongly advise you to use policies instead for better and more fine-grained access control over the bucket and its objects. However, for very basic permission control needs, you might consider using bucket ACLs to grant permissions to the objects and ACLs related to the bucket.
