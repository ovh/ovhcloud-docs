...
description: "Object Lock is a feature that allows you to store objects using a Write Once, Read Many (WORM) model and can be used for scenarios where data mustn't be changed"
zendesk_id: 10694926634003
zendesk_hash: 355e7fb81c56a84c
content_hash: 5e0b25292d3a9abb
updated: 2026-04-27
---
...
Object Lock is a feature that allows you to store objects using a **W**rite **O**nce, **R**ead **M**any (WORM) model and can be used for scenarios where data mustn't be changed or deleted after it has been written.
...
**NOTE:** Compliance mode should only be selected if no user, including the administrator, should be able to delete or modify objects during the retention period.
...
Designed for any situation where you are not sure how long you want your objects to stay immutable, Legal hold is an ON/OFF switch that can be applied to every object in a locked container, independently of the lock configuration, object retention, or object age. It can be applied to objects that are locked.
...
See our [Getting Started with Object Storage](/guides/storage-and-backup/object-storage/getting-started) guide for more information.
...
All the following examples use AWS CLI.

To learn more about the AWS CLI, please read this [guide](/guides/storage-and-backup/object-storage/getting-started).
...
*Read&#x20;*[*this guide*](/guides/storage-and-backup/object-storage/identity-and-access-management)*&#x20;to learn more about IAM.*
...
To manage an Object Storage bucket, log in to your [OVHcloud Control Panel](https://manager.us.ovhcloud.com/#/public-cloud/pci/projects/) and select your project.

Click on `Object Storage` in the navigation bar, then click on `Create object container`.
...
Click on `Object Storage` in the navigation bar, then on the `My containers` tab, and then on the `name of your container`.

From the `General Information` tab, click `Configure Retention`, enable retention, and then set the applicable retention mode and period. Next, click the `Save` button.
...
Before placing an object under Legal Hold, ensure that the necessary permissions are granted. The user must have the `s3:PutObjectLegalHold`.
