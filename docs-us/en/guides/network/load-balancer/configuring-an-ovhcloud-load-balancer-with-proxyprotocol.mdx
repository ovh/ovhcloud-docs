...
description: "The OVHcloud Load Balancer acts as a proxy. Like a human proxy, it acts as an intermediary, so the client addresses the proxy and the proxy addresses the servic"
zendesk_id: 29040083217555
zendesk_hash: 626ef582c7c7ff35
content_hash: 7f18b2e663f7345a
updated: 2026-01-12
---
...
If you are using an HTTP frontend, we recommend the guide "[Configuring an OVHcloud Load Balancer service - HTTP headers](https://support.us.ovhcloud.com/hc/en-us/articles/29028259702931)," which describes the standard way to use HTTP headers to retrieve the IP address, port, and source protocol.

 

## Requirements

- An OVHcloud Load Balancer service with a functional TCP frontend and server cluster
- Nginx or Apache with mod\_proxyprotocol on an OVHcloud server

:::info

This is an advanced guide. It assumes you're already familiar with the main features of your OVHcloud Load Balancer service, especially **frontends** and **clusters**. If you're not, we recommend checking out the guide "[Configure an OVHcloud Load Balancer HTTP/HTTPS service](https://support.us.ovhcloud.com/hc/en-us/articles/29029482815507)." While this guide focuses more on TCP services, the general principles are the same.

:::

:::warning

**NOTE:** Since ProxyProtocol fields can be forged by a malicious client, they should only be considered if they come from a trusted source.
...
The list of IPv4 outbound addresses potentially used by your OVHcloud Load Balancer service can be found in the [OVHcloud Control Panel](https://manager.us.ovhcloud.com/) on the homepage of your OVHcloud Load Balancer service under **Outbound IPv4**.

![IPLB\_informalPanel.PNG](/images/network/load-balancer/configuring-an-ovhcloud-load-balancer-with-proxyprotocol/48141286201491.png)
...
You can list the IP addresses used by your OVHcloud Load Balancer service with the following call:
...
ProxyProtocol was developed by the [HAProxy](http://haproxy.org/) Load Balancer team as the TCP counterpart to standard HTTP headers like X-Forwarded-For. It was designed to forward at a minimum:
...
Indeed, this protocol exists in two versions:
...
ProxyProtocol must be enabled for each server registered in a server farm. Since this feature is intrusive and cannot be activated transparently, it allows you to test it on a particular machine and then progressively deploy the configuration to an active farm.

Your OVHcloud Load Balancer service supports four ProxyProtocol modes:
...
In the `Server clusters` section, select the farm containing the server on which to enable ProxyProtocol, then click the edit button for the relevant server.

ProxyProtocol is configured via the `ProxyProtocol version` option. You will find the four modes described above.

![IPLB\_server\_advancedSettings.png](/images/network/load-balancer/configuring-an-ovhcloud-load-balancer-with-proxyprotocol/48141286202899.png)

Once the desired mode is selected, click `Update`, then apply your changes in the relevant zone.
...
Activating ProxyProtocol via the API is done in the same way as from the OVHcloud Control Panel. The corresponding API field in the server is proxyProtocolVersion.
...
4.
5.     # Trust the proxy protocol provided information from your OVHcloud Load Balancer service
...
This example uses the HTTP protocol for simplicity. If you are using HTTP, we strongly recommend using HTTP headers instead of ProxyProtocol, unless your OVHcloud Load Balancer service is configured in TCP. This can happen in the case of SSL termination for HTTP/2, for example.
...
Regardless of the approach chosen, we strongly recommend restricting connections to your servers to the outbound addresses of your OVHcloud Load Balancer service. This can be easily configured with `iptables`:
...
<details>
<summary>With mod-proxy-protocol</summary>
...
<details>
<summary>With mod_remoteip</summary>
