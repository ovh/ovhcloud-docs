...
description: "- To help you understand the distinctions between TCP, HTTP, and TLS frontends on an OVHcloud Load Balancer, enabling you to determine if a TCP frontend is the"
zendesk_id: 19805896005139
zendesk_hash: 6d84284b8371db50
content_hash: aff68f03a5ac59a0
updated: 2026-02-02
---
...
- If a TCP frontend is deemed desirable, to provide step-by-step instructions on how to configure it to effectively balance HTTP/2 traffic across your backend servers.
...
To enable HTTP/2 on existing HTTP and TLS frontends, you must make the following refresh call via the API, where **\{serviceName}** is the internal name of your Load Balancer.
...
Please note that multiple routes are not supported in [Pack 1](https://us.ovhcloud.com/network/load-balancer/).
...
**NOTE:** The order in which you create elements is important: the routes must be configured before you attach them to rules.
...
**NOTE:** The weight parameter allows you to define the evaluation order of your routes; the first one that is validated will be executed.
...
```
curl -I --http2 https://us.ovhcloud.com/
HTTP/2 200
```
