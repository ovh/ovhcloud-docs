...
description: "Velero is an Open-Source tool for safely backing up and restoring, performing disaster recovery, and migrating Kubernetes cluster resources."
zendesk_id: 1500004946921
zendesk_hash: 071ecda584cdd0ba
content_hash: cdfae28d6278855a
updated: 2025-09-23
---

# Backing-up an OVHcloud Managed Kubernetes cluster using Velero

[Velero](https://velero.io/) is an Open-Source tool for safely backing up and restoring, performing disaster recovery, and migrating Kubernetes cluster resources.
...
For Persistent Volumes backup, we are using the [CSI snapshot support for Velero](https://velero.io/docs/v1.8/csi/), which enables Velero to backup and restore CSI-backed volumes using the Kubernetes CSI Snapshot Beta APIs.

 

## Before you begin

This tutorial presupposes that you already have a working OVHcloud Managed Kubernetes cluster and some basic knowledge of how to operate it. If you want to know more about those topics, please look at our [deploying a Hello World application](https://support.us.ovhcloud.com/hc/en-us/articles/1500004910382) documentation.
...
Velero needs an S3™-compatible bucket as a storage backend to store the data from your cluster. In this section, you will create your S3 bucket on OVHcloud Object Storage.
...
- [Get OpenStack RC File v3 from Horizon](https://support.us.ovhcloud.com/hc/en-us/articles/1500009282941)
...
| links      | {u'self': u'https://auth.cloud.ovh.us/v3/users/xxxx/credentials/OS-EC2/xxxx'}
| project_id | xxxx
| secret     | xxxx
| trust_id   | None
| user_id    | xxxx
...
[default]
region = <s3_region>
endpoint_url = https://s3.<s3_region>.io.cloud.ovh.us/
```

:::info

Replace `s3_region` by the Public Cloud Region with no digits.
...
velero install \
  ...
  --backup-location-config region=<s3_region>,s3ForcePathStyle="true",s3Url=https://s3.<s3_region>.io.cloud.ovh.us,checksumAlgorithm="" \
  --snapshot-location-config region=<s3_region>,enableSharedConfig=true
```

:::info

Starting with version 1.14, the plugin-for-csi is integrated into Velero. You can simply remove it from the install example if you install version 1.14 or newer. For upgrading an older version, follow the upgrade notes: [Upgrade-to-1.14](https://velero.io/docs/v1.14/upgrade-to-1.14/). Please refer to those links to check Velero's plugins compatibility: [velero-plugin-for-aws](https://github.com/vmware-tanzu/velero-plugin-for-aws?tab=readme-ov-file#compatibility) and [velero-plugin-for-csi](http://github.com/vmware-tanzu/velero-plugin-for-csi?tab=readme-ov-file#compatibility).

:::

To allow Velero to do Volume Snapshots, we need to deploy a new `VolumeSnapshotClass`. Create a `velero-snapclass.yaml` file with this content:
...
Check that the web server responds as expected:
...
Your namespace with resources and PVC has been correctly restored.
