...
description: "Helm is a package manager for Kubernetes. It works with packages of pre-configured Kubernetes resources, called Helm charts."
zendesk_id: 1500004934541
zendesk_hash: c8e7bc67c3aedf49
content_hash: f8aa36333ac0be46
updated: 2026-02-18
---
...
![Diagram use - Install helm](/images/public-cloud/managed-kubernetes/install-helm/diagram-use.jpg)

A *chart* is a Helm package. Inside the chart, you have all the resource definitions necessary to run an application, tool, or service inside a Kubernetes cluster. It's the Helm equivalent of a Debian pkg for Linux, a Maven file for Java, or a `package.json` for Node.js.
...
When a chart is installed in a Kubernetes cluster, the running instance is called a *release*. Multiple releases of a single chart can be installed at the same time in a cluster (think, for example, several instances of the WordPress chart for several different blog instances running in the cluster).
...
**NOTE:** This guide supposes you're using Helm 3, the latest major version of Helm. The legacy version Helm 2 is considered deprecated. If you want to use Helm 2, please refer to the [official documentation](https://v2.helm.sh/).

:::

Install the Helm CLI on your machine from a package manager:
...
You installed the Helm CLI on your machine; now you can use it to deploy applications in an OVHcloud Managed Kubernetes cluster.
...
Once the repository is added, run `helm repo update` to make sure we get the latest list of charts.
