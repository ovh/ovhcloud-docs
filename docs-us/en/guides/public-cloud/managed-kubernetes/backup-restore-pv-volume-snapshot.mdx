...
description: "Learn how to back up and restore persistent volumes on an OVHcloud Managed Kubernetes cluster using&#x20;Kubernetes Volume Snapshots."
zendesk_id: 13948542930067
zendesk_hash: ff2d16b9bfa6b009
content_hash: 48f4d7dbb2f2c59f
updated: 2026-04-17
---

# Backing up and restoring your Persistent Volume with Volume Snapshots on OVHcloud Managed Kubernetes

**Learn how to back up and restore persistent volumes on an OVHcloud Managed Kubernetes cluster using&#x20;**[**Kubernetes Volume Snapshots**](https://kubernetes.io/docs/concepts/storage/volume-snapshots/)**.**

Volume Snapshots are a Kubernetes feature released in General Availability (GA).
...
The tutorial also assumes that you’re familiar with [Kubernetes Persistent Volumes](https://kubernetes.io/docs/concepts/storage/persistent-volumes/). You also need to know how PVs are handled on the OVHcloud Managed Kubernetes service. Please refer to the [Persistent Volumes on OVHcloud Managed Kubernetes](/guides/public-cloud/managed-kubernetes/set-up-persistent-volume) guide.
...
If you look at the `deployment` part of this manifest, you will see that we have defined a `.spec.strategy.type`. It specifies the strategy used to replace old pods with new ones, and we have set it to `Recreate`, so that all existing pods are killed before new ones are created.\
...
Create a `VolumeSnapshot` in an `nginx-example-snapshot.yml` file with the following content:
...
Then, create an `nginx-example-restore.yml` file with the following content:
...
The volume should have a status of `Bound`. Now you can restore the deployment to its replica value of 1, and wait until the pod is `Running` again:
...
Now you can verify that the `access.log` file is back and that its content is still there:
...
You can proceed to clean up by deleting everything. Delete the `nginx-example` namespace:
