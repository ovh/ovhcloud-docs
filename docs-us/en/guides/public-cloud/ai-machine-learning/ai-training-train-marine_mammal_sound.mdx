...
description: "Learn how to train a model with AI Training to classify sounds."
zendesk_id: 37646100448275
zendesk_hash: 9493c8c4596eea00
content_hash: 1e1d032566f7040b
updated: 2025-06-05
---

# AI Training - Tutorial - Train a model to recognize marine mammal sounds

**Learn how to train a model with AI Training to classify sounds.**

This is the next step after you have designed the model with AI Notebooks. You can see the Notebook step in the tutorial: [Audio analysis and classification with AI](/guides/public-cloud/ai-machine-learning/ai-notebooks-marine-mammal-sounds-classification).
...
- The ovhai CLI interface installed on your system (more information [here](/guides/public-cloud/ai-machine-learning/cli-install-client))
- [Docker](https://www.docker.com/get-started) installed and configured to build images.
- An OCI / Docker image registry. You can use a public registry (such as [Docker Hub](https://hub.docker.com/) for example) or a private registry. Refer to the [Creating a private registry](/guides/public-cloud/managed-private-registry/create-private-registry) documentation to create a private registry based on Harbor.
...
To train the model you'll need data and a place where to save the trained model. You can reuse the previous object storage used in the Notebook tutorial [Audio analysis and classification with AI](/guides/public-cloud/ai-machine-learning/ai-notebooks-marine-mammal-sounds-classification) or follow the step [Uploading your dataset on Public Cloud Storage](/guides/public-cloud/ai-machine-learning/ai-notebooks-marine-mammal-sounds-classification#uploading-your-dataset-on-public-cloud-storage) of this same tutorial.
...
First, you need to create a Python script that is in charge of doing the training. You can copy and paste the following code in a file named `train-audio-classification.py`:
...
########################################################################################################################################################
# The goal of this script is to train a pre-construct model to recognize marine mammal sounds.                                                         #
...
Then, create a Dockerfile compliant with AI Training. You can copy and paste the following code in a file named `Dockerfile`:
...
```bash
$ docker build . -f Dockerfile -t my-registry.c1.va1.container-registry.ovh.us/marine-mammal-job:1.0.0
...
 => => naming to my-registry.c1.va1.container-registry.ovh.us/marine-mammal-job:1.0.0

$ docker push my-registry.c1.va1.container-registry.ovh.us/marine-mammal-job:1.0.0                                                     
The push refers to repository [my-registry.c1.va1.container-registry.ovh.us/marine-mammal-job]
6e5b7acfda9e: Pushed 
..
1.0.0: digest: sha256:72f19493662aafe3d0a3dc35ea5ab76b8472bd6a709de2da1a52e7ebf8ab7ad1 size: 3054 
```

Once your Docker image is created and pushed into the registry, you can directly use the `ovhai` command to create your model training. You can launch the training specifying more or less GPU depending on the speed you want for your training.

:::info

If your images are stored in a private registry, please follow the documentation [Registries - Use & manage your registries](/guides/public-cloud/ai-machine-learning/manage-registries) to add your registry.
...
ovhai job run \
    ...
    --volume marine-mammal-sounds@US-EAST-VA/csv/:/workspace/data:RO:cache \
    <registry name>/marine-mammal-job:1.0.0
```
...
$ ovhai job run \
        ...
        --volume marine-mammal-sounds@US-EAST-VA/csv/:/workspace/data:RO:cache \
        --unsecure-http \
        my-registry.c1.va1.container-registry.ovh.us/marine-mammal-job:1.0.0
...
User:       my-user
Spec:
  Image:                my-registry.c1.va1.container-registry.ovh.us/marine-mammal-job:1.0.0
 Command:              
  Env Vars:             ~
...
Status:
  ...
  Data Sync:       ~
  Duration:        0s
  Url:             https://c0c0878c-5564-4660-889a-65724f6e3056.job.us-east-va.ai.cloud.ovh.us
  Info Url:        https://ui.us-east-va.ai.cloud.ovh.us/job/c0c0878c-5564-4660-889a-65724f6e3056
  Ssh Url:         ~
  Monitoring Url:  ~
  Volumes:
  - Mount Path:     /workspace/saved_model
...
For more explanations about the CLI command for AI Training, please read this guide: [CLI Reference](/guides/public-cloud/ai-machine-learning/cli-overview).
