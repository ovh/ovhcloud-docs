...
description: "Learn how to deploy the Speech to Text Application created in this blog article using Streamlit and pre-trained models."
zendesk_id: 37306727494163
zendesk_hash: 758b679812f5ecc9
content_hash: 134083b9ca218c74
updated: 2025-06-27
---

# AI Deploy - Tutorial - Create and deploy a Speech to Text application using Streamlit

**Learn how to deploy the Speech to Text Application created in this [blog article](https://blog.ovhcloud.com/how-to-build-a-speech-to-text-application-with-python-1-3/) using [Streamlit](https://streamlit.io/) and pre-trained models.**
...
![PaaS\_AI\_deploy\_speechToTextStreamlit01.png](/images/public-cloud/ai-machine-learning/ai-deploy-tutorial-create-and-deploy-a-speech-to-text-application-using-streamli/37307503088147.png)

Overview of the Speech-to-Text app:

![PaaS\_AI\_deploy\_speechToTextStreamlit02.png](/images/public-cloud/ai-machine-learning/ai-deploy-tutorial-create-and-deploy-a-speech-to-text-application-using-streamli/37307518690835.png)
...
- [The OVHcloud AI CLI](https://cli.us-east-va.ai.cloud.ovh.us/) **and** [Docker](https://www.docker.com/get-started) installed on your local computer, **or** only access to a Debian Docker Instance on the [Public Cloud](https://us.ovhcloud.com/manager/)
- To deploy your app, you must have the full code of the application, either by cloning the [GitHub repository](https://github.com/ovh/ai-training-examples/tree/main/apps/streamlit/speech-to-text) or by following our [blog article](https://blog.ovhcloud.com/how-to-build-a-speech-to-text-application-with-python-1-3/) that taught you how to build this app step by step
- If you want the diarization option (speaker differentiation), you will need an access token. This token will be requested at the launch of the application. To create your token, follow the steps indicated on the [model page](https://huggingface.co/pyannote/speaker-diarization). If the token is not specified, the application will be launched without this feature.
...
We can now install our needed system packages. To do this, use `apt-get`, which is a command-line tool that helps in handling packages:
...
Finally, create a `data` folder which will temporarily store users' audio files until they are transcribed. Then, give the correct access rights to the **OVHcloud user** (`42420:42420`):
...
The `-t` argument allows you to choose the identifier to give to your image. Usually, image identifiers are composed of a **name** and a **version tag** `<name>:<version>`. For this example, we chose **streamlit\_app:latest**.
...
To do this, we will use **AI Training**. This will allow us to launch a Python script from *GitHub* that will **download the models and store them in an OVHcloud volume** named `speech_to_text_app_models`. When the models will be downloaded and added to this volume, the status of the job will automatically switch from `Running` to `Done` and the **job will be immediately stopped**. This operation should be quite fast.
...
ovhai job run <shared-registry-address>/streamlit_app:latest \
    --cpu 12 \
    --volume speech_to_text_app_models@US-EAST-VA/:/workspace/speech_to_text_app_models:RW \
...
When you run this command, an `Info url` will appear. Opening it will allow you to **track the status of the job**. Once the *GitHub* repository is recovered, the Python script will be launched and the job status will switch to `Running`. Then, you just have to wait for the job to end.
...
ovhai app run \
      ...
      --cpu 6 \
      --volume speech_to_text_app_models@US-EAST-VA/:/workspace/models:RO \
...
`--cpu 6` indicates that we request 6 CPUs for our app. If you would like to deploy with GPUs, certain changes will need to be made in your `app.py` file to ensure driver compatibility.
