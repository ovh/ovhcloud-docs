...
description: "Learn about the technical capabilities and limitations of the Public Cloud AI Deploy offer."
zendesk_id: 35408540184339
zendesk_hash: 24ac55cce3b54f65
content_hash: b70689596ee14b9a
updated: 2026-08-26
---
...
**Learn about the technical capabilities and limitations of the Public Cloud AI Deploy offer.**
...
| ------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **AI environments**                        |                                                                                                                                                                                                                                          |
| Easy customization                         | AI Deploy allows the installation of almost any Conda or Pip packages. You can easily customize your environment to suit your needs. It also allows you to use your own environment by specifying your Docker image.                     |
| **Management**                             |                                                                                                                                                                                                                                          |
...
| **Compute resources**                      |                                                                                                                                                                                                                                          |
| Guaranteed compute resources               | Select the number of CPUs or GPUs required during the creation of the AI Deploy app. Once launched, you will keep these resources as long as your app is running.                                                                        |
...
| Monitoring tools                           | Each AI Deploy app comes with a native Grafana dashboard, allowing you to track and monitor your CPU, GPU, RAM, and storage resources.                                                                                                   |
| **Storage**                                |                                                                                                                                                                                                                                          |
...
AI Deploy is compliant with the OVHcloud AI CLI. Discover how to [install the OVHcloud AI CLI](https://support.us.ovhcloud.com/hc/en-us/articles/34692499017619).
...
To see information about your app, you can do so with the `ovhai` CLI using this command:
...
You are also able to check it from the [OVHcloud Control Panel](https://us.ovhcloud.com/manager/) in your app information by clicking the `Go to Graph Dashboard` button.
...
<Api version="v1" section="/cloud" method="GET" route={"/cloud/project/\{serviceName\}/ai/app"} regions={["us"]} />

 

### Planned features

We continuously improve our offers. You can follow, vote, and submit ideas to add to [our roadmap](https://github.com/orgs/ovh/projects/16/views/1?pane=info\&sliceBy%5Bvalue%5D=AI+Deploy).
...
You can either choose the number of `GPUs` or `CPUs` for an AI Deploy app, not both. By default, an app uses one CPU instance. The memory resource is not customizable.
...
The maximum number of CPUs/GPUs, memory per CPU/GPU, and local storage is available on the [OVHcloud website](https://us.ovhcloud.com/public-cloud/prices/#ai-&-machine-learning), Control Panel and the `ovhai` CLI.
...
AI Deploy offers the possibility to choose between two [scaling strategies](https://support.us.ovhcloud.com/hc/en-us/articles/35570653416211): **static** or **automatic**.
...
Each AI Deploy app comes with a local storage space, which is ephemeral. When you delete your app, this storage space is also deleted. This storage space depends on the selected instances during AI Deploy app deployment. Please refer to the compute resources section for more information.
...
You can attach data volumes from Public Cloud Object Storage. The Object Storage bucket should be in the same region as your AI Deploy app. Attached storage allows you to work on several TB of data while being persistent when you delete your AI Deploy app.
...
AI Deploy authorizes the deployment of your own Docker images and applications from the [OVHcloud portfolio](https://support.us.ovhcloud.com/hc/en-us/articles/35572480827155).

Docker images can be hosted in a public or private registry.

The use of `docker-compose` is not possible.

Please be aware that images need to be built with an **AMD architecture**. For more information, see [this guide](https://support.us.ovhcloud.com/hc/en-us/articles/36029131111955).
