...
description: "Learn how to build your own Docker image for AI Deploy. After detailing major guidelines, we will cover a quick example."
zendesk_id: 37138294111251
zendesk_hash: 66db86ad83b85b0b
content_hash: 40af48645d7dd604
updated: 2025-06-27
---
...
![PaaS\_AI\_deploy\_customDocker01.png](/images/public-cloud/ai-machine-learning/ai-deploy-tutorial-build-use-a-custom-docker-image/37140912090259.png)
...
When you invoke a build, you can set the `--platform` flag to specify the target platform for the build output, `linux/amd64`. This is especially relevant if you use newer Apple computers (M1/M2/... chipsets) or ARM-based computers.
...
# Install a few Python requirements, such as pandas, scikit-learn, taipy...
...
If you need to write data somewhere, e.g., output from your AI model (generated images), your code application should include a storage connection. For example, you can use the Python `Boto3` library when using Python and S3™ compatible Object Storage.
...
Inside your Dockerfile, you will need to expose your model or application so anyone can use it. The easiest way is to expose the API via REST endpoint. The most popular open source frameworks for exposing APIs are [Flask](https://flask.palletsprojects.com/en/2.3.x/) and [Fast API](https://fastapi.tiangolo.com/).

You can find a basic example in the section below, and more advanced tutorials in our [AI Deploy documentation](https://support.us.ovhcloud.com/hc/en-us/sections/34501047157395-AI-Deploy).
...
AI Deploy is fully compliant with multiple frontend frameworks, such as [Streamlit](https://streamlit.io/), [Gradio](https://gradio.app/), or [Taipy](https://www.taipy.io/). You can also build your own frontend with your favorite tools, such as HTML/CSS.
...
```console
# Start from the official Python image since we don't need GPU
FROM python:3.9
...
Example: If you want to push an image named `custom-image` inside a registry `my-registry.us-east-va.ai.cloud.ovh.us`:

```console
# Add a new registry into OVHcloud AI Tools
ovhai registry add my-registry.us-east-va.ai.cloud.ovh.us

docker login -u <registry-user> -p <registry-password> my-registry.us-east-va.ai.cloud.ovh.us
docker tag custom-image:latest my-registry.us-east-va.ai.cloud.ovh.us/custom-image:latest
docker push my-registry.us-east-va.ai.cloud.ovh.us/custom-image:latest
```

If you want to know the exact commands to push on the shared registry, navigate to the `AI Dashboard` and click the `Docker registries` tab.

![manager\_aiDashboard\_dockerTab.png](/images/public-cloud/ai-machine-learning/ai-deploy-tutorial-build-use-a-custom-docker-image/41947836921619.png)

From there, scroll down to the **Shared Docker Registries** area.

![manager\_aiDashboard\_dockerTab\_sharedRepo.png](/images/public-cloud/ai-machine-learning/ai-deploy-tutorial-build-use-a-custom-docker-image/40427615809171.png)
