...
description: "Learn how to create application tokens for AI Training."
zendesk_id: 34761257426707
zendesk_hash: f96a399a47ccfad9
content_hash: df1df0fc94cb864d
updated: 2025-12-08
---
...
- A working `ovhai` CLI (learn more [here](/guides/public-cloud/ai-machine-learning/cli-install-client))
...
The tokens can be scoped to apply only to jobs matching a specific label. Upon creating a token simply provide a `name=value` pair and when running a new job add the label to the job to be accessible `ovhai job run -l name=value`. If you create a new job using an app token the `label-selector` is automatically added to the created job so that it falls within the token scope.
