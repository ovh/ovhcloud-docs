...
description: "Learn about the basic commands needed to manipulate your data on object storage through the ovhai CLI."
zendesk_id: 34772059068435
zendesk_hash: ce6be60207e2b914
content_hash: 6bff74940ac52f0a
updated: 2025-02-20
---
...
- [Listing available regions of data stores](#listing-available-regions-of-data-stores)
- [Adding new data stores](#adding-new-data-stores)
- [Listing containers and objects](#listing-containers-and-objects)
...
At this time, **S3™ \* compatible and Git data stores are supported**. To know more about S3™ compatible storage, please read [this documentation](/guides/public-cloud/ai-machine-learning/compliance).
...
Use this command to list all objects for container `container-1` in region `US-EAST-VA`:

```console
ovhai bucket object list container-1@US-EAST-VA
```
...
Use this command to list only the objects in the container `container-1`, region `US-EAST-VA` that begin with prefix `seg-01`:

```console
ovhai bucket object list container-1@US-EAST-VA --prefix seg-01
```
...
Use this command to upload the local file (or directory) `/tmp/directory/file1` into the container `my-container` in region `US-EAST-VA` while keeping the full path as object name:

```console
ovhai bucket object upload my-container@US-EAST-VA tmp/directory/file1
```
...
Use this command to upload the local file (or directory) `/tmp/directory/file1` into the container `my-container` in region `US-EAST-VA` while keeping only the file name as object name:

```console
ovhai bucket object upload my-container@US-EAST-VA tmp/directory/file1 --remove-prefix tmp/directory/
```
...
Use this command to upload the local file (or directory) `/tmp/directory/file1` into the container `my-container` in region `US-EAST-VA` while adding a prefix to the full path as object name:

```console
ovhai bucket object upload my-container@US-EAST-VA tmp/directory/file1 --add-prefix /root/
```
...
Use this command to download the whole container `my-container` in region `US-EAST-VA`:

```console
ovhai bucket object download my-container@US-EAST-VA
```
...
Use this command to download all the objects from `my-container` in region `US-EAST-VA` matching the prefix `tmp/directory/`:

```console
ovhai bucket object download my-container@US-EAST-VA --prefix tmp/directory/
```
...
Use this command to delete all the content of a container named `my-container`, in region `US-EAST-VA`, **without** deleting the container:

```console
ovhai bucket object delete my-container@US-EAST-VA --all
```
...
Use this command to delete a single object named `file1` inside the container `my-container` of region `US-EAST-VA`:

```console
ovhai bucket object delete my-container@US-EAST-VA file1
```
...
```console
ovhai bucket delete US-EAST-VA my-container
```
...
Use this command to copy a single object `path/to/object` in a container `my-container` in region `US-EAST-VA`:

```console
ovhai bucket object copy my-container@US-EAST-VA path/to/object new/path/to/object
```
...
```console
ovhai bucket object copy source-container@US-EAST-VA --container dest-container path/to/object new/path/to/object
```
...
```console
ovhai bucket object copy source-container@US-EAST-VA --container dest-container --prefix path/to/ new/path/to/
```
...
```console
ovhai bucket object move my-container@US-EAST-VA path/to/object new/path/to/object
```
...
```console
ovhai bucket object move my-container@US-EAST-VA --prefix path/to/ new/path/to/
```
