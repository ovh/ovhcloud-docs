---
title: "AI Training - Tutorial - Build and use a custom Docker image"
description: "Learn the process of building your own job image for specific needs."
zendesk_id: 36029131111955
zendesk_hash: b8d5a77b3b23fcc4
content_hash: 2369885481c6752d
updated: 2025-06-27
---

# AI Training - Tutorial - Build and use a custom Docker image

**Learn the process of building your own job image for specific needs.**
...
Your AI Training job **has to be containerized** inside a Docker image. Containers provide isolation but also flexibility. The Docker images that you build can be used locally, with OVHcloud AI Training, but also with other cloud providers.
...
AI Training accepts images from **public** or **private** repositories. Find more information about using public and private registries [in this documentation](/guides/public-cloud/ai-machine-learning/manage-registries).
...
Before continuing, please make sure you are in the root directory of your project, containing job files (Python files, requirements.txt, etc.).

Once you are in it, create a new file and name it `Dockerfile` by using a text editor (vi, vim, nano, etc.).
...
There are many official images available on [Docker Hub](https://hub.docker.com/). For example, we could use the basic [Python images](https://hub.docker.com/_/python) which come in many versions and flavors (alpine, slim, etc.), each designed for a specific use case.
...
**NOTE:** If you want to be able to use **GPU** hardware in your **AI Training jobs**, your base image should have **cuda drivers installed on**.
...
**NOTE:** Declaring the `/workspace` directory as the `HOME` environment variable is necessary to ensure compatibility with AI Training.
...
**NOTE:** This command could allow you to load your dataset within your Docker image. However, this would increase its size considerably. A best practice is to put data outside, such as [Object Storage](https://us.ovhcloud.com/public-cloud/object-storage/) then link it to the AI Training job during its launch.
...
As mentioned, the home directory for the "OVHcloud" user (with UID 42420) will be `/workspace`. We can then replace `<specific-directory>` by `/workspace`. Keep in mind that you can change the ownership of any other useful directory.

:::warning

**NOTE:** Depending on the models and frameworks you use, other folders may be created during training at the root of your environment. This can be the case for folders like `/runs` or `/logs`. In this case, do not forget to give access rights to these folders as well: `RUN chown -R 42420:42420 /runs /logs`.
...
Here is an example of a `requirements.txt` file. As you can see, we have indicated each required library of our project. Please replace `<version>` with the applicable version of each library, e.g., `1.2.3`:
...
**NOTE:** Don't forget the `--user=42420:42420` argument if you want to simulate the exact same behavior that will occur on **AI Training jobs**. It executes the docker container as the specific OVHcloud user (user **42420:42420**).
...
Pushing our image to a registry is needed for AI Training to pull it.

AI Training provides, for each Public Cloud project, a default registry called **shared registry** where all users of the same Public Cloud project can push their custom images.

:::warning

This shared registry can help you perform your tests, but **should not be used in production**, as we reserve the right to delete its content if deemed necessary. The images pushed to this registry are for AI Tools workloads only, and will not be accessible for external uses. This is why it can be interesting to add other registries. Learn how to do it by following [this documentation](/guides/public-cloud/ai-machine-learning/manage-registries).
...
For example, if we want to push the first version of our built image named `cnn_image_segmentation_project` inside a registry `registry.us-east-va.ai.cloud.ovh.us`, we would use:

```console
    docker login -u <registry-user> -p <registry-password> my-registry.ai.cloud.ovh.us
    docker tag cnn_image_segmentation_project:v1.0.0 my-registry.ai.cloud.ovh.us/cnn_image_segmentation_project:v1.0.0
    docker push my-registry.ai.cloud.ovh.us/cnn_image_segmentation_project:v1.0.0
```

If you want to know the exact commands to push on your shared registry, please navigate to the `Docker registries` tab of the `AI Dashboard` and scroll down.

![Manager AI dashboard docker tab](/images/public-cloud/ai-machine-learning/ai-training-build-use-custom-image/manager-ai-dashboard-docker-tab.png)
