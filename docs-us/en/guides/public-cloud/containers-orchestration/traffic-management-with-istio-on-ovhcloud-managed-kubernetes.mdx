...
zendesk_id: 1500004944661
zendesk_hash: 88e146d8557b36e9
content_hash: af91199ff6be8730
updated: 2026-02-19
---
...
It also supposes you have a basic knowledge of Istio and that you have installed it in your Kubernetes cluster. If it isn’t the case, please follow [Istio's tutorial](https://istio.io/latest/docs/setup/install/istioctl/). We are going to use the [Bookinfo](https://istio.io/docs/examples/bookinfo/) example application, as we did in that tutorial, so if you haven’t installed it, please do so now.
...
Then wait a few moments for the destination rules to propagate and verify them:
...
[A/B Testing](https://en.wikipedia.org/wiki/A/B_testing) is used when we want to try two different versions of an application and compare user interaction and engagement to choose the best one. It requires being able to push the two versions into production at the same time, split traffic between the two versions, and collect metrics to be able to make an informed choice.

A/B Testing used to be a difficult problem with traditional deployment methods, and it’s very hard to do it directly in Kubernetes since there is no notion of versions, but Istio makes it rather simple.

Let’s use the Bookinfo application to show how easily you can do A/B Testing on Kubernetes with Istio. The Bookinfo application is composed of four separate microservices:

- `productpage`: it calls the `reviews` and `details` service and builds the page
- `reviews`: it contains book reviews, and it calls the `rating` service
- `details`: it contains the book information
- `ratings`: it contains the book rating information

To set up A/B Testing on Bookinfo, we are using the `reviews` microservice, as it has three versions:
...
![bookinfo-architecture.png](/images/public-cloud/containers-orchestration/traffic-management-with-istio-on-ovhcloud-managed-kubernetes/1500011256381.png)
...
Now, on the `/productpage` of the Bookinfo app (accessing through the `http://$GATEWAY_URL/productpage` URL, at every refresh, you will see that stars change alternatively between back (`v2`) and red (`v3`).

![ab-testing-001.png](/images/public-cloud/containers-orchestration/traffic-management-with-istio-on-ovhcloud-managed-kubernetes/1500011004062.png)

 

### Canary Testing with Istio

Similar to A/B Testing, [Canary Testing](https://whatis.techtarget.com/definition/canary-canary-testing) is pushing a new version of a service to a small group of users. The idea is to test the new release in a reduced number of real-world users, to quickly discover potential bugs while minimizing the number of users impacted.
...
![Canary Release v1 to v2 progressing release](/images/public-cloud/containers-orchestration/traffic-management-with-istio-on-ovhcloud-managed-kubernetes/public-cloud-containers-orchestration-managed-kubernetes-traffic-management-with.png)
...
![canary-testing-001.png](/images/public-cloud/containers-orchestration/traffic-management-with-istio-on-ovhcloud-managed-kubernetes/1500011004122.png)
...
![canary-testing-002.png](/images/public-cloud/containers-orchestration/traffic-management-with-istio-on-ovhcloud-managed-kubernetes/1500011256841.png)
...
![rolling-deployment-001.png](/images/public-cloud/containers-orchestration/traffic-management-with-istio-on-ovhcloud-managed-kubernetes/1500011256981.png)

Doing a Rolling Deployment on Istio is rather simple; you can take as a base the examples of Canary Testing and A/B Testing.

Let’s apply the `reviews-all-v1.yaml` file again to redirect all the `reviews` traffic to the `v1`:

```
kubectl -n istio-apps apply -f reviews-all-v1.yaml
```

The `v1` is then our initial release. Let’s say we want to ship `v2` as a new release, but using Rolling Deployment to be sure there is no downtime. The easiest way would be pass by an intermediate state where traffic is split 50%-50% between `v1` and `v2`:
...
![rolling-deployment-002.png](/images/public-cloud/containers-orchestration/traffic-management-with-istio-on-ovhcloud-managed-kubernetes/1500011004422.png)
...
![rolling-deployment-003.png](/images/public-cloud/containers-orchestration/traffic-management-with-istio-on-ovhcloud-managed-kubernetes/1500011257061.png)

 

### What’s next?

Now that you have seen some of the traffic management capabilities of Istio, you can explore other [examples of Istio traffic management](https://istio.io/docs/tasks/traffic-management/): [fault injection](https://istio.io/docs/tasks/traffic-management/fault-injection/), [circuit breaking](https://istio.io/docs/tasks/traffic-management/circuit-breaking/), [mirroring](https://istio.io/docs/tasks/traffic-management/mirroring/)…
