---
title: "How to Configure the OIDC Provider on an OVHcloud Managed Kubernetes Cluster"
description: "The OVHcloud Managed Kubernetes service provides you Kubernetes clusters without the hassle of installing or operating them."
zendesk_id: 13983123066003
zendesk_hash: c891634ad42fbdb8
content_hash: edb54c6dd44648ed
updated: 2023-02-17
---
...
This guide will cover the features that allow you to secure your OVHcloud Managed Kubernetes cluster with OpenID Connect (OIDC).
...
**Warning:** Several elementary validations are made (e.g., on provider URL, CA content, and required claims), but if you misconfigure your OIDC provider, errors will be logged by the Kubernetes API Server, and will only be visible through the cluster’s audit logs in the OVHcloud Control Panel.

## Instructions

### Configure OIDC provider through the OVHcloud Control Panel

Log into the [OVHcloud Control Panel](https://us.ovhcloud.com/manager/). Go to the `Public Cloud` section, and select the Public Cloud project concerned.
...
![oidc1.png](/images/public-cloud/containers-orchestration/how-to-configure-the-oidc-provider-on-an-ovhcloud-managed-kubernetes-cluster/14002588399891.png)

Click on your Kubernetes cluster.

![oidc2.png](/images/public-cloud/containers-orchestration/how-to-configure-the-oidc-provider-on-an-ovhcloud-managed-kubernetes-cluster/14002603264787.png)

Click the `...` button in the `OIDC provider` section.

![oidc3.png](/images/public-cloud/containers-orchestration/how-to-configure-the-oidc-provider-on-an-ovhcloud-managed-kubernetes-cluster/14002603375891.png)

Enter the required information into the popup (note: each field corresponds to a flag from the [API Server configuration documentation](https://kubernetes.io/docs/reference/access-authn-authz/authentication/#configuring-the-api-server):
...
To simplify things, we are using the [API Explorer](https://api.us.ovhcloud.com/) which allows us to explore, learn and interact with the API.

Log into the API Explorer using your OVHcloud credentials.

![oidc4.png](/images/public-cloud/containers-orchestration/how-to-configure-the-oidc-provider-on-an-ovhcloud-managed-kubernetes-cluster/14002603343891.png)

If you go to the [Kubernetes section](https://api.us.ovhcloud.com/console/#/kube) of the API Explorer, you will see the available endpoints:

![oidc5.png](/images/public-cloud/containers-orchestration/how-to-configure-the-oidc-provider-on-an-ovhcloud-managed-kubernetes-cluster/14002603427859.png)

#### API endpoints

- Get an existing cluster’s OIDC configuration:

<Api version="v1" section="/cloud/project" method="GET" route={"/cloud/project/\{serviceName\}/kube/\{kubeID\}/openIdConnect"} regions={["us"]} />
...
<Api version="v1" section="/cloud/project" method="PUT" route={"/cloud/project/\{serviceName\}/kube/\{kubeID\}/openIdConnect"} regions={["us"]} />
...
<Api version="v1" section="/cloud/project" method="POST" route={"/cloud/project/\{serviceName\}/kube/\{kubeID\}/openIdConnect"} regions={["us"]} />
...
<Api version="v1" section="/cloud/project" method="DELETE" route={"/cloud/project/\{serviceName\}/kube/\{kubeID\}/openIdConnect"} regions={["us"]} />

### Configure OIDC provider through Terraform

Since version 0.23+ of our [OVHcloud Terraform provider](https://registry.terraform.io/providers/ovh/ovh/latest/docs), you can also add IP restrictions through Terraform.

#### Getting your cluster/API tokens information

The “OVHcloud provider” needs to be configured with a set of credentials:
...
In order to retrieve this information, please follow our [First Steps with the OVHcloud APIs](https://support.us.ovhcloud.com/hc/en-us/articles/360018130839) tutorial.
...
![oidc6.png](/images/public-cloud/containers-orchestration/how-to-configure-the-oidc-provider-on-an-ovhcloud-managed-kubernetes-cluster/14002588578067.png)
...
![oidc7.png](/images/public-cloud/containers-orchestration/how-to-configure-the-oidc-provider-on-an-ovhcloud-managed-kubernetes-cluster/14002603539475.png)
...
Now, define the resources you want to create in a new file called `ovh_kube_cluster_oidc.tf`:
...
**Note:&#x20;**&#x44;on’t forget to replace `<service_name>`, `<cluster_id>`, and all the other information with the real data.
...
It’s the first command to execute for a new configuration, or it can be done after doing a checkout of an existing configuration in a given git repository.
