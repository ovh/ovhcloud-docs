...
description: "Learn how to use Stash to back up and restore persistent volumes on an OVHcloud Managed Kubernetes cluster."
zendesk_id: 13937115708691
zendesk_hash: 2ba3d6928328d552
content_hash: dcd42125c2b11708
updated: 2025-01-23
---

# Backing Up Persistent Volumes Using Stash

**Learn how to use [Stash](https://stash.run/) to back up and restore persistent volumes on an OVHcloud Managed Kubernetes cluster.**
...
We are using our Public Cloud’s Swift Object Storage with the Swift S3 API as the storage backend for Stash. Stash uses the Amazon S3® protocol to store the cluster backups on an S3™ \* compatible object storage.
...
You also need to have [Helm](https://docs.helm.sh/) installer on your workstation and your cluster, please refer to the [How to Install Helm on OVHcloud Managed Kubernetes Service](https://support.us.ovhcloud.com/hc/en-us/articles/1500004934541) tutorial.

 

## Create the Object Storage bucket for Stash

Stash needs an Object Storage bucket as a storage backend to store your cluster's data. In this section, you will create your Object Storage bucket on Swift.

### Prepare your working environment

Before creating your Object Storage bucket you need to:

- [Prepare your environment to use the OpenStack API](https://support.us.ovhcloud.com/hc/en-us/articles/1500009262762)

- [Get OpenStack RC File v3 from Horizon](https://support.us.ovhcloud.com/hc/en-us/articles/1500009262762)
...
Save the **access** and **secret** parameters from the results.
...
endpoint_url = https://s3.<public cloud region without digit>.cloud.ovh.us
s3 =
...
You should receive your license by email. Save it as you will need it in the `helm install` command.
...
As suggested during the chart install, to check if Stash operator pods have started, we can run the suggested command and customize it to see only our stash Pod:
...
Stash provides a CLI using kubectl plugin to work with the stash `Objects` quickly. Download pre-built binaries from [stashed/cli](https://github.com/stashed/cli/releases) Github release and put the binary to some directory in your `PATH`.
...
In Kubernetes, a [Volume Snapshot](https://kubernetes.io/docs/concepts/storage/volume-snapshots/) represents a snapshot of a volume on a storage system. It was introduced as an Alpha feature in Kubernetes v1.12 and has been promoted to a Beta feature in Kubernetes 1.17.

In order to support `VolumeSnapshot`, your `PersistentVolumes` need to use a StorageClass with a [CSI](https://github.com/container-storage-interface/spec) driver that supports the feature. Currently, OVHcloud Managed Kubernetes cluster propose you two of these: `StorageClasses`, `csi-cinder-classic`, and `csi-cinder-high-speed`.
...
For this example, we will use a small Nginx web server with a `PersistentVolume` to store the access logs.
...
Create a `repository.yaml` file, replacing `<public cloud region>` with the region, without digits and in lowercase (e.g., `us-east-va-1`):
...
spec:
  backend:
    s3:
      endpoint: s3.<public cloud region>.cloud.ovh.us
      bucket: s3-stash
...
If everything goes well, Stash will create a `CronJob` to take periodic snapshot of `nginx-logs` volume of the deployment with the schedule specified in `spec.schedule` field of `BackupConfiguration` CRD (a backup every five minutes).
...
Wait for the next backup schedule. Run the following command to watch `BackupSession` CRD:
...
The snapshots are visible in the [OVHcloud Control Panel](https://us.ovhcloud.com/manager/). To see them, go to Object Storage section, where you will find the Object Storage bucket you have created. By clicking on the bucket you will see the list of objects, including all the snapshots beginning with the `/backup/demo/deployment/stash-demo` we had defined in the `Repository`:

![stash1.png](/images/public-cloud/containers-orchestration/backing-up-persistent-volumes-using-stash/13938525457299.png)
...
Before restoring we need to pause the `BackupConfiguration` to prevent any snapshotting during the restore process. With the `BackupConfiguration`, Stash will stop taking any further backups for `nginx-deployment`.
