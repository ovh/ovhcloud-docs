...
description: "On a server with a hardware RAID configuration, the RAID array is managed by a physical component called a RAID controller."
zendesk_id: 22654971011731
zendesk_hash: 71ffd8290459b980
content_hash: 947e9eba5b5a049f
updated: 2025-07-09
---
...
- [Using MegaRaid RAID controller](#using-the-megaraid-raid-controller)
- [Using LSI RAID controller](#using-the-lsi-raid-controller)

 

## Requirements

- A [dedicated server](https://us.ovhcloud.com/bare-metal/) with a hardware RAID configuration
- Administrative (root) access to the server via SSH

:::warning

**NOTE:** It is not advisable to reconfigure your RAID controller using MegaCli and lsiutil if you're unfamiliar with these tools, as you could risk losing your data. Please make a backup before making any changes.
...
<details>
<summary>Step 1: Retrieve RAID information</summary>

Before verifying your RAID state, verify that you have a MegaRaid controller:
...
<details>
<summary>Step 2: Determine the disk's state</summary>

First, you must list the device Id for each drive to fully test them with smartmontools:
...
**NOTE:** If one of your hard drives is showing SMART errors, you should perform a full backup of your data as soon as possible and contact our support team. Our support team will need the slot number and device ID to identify the faulty disk.
...
<details>
<summary>Step 3: Verify the health of the RAID controller</summary>
...
<details>
<summary>Step 4: Resynchronizing the RAID</summary>

If you had one or more hard drives replaced, the RAID will re-synchronize automatically. You can use this command to see which hard drives are currently rebuilding:
...
<details>
<summary>Step 5a: Using CacheCade</summary>

:::info

CacheCade is a module from LSI used to improve the random read performance of hard drives using an SSD as a front caching device.
...
<details>
<summary>Step 5b: Checking the status of the backup battery unit</summary>
...
the most important value to check is if `Battery State` is **Optimal**. If there are indicators of a failing battery, create a backup of your data and provide the output of this command to the support, when creating the Ticket.
...
**NOTE:** This RAID controller card is deprecated and no longer available for new servers. It is gradually replaced by MegaRaid controllers.
...
Before verifying the RAID state, ensure that an LSI RAID controller card is installed with the following command:
...
**NOTE:** Caution, the values (1,0 21) may differ depending on the version. Be very careful when handling this type of control.
...
Since the LSI card uses sg-map, we must test the /dev/sgX (X being the device number, like /dev/sg1, for example) corresponding to the hard drives to test them with smartmontools.
...
To list the right devices within one command, use the following:
...
**NOTE:** If one of your hard drives is showing SMART errors, you should perform a full backup of your data as soon as possible and contact our support team.
...
<details>
<summary>Step 3: Resynchronize the RAID</summary>

If you had one or more hard drives replaced, the RAID will re-synchronize automatically. To see if the RAID is in re-sync and monitor the resync progression, use this command:

:::warning

**NOTE:** Caution, the values (3,0 21) may differ depending on the version. Be very careful when handling this type of control.
...
**NOTE:** The percentage value shown in the command result is NOT the completion percentage. It is the remaining percentage.
...
**WARNING:** This RAID controller card is deprecated. We highly recommend that you [contact OVHcloud Support](/guides/account/account-information/contact-us-support) to schedule an intervention to replace the RAID controller with an LSI, as 3ware RAID controllers are proven to be rather unstable. This type of intervention requires a reinstallation of your server. Be sure to backup your data first.
