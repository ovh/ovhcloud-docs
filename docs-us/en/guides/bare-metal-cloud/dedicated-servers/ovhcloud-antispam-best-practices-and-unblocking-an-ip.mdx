...
description: "For every IP available with OVHcloud products and services, as an Internet Service Provider, we will register and reserve it with organizations such as RIPE or"
zendesk_id: 16100926574995
zendesk_hash: 53fd67d260537db5
content_hash: d2f469a27ce1be9b
updated: 2026-01-06
---
...
- If you are indeed sending SPAM or illegitimate emails, we strongly recommend you to resolve the issue **before** unblocking the IP. Please consult the second part of this guide for email [best practices](#best-practices).
...
In your [OVHcloud Control Panel](https://manager.us.ovhcloud.com/), go to the `Bare Metal Cloud` section and, in the **Network** section, click on `IP`.
...
![IP\_antispam\_alert.PNG](/images/bare-metal-cloud/dedicated-servers/ovhcloud-antispam-best-practices-and-unblocking-an-ip/47967840770835.png)

Click on the *more options* `...` next to the corresponding IP/service and select `Anti-spam unblocking`.

![IP\_antispam\_blocked.PNG](/images/bare-metal-cloud/dedicated-servers/ovhcloud-antispam-best-practices-and-unblocking-an-ip/47967840772627.png)

In the pop-up windows, click `Unblock the IP` at the bottom and confirm.

![IP\_antispam\_unblock.png](/images/bare-metal-cloud/dedicated-servers/ovhcloud-antispam-best-practices-and-unblocking-an-ip/47967840774547.png)
...
<Api version="v1" section="/ip" method="GET" route={"/ip"} regions={["us"]} />
...
<Api version="v1" section="/ip" method="GET" route={"/ip/\{ip\}/spam"} regions={["us"]} />
...
<Api version="v1" section="/ip" method="GET" route={"/ip/\{ip\}/spam/\{ipSpamming\}"} regions={["us"]} />
...
<Api version="v1" section="/ip" method="GET" route={"/ip/\{ip\}/spam/\{ipSpamming\}/stats"} regions={["us"]} />
...
**WARNING:** IMPORTANT! Do not unblock the IP under any circumstances without having suspended the sending of emails from your server, otherwise you will immediately get blocked for a second time (and a longer duration).

To unblock your IP, use the following call:

<Api version="v1" section="/ip" method="POST" route={"/ip/\{ip\}/spam/\{ipSpamming\}/unblock"} regions={["us"]} />
...
In some cases, the antispam alert could be false positive. If you have checked and found that **Message-ID** are from your legitimate email, you should then ensure that your email messages comply with the RFC and the [Best Practices](#best-practices) indicated below.

 

#### RFC

RFCs (Request For Comments) are documents intended to describe technical aspects of the Internet. They are produced and published by the IETF (Internet Engineering Task Force), a group which basically produces and defines standards. For more information, see: [RFC](https://en.wikipedia.org/wiki/Request_for_Comments), [IETF](https://www.ietf.org/) and [Internet Draft](https://en.wikipedia.org/wiki/Internet_Draft).
...
- configure [Reverses](https://support.us.ovhcloud.com/hc/en-us/articles/360002181530) on all IPs correctly.
...
- **SPF**: Sender Policy Framework is a standard for verifying the domain of the sender. It is based on [RFC4408](https://datatracker.ietf.org/doc/rfc4408/) and consists of adding an SPF or TXT field to the domain DNS, which contains the list of IPs authorized to send emails from this domain.
...
For more information on the above services, please consult our guide on [How to Optimize the Sending of Emails](https://support.us.ovhcloud.com/hc/en-us/articles/4407065448339).
