...
zendesk_id: 7154925809683
zendesk_hash: 84c9d8dbc54938d7
content_hash: 5da886c1e7de863a
updated: 2026-06-15
---
...
- An [Additional IP address](https://us.ovhcloud.com/network/additional-ip/) or an Additional IP block (RIPE or ARIN)
...
| ------------------ | -------------------------------------------------------------------------------------------------------- | -------------- |
| ADDITIONAL\_IP     | An Additional IP address assigned to your service                                                        | 203.0.113.1    |
...
To obtain the name of your network interface to edit the appropriate network file, you can run one of the following commands:
...
<Tab label="Debian 12 & Ubuntu 20.04+">
...
<Tab label="Alma Linux / Rocky Linux">
...
![Whm control panel](/images/bare-metal-cloud/dedicated-servers/configuring-ip-aliasing/whm-control-panel.png)
...
![Whm add IP](/images/bare-metal-cloud/dedicated-servers/configuring-ip-aliasing/whm-add-ip.png)

:::warning

**NOTE:** Please note that if you have more than one IP to configure on the same block and you add them all at once, the WHM system will force you to use the subnet mask `255.255.255.0`. We do not recommend using this configuration. Instead, you need to add each IP individually to use the proper subnet mask `255.255.255.255`.
...
![Whm i pfunctions](/images/bare-metal-cloud/dedicated-servers/configuring-ip-aliasing/whm-i-pfunctions.png)
...
![Windows console IP configuration output](/images/bare-metal-cloud/dedicated-servers/configuring-ip-aliasing/windows-console-ip-config-output.png)
...
<details>
<summary>Via a command-line interface (recommended)</summary>

In the commands below, you need to replace:

| Command          | Value                                                                 |
| ---------------- | --------------------------------------------------------------------- |
| NETWORK\_ADAPTER | Name of the network adapter (in our example: Ethernet 2)              |
| IP\_ADDRESS      | Server IP address (in our example: 192.0.2.28)                        |
| SUBNET\_MASK     | Subnet mask (in our example: 255.255.255.0)                           |
| GATEWAY          | Default gateway (in our example: 192.0.2.254)                         |
| ADDITIONAL\_IP   | Address of Additional IP you want to add (in our example 203.0.113.1) |

:::warning

**NOTE:** Be careful – the server will no longer be accessible if you enter incorrect information. You will then have to make the corrections in [Winrescue mode](/guides/bare-metal-cloud/dedicated-servers/rescue-mode) or via the [IPMI](/guides/bare-metal-cloud/dedicated-servers/use-ipmi-console).
...
![Windows IPv4 properties](/images/bare-metal-cloud/dedicated-servers/configuring-ip-aliasing/windows-ipv4-properties.png)

In the `IP Address` section, click `Add`:

![Windows advanced tcp i psettings](/images/bare-metal-cloud/dedicated-servers/configuring-ip-aliasing/windows-advanced-tcp-i-psettings.png)

Type in your Additional IP and the subnet mask **255.255.255.255**. Then click on `Add`

![Windows add tcp IP address](/images/bare-metal-cloud/dedicated-servers/configuring-ip-aliasing/windows-add-tcp-ip-address.png)
...
![Windows console IP configuration output - step 2](/images/bare-metal-cloud/dedicated-servers/configuring-ip-aliasing/windows-console-ip-config-output-2.png)
...
![Plesk control panel](/images/bare-metal-cloud/dedicated-servers/configuring-ip-aliasing/plesk-control-panel.png)
...
![Plesk IP addresses](/images/bare-metal-cloud/dedicated-servers/configuring-ip-aliasing/plesk-i-paddresses.png)

Enter your Additional IP in the form `xxx.xxx.xxx.xxx/32` into the field "IP address and subnet mask", then click on `OK`.

![Plesk add IP address](/images/bare-metal-cloud/dedicated-servers/configuring-ip-aliasing/plesk-add-ip-address.png)
...
![Plesk verify IP added](/images/bare-metal-cloud/dedicated-servers/configuring-ip-aliasing/plesk-verify-i-padded.png)
