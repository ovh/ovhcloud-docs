...
description: "Learn how to reconfigure the disk mirror of your Windows system if it needs to be rebuilt due to corruption or disk failure."
zendesk_id: 17609776186771
zendesk_hash: 273226d2f9231052
content_hash: 7d37a3972b2355ba
updated: 2023-10-01
---

# Configuring a Software Mirror (RAID) on Windows

**Learn how to reconfigure the disk mirror of your Windows system if it needs to be rebuilt due to corruption or disk failure.**
...
- A Windows [dedicated server](https://us.ovhcloud.com/bare-metal/) with a software mirror
...
![Rai dwindows configuration - step 1](/images/bare-metal-cloud/dedicated-servers/configuring-software-mirror-raid-windows/config-rai-dwindows-01.png)

 

Enter "cmd" and click on `OK`.

![Rai dwindows configuration - step 2](/images/bare-metal-cloud/dedicated-servers/configuring-software-mirror-raid-windows/config-rai-dwindows-02.png)

The method to use depends on the partition style of your disks. Follow the instructions below for **MBR** or skip to the [subsequent section](#GPT) for **GPT**. If you are unsure, run `diskpart` at the command prompt and enter `list disk`. Check the "Gpt" column in the output.

 

### Rebuilding the mirror (MBR partition scheme)[]()
...
**WARNING:** DiskPart executes commands without issuing warnings or asking for confirmation. Any changes done in DiskPart are irreversible. Entering commands while the wrong disk or volume is selected may therefore cause immediate data loss and/or prevent your system from booting. We recommend to proceed with caution and double-check each command.
...
In this example, `Disk 1` is a replacement drive that has been installed to replace the defective `Disk M0` which had been physically removed previously.

:::info

The following code sections are for illustration only, based on the example output above. You will need to adjust the instructions according to your actual configuration by replacing the values in the commands with your disk and volume identifiers.
...
#### Initializing the replacement disk
...
Back at the command prompt, copy the boot files from the boot (EFI) partition on the first disk (`Disk 0`) to the boot partition on the second disk (`Disk 1`).

Type the following three commands and execute each one with `Enter`:
