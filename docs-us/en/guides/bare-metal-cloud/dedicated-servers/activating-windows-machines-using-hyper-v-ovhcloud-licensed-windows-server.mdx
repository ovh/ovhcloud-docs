...
description: "Learn about creating virtual machines within Hyper-V on a Windows Server licensed with OVHcloud."
zendesk_id: 45611139987091
zendesk_hash: edf0f95928540f15
content_hash: 8a9a38bf5c61d6d3
updated: 2026-06-15
---

# Activating Windows machines using Hyper-V on an OVHcloud licensed Windows Server

**Learn about creating virtual machines within Hyper-V on a Windows Server licensed with OVHcloud.**
...
- A [dedicated server](https://us.ovhcloud.com/bare-metal/) with Windows Server installed
...
The steps below are for standard activation only. If you have a Windows Datacenter license, we recommend proceeding with [Automatic Virtual Machine Activation (AVMA)](https://learn.microsoft.com/en-us/windows-server/get-started/automatic-vm-activation?tabs=server2019#system-requirements).
...
We see that the "NAT" Virtual Switch has been created successfully. Once it's been created, we will need to confirm the InterfaceIndex or "interface ID" for the following step like this:
...
The IPAddress value will be the internal gateway IP for our VM to be configured; it will connect the WinNAT service inside the host to reach internet. The PrefixLength will be the netmask prefix of the IP given before, and lastly the InterfaceIndex will be the ID of the virtual switch created on the step before (in our case, its ID is 24).
...
![Example - Activating Windows machines using hyper v OVHcloud licensed Windows server](/images/bare-metal-cloud/dedicated-servers/activating-windows-machines-using-hyper-v-ovhcloud-licensed-windows-server/example.png)

Since the .ISO used has enabled the "Evaluation mode", we need to transfer this guest OS to the Standard version. Launch the following command on your CMD:
