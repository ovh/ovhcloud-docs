...
description: "Our maintenance interventions are limited to the hardware aspect of your server. Following a maintenance intervention, actions may be required on your server's"
zendesk_id: 30897799384211
zendesk_hash: 7ff0213bcd59cc9a
content_hash: a60e070442d75b5f
updated: 2026-04-21
---

import { Tab, Tabs } from '@rspress/core/theme';

# Finalize a maintenance intervention on your dedicated server

Our maintenance interventions are limited to the hardware aspect of your server. Following a maintenance intervention, actions may be required on your server's software.
...
Find out how to identify the names of your network interfaces in [this subpart of this guide](#locate-the-name-of-your-network-interfaces).
...
If you experience an operating system boot problem that appears to be related to its EFI boot partition, go directly to this [section](#problems-with-your-efi-partition).
...
1\. Reboot the server in [rescue mode](https://support.us.ovhcloud.com/hc/en-us/articles/115001754490).\
...
If the `70-persistent-net.rules` file does not exist (refer to the section of this guide on [the names of your network interfaces](#locate-the-name-of-your-network-interfaces)), check the following files in the `/etc/systemd/network/` tree:
...
3\. Check the configuration file `/mnt/etc/sysconfig/network-scripts/ifcfg-eth0`. 4. Back up the files and edit them to correct the MAC address:

```bash
root@rescue:~# cp /mnt/etc/sysconfig/network-scripts/ifcfg-eth0 /mnt/etc/sysconfig/network-scripts/ifcfg-eth0.`date +%s`
```

5\. Enter the new MAC address in the line `HWADDR=xx:xx:xx:xx:xx:xx`. 6. Please remember to unmount the partition `/` before rebooting the server.
...
1\. Reboot the server in [rescue mode](https://support.us.ovhcloud.com/hc/en-us/articles/115001754490) and locate the partition `/`:
...
In our example, the system uses *LVM*:
...
root@rescue:~# lvdisplay
  ...
  LV Creation host, time rescue.ovh.us, 2019-06-03 16:21:38 -0400
  LV Status              available
  # open                 0
  LV Size                128.00 GiB
  Current LE             32768
  ...
  LV Write Access        read/write
  LV Creation host, time rescue.ovh.us, 2019-06-03 16:21:44 -0400
  LV Status              available
  # open                 0
  LV Size                17.34 TiB
  Current LE             4545078
...
Click the tab for your virtualization option.

:::

<Tabs>

<Tab label="Proxmox/Debian (excluding Debian 12+)">
...
![DS\_troubleshooting\_finalizeMaintenance00.png](/images/bare-metal-cloud/dedicated-servers/finalize-a-maintenance-intervention-on-your-dedicated-server/30901999677587.png)
...
- The new MAC address (viewable in the tab `Network interfaces` in the [OVHcloud Control Panel](https://us.ovhcloud.com/manager/) or in the return of the command `ip link`).
...
1\. Restart the server in [rescue mode](https://support.us.ovhcloud.com/hc/en-us/articles/115001754490) and use the `chroot` command:
...
After a motherboard has been replaced, you cannot modify the new MAC addresses through the `esxi.conf` file via the tools integrated in rescue mode. This will require manual intervention.

#### Version 7.0 or newer

The following procedure applies only to **versions 7.0 and later**. From this release on, the state.tgz file is encrypted. You will need to reboot the network from the Direct Console menu via your KVM or IPMI.

Please refer to the screenshot below:

![DS\_troubleshooting\_finalizeMaintenance01.png](/images/bare-metal-cloud/dedicated-servers/finalize-a-maintenance-intervention-on-your-dedicated-server/30901999683859.png)

#### Version 6.7 or older

The procedure described below applies only to **versions 6.7 and earlier**.

1\. Restart the server in [rescue mode](https://support.us.ovhcloud.com/hc/en-us/articles/115001754490) to mount the partition `/`:
...
**NOTE:** Do not change the virtual MAC address.
...
**NOTE:** If there is a vRack interface, remember to also change its MAC address.
...
1\. Log in as an administrator from the [IPMI/KVM](https://support.us.ovhcloud.com/hc/en-us/articles/360007816120) interface.\
2\. Open the `Run` utility (Windows logo key + `R`), and through the `run` prompt, run the `devmgmt.msc` command:

![DS\_troubleshooting\_finalizeMaintenance02.png](/images/bare-metal-cloud/dedicated-servers/finalize-a-maintenance-intervention-on-your-dedicated-server/30901970600979.png)
...
![DS\_troubleshooting\_finalizeMaintenance03.png](/images/bare-metal-cloud/dedicated-servers/finalize-a-maintenance-intervention-on-your-dedicated-server/30901970609939.png)
...
![DS\_troubleshooting\_finalizeMaintenance04.png](/images/bare-metal-cloud/dedicated-servers/finalize-a-maintenance-intervention-on-your-dedicated-server/30901999697555.png)

4\. Then click `Load Hive...`.

![DS\_troubleshooting\_finalizeMaintenance05.png](/images/bare-metal-cloud/dedicated-servers/finalize-a-maintenance-intervention-on-your-dedicated-server/30901999701267.png)

:::info

By default, you will be in the WinPE tree. Don't forget to browse to `C:\`.

:::

5\. Locate the `SYSTEM` registry.

![DS\_troubleshooting\_finalizeMaintenance06.png](/images/bare-metal-cloud/dedicated-servers/finalize-a-maintenance-intervention-on-your-dedicated-server/30901970628755.png)
...
**NOTE:** As long as we are on WinPE, the value `CurrentControlSet` should be equal to `CurrentSet001`.

:::

You should see several subkeys named like this: 0000, 0001, etc.

8\. Click each subkey to check the `DriverDesc` value that should match your network interface. You can check the name of your interface from a terminal using the following command:
...
![DS\_troubleshooting\_finalizeMaintenance07.png](/images/bare-metal-cloud/dedicated-servers/finalize-a-maintenance-intervention-on-your-dedicated-server/30901970635923.png)

10\. You will need to unload the registry to apply the recent changes. Click OVH\_TEST (created earlier) then click `File` (in the top left-hand corner) and select `Unload hive...`.
...
1\. Restart the server in [rescue mode](https://support.us.ovhcloud.com/hc/en-us/articles/115001754490), mount the partition `/` and then use the `chroot` command:
...
There are two methods, via rescue mode or via BIOS.
...
- Go to the BIOS `boot` menu:
  ...
  - Choose the path to your `Path for boot partition option` and select the file system that contains your boot partition.
  - Select the `EFI\centos6\bootx64.efi` file and choose `Create`.

The image below is a summary of the actions mentioned above:

![DS\_troubleshooting\_finalizeMaintenance08.gif](/images/bare-metal-cloud/dedicated-servers/finalize-a-maintenance-intervention-on-your-dedicated-server/30901970643219.gif)

The *second sequence* is to make the selected partition active in the previous sequence.

- In the BIOS `boot` menu:

  - Choose `UEFI Hard Disk BBS Priorities`.
  - Choose `UEFI Boot Order #1` then select `CENTOS6` to position it as the first start choice in the current list.

The image below is a summary of the actions mentioned above:

![DS\_troubleshooting\_finalizeMaintenance09.gif](/images/bare-metal-cloud/dedicated-servers/finalize-a-maintenance-intervention-on-your-dedicated-server/30901999725587.gif)
