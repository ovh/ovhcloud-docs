...
description: "The OVHcloud Command Line Interface (CLI) is a tool designed to interact with OVHcloud services directly from your terminal. It allows you to automate tasks, ma"
zendesk_id: 48181801023763
zendesk_hash: 46acbd342a0a7432
content_hash: 501a144e3bdfed0d
updated: 2026-01-14
---
...
Default settings can be set using a configuration file named `.ovh.conf` and located in your `${HOME}` directory. You can check [this guide](https://support.us.ovhcloud.com/hc/en-us/articles/360018130839) to learn how to create API credentials.
...
```
ovhcloud cloud instance create us-east-va --cloud-project <PROJECT_ID> --name my-instance --flavor-selector --image-selector --network.public --ssh-key.name <SSH_KEY_NAME>
```
...
The actions currently available in the CLI correspond to those offered by the main OVHcloud API. Actions specific to each product, accessible via their own APIs, are not yet covered, but will gradually be added depending on the product.
...
- [CLI Documentation](https://github.com/ovh/ovhcloud-cli/blob/main/README.md)
- [API Reference](https://api.us.ovhcloud.com/)
