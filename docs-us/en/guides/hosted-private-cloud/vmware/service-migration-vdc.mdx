...
![Decision tree - Service migration vDC](/images/hosted-private-cloud/vmware/service-migration-vdc/newdecisiontree.png)
...
![Add vDC](/images/hosted-private-cloud/vmware/service-migration-vdc/add-vdc.gif)
...
In the OVHcloud Control Panel, you will see your new vDC attached to your existing service. You can order new resources (selected in Step 1) in the new destination vDC. You can see billing information for OVHcloud products and services in [this guide](/guides/account/billing/overview).
...
For connections to the VMware platform, you can choose to block access to vSphere by default. Please refer to our guide on the [vCenter access policy](/guides/hosted-private-cloud/vmware/authorise-ip-vcenter-access) for details.
...
To do this, please refer to our guides on [changing user rights](/guides/hosted-private-cloud/vmware/introduction-ovhcloud-control-panel), [changing the user password](/guides/hosted-private-cloud/vmware/change-user-password), and [associating an email with a vSphere user](/guides/hosted-private-cloud/vmware/vsphere-edit-user).
...
As part of a migration process, the new vDC will (by default) be linked to the same vRack as the source vDC. Please consult our guide to [Using Private Cloud within a vRack](/guides/hosted-private-cloud/vmware/use-private-cloud-within-vrack).
...
Setting up a new vDC involves reconfiguring VMware High Availability (HA), including boot order and priority. Please consult our guide about [VMware HA configuration](/guides/hosted-private-cloud/vmware/ha-high-availability).
...
Setting up a new vDC involves reconfiguring the VMware Distributed Resource Scheduler (DRS) feature, in particular the affinity or anti-affinity rules for groups of hosts and VMs. Please consult our guide about [configuring VMware DRS](/guides/hosted-private-cloud/vmware/drs-distributed-ressource-scheduler).
...
If vSAN was enabled on your source VDC, you will need to enable it again on the destination vDC. Please refer to our guide on [using VMware Hyperconvergence with vSAN](/guides/hosted-private-cloud/vmware/hyperconvergence-with-vsan).
...
As a first step, please read our documentation on [Getting Started with NSX](/guides/hosted-private-cloud/vmware/nsx-first-steps).
...
For further information on the Distributed Firewall, refer to our guide [Distributed Firewall Management in NSX](/guides/hosted-private-cloud/vmware/nsx-manage-distributed-firewall).
...
- *Step 4.8.3.1 Create the T1 and Segments*
  ...
  To create T1 Gateways, follow this documentation: [Adding a New Tier-1 Gateway](/guides/hosted-private-cloud/vmware/nsx-add-new-tier1-gateway). This documentation also guides you on how to create segments. Before creating segments, it is necessary to inventory the VXLANs used in the source vDC and create a segment for each VXLAN used in your infrastructure.

  Afterward, you can connect them to the provided T0 in NSX.

  For further information on segments, this documentation can be helpful: [Segment Management in NSX](/guides/hosted-private-cloud/vmware/nsx-segment-management).
...
- *Step 4.8.3.2 DHCP*
  ...
  To recreate DHCP and associate them with your segments and T1 Gateways, please see the [DHCP Configuration in NSX](/guides/hosted-private-cloud/vmware/nsx-configure-dhcp-onsegment) guide.

- *Step 4.8.3.3 DNS*

  To recreate DNS and associate them with your T1 Gateways, see the [Configuring DNS Forwarder in NSX](/guides/hosted-private-cloud/vmware/nsx-configure-dns-forwarder) guide.

- *Step 4.8.3.4 NAT Rules*

  To recreate your NAT rules and associate them with your T1 Gateways, see the [Configuring NAT Redirection](/guides/hosted-private-cloud/vmware/nsx-configure-nat-redirection) guide.

- *Step 4.8.3.5 NSX Load Balancing*

  To recreate your Load Balancers, follow the instructions in our [Load Balancing Configuration](/guides/hosted-private-cloud/vmware/nsx-configure-loadbalancing) guide.

- *Step 4.8.3.6 Firewall for T0 T1 Gateways*

  To recreate the firewall rules associated with your previous edges, see our [Gateway Firewall Management in NSX](/guides/hosted-private-cloud/vmware/nsx-manage-gateway-firewall) guide.

- *Step 4.8.3.7 VPN*

  - Step 4.8.3.7.1 IPSec

    To recreate your IPsec sessions, here is the documentation: [How to Set Up an IPsec Tunnel with NSX](/guides/hosted-private-cloud/vmware/set-up-ipsec-tunnel-nsx).
...
* *Step 4.8.3.8 Reconfiguration of the Initial IP Block*
  ...
  **NOTE:** This step will result in a service interruption as all traffic will be redirected to the T0 VIP (you will no longer be able to use this IP block with VMs on the VM Network portgroup). It is up to you to decide whether you want to point the primary IP block of NSX-V to the NSX-T0 before or after migrating your VMs ([see Step 5.2](#step-5-migrate-your-workload)).
  ...
  Next, in the [OVHcloud Control Panel](https://us.ovhcloud.com/manager/), follow the instructions in our [How to Move an Additional IP](/guides/bare-metal-cloud/dedicated-servers/move-failover-ip) guide to move the initial NSX-V block to the PCC service you are migrating. Specify the VIP IP of the T0 as the "next hop," as shown in the example below:

  ![Move IP next hop](/images/hosted-private-cloud/vmware/service-migration-vdc/move-ip-next-hop.png)
...
The next step depends on the current configuration per [Virtual Protection Group](/guides/hosted-private-cloud/vmware/zerto-virtual-replication-vsphere-drp):
...
To "move" the Private Gateway to the destination vDC, you must first disable it by following the steps to [disable the private gateway](/guides/hosted-private-cloud/vmware/enable-private-gateway).
...
Then enable it again by following the steps in [enable the private gateway](/guides/hosted-private-cloud/vmware/enable-private-gateway) and choosing the datacenterId of the new vDC.
...
![Remove NSX-V - Service migration vDC - step 1](/images/hosted-private-cloud/vmware/service-migration-vdc/remove-nsxv-01.png)
...
**NOTE:** If the process takes longer than two hours [open a support ticket](/guides/account/account-information/contact-us-support).
...
![Remove NSX-V - Service migration vDC - step 2](/images/hosted-private-cloud/vmware/service-migration-vdc/remove-nsxv-02.png)
...
![Remove NSX-V - Service migration vDC - step 3](/images/hosted-private-cloud/vmware/service-migration-vdc/remove-nsxv-03.png)
...
![Remove NSX-V - Service migration vDC - step 5](/images/hosted-private-cloud/vmware/service-migration-vdc/remove-nsxv-05.png)

**IMPORTANT!** Open an [OVHcloud support ticket](/guides/account/account-information/contact-us-support) and state the following:
...
**NOTE:** During segment creation, if you want to use vRack as a segment, follow [this guide](/guides/hosted-private-cloud/vmware/nsx-segment-management#creating-a-segment-on-a-vlan).
