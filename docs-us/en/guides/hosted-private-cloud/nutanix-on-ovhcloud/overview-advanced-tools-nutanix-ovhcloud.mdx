---
title: "Overview of Advanced Tools for Nutanix on OVHcloud"
description: "This guide will show you all the administration tools, other than the Prism Central and Prim Element web interfaces, which are:"
zendesk_id: 17268631052051
zendesk_hash: c54a224a5ab598ca
content_hash: e01c79f923809546
updated: 2023-09-29
---

# Overview of Advanced Tools for Nutanix on OVHcloud
...
**NOTE:** Some commands can only be used if you have SSH access to **Prism Central** and **Prism Element**.
...
It works in **Microsoft**, **Linux**, and **MacOS** environments but is mainly used in a **Microsoft** environment.\
All of the examples below have been run in a **Microsoft** environment.
...
ncli
 ...
 ctr create name="NewContainer" sp-name="default-storage-pool-10728992352041"
...
Download and install the installer that corresponds to your version of Windows, in 32 bits or more generally in 64 bits.\
...
- `< PRISM CENTRAL USERNAME >` Replace this item with the **Prism Central** username
...
All of the examples shown below must be run on a virtual machine running Linux with the **curl** tool.\
...
In the top right-hand corner of the **Prism Central** interface, click `Username`, then `REST API Explorer`. The list of command groups is displayed on the left.

![Overview advanced tools Nutanix OVHcloud - step 1](/images/hosted-private-cloud/nutanix-on-ovhcloud/overview-advanced-tools-nutanix-ovhcloud/advancedtools-01.png)

 

Click the `drop-down icons` to the right of one of the command groups to display all possible commands in that group.

![Overview advanced tools Nutanix OVHcloud - step 2](/images/hosted-private-cloud/nutanix-on-ovhcloud/overview-advanced-tools-nutanix-ovhcloud/advancedtools-02.png)

 

Click `Expand operations` to list the syntax for a particular command.

![Overview advanced tools Nutanix OVHcloud - step 3](/images/hosted-private-cloud/nutanix-on-ovhcloud/overview-advanced-tools-nutanix-ovhcloud/advancedtools-03.png)

![Overview advanced tools Nutanix OVHcloud - step 4](/images/hosted-private-cloud/nutanix-on-ovhcloud/overview-advanced-tools-nutanix-ovhcloud/advancedtools-04.png)
...
We will see how to create two virtual machines in an automated way, one in a Linux environment and the other on Windows.

#####  

##### **Creating a virtual machine on Linux**

You can install Linux from pre-installed images, and you can customize the configuration with cloud-init, which uses the yaml file format. To import images into a Nutanix cluster, you can refer to our guide on [importing ISO images](/guides/hosted-private-cloud/nutanix-on-ovhcloud/import-iso-images-nutanix-using-prism-central).

Follow these instructions to create a virtual machine from a Ubuntu cloud-init customizable image.
...
- `< UUID-NETWORK >` must be replaced with the UUID of the network on which the virtual machine will operate.
...
You can install virtual machines on Windows from a prepared image using the Windows-integrated **sysprep** command and apply a customization file to it.
